...
/proc/do_after(mob/user, delay, atom/target = null, needhand = 1, progress = 1, var/incapacitation_flags = INCAPACITATION_DEFAULT, var/same_direction = 0, var/can_move = 0)
	...
	user.curr_interaction++
	var/interact_id = user.curr_interaction /* id for the current do_after interaction */
	...
	while (world.time < endtime)
		...
		/* if user has started a new interaction, leave this one */
		if(interact_id != user.curr_interaction)
			. = 0
			break
