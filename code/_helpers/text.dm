...
/proc/sanitize(var/input, var/max_length = MAX_MESSAGE_LEN, var/encode = 1, var/trim = 1, var/extra = 1)
	...
	if(max_length)
		...
		if(length(input) >= max_length)
			...
			to_chat(usr, "<span class='warning'>My message is too long by [overflow] character\s.</span>")
			return
...
//If I have a problem with sanitize() in chat, when quotes and >, < are displayed as html entites -
