/*
	...
	This is calculated by looking for border items, or in the case of clicking diagonally from myself, dense items.
	This proc will NOT notice if I am trying to attack a window on the other side of a dense object in its turf.  There is a window helper for that.

	Note that in all cases the neighbor is handled simply; this is usually the user's mob, in which case it is up to me
...
/*
	Adjacency (to turf):
	* If I am in the same turf, always true
	* If I am vertically/horizontally adjacent, ensure there are no border objects
	* If I am diagonally adjacent, ensure I can pass through at least one of the mutually adjacent square.
...
* If I am in the same turf, always true
* If I am not adjacent, then false
*/
...
// This is necessary for storage items not on my person.
/obj/item/Adjacent(var/atom/neighbor, var/recurse = 1)
	...
	return ..()
/*
	Special case: This allows me to reach a door when it is visally on top of,
	but technically behind, a fire door

	I could try to rewrite this to be faster, but I'm not sure anything would be.
...
/*
	This checks if I there is uninterrupted airspace between that turf and this one.
	This is defined as any dense ATOM_FLAG_CHECKS_BORDER object, or any dense object without throwpass.
	The border_only flag allows me to not objects (for source and destination squares)
*/
/turf/proc/ClickCross(var/target_dir, var/border_only, var/target_atom = null)
	for(var/obj/O in src)
		if( !O.density || O == target_atom || O.throwpass) continue // throwpass is used for anything I can click through
