			...
			if(iscarbon(usr))
				...
				if(!C.stat && !C.stunned && !C.paralysis && !C.restrained())
					if(C.internal)
						...
						if(no_mask)
							to_chat(C, "<span class='notice'>I am not wearing a suitable mask or helmet.</span>")
							return 1
						else
							...
							if(best)
								to_chat(C, "<span class='notice'>I am now running on internals from [tankcheck[best]] [from] my [nicename[best]].</span>")
							...
							if(C.internal)
								...
								to_chat(C, "<span class='notice'>I don't have a[breathes=="oxygen" ? "n oxygen" : addtext(" ",breathes)] tank.</span>")
		if("act_intent")
...
//					return 1
		...
		if("inventory")
			if(isrobot(usr))
				...
				if(R.module)
					...
					to_chat(R, "I haven't selected a module yet.")
		...
		if("health" || "healthback") //so that if I misclick, I still get the health report
			if(ishuman(usr))
				var/mob/living/carbon/human/S = usr
				S.help_shake_act(S)
