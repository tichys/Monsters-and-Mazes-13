...
/*
	...
	that I can grab with a telekinetic grab.
...
/*
	...
	* If I have not grabbed something, do a normal tk attack
	* If I have something, throw it at the target.  If it is already adjacent, do a normal attackby()
	* If I click what I am holding, or attack_self(), do an attack_self_tk() on it.
	* Deletes itself if it is ever not in my hand, or if I should have no access to TK.
*/
/obj/item/tk_grab
...
/obj/item/tk_grab/dropped(mob/user as mob)
	if(focus && user && loc != user && loc != user.loc) // drop_item() gets called when I tk-attack a table/closet with an item
