...
/datum/controller/vote
	...
	proc/autorestart()
		initiate_vote("restart","the server", 1)
		log_debug("The server has called a restart vote.")
	...
	proc/initiate_vote(var/vote_type, var/initiator_key, var/automatic = 0)
		if(!mode)
			...
			switch(vote_type)
				...
				if("add_antagonist")
					if(!is_addantag_allowed(automatic))
						if(!automatic)
							to_chat(usr, "The add antagonist vote is unavailable at this time. The game may not have started yet, the game mode may disallow adding antagonists, or I don't have required permissions.")
						return 0
			...
			log_vote(text)
			to_world("<span class='vote'><b>[text]</b>\nType <b>vote</b> or click <a href='?src=\ref[src]'>here</a> to place my votes.\nI have [config.vote_period/10] seconds to vote.</span>")
