...
/decl/communication_channel/proc/can_ignore(var/client/C)
	...
	if(!show_preference_setting)
		return FALSE
	// If you're trying to see the channel, I can't ignore it
...
/decl/communication_channel/proc/can_communicate(var/datum/communicator, var/message)
	...
	if(can_ignore(C))
		to_chat(communicator, "<span class='warning'>Couldn't send message - I have [name] muted.</span>")
		return FALSE

	if(C && mute_setting && (C.prefs.muted & mute_setting))
		to_chat(communicator, "<span class='danger'>I cannot use [name] (muted).</span>")
		return FALSE
