...
/datum/extension/holster/proc/holster(var/obj/item/I, var/mob/living/user)
	...
	if(can_holster(I))
		...
		user.visible_message("<span class='notice'>\The [user] holsters \the [holstered].</span>", "<span class='notice'>I holster \the [holstered].</span>")
	...
	if(avoid_intent || user.a_intent != I_HELP)
		...
		if(user.a_intent == I_HURT)
			...
			usr.visible_message(
				...
				"<span class='warning'>I draw \the [holstered], ready to go!</span>"
				)
		else
...
/atom/proc/holster_verb(var/holster_name in get_holsters())
	...
	if(!H.holstered)
		...
		if(!istype(W, /obj/item))
			to_chat(usr, "<span class='warning'>I'm not holding anything to holster.</span>")
			return
		H.holster(W, usr)
	else
		H.unholster(usr, 1)
