/*	Note from Carnie:
	...
	-	I must not assign key= or ckey= after transfer_to() since the transfer_to transfers the client for you.
		By setting key or ckey explicitly after transfering the mind with transfer_to I will cause bugs like DCing
		the player.

	-	IMPORTANT NOTE 2, if I want a player to become a ghost, use mob.ghostize() It does all the hard work for you.
	...
	-	When creating a new mob which will be a new IC character (e.g. putting a shade in a construct or randomly selecting
		...
		However if I want that mind to have any special properties like being a traitor etc I will have to do that
		myself.
	...
	if(href_list["add_antagonist"])
		...
		switch (new_obj_type)
			...
			if("download","capture","absorb")
				...
				if (isnull(target_number))//Ordinarily, I wouldn't need isnull. In this case, the value may already exist.
					return
				...
				for(var/obj/item/weapon/implant/loyalty/I in H.contents)
					for(var/obj/item/organ/external/organs in H.organs)
						if(I in organs.implants)
							qdel(I)
							break
				to_chat(H, "<span class='notice'><font size =3><B>My loyalty implant has been deactivated.</B></font></span>")
				log_admin("[key_name_admin(usr)] has de-loyalty implanted [current].")
			if("add")
				to_chat(H, "<span class='danger'><font size =3>I somehow have become the recepient of a loyalty transplant, and it just activated!</font></span>")
