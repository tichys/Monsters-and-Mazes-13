...
/datum/uplink_item/item/services/fake_crew_annoncement
	...
	desc = "Creates a fake crew arrival announcement as well as fake crew records, using my current appearance (including held items!) and worn id card. Prepare well!"
	item_cost = 16
...
/obj/item/device/uplink_service/attack_self(var/mob/user)
	...
	update_icon()
	user.visible_message("<span class='notice'>\The [user] activates \the [src].</span>", "<span class='notice'>I activate \the [src].</span>")
...
/obj/item/device/uplink_service/fake_update_announcement/enable(var/mob/user = usr)
	var/title = sanitize(input(user, "Enter my announcement title.", "Announcement Title") as null|text)
	if(!title)
		return
	var/message = sanitize(input(user, "Enter my announcement message.", "Announcement Title") as null|text)
	if(!message)
		return
