#define NO_GUARANTEE_NO_EXTRA_COST_DESC(X) "Installs an uplink into " + X + " if, and only if, found on my person. Has no TC cost."
...
/decl/uplink_source/pda/setup_uplink_source(var/mob/M, var/amount)
	...
	to_chat(M, "<span class='notice'>A portable object teleportation relay has been installed in my [P.name]. Simply enter the code \"[pda_pass]\" in my new program to unlock its hidden features.</span>")
...
/decl/uplink_source/radio/setup_uplink_source(var/mob/M, var/amount)
	...
	to_chat(M, "<span class='notice'>A portable object teleportation relay has been installed in my [R.name]. Simply dial the frequency [format_frequency(freq)] to unlock its hidden features.</span>")
	M.mind.store_memory("<B>Radio Freq:</B> [format_frequency(freq)] ([R.name]).")

/decl/uplink_source/implant
	name = "Implant"
	desc = "Teleports an uplink implant into my head. Costs at least half the initial TC amount."
...
/decl/uplink_source/unit
	name = "Uplink Unit"
	desc = "Teleports an uplink unit to my location. Costs 10% of the initial TC amount."
...
/decl/uplink_source/telecrystals
	name = "Telecrystals"
	desc = "Get my telecrystals in pure form, without the means to trade them for goods."
...
/decl/uplink_source/proc/put_on_mob(var/mob/M, var/atom/movable/AM, var/text)
	...
	if(O)
		to_chat(M, "<span class='notice'>[text] can be found in my [O.name].</span>")
	else if(M.put_in_hands(AM))
		to_chat(M, "<span class='notice'>[text] appear in my hands.</span>")
	else
		AM.dropInto(M.loc)
		to_chat(M, "<span class='notice'>[text] appear at my location.</span>")
...
/proc/setup_uplink_source(var/mob/M, var/amount = DEFAULT_TELECRYSTAL_AMOUNT)
	...
	to_chat(M, "<span class='warning'>Either by choice or circumstance I will be without an uplink.</span>")
	return FALSE
