...
/mob/living/proc/convert_to_faction(var/datum/mind/player, var/datum/antagonist/faction)
	...
	if(world.time < player.rev_cooldown)
		to_chat(src, "<span class='danger'>I must wait five seconds between attempts.</span>")
		return

	to_chat(src, "<span class='danger'>I am attempting to convert \the [player.current]...</span>")
	...
	var/choice = alert(player.current,"Asked by [src]: Do I want to join the [faction.faction_descriptor]?","Join the [faction.faction_descriptor]?","No!","Yes!")
	if(choice == "Yes!" && faction.add_antagonist_mind(player, 0, faction.faction_role_text, faction.faction_welcome))
		...
		return
	if(choice == "No!")
		to_chat(player, "<span class='danger'>I reject this traitorous cause!</span>")
