...
/datum/antagonist/cultist
	...
	welcome_text = "I have a tome in my possession; one that will help I start the cult. Use it well and remember - there are others."
	...
	var/conversion_blurb = "I catch a glimpse of the Realm of Nar-Sie, the Geometer of Blood. I now see how flimsy the world is, I see that it should be open to the knowledge of That Which Waits. Assist my new compatriots in their dark dealings. Their goals are mine, and mine are theirs. I serve the Dark One above all else. Bring It back."
...
/datum/antagonist/cultist/remove_antagonist(var/datum/mind/player, var/show_message, var/implanted)
	if(!..())
		return 0
	to_chat(player.current, "<span class='danger'>An unfamiliar white light flashes through my mind, cleansing the taint of the dark-one and the memories of my time as his servant with it.</span>")
	player.memory = ""
	if(show_message)
...
/datum/antagonist/cultist/proc/update_cult_magic(var/list/to_update)
	if(CULT_RUNES_1 in to_update)
		for(var/datum/mind/H in GLOB.cult.current_antagonists)
			if(H.current)
				to_chat(H.current, "<span class='cult'>The veil between this world and beyond grows thin, and my power grows.</span>")
				add_cult_magic(H.current)
	if(CULT_RUNES_2 in to_update)
		for(var/datum/mind/H in GLOB.cult.current_antagonists)
			if(H.current)
				to_chat(H.current, "<span class='cult'>I feel that the fabric of reality is tearing.</span>")
				add_cult_magic(H.current)
...
/datum/antagonist/cultist/proc/offer_uncult(var/mob/M)
	...
	to_chat(M, "<span class='cult'>Do I want to abandon the cult of Nar'Sie? <a href='?src=\ref[src];confirmleave=1'>ACCEPT</a></span>")
