...
/area/Entered(A)
	...
	if(oldarea.has_gravity != newarea.has_gravity)
		if(newarea.has_gravity == 1 && !MOVING_DELIBERATELY(L)) // Being ready when I change areas allows me to avoid falling.
			thunk(L)
		L.update_floating()
	...
	if(istype(mob,/mob/living/carbon/human/))
		...
		if(prob(H.skill_fail_chance(SKILL_EVA, 100, SKILL_PROF)))
			if(!MOVING_DELIBERATELY(H))
				...
				H.AdjustStunned(3)
				H.AdjustWeakened(3)
			to_chat(mob, "<span class='notice'>The sudden appearance of gravity makes I fall to the floor!</span>")
