	...
	if(!isbroken)
		if(prob(1+ damage * 5))
			user.visible_message(
				...
				"<span class='warning'>I hit the pylon, and its crystal breaks apart!</span>",
				"I hear a tinkle of crystal shards"
				)
...
/obj/effect/gateway
	name = "gateway"
	desc = "I'm pretty sure that abyss is staring back."
	icon = 'icons/obj/cult.dmi'
	icon_state = "hole"
	density = 1
	unacidable = 1
...
/obj/effect/gateway/active/Crossed(var/atom/A)
	...
	if(M.stat < DEAD)
		...
		to_chat(new_mob, "<B>My form morphs into that of a corgi.</B>")//Because we don't have cluwnes
