...
/mob/observer/ghost/cultify()
	if(icon_state != "ghost-narsie")
		...
		overlays = 0
		set_invisibility(0)
		to_chat(src, "<span class='sinister'>Even as a non-corporal being, I can feel Nar-Sie's presence altering you. I am now visible to everyone.</span>")

/mob/living/cultify()
	if(iscultist(src) && client)
		var/mob/living/simple_animal/construct/harvester/C = new(get_turf(src))
		mind.transfer_to(C)
		to_chat(C, "<span class='sinister'>The Geometer of Blood is overjoyed to be reunited with its followers, and accepts my body in sacrifice. As reward, I have been gifted with the shell of an Harvester.<br>My tendrils can use and draw runes without need for a tome, my eyes can see beings through walls, and my mind can open any door. Use these assets to serve Nar-Sie and bring him any remaining living human in the world.<br>I can teleport myself back to Nar-Sie along with any being under myself at any time using my \"Harvest\" spell.</span>")
		dust()
	else if(client)
