...
/mob/observer/ghost/proc/ghost_ability_check()
	...
	if(T.holy)
		to_chat(src, "<span class='notice'>I may not use my abilities on the blessed ground.</span>")
		return 0
	if(ghost_magic_cd > world.time)
		to_chat(src, "<span class='notice'>I need [round((ghost_magic_cd - world.time) / 10)] more seconds before I can use my abilities.</span>")
		return 0
	return 1
...
/mob/observer/ghost/proc/bloody_doodle_proc(var/bloodless = 0)
	...
	if(!bloodless)
		...
		choice = input(src, "What blood would I like to use?") as null|anything in choices
		if(!choice)
			return
...
/mob/observer/ghost/proc/move_item()
	set category = "Cult"
	set name = "Move item"
	set desc = "Move a small item to where I am."
	...
	var/obj/item/choice = input(src, "What item would I like to pull?") as null|anything in choices
...
/mob/observer/ghost/proc/whisper_to_cultist()
	...
	set desc = "Whisper to a human of my choice. They won't understand I unless they're a cultist though."
...
/mob/observer/ghost/proc/whisper_proc(var/anyone = 0)
	...
	var/mob/living/choice = input(src, "Whom do I want to whisper to?") as null|anything in choices
	if(!choice)
		return

	var/message = sanitize(input("Decide what I want to whisper.", "Whisper", ""))
	...
	if(message)
		if(iscultist(choice) || anyone)
			to_chat(choice, "<span class='notice'>I hear a faint whisper... It says... \"[message]\"</span>")
			log_and_message_admins("used ghost magic to say '[message]' to \the [choice] and was heard - [x]-[y]-[z]")
		else
			to_chat(choice, "<span class='notice'>I hear a faint whisper, but I can't make out the words.</span>")
			log_and_message_admins("used ghost magic to say '[message]' to \the [choice] but wasn't heard - [x]-[y]-[z]")
		to_chat(src, "I whisper to \the [choice]. Perhaps they heard you.")
...
/mob/observer/ghost/proc/bite_someone()
	...
	var/mob/living/carbon/human/choice = input(src, "Whom do I want to scratch?") as null|anything in choices
	if(!choice)
		return
	...
	to_chat(choice, "<span class='danger'>Something invisible [method] me!</span>")
	choice.apply_effect(5, PAIN, 0)
	to_chat(src, "<span class='notice'>I [method] \the [choice].</span>")
...
/mob/observer/ghost/proc/chill_someone()
	...
	var/mob/living/carbon/human/choice = input(src, "Whom do I want to scare?") as null|anything in choices
	if(!choice)
		return
	...
	to_chat(choice, "<span class='danger'>I feel as if something cold passed through me!</span>")
	...
	to_chat(src, "<span class='notice'>I pass through \the [choice], giving them a sudden chill.</span>")
...
/mob/observer/ghost/proc/whisper_to_anyone()
	...
	set desc = "Whisper to a human of my choice."
...
/mob/observer/ghost/proc/bloodless_doodle()
	...
	set desc = "Write a short message in blood on the floor or a wall. I don't need blood nearby to use this."
...
/mob/observer/ghost/proc/toggle_visiblity()
	...
	set desc = "Allows I to become visible or invisible at will."
	...
	if(invisibility == 0)
		ghost_magic_cd = world.time + 60 SECONDS
		to_chat(src, "<span class='info'>I am now invisible.</span>")
		visible_message("<span class='emote'>It fades from sight...</span>")
		set_invisibility(INVISIBILITY_OBSERVER)
		mouse_opacity = 1
	else
		ghost_magic_cd = world.time + 60 SECONDS
		to_chat(src, "<span class='info'>I am now visible.</span>")
		set_invisibility(0)
