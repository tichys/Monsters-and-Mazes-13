	...
	for(var/mob/living/carbon/M in oviewers(8, src))
		if(M.stat == CONSCIOUS)
			if(M.status_flags & GODMODE)
				continue
			if(!iscultist(M))
				to_chat(M, "<span class='danger'> I feel my sanity crumble away in an instant as I gaze upon [src.name]...</span>")
...
/obj/singularity/narsie/proc/narsiewall(var/turf/T)
	T.desc = "An opening has been made on that wall, but who can say if what I seek truly lies on the other side?"
...
/obj/singularity/narsie/proc/acquire(const/mob/food)
	...
	if (ishuman(target))
		...
		to_chat(target, "<span class='danger'>[capname] HAS CHOSEN I TO LEAD HIM TO HIS NEXT MEAL.</span>")
