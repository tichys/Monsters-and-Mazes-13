...
/obj/item/weapon/book/tome/attack_self(var/mob/user)
	if(!iscultist(user))
		...
		to_chat(user, "Hold \the [src] in my hand while drawing a rune to use it.")
...
/obj/item/weapon/book/tome/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(A.reagents && A.reagents.has_reagent(/datum/reagent/water/holywater))
		to_chat(user, "<span class='notice'>I unbless \the [A].</span>")
...
/mob/proc/make_rune(var/rune, var/cost = 5, var/tome_required = 0)
	...
	if(istype(get_active_hand(), /obj/item/weapon/book/tome) || istype(get_inactive_hand(), /obj/item/weapon/book/tome))
		...
		to_chat(src, "<span class='warning'>This rune is too complex to draw by memory, I need to have a tome in my hand to draw it.</span>")
		return
	...
	if(T.holy)
		to_chat(src, "<span class='warning'>This place is blessed, I may not draw runes on it - defile it first.</span>")
		return
	if(!istype(T, /turf/simulated))
		to_chat(src, "<span class='warning'>I need more space to draw a rune here.</span>")
		return
	...
	if(has_tome)
		if(has_robes && cult_ground)
			self = "Feeling greatly empowered, I slice open my finger and make a rune on the engraved floor. It shifts when my blood touches it, and starts vibrating as I begin to chant the ritual that binds my life essence with the dark arcane energies flowing through the surrounding world."
			timer = 10
			damage = 0.2
		else if(has_robes)
			self = "Feeling empowered in my robes, I slice open my finger and start drawing a rune, chanting the ritual that binds my life essence with the dark arcane energies flowing through the surrounding world."
			timer = 30
			damage = 0.8
		else if(cult_ground)
			self = "I slice open my finger and slide it over the engraved floor, watching it shift when my blood touches it. It vibrates when I begin to chant the ritual that binds my life essence with the dark arcane energies flowing through the surrounding world." // Sadly, I don't have access to the bell nor the candelbarum
			timer = 20
			damage = 0.8
		else
			self = "I slice open one of my fingers and begin drawing a rune on the floor whilst chanting the ritual that binds my life essence with the dark arcane energies flowing through the surrounding world."
			timer = 40
	else
		self = "Working without my tome, I try to draw the rune from my memory"
		if(has_robes && cult_ground)
			self += ". I feel that I remember it perfectly, finishing it with a few bold strokes. The engraved floor shifts under my touch, and vibrates once I begin my chants."
			timer = 30
		else if(has_robes)
			self += ". I don't remember it well, but I feel strangely empowered. I begin chanting, the unknown words slipping into my mind from beyond."
			timer = 50
		else if(cult_ground)
			self += ", watching as the floor shifts under my touch, correcting the rune. I begin my chants, and the ground starts to vibrate."
			timer = 40
		else
			self += ", having to cut my finger two more times before I make it resemble the pattern in my memory. It still looks a little off."
			timer = 80
			damage = 2
	visible_message("<span class='warning'>\The [src] slices open a finger and begins to chant and paint symbols on the floor.</span>", "<span class='notice'>[self]</span>", "I hear chanting.")
...
/mob/living/carbon/human/make_rune(var/rune, var/cost, var/tome_required)
	if(should_have_organ(BP_HEART) && vessel && !vessel.has_reagent(/datum/reagent/blood, species.blood_volume * 0.7))
		to_chat(src, "<span class='danger'>I am too weak to draw runes.</span>")
		return
	..()
...
/mob/proc/cult_communicate()
	...
	if(incapacitated())
		to_chat(src, "<span class='warning'>Not when I am incapacitated.</span>")
		return
...
/mob/living/carbon/cult_communicate()
	if(incapacitated(INCAPACITATION_RESTRAINED))
		to_chat(src, "<span class='warning'>I need at least my hands free to do this.</span>")
		return
	..()
