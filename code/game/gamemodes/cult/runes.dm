/obj/effect/rune
	...
	var/blood
	var/bcolor
	var/strokes = 2 // IF I EVER SET THIS TO MORE THAN TEN, EVERYTHING WILL BREAK
	var/cultname = ""
...
/obj/effect/rune/attackby(var/obj/item/I, var/mob/living/user)
	if(istype(I, /obj/item/weapon/book/tome) && iscultist(user))
		user.visible_message("<span class='notice'>[user] rubs \the [src] with \the [I], and \the [src] is absorbed by it.</span>", "I retrace my steps, carefully undoing the lines of \the [src].")
		qdel(src)
		return
	else if(istype(I, /obj/item/weapon/nullrod))
		user.visible_message("<span class='notice'>[user] hits \the [src] with \the [I], and it disappears, fizzling.</span>", "<span class='notice'>I disrupt the vile magic with the deadening field of \the [I].</span>", "I hear a fizzle.")
		qdel(src)
		return
...
/obj/effect/rune/attack_hand(var/mob/living/user)
	...
	if(GLOB.cult.powerless)
		to_chat(user, "I read the words, but nothing happens.")
		return fizzle(user)
	cast(user)
...
/obj/effect/rune/proc/fizzle(var/mob/living/user)
	visible_message("<span class='warning'>The markings pulse with a small burst of light, then fall dark.</span>", "I hear a fizzle.")
...
/obj/effect/rune/convert/cast(var/mob/living/user)
	...
	to_chat(target, "<span class='cult'>My blood pulses. My head throbs. The world goes red. All at once I am aware of a horrible, horrible truth. The veil of reality has been ripped away and in the festering wound left behind something sinister takes root.</span>")
	if(!GLOB.cult.can_become_antag(target.mind, 1))
		to_chat(target, "<span class='danger'>Are I going insane?</span>")
	else
		to_chat(target, "<span class='cult'>Do I want to join the cult of Nar'Sie? I can choose to ignore offer... <a href='?src=\ref[src];join=1'>Join the cult</a>.</span>")
		...
		if(!iscultist(target) && target.loc == get_turf(src)) // They hesitated, resisted, or can't join, and they are still on the rune - burn them
			if(target.stat == CONSCIOUS)
				...
				switch(target.getFireLoss())
					if(0 to 25)
						to_chat(target, "<span class='danger'>My blood boils as I force myself to resist the corruption invading every corner of my mind.</span>")
					if(25 to 45)
						to_chat(target, "<span class='danger'>My blood boils and my body burns as the corruption further forces itself into my body and mind.</span>")
						target.take_overall_damage(0, 3)
					if(45 to 75)
						to_chat(target, "<span class='danger'>I begin to hallucinate images of a dark and incomprehensible being and my entire body feels like its engulfed in flame as my mental defenses crumble.</span>")
						target.take_overall_damage(0, 5)
					if(75 to 100)
						to_chat(target, "<span class='cult'>My mind turns to ash as the burning flames engulf my very soul and images of an unspeakable horror begin to bombard the last remnants of mental resistance.</span>")
						target.take_overall_damage(0, 10)
...
/obj/effect/rune/teleport/cast(var/mob/living/user)
	if(user.loc == src)
		...
		if(do_after(user, 30))
			user.visible_message("<span class='warning'>\The [user] disappears in a flash of red light!</span>", "<span class='warning'>I feel as my body gets dragged into the dimension of Nar-Sie!</span>", "I hear a sickening crunch.")
			user.forceMove(src)
			showOptions(user)
			var/warning = 0
			while(user.loc == src)
				...
				if(user.getFireLoss() > 50)
					to_chat(user, "<span class='danger'>My body can't handle the heat anymore!</span>")
					leaveRune(user)
					return
				if(warning == 0)
					to_chat(user, "<span class='warning'>I feel the immerse heat of the realm of Nar-Sie...</span>")
					++warning
				if(warning == 1 && user.getFireLoss() > 15)
					to_chat(user, "<span class='warning'>My burns are getting worse. I should return to my realm soon...</span>")
					++warning
...
/obj/effect/rune/teleport/proc/leaveRune(var/mob/living/user)
	...
	user.visible_message("<span class='warning'>\The [user] appears in a flash of red light!</span>", "<span class='warning'>I feel as my body gets thrown out of the dimension of Nar-Sie!</span>", "I hear a pop.")
...
/obj/effect/rune/tome/cast(var/mob/living/user)
	...
	visible_message("<span class='notice'>\The [src] disappears with a flash of red light, and in its place now a book lies.</span>", "I hear a pop.")
	qdel(src)
...
/obj/effect/rune/wall/cast(var/mob/living/user)
	...
	to_chat(user, "<span class='warning'>My blood flows into the rune, and I feel that the very space over the rune thickens.</span>")
...
/obj/effect/cultwall/attack_hand(var/mob/living/user)
	if(iscultist(user))
		user.visible_message("<span class='notice'>\The [user] touches \the [src], and it fades.</span>", "<span class='notice'>I touch \the [src], whispering the old ritual, making it disappear.</span>")
		qdel(src)
	else
		to_chat(user, "<span class='notice'>I touch \the [src]. It feels wet and becomes harder the further I push my arm.</span>")

/obj/effect/cultwall/attackby(var/obj/item/I, var/mob/living/user)
	if(istype(I, /obj/item/weapon/nullrod))
		user.visible_message("<span class='notice'>\The [user] touches \the [src] with \the [I], and it disappears.</span>", "<span class='notice'>I disrupt the vile magic with the deadening field of \the [I].</span>")
		qdel(src)
	else if(I.force)
		user.visible_message("<span class='notice'>\The [user] hits \the [src] with \the [I].</span>", "<span class='notice'>I hit \the [src] with \the [I].</span>")
		take_damage(I.force)
...
/obj/effect/rune/ajorney/cast(var/mob/living/user)
	...
	user.visible_message("<span class='warning'>\The [user]'s eyes glow blue as \he freezes in place, absolutely motionless.</span>", "<span class='warning'>The shadow that is my spirit separates itself from my body. I am now in the realm beyond. While this is a great sight, being here strains my mind and body. Hurry...</span>", "I hear only complete silence for a moment.")
	announce_ghost_joinleave(user.ghostize(1), 1, "I feel that they had to use some [pick("dark", "black", "blood", "forgotten", "forbidden")] magic to [pick("invade", "disturb", "disrupt", "infest", "taint", "spoil", "blight")] this place!")
...
/obj/effect/rune/defile/cast(var/mob/living/user)
	...
	for(var/turf/T in range(1, src))
		if(T.holy)
			...
			T.cultify()
	visible_message("<span class='warning'>\The [src] embeds into the floor and walls around it, changing them!</span>", "I hear liquid flow.")
	qdel(src)
...
/obj/effect/rune/armor/cast(var/mob/living/user)
	...
	visible_message("<span class='warning'>\The [src] disappears with a flash of red light, and a set of armor appears on \the [user].</span>", "<span class='warning'>I am blinded by the flash of red light. After you're able to see again, I see that I am now wearing a set of armor.</span>")

	var/obj/O = user.get_equipped_item(slot_head) // This will most likely kill I if I am wearing a spacesuit, and it's 100% intended
...
/obj/effect/rune/offering/cast(var/mob/living/user)
	...
	if(victim)
		to_chat(user, "<span class='warning'>I am already sarcificing \the [victim] on this rune.</span>")
		return
	if(cultists.len < 3)
		to_chat(user, "<span class='warning'>I need three cultists around this rune to make it work.</span>")
		return fizzle(user)
	var/turf/T = get_turf(src)
	...
	if(victim && victim.loc == T && victim.stat == DEAD)
		...
		if(victim.mind == GLOB.cult.sacrifice_target)
			for(var/datum/mind/H in GLOB.cult.current_antagonists)
				if(H.current)
					to_chat(H.current, "<span class='cult'>My objective is now complete.</span>")
		//TODO: other rewards?
		...
		to_chat(usr, "<span class='warning'>The Geometer of Blood accepts this sacrifice, my objective is now complete.</span>")
		...
		*/
		to_chat(victim, "<span class='cult'>The Geometer of Blood claims my body.</span>")
		victim.dust()
	if(victim)
...
/obj/effect/rune/drain/cast(var/mob/living/user)
	...
	user.visible_message("<span class='warning'>Blood flows from \the [src] into \the [user]!</span>", "<span class='cult'>The blood starts flowing from \the [src] into my frail mortal body. [capitalize(english_list(heal_user(user), nothing_text = "I feel no different"))].</span>", "I hear liquid flow.")
	user.setClickCooldown(DEFAULT_ATTACK_COOLDOWN)

/obj/effect/rune/drain/proc/heal_user(var/mob/living/carbon/human/user)
	if(!istype(user))
		return list("I feel no different")
	...
	if(charges >= 15)
		for(var/obj/item/organ/external/e in user.organs)
			if(e && e.status & ORGAN_BROKEN)
				...
				statuses += "bones in my [e.name] snap into place"
				charges -= 15
				if(charges < 15)
					break
	if(!charges)
	...
	for(var/obj/item/organ/I in user.internal_organs)
		if(I.damage)
			damaged += I
	if(damaged.len)
		statuses += "I feel pain inside for a moment that passes quickly"
...
/obj/effect/rune/weapon/cast(var/mob/living/user)
	if(!istype(user.get_equipped_item(slot_head), /obj/item/clothing/head/culthood) || !istype(user.get_equipped_item(slot_wear_suit), /obj/item/clothing/suit/cultrobes) || !istype(user.get_equipped_item(slot_shoes), /obj/item/clothing/shoes/cult))
		to_chat(user, "<span class='warning'>I need to be wearing my robes to use this rune.</span>")
		return fizzle(user)
	var/turf/T = get_turf(src)
...
/obj/effect/rune/shell/cast(var/mob/living/user)
	...
	if(!target)
		to_chat(user, "<span class='warning'>I need ten sheets of metal to fold them into a construct shell.</span>")
		return fizzle(user)

	speak_incantation(user, "Da A[pick("'","`")]ig Osk!")
	target.use(10)
	var/obj/O = new /obj/structure/constructshell/cult(get_turf(src))
	visible_message("<span class='warning'>The metal bends into \the [O], and \the [src] imbues into it.</span>", "I hear a metallic sound.")
	qdel(src)
...
/obj/effect/rune/revive/cast(var/mob/living/user)
	...
	target.visible_message("<span class='warning'>\The [target]'s eyes glow with a faint red as \he stands up, slowly starting to breathe again.</span>", "<span class='warning'>Life... I'm alive again...</span>", "I hear liquid flow.")
		...
		for(var/mob/living/carbon/M in viewers(src))
			...
			if(!(M in previous))
				if(M.should_have_organ(BP_HEART))
					to_chat(M, "<span class='danger'>My blood boils!</span>")
				else
					to_chat(M, "<span class='danger'>I feel searing heat inside!</span>")
...
/obj/effect/rune/tearreality/cast(var/mob/living/user)
	if(!GLOB.cult.allow_narsie)
		return
	if(the_end_comes)
		to_chat(user, "<span class='cult'>I am already summoning! Be patient!</span>")
		return
	var/list/mob/living/cultists = get_cultists()
	if(cultists.len < 5)
		return fizzle()
	for(var/mob/living/M in cultists)
		...
		to_chat(M, "<span class='cult'>I am staring to tear the reality to bring Him back... stay around the rune!</span>")
...
/obj/effect/rune/tearreality/attack_hand(var/mob/living/user)
	...
	if(HECOMES && !iscultist(user))
		var/input = input(user, "Are I SURE I want to sacrifice myself?", "DO NOT DO THIS") in list("Yes", "No")
		if(input != "Yes")
			return
		speak_incantation(user, "Uhrast ka'hfa heldsagen ver[pick("'","`")]lot!")
		to_chat(user, "<span class='warning'>In the last moment of my humble life, I feel an immense pain as fabric of reality mends... with my blood.</span>")
		for(var/mob/M in GLOB.living_mob_list_)
			if(iscultist(M))
				to_chat(M, "I see a vision of \the [user] keeling over dead, his blood glowing blue as it escapes \his body and dissipates into thin air; I hear an otherwordly scream and feel that a great disaster has just been averted.")
			else
				to_chat(M, "I see a vision of [name] keeling over dead, his blood glowing blue as it escapes his body and dissipates into thin air; I hear an otherwordly scream and feel very weak for a moment.")
...
/obj/effect/rune/imbue
	cultname = "otherwordly abomination that shouldn't exist and that I should report to my local god as soon as I see it, along with the instructions for making this"
	var/papertype
