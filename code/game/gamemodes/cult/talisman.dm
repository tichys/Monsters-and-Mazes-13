...
/obj/item/weapon/paper/talisman/attack_self(var/mob/living/user)
	if(iscultist(user))
		to_chat(user, "Attack my target to use this talisman.")
	else
...
/obj/item/weapon/paper/talisman/emp/afterattack(var/atom/target, var/mob/user, var/proximity)
	...
	user.visible_message("<span class='danger'>\The [user] invokes \the [src] at [target].</span>", "<span class='danger'>I invoke \the [src] at [target].</span>")
	target.emp_act(1)
	qdel(src)
