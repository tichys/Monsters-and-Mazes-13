...
/datum/universal_state/bluespace_jump/OnEnter()
	...
	for(var/mob/goast in GLOB.ghost_mob_list)
		goast.mouse_opacity = 0	//can't let I click that Dave
...
/datum/universal_state/bluespace_jump/OnTouchMapEdge(var/atom/A)
	if((A.z in affected_levels) && (A in bluespaced))
		if(ismob(A))
			to_chat(A,"<span class='warning'>I drift away into the shifting expanse, never to be seen again.</span>")
...
/datum/universal_state/bluespace_jump/proc/apply_bluespaced(var/mob/living/M)
	bluespaced += M
	if(M.client)
		to_chat(M,"<span class='notice'>I feel oddly light, and somewhat disoriented as everything around me shimmers and warps ever so slightly.</span>")
...
/datum/universal_state/bluespace_jump/proc/clear_bluespaced(var/mob/living/M)
	if(M.client)
		to_chat(M,"<span class='notice'>I feel rooted in material world again.</span>")
...
/obj/effect/bluegoast/proc/mirror(var/atom/movable/am, var/old_loc, var/new_loc)
	...
	if(nloc == new_loc)
		reality++
		if(reality > 5)
			to_chat(daddy, "<span class='notice'>Yep, it's certainly the other one. My existance was a glitch, and it's finally being mended...</span>")
			blueswitch()
		else if(reality > 3)
			to_chat(daddy, "<span class='danger'>Something is definitely wrong. Why do I think I am the original?</span>")
		else
			to_chat(daddy, "<span class='warning'>I feel a bit less real. Which one of I two was original again?..</span>")
