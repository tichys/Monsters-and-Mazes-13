...
/datum/controller/gameticker/proc/pregame()
	do
		...
		to_world("Please, setup my character and select ready. Game will start in [pregame_timeleft] seconds")
...
/datum/controller/gameticker/proc/declare_completion()
	...
	for(var/mob/Player in GLOB.player_list)
		if(Player.mind && !isnewplayer(Player))
			if(Player.stat < DEAD)
				to_chat(Player, "<font color='green'>I survived the events on Genesis.</font>")
			else
				if(isobserver(Player))
					var/mob/observer/ghost/O = Player
					if(!O.started_as_observer)
						to_chat(Player, "<font color='red'><b>I did not survive the events on Genesis...</b></font>")
				else
					to_chat(Player, "<font color='red'><b>I did not survive the events on Genesis...</b></font>")
	to_world("<br>")
...
/datum/controller/gameticker/proc/attempt_late_antag_spawn(var/list/antag_choices)
	...
	while(antag_choices.len && antag)
		...
		if (needs_ghost)
			...
			antag_pool.Cut()
			to_world("<b>A ghost is needed to spawn \a [antag.role_text].</b>\nGhosts may enter the antag pool by making sure their [antag.role_text] preference is set to high, then using the toggle-add-antag-candidacy verb. I have 3 minutes to enter the pool.")
