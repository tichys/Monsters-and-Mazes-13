/obj/structure/deity/blood_forge
	...
	var/text_modifications = list("Cost" = "Blood",
								"Dip" = "fire. Pain envelopes I as blood seeps out of my hands and I begin to shape it into something more useful",
								"Shape" = "I shape the fire as more and more blood comes out.",
								"Out" = "flames")
...
/obj/structure/deity/blood_forge/proc/craft_item(var/path, var/blood_cost, var/mob/user)
	...
	busy = 1
	to_chat(user, "<span class='notice'>I dip my hands into \the [src]'s [text_modifications["Dip"]]</span>")
	...
	user.visible_message("\The [user] pull out \the [I] from the [text_modifications["Out"]].", "I pull out the completed [I] from the [text_modifications["Out"]].")
	busy = 0
...
/obj/structure/deity/blood_stone/attack_hand(var/mob/user)
	...
	user.visible_message("<span class='warning'>\The [user] calmly slices their finger on \the [src], smeering it over the black stone.</span>","<span class='warning'>I slowly slide my finger down one of \the [src]'s sharp edges, smeering it over its smooth surface.</span>")
	while(do_after(H,50,src))
		user.audible_message("\The [user] utters something under their breath.", "<span class='cult'>I mutter a dark prayer to my master as I feel the stone eat away at my lifeforce.</span>")
