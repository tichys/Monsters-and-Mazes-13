...
/obj/structure/deity/gateway/Process()
	...
	if(target)
		if(get_turf(target) != get_turf(src))
			...
			to_chat(target,"<span class='danger'>\The [src] sucks at my lifeforce!</span>")
	if(start_time && world.time > start_time + 3000)
		start_time = 0
		to_chat(target,"<span class='danger'>I have been sucked into \the [src], my soul used to fuel \the [linked_god]'s minions.</span>")
		target.dust()
		active_souls++
		...
		if(T)
			...
			to_chat(T, "<span class='danger'>I feel my lifeforce begin to drain into \the [src]!</span>")
...
/obj/structure/deity/radiant_statue/attack_deity(var/mob/living/deity/deity)
	if(activate_charging())
		to_chat(deity,"<span class='notice'>I activate \the [src], and it begins to charge as long as at least one of my followers is nearby.</span>")
	else
	...
	if(charging)
		...
		if(followers.len)
			for(var/m in followers)
				...
				L.adjustFireLoss(-5)
				if(prob(50))
					to_chat(L, "<span class='notice'>I feel a pleasant warmth spread throughout my body...</span>")
...
/obj/structure/deity/blood_forge/starlight
	...
	text_modifications = list("Cost" = "Burn",
								"Dip" = "fire. Pain envelopes I as dark burns mar my hands and I begin to shape it into something more useful",
								"Shape" = "I shape the fire, ignoring the painful burns it gives I in the process.",
								"Out" = "flames")
