...
/obj/structure/deity/altar/Destroy()
	if(target)
		remove_target()
	if(linked_god)
		to_chat(src, "<span class='danger'>I have lost an altar!</span>")
	return ..()
...
/obj/structure/deity/altar/Process()
	...
	if(!cycles_before_converted)
		...
		GLOB.godcult.add_antagonist_mind(target.mind,1, "Servant of [linked_god]","My loyalty may be faulty, but I know that it now has control over you...", specific_god=linked_god)
		remove_target()
		return
	...
	switch(cycles_before_converted)
		...
		if(3)
			text = "I feel like my thought are being overriden..."
		if(2)
			text = "I can't.... concentrate.. must... resist!"
		if(1)
...
/obj/structure/deity/altar/OnTopic(var/user, var/list/href_list)
	if(href_list["resist"])
		...
		M.visible_message("<span class='warning'>\The [M] writhes on top of \the [src]!</span>", "<span class='notice'>I struggle against the intruding thoughts, keeping them at bay!</span>")
		to_chat(linked_god, "<span class='warning'>\The [M] slows its conversion through willpower!</span>")
		cycles_before_converted++
		if(prob(50))
			to_chat(M, "<span class='danger'>The mental strain is too much for me! I feel my body weakening!</span>")
			M.adjustToxLoss(15)
			M.adjustHalLoss(30)
