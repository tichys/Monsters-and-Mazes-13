...
/obj/structure/deity/pylon/proc/add_intuned(var/mob/living/L)
	if(L in intuned)
		return
	to_chat(L, "<span class='notice'>I place my hands on \the [src], feeling myself intune to its vibrations.</span>")
	intuned += L
...
/obj/structure/deity/pylon/proc/remove_intuned(var/mob/living/L)
	if(!(L in intuned))
		return
	to_chat(L, "<span class='warning'>I no longer feel intuned to \the [src].</span>")
	intuned -= L
