/proc/valid_deity_structure_spot(var/type, var/turf/target, var/mob/living/deity/deity, var/mob/living/user)
	...
	if(flags & DEITY_STRUCTURE_NEAR_IMPORTANT && !deity.near_structure(target))
		if(user)
			to_chat(user, "<span class='warning'>I need to be near \a [deity.get_type_name(/obj/structure/deity/altar)] to build this!</span>")
		return 0
	...
	if(flags & DEITY_STRUCTURE_ALONE)
		for(var/structure in deity.structures)
			if(istype(structure,type) && get_dist(target,structure) <= 3)
				if(user)
					to_chat(user, "<span class='warning'>I am too close to another [deity.get_type_name(type)]!</span>")
				return 0
	return 1
...
/obj/structure/deity/attackby(obj/item/W as obj, mob/user as mob)
	...
	user.visible_message(
		...
		"<span class='danger'>I hit \the [src] with \the [W]!</span>",
		"<span class='danger'>I hear something breaking!</span>"
		)
	take_damage(W.force)
