...
/datum/game_mode/malfunction/verb/ai_self_destruct()
	...
	set desc = "Activates or deactivates self destruct sequence of my physical mainframe."
...
/datum/game_mode/malfunction/verb/ai_toggle_apu()
	...
	set desc = "Activates or deactivates my APU generator, allowing I to operate even without power."
	var/mob/living/silicon/ai/user = usr
...
/datum/game_mode/malfunction/verb/boost_research()
	...
	set desc = "Uses my special hardware piece to instantly advance all research by one level."
	var/mob/living/silicon/ai/user = usr
	...
	if(HW.spent)
		to_chat(user, "I attempt to activate my hardware piece, but it does not work. It must be damaged.")
		return

	var/choice = alert("Really activate my hardware? It will advance my research by one tier, but may only be used once.", "Rapid System Upgrade", "YES", "NO")
	if(choice != "YES")
		return
	...
	to_chat(user, "I activate my hardware piece. I have advanced research in all ability trees by one.")


/datum/game_mode/malfunction/verb/ai_destroy_station()
	set category = "Hardware"
	set name = "Destroy Installation"
	set desc = "Activates or deactivates self destruct sequence of this installation. Sequence takes two minutes, and if I am shut down before timer reaches zero it will be cancelled."
	var/mob/living/silicon/ai/user = usr
	...
	if(user.system_override != 2)
		to_chat(user, "I do not have access to self-destruct system.")
		return
