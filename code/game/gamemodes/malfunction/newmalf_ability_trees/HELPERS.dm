...
/datum/game_mode/malfunction/verb/ai_select_hardware()
	...
	set desc = "Allows I to select hardware piece to install"
	...
	if(user.hardware)
		to_chat(user, "I have already selected my hardware.")
		return
	...
	var/choice = input("Select desired hardware. I may only choose one hardware piece!: ") in possible_choices
	if(choice == "CANCEL")
		return
	var/note = null
	...
	var/confirmation = alert("[note] - Is this what I want?", "Hardware selection", "Yes", "No")
	if(confirmation != "Yes")
...
/datum/game_mode/malfunction/verb/ai_select_research()
	...
	set desc = "Allows I to select my next research target."
	var/mob/living/silicon/ai/user = usr
	...
	var/datum/malf_research_ability/tar = input("Select my next research target") in res.available_abilities
	if(!tar)
		return
	res.focus = tar
...
/proc/ability_prechecks(var/mob/living/silicon/ai/user = null, var/check_price = 0, var/override = 0)
	if(!user)
		return 0
	if(!istype(user))
		to_chat(user, "GAME ERROR: I tried to use ability that is only available for malfunctioning AIs, but I am not AI! Please report this.")
		return 0
	if(!user.malfunctioning)
		to_chat(user, "GAME ERROR: I tried to use ability that is only available for malfunctioning AIs, but I am not malfunctioning. Please report this.")
		return 0
	if(!user.research)
		to_chat(user, "GAME ERROR: No research datum detected. Please report this.")
		return 0
	if(user.research.max_cpu < check_price)
		to_chat(user, "My CPU storage is not large enough to use this ability. Hack more APCs to continue.")
		return 0
	if(user.research.stored_cpu < check_price)
		to_chat(user, "I do not have enough CPU power stored. Please wait a moment.")
		return 0
	if(user.hacking && !override)
		to_chat(user, "My system is busy processing another task. Please wait until completion.")
		return 0
...
/proc/announce_hack_failure(var/mob/living/silicon/ai/user = null, var/text)
	...
	switch(user.hack_fails)
		if(1)
			fulltext = "We have detected a hack attempt into my [text]. The intruder failed to access anything of importance, but disconnected before we could complete our traces."
		if(2)
		...
		if(4)
			fulltext = "We have finished our traces and it seems the recent hack attempts are originating from my AI system [user.name]. We recommend investigation."
		else
			fulltext = "Another hack attempt has been detected, targeting [text]. The source still seems to be my AI system [user.name]."
