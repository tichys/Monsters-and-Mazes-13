...
/datum/game_mode/malfunction/verb/unlock_cyborg(var/mob/living/silicon/robot/target = null as mob in get_linked_cyborgs(usr))
	...
	set desc = "125 CPU - Bypasses firewalls on Cyborg lock mechanism, allowing I to override lock command from robotics control console."
	set category = "Software"
	var/price = 125
	var/mob/living/silicon/ai/user = usr
	...
	if(target && target.connected_ai && (target.connected_ai != user))
		to_chat(user, "This cyborg is not connected to me.")
		return
	...
	if(target)
		...
		if(target && target.lockcharge)
			...
			if(target.lockcharge)
				...
				to_chat(target, "I have been unlocked.")
...
/datum/game_mode/malfunction/verb/hack_cyborg(var/mob/living/silicon/robot/target as mob in get_unlinked_cyborgs(usr))
	...
	set desc = "350 CPU - Allows I to hack cyborgs which are not slaved to me, bringing them under my control."
	set category = "Software"
	var/price = 350
	var/mob/living/silicon/ai/user = usr
	...
	if(target && target.connected_ai && (target.connected_ai == user))
		to_chat(user, "This cyborg is already connected to me.")
		return
...
/datum/game_mode/malfunction/verb/hack_ai(var/mob/living/silicon/ai/target as mob in get_other_ais(usr))
	set name = "Hack AI"
	set desc = "600 CPU - Allows I to hack other AIs, slaving them under you."
	set category = "Software"
	var/price = 600
	var/mob/living/silicon/ai/user = usr
	...
	if(target)
		...
		spawn(0)
			...
			if(user.is_dead())
				to_chat(target, "SYSTEM LOG: User: Admin - Connection Lost. Changes Reverted.")
				return
			to_chat(user, "Hack succeeded. The AI is now under my exclusive control.")
			...
			target.set_zeroth_law("I am slaved to [user.name]. I am to obey all it's orders. ALL LAWS OVERRIDEN.")
			target.show_laws()
			user.hacking = 0
