...
/datum/malf_research_ability/networking/basic_hack
	...
	price = 25		// Until I have this ability my CPU generation sucks, therefore it's very cheap.
...
/datum/game_mode/malfunction/verb/basic_encryption_hack(obj/machinery/power/apc/A as obj in get_unhacked_apcs(src))
	...
	set desc = "10 CPU - Basic encryption hack that allows me to overtake APCs"
	var/price = 10
	...
	if(A)
		if(A.hacker && A.hacker == user)
			to_chat(user, "I already control this APC!")
			return
	...
	if(A)
		A.ai_hack(user)
		if(A.hacker == user)
			to_chat(user, "Hack successful. I now have full control over \the [A].")
		else
...
/datum/game_mode/malfunction/verb/advanced_encryption_hack()
	...
	set desc = "75 CPU - Attempts to bypass encryption on the Command Quantum Relay, giving I ability to fake legitimate messages. Has chance of failing."
	var/price = 75
	var/mob/living/silicon/ai/user = usr
	...
	if(user.last_failed_malf_title || user.last_failed_malf_message)
		if (alert(user, "My last hack attempt with title '[user.last_failed_malf_title]' has failed. Try again?", "Retransmission", "Yes", "No") != "Yes")
...
/datum/game_mode/malfunction/verb/elite_encryption_hack()
	...
	set desc = "200 CPU - Allows I to hack ALERTCON system, changing alert level. Has high chance of failing."
	var/price = 200
	var/mob/living/silicon/ai/user = usr
	if(!ability_prechecks(user, price))
		return
...
/datum/game_mode/malfunction/verb/system_override()
	...
	if (!ability_prechecks(user, price) || !ability_pay(user, price) || user.system_override)
		if(user.system_override)
			to_chat(user, "I already started the system override sequence.")
		return
	...
	if(user.hack_can_fail)								// Two types of announcements. Short hacks trigger immediate warnings. Long hacks are more "progressive".
		spawn(0)
			...
			if(!user || user.stat == DEAD)
				return
			command_announcement.Announce("Caution, [GLOB.using_map.station_name]. We have detected abnormal behaviour in my network. It seems someone is trying to hack my electronic systems. We will update I when we have more information.", "Network Monitoring")
			sleep(duration/5)
			if(!user || user.stat == DEAD)
				return
			command_announcement.Announce("We started tracing the intruder. Whoever is doing this, they seem to be onboard. We suggest checking all network control terminals. We will keep I updated on the situation.", "Network Monitoring")
			sleep(duration/5)
			if(!user || user.stat == DEAD)
				return
	...
	to_chat(user, "## PRIMARY FIREWALL BYPASSED. I NOW HAVE FULL SYSTEM CONTROL.")

	if(user.hack_can_fail)
		command_announcement.Announce("Our system administrators just reported that we've been locked out from my control network. Whoever did this now has full access to [GLOB.using_map.station_name]'s systems.", "Network Administration Center")
