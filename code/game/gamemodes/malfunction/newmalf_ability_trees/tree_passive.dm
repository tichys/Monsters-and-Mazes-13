...
/datum/malf_research_ability/passive/relay_suppression/research_finished(var/mob/living/silicon/ai/user)
	...
	if(!user)
		return
	to_chat(user, "<span class='notice'>I have suppressed the IDS system of nearby quantum relay. My hacks will no longer be prevented or detected.</span>")
...
/datum/malf_research_ability/passive/relay_override/research_finished(var/mob/living/silicon/ai/user)
	...
	if(!user)
		return
	to_chat(user, "<span class='notice'>I have completely overtaken a nearby quantum relay. No remote communications will work.</span>")
...
/datum/game_mode/malfunction/verb/intellicard_interception()
	...
	set desc = "Free - Toggles my intellicard interface on or off."
	...
	to_chat(user, "My intellicard transfer interface is now [user.uncardable ? "disabled" : "enabled"].")
...
/datum/game_mode/malfunction/verb/subtle_algorithms()
	...
	to_chat(user, "I [user.hacked_apcs_hidden ? "enable" : "disable"] subtle algorithms on all of my hacked APCs.")
