...
datum/objective/silence
	explanation_text = "Do not allow anyone to escape.  Only allow the shuttle to be called when everyone is dead and my story is the only one left."
...
datum/objective/harm
	...
	find_target()
		..()
		if(target && target.current)
			explanation_text = "Make an example of [target.current.real_name], the [target.assigned_role]. Break one of their bones, detach one of their limbs or disfigure their face. Make sure they're alive when I do it."
		else
			explanation_text = "Free Objective"
		return target
	...
	find_target_by_role(role, role_type=0)
		..(role, role_type)
		if(target && target.current)
			explanation_text = "Make an example of [target.current.real_name], the [!role_type ? target.assigned_role : target.special_role]. Break one of their bones, detach one of their limbs or disfigure their face. Make sure they're alive when I do it."
		else
			explanation_text = "Free Objective"
		return target
...
/datum/objective/ninja_highlander
   explanation_text = "I aspire to be a Grand Master of the Spider Clan. Kill all of my fellow acolytes."
...
/datum/objective/cult/sacrifice/find_target()
	...
	if(possible_targets.len > 0)
		target = pick(possible_targets)
	if(target) explanation_text = "Sacrifice [target.name], the [target.assigned_role]. I will need the sacrifice rune (Hell blood join) and three acolytes to do so."
