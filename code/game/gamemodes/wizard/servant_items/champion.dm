/obj/item/clothing/head/champhelm
	...
	desc = "A spiky, golden crown. It's probably worth more than my bank account."
...
/obj/item/clothing/shoes/jackboots/medievalboots
	name = "leather boots"
	desc = "Old-fashioned leather boots. Probably not something I want to get kicked with."
...
/obj/item/weapon/excalibur/pickup(var/mob/living/user as mob)
	if(user.mind)
		if(!GLOB.wizards.is_antagonist(user.mind) || user.mind.special_role != ANTAG_SERVANT)
			...
			to_chat(user,"<span class='danger'>\The [src] heats up in my hands, burning me!</span>")
...
/obj/item/weapon/excalibur/Process()
	if(istype(loc, /mob/living))
		if(istype(loc, /mob/living/carbon/human))
			...
			M.adjustFireLoss(2)
		if(prob(2))
			to_chat(loc,"<span class='danger'>\The [src] is burning me!</span>")
	return 1
