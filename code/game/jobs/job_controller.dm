...
/datum/controller/occupations
	...
	proc/CheckLatejoinBlockers(var/mob/new_player/joining, var/datum/job/job)
		...
		if(job.minimum_character_age && (joining.client.prefs.age < job.minimum_character_age))
			to_chat(joining, "<span class='warning'>My character's in-game age is too low for this job.</span>")
			return FALSE
		if(!job.player_old_enough(joining.client))
			to_chat(joining, "<span class='warning'>My player age (days since first seen on the server) is too low for this job.</span>")
			return FALSE
	...
	proc/CheckUnsafeSpawn(var/mob/living/spawner, var/turf/spawn_turf)
		...
		if(airstatus || radlevel > 0)
			var/reply = alert(spawner, "Warning. My selected spawn location seems to have unfavorable conditions. \
			I may die shortly after spawning. \
	...
	proc/GiveRandomJob(var/mob/new_player/player)
		...
		for(var/datum/job/job in shuffle(occupations))
			...
			if(job.title in GLOB.command_positions) //If I want a command position, select it!
				continue
		...
		for(var/level = 1 to 3)
			for(var/command_position in GLOB.command_positions)
				...
				for(var/mob/V in candidates)
					// Log-out during round-start? What a bad boy, no head position for me!
 ...
 **/
	proc/DivideOccupations()
		...
		// For those who wanted to be assistant if their preferences were filled, here I go.
		for(var/mob/new_player/player in unassigned)
		...
		if(H.client.prefs.Gear() && job.loadout_allowed)
			for(var/thing in H.client.prefs.Gear())
				...
				if(G)
					...
					if(!permitted)
						to_chat(H, "<span class='warning'>My current species, job, or branch status does not permit me to spawn with [thing]!</span>")
						continue
	...
	proc/EquipRank(var/mob/living/carbon/human/H, var/rank, var/joined_late = 0)
		...
		if(H.mind && job.head_position)
			...
			if(department_account)
				...
				remembered_info += "<b>My department's account funds are:</b> T[department_account.money]<br>"
		...
		to_chat(H, "<B>I am [job.total_positions == 1 ? "the" : "a"] [alt_title ? alt_title : rank].</B>")

		if(job.supervisors)
			to_chat(H, "<b>As the [alt_title ? alt_title : rank] I answer directly to [job.supervisors]. Special circumstances may change this.</b>")

		to_chat(H, "<b>To speak on my department's radio channel use :h. For the use of other channels, I can examine my headset.</b>")
		
		if(job.objective)
			to_chat(H, "\n<b>My Objective: </b>[job.objective]\n")
...
/datum/controller/occupations/proc/get_spawnpoint_for(var/client/C, var/rank)
	...
	if(spawnpoint)
		if(!(spawnpoint in GLOB.using_map.allowed_spawns))
			if(H)
				to_chat(H, "<span class='warning'>My chosen spawnpoint ([C.prefs.spawnpoint]) is unavailable for the current map. Spawning I at one of the enabled spawn points instead. To resolve this error head to my character's setup and choose a different spawn point.</span>")
			spawnpos = null
		else
	...
	if(spawnpos && !spawnpos.check_job_spawning(rank))
		if(H)
			to_chat(H, "<span class='warning'>My chosen spawnpoint ([spawnpos.display_name]) is unavailable for my chosen job ([rank]). Spawning I at another spawn point instead.</span>")
		spawnpos = null
