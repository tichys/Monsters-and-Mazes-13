...
/obj/machinery/sleeper/CanUseTopic(user)
	if(user == occupant)
		to_chat(usr, "<span class='warning'>I can't reach the controls from the inside.</span>")
		return STATUS_CLOSE
	return ..()
...
/obj/machinery/sleeper/OnTopic(user, href_list)
	...
	if(href_list["chemical"] && href_list["amount"])
		if(occupant && occupant.stat < DEAD)
			if(href_list["chemical"] in available_chemicals) // My hacks are bad and I should feel bad
	...
	if(istype(I, /obj/item/weapon/reagent_containers/glass))
		...
		if(!beaker)
			if(!user.unEquip(I, src))
				return
			beaker = I
			user.visible_message("<span class='notice'>\The [user] adds \a [I] to \the [src].</span>", "<span class='notice'>I add \a [I] to \the [src].</span>")
		else
...
/obj/machinery/sleeper/proc/toggle_filter()
	if(!occupant || !beaker)
		filtering = 0
		return
	to_chat(occupant, "<span class='warning'>I feel like my blood is being sucked away.</span>")
	filtering = !filtering
...
/obj/machinery/sleeper/proc/toggle_pump()
	if(!occupant || !beaker)
		pump = 0
		return
	to_chat(occupant, "<span class='warning'>I feel a tube jammed down my throat.</span>")
	pump = !pump
