...
/obj/machinery/alarm/OnTopic(user, href_list, var/datum/topic_state/state)
	...
	if(href_list["temperature"])
		...
		var/input_temperature = input(user, "What temperature would I like the system to mantain? (Capped between [min_temperature] and [max_temperature]C)", "Thermostat Controls", target_temperature - T0C) as num|null
		...
		if(href_list["command"])
			...
			switch(href_list["command"])
				if("set_external_pressure")
					var/input_pressure = input(user, "What pressure I like the system to mantain?", "Pressure Controls") as num|null
...
//				to_chat(user, "I pop the Air Alarm's maintence panel open.")
			...
			if (wiresexposed && isWirecutter(W))
				user.visible_message("<span class='warning'>[user] has cut the wires inside \the [src]!</span>", "I have cut the wires inside \the [src].")
		...
		if(1)
			if(isCoil(W))
				...
				if (C.use(5))
					...
					to_chat(user, "<span class='warning'>I need 5 pieces of cable to do wire \the [src].</span>")
					return
		...
		switch(buildstage)
			if(2)
				if(isMultitool(W))
					...
					if (src.detecting)
						user.visible_message("<span class='notice'>\The [user] has reconnected [src]'s detecting unit!</span>", "<span class='notice'>I have reconnected [src]'s detecting unit.</span>")
					else
						user.visible_message("<span class='notice'>\The [user] has disconnected [src]'s detecting unit!</span>", "<span class='notice'>I have disconnected [src]'s detecting unit.</span>")
				else if(isWirecutter(W))
					user.visible_message("<span class='notice'>\The [user] has cut the wires inside \the [src]!</span>", "<span class='notice'>I have cut the wires inside \the [src].</span>")
					new/obj/item/stack/cable_coil(get_turf(src), 5)
					playsound(src.loc, 'sound/items/Wirecutter.ogg', 50, 1)
					buildstage = 1
					update_icon()
			if(1)
				if(istype(W, /obj/item/stack/cable_coil))
					...
					if (C.use(5))
						...
						to_chat(user, "<span class='warning'>I need 5 pieces of cable to wire \the [src].</span>")
						return
				else if(isCrowbar(W))
					to_chat(user, "I pry out the circuit!")
					playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
