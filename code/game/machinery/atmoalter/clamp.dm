...
/obj/machinery/clamp/attack_hand(var/mob/user)
	...
	if(!open)
		...
		close()
	to_chat(user, "<span class='notice'>I turn [open ? "off" : "on"] \the [src]</span>")
...
/obj/machinery/clamp/MouseDrop(obj/over_object as obj)
	...
	if(open && over_object == usr && Adjacent(usr))
		to_chat(usr, "<span class='notice'>I begin to remove \the [src]...</span>")
		if (do_after(usr, 30, src))
			to_chat(usr, "<span class='notice'>I have removed \the [src].</span>")
		...
		to_chat(usr, "<span class='warning'>I can't remove \the [src] while it's active!</span>")
...
/obj/item/clamp/afterattack(var/atom/A, mob/user as mob, proximity)
	...
	if (istype(A, /obj/machinery/atmospherics/pipe/simple))
		to_chat(user, "<span class='notice'>I begin to attach \the [src] to \the [A]...</span>")
		if (do_after(user, 30, src))
			if(!user.unEquip(src))
				return
			to_chat(user, "<span class='notice'>I have attached \the [src] to \the [A].</span>")
