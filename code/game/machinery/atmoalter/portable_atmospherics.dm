...
/obj/machinery/portable_atmospherics/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	if ((istype(W, /obj/item/weapon/tank) && !( src.destroyed )))
		...
		if(connected_port)
			disconnect()
			to_chat(user, "<span class='notice'>I disconnect \the [src] from the port.</span>")
			update_icon()
			return
		else
			...
			if(possible_port)
				if(connect(possible_port))
					to_chat(user, "<span class='notice'>I connect \the [src] to the port.</span>")
					update_icon()
					return
				else
...
/obj/machinery/portable_atmospherics/powered/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/cell))
		...
		if(!user.unEquip(I, src))
			return
		cell = I
		user.visible_message("<span class='notice'>[user] opens the panel on \the [src] and inserts \the [I].</span>", "<span class='notice'>I open the panel on \the [src] and insert \the [I].</span>")
		power_change()
		return
	...
	if(isScrewdriver(I))
		...
		user.visible_message("<span class='notice'>[user] opens the panel on \the [src] and removes \the [cell].</span>", "<span class='notice'>I open the panel on \the [src] and remove \the [cell].</span>")
