	...
	if(istype(O, /obj/item/weapon/reagent_containers/glass))
		...
		for(var/obj/item/weapon/reagent_containers/food/snacks/grown/G in P.contents)
			...
			if(ingredients >= capacity)
				to_chat(user, "<span class='notice'>I fill \the [src] to its capacity.</span>")
				break
		P.finish_bulk_removal() //Now do the UI stuff once.
		if(!hadPlants)
			to_chat(user, "<span class='notice'>\The [P] has no produce inside.</span>")
		else if(ingredients < capacity)
			to_chat(user, "<span class='notice'>I empty \the [P] into \the [src].</span>")


	else if(!istype(O, /obj/item/weapon/reagent_containers/food/snacks/grown))
		to_chat(user, "<span class='notice'>I cannot put this in \the [src].</span>")
	else if(user.unEquip(O, src))
		ingredients++
		to_chat(user, "<span class='notice'>I put \the [O] in \the [src]</span>")
	update_icon()
