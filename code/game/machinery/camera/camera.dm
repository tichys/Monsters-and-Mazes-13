...
//		to_chat(user, "<span class='notice'>I start to [panel_open ? "close" : "open"] the camera's panel.</span>")
		...
		"<span class='notice'>I screw the camera's panel [panel_open ? "open" : "closed"].</span>")
		...
		if(weld(W, user))
			if(assembly)
				...
				if(stat & BROKEN)
					assembly.state = 2
					to_chat(user, "<span class='notice'>I repaired \the [src] frame.</span>")
					cancelCameraAlarm()
				else
					assembly.state = 1
					to_chat(user, "<span class='notice'>I cut \the [src] free from the wall.</span>")
	...
	// OTHER
		...
		var/itemname = X.name
		var/info = X.info
		to_chat(U, "I hold \a [itemname] up to the camera ...")
		for(var/mob/living/silicon/ai/O in GLOB.living_mob_list_)
			if(!O.client) continue
			if(U.name == "Unknown") to_chat(O, "<b>[U]</b> holds \a [itemname] up to one of my cameras ...")
			else to_chat(O, "<b><a href='byond://?src=\ref[O];track2=\ref[O];track=\ref[U];trackname=[U.name]'>[U]</a></b> holds \a [itemname] up to one of my cameras ...")
