...
/mob/living/silicon/ai/proc/ai_store_location(loc as text)
	...
	set desc = "Stores my current camera location by the given name"
...
/mob/living/silicon/ai/proc/ai_camera_track(var/target_name in trackable_mobs())
	...
	set desc = "Select who I would like to track."

	if(src.stat == 2)
		to_chat(src, "I can't follow [target_name] with cameras because I am dead!")
		return
	if(!target_name)
