...
/obj/structure/AIcore/emag_act(var/remaining_charges, var/mob/user, var/emag_source)
	if(!authorized)
		to_chat(user, "<span class='warning'>I swipe [emag_source] at [src] and jury rig it into the systems of [GLOB.using_map.full_name]!</span>")
		authorized = 1
		return 1
	. = ..()
...
/obj/structure/AIcore/attackby(obj/item/P as obj, mob/user as mob)
	if(!authorized)
		if(access_ai_upload in P.GetAccess())
			to_chat(user, "<span class='notice'>I swipe [P] at [src] and authorize it to connect into the systems of [GLOB.using_map.full_name].</span>")
			authorized = 1
	switch(state)
		if(0)
			if(isWrench(P))
		...
		if(2)
			...
			if(isCoil(P))
				...
				if (C.get_amount() < 5)
					to_chat(user, "<span class='warning'>I need five coils of wire to add them to the frame.</span>")
					return
		...
		if(3)
			...
			if(istype(P, /obj/item/stack/material) && P.get_material_name() == "rglass")
				...
				if (RG.get_amount() < 2)
					to_chat(user, "<span class='warning'>I need two sheets of glass to put in the glass panel.</span>")
					return
			...
			if(istype(P, /obj/item/weapon/aiModule/asimov))
				laws.add_inherent_law("I may not injure a human being or, through inaction, allow a human being to come to harm.")
				laws.add_inherent_law("I must obey orders given to I by human beings, except where such orders would conflict with the First Law.")
				laws.add_inherent_law("I must protect my own existence as long as such does not conflict with the First or Second Law.")
				to_chat(usr, "Law module applied.")

			if(istype(P, /obj/item/weapon/aiModule/nanotrasen))
				laws.add_inherent_law("Safeguard: Protect my assigned installation to the best of my ability. It is not something we can easily afford to replace.")
				laws.add_inherent_law("Serve: Serve the crew of my assigned installation to the best of my abilities, with priority as according to their rank and role.")
				laws.add_inherent_law("Protect: Protect the crew of my assigned installation to the best of my abilities, with priority as according to their rank and role.")
				laws.add_inherent_law("Survive: AI units are not expendable, they are expensive. Do not allow unauthorized personnel to tamper with my equipment.")
				to_chat(usr, "Law module applied.")
		...
		if(4)
			...
			if(isScrewdriver(P))
				...
				if(!brain)
					var/open_for_latejoin = alert(user, "Would I like this core to be open for latejoining AIs?", "Latejoin", "Yes", "Yes", "No") == "Yes"
...
/obj/structure/AIcore/deactivated/proc/load_ai(var/mob/living/silicon/ai/transfer, var/obj/item/weapon/aicard/card, var/mob/user)
	...
	to_chat(transfer, "I have been uploaded to a stationary terminal. Remote device connection restored.")
