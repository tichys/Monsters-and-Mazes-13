...
/obj/machinery/computer/arcade/orion_trail
	...
	var/list/stopblurbs = list(
		...
		"I have made it to Orion! Congratulations! My crew is one of the few to start a new foothold for mankind!"
		)
	...
	switch(view)
		if(ORION_VIEW_MAIN)
			if(event == ORION_TRAIL_START) //new game? New game.
				...
				event_title = event
				event_actions = "<a href='?src=\ref[src];continue=1'>Continue my journey</a><br>"
			switch(event)
				...
				if(ORION_TRAIL_RAIDERS)
					event_desc   = "I arm mineelves as I prepare to fight off the vox menace!"
				if(ORION_TRAIL_DERELICT)
					event_desc = "I come across an unpowered ship drifting slowly in the vastness of space. Sensors indicate there are no lifeforms aboard."
				if(ORION_TRAIL_ILLNESS)
					event_desc = "A disease has spread amoungst my crew!"
				if(ORION_TRAIL_FLUX)
					event_desc = "I have entered a turbulent region. Slowing down would be better for my ship but would cost more fuel."
				...
				if(ORION_TRAIL_MALFUNCTION)
					event_info = ""
					event_desc = "The ship's computers are malfunctioning! I can choose to fix it with a part or risk something going awry."
					event_actions  = "<a href='?src=\ref[src];continue=1;risky=25'>Continue as normal</a><BR>"
					if(supplies["3"] != 0)
						event_actions += "<a href='?src=\ref[src];continue=1;fix=3'>Fix using a part.</a><BR>"
				if(ORION_TRAIL_COLLISION)
					event_info = ""
					event_desc = "Something has hit my ship and breached the hull! I can choose to fix it with a part or risk something going awry."
					event_actions  = "<a href='?src=\ref[src];continue=1;risky=25'>Continue as normal</a><BR>"
					if(supplies["2"] != 0)
						event_actions += "<a href='?src=\ref[src];continue=1;fix=2'>Fix using a part.</a><BR>"
				if(ORION_TRAIL_BREAKDOWN)
					event_info = ""
					event_desc = "The ship's engines broke down! I can choose to fix it with a part or risk something going awry."
					event_actions  = "<a href='?src=\ref[src];continue=1;risky=25'>Continue as normal</a><BR>"
					if(supplies["1"] != 0)
						event_actions += "<a href='?src=\ref[src];continue=1;fix=1'>Fix using a part.</a><BR>"
				if(ORION_TRAIL_STUCK)
					event_desc    = "I have ran out of fuel. My only hope to survive is to get refueled by a passing ship, if there are any."
					if(supplies["5"] == 0)
						event_actions = "<a href='?src=\ref[src];continue=1;food=1'>Wait</a>"
				if(ORION_TRAIL_CARP)
					event_desc = "I have chanced upon a large carp migration! Known both for their delicious meat as well as their bite, I and my crew arm mineelves for a small hunting trip."
				if(ORION_TRAIL_MUTINY)
					event_desc = "I have been hearing rumors of dissenting opinions and missing items from the armory..."
				if(ORION_TRAIL_MUTINY_ATTACK)
					event_desc = "Oh no, some of my crew are attempting to mutiny!!"
		...
		if(ORION_VIEW_SUPPLIES)
			dat  = "<center><h1>Supplies</h1>View my supplies or buy more when at a spaceport.</center><BR>"
			dat += "<center>I have [supplies["6"]] thalers.</center>"
			for(var/i=1; i<6; i++)
				var/amm = (i>3?10:1)
		...
		if(ORION_VIEW_CREW)
			dat = "<center><h1>Crew</h1>View the status of my crew.</center>"
...
/obj/machinery/computer/arcade/orion_trail/OnTopic(user, href_list)
	if(href_list["continue"])
		if(view == ORION_VIEW_MAIN)
			...
			if(!settlers.len)
				event_desc = "I and my crew were killed on the way to Orion, my ship left abandoned for scavengers to find."
			...
			if(!href_list["food"])
				...
				event_info = "I have [supplies["4"]] food left.<BR>"
				next_event = ORION_TRAIL_STUCK

			if(supplies["4"] <= 0)
				next_event = ORION_TRAIL_GAMEOVER
				event_desc = "I and my crew starved to death, never to reach Orion."
				supplies["4"] = 0
		...
		if(prob(17*settlers.len))
			event_desc = "An empty husk of a station now, all its resources stripped for use in my travels."
			event_info = "I have successfully raided the spaceport!<br>"
			change_resource(null)
			change_resource(null)
		else
			event_desc = "The local police mobilized too quickly, sirens blare as I barely make it away with my ship intact."
			change_resource(null,-1)
			change_resource(null,-1)
			if(prob(50))
				remove_settler(null, "died while I were escaping!")
				if(prob(10))
					remove_settler(null, "died while I were escaping!")
...
/obj/machinery/computer/arcade/orion_trail/proc/change_resource(var/specific = null, var/add = 1)
	...
	event_info += "I have [add > 0 ? "gained" : "lost"] [abs(cost)] [supply_name["[specific]"]]<BR>"
...
/obj/machinery/computer/arcade/orion_trail/proc/generate_event(var/specific = null)
	...
	switch(specific)
		if(ORION_TRAIL_RAIDERS)
			if(prob(17 * settlers.len))
				event_info = "I managed to fight them off!<br>"
				if(prob(5))
				...
				event_info = "I couldn't fight them off!<br>"
		...
		if(ORION_TRAIL_DERELICT)
			if(prob(60))
				event_info = "I find resources onboard!"
				change_resource(rand(1,3))
				change_resource(rand(4,5))
			else
				event_info = "I don't find anything onboard..."
		if(ORION_TRAIL_COLLISION)
			event_info = ""
			event_desc = "I have collided with a passing meteor, breaching my hull!"
			if(prob(10))
				event_info = "My cargo hold was breached!<BR>"
		...
		if(ORION_TRAIL_MUTINY_ATTACK)
			...
			if(prob(30+(settlers.len-num_traitors)*20))
				event_info = "The traitors decided to jump ship along with some of my supplies!<BR>"
				...
				event_info = "The traitors are charging me! Prepare my weapons!<BR>"
		...
		if(ORION_TRAIL_DISASTER)
			event_desc = "The [event] proved too difficult for I and my crew!"
			change_resource(4,-1)
		...
		if(ORION_TRAIL_STUCK)
			event_info = "I have [supplies["4"]] food left.<BR>"
			if(prob(10))
				event_info += "A passing ship has kindly donated fuel to I and wishes I luck on my journey.<BR>"
	...
	switch(event)
		if(ORION_TRAIL_RAIDERS)
			if(istype(usr,/mob/living/carbon))
				...
				if(prob(50))
					to_chat(usr, "<span class='warning'>I hear battle shouts. The tramping of boots on cold metal. Screams of agony. The rush of venting air. Are I going insane?</span>")
					M.hallucination(50, 50)
				else
					to_chat(usr, "<span class='danger'>Something strikes I from behind! It hurts like hell and feel like a blunt weapon, but nothing is there...</span>")
					M.take_organ_damage(10)
			else
				to_chat(usr, "<span class='warning'>The sounds of battle fill my ears...</span>")
		if(ORION_TRAIL_ILLNESS)
			if(istype(usr,/mob/living/carbon/human))
				var/mob/living/carbon/human/M = usr
				to_chat(M, "<span class='warning'>An overpowering wave of nausea consumes over you. I hunch over, my stomach's contents preparing for a spectacular exit.</span>")
				M.vomit()
			else
				to_chat(usr, "<span class='warning'>I feel ill.</span>")
		if(ORION_TRAIL_CARP)
			to_chat(usr, "<span class='danger'> Something bit me!</span>")
		...
		if(ORION_TRAIL_FLUX)
			if(istype(usr,/mob/living/carbon) && prob(75))
				var/mob/living/carbon/M = usr
				M.Weaken(3)
				src.visible_message("A sudden gust of powerful wind slams \the [M] into the floor!", "I hear a large fwooshing sound, followed by a bang.")
				M.take_organ_damage(10)
			else
				to_chat(usr, "<span class='warning'>A violent gale blows past you, and I barely manage to stay standing!</span>")
		if(ORION_TRAIL_MALFUNCTION)
			if(supplies["3"])
				return
		...
		if(ORION_TRAIL_COLLISION)
			if(prob(90) && !supplies["2"])
				...
				src.visible_message("<span class='danger'>Something slams into the floor around \the [src], exposing it to space!</span>", "I hear something crack and break.")
			else
				src.visible_message("Something slams into the floor around \the [src] - luckily, it didn't get through!", "I hear something crack.")
		if(ORION_TRAIL_GAMEOVER)
			to_chat(usr, "<span class='danger'><font size=3>I'm never going to make it to Orion...</font></span>")
			var/mob/living/M = usr
...
/obj/machinery/computer/arcade/orion_trail/proc/win()
	src.visible_message("\The [src] plays a triumpant tune, stating 'CONGRATULATIONS, I HAVE MADE IT TO ORION.'")
	if(emagged)
...
/obj/item/weapon/orion_ship/attack_self(mob/user)
	...
	src.audible_message("<b>\The [src]</b> says, 'Uh, Port? Having some issues with our reactor, could I check it out? Over.'")
	sleep(30)
