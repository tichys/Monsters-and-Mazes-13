// Allows I to monitor messages that passes the server.

/obj/machinery/computer/message_monitor
	name = "messaging monitor console"
	desc = "Used to access and maintain data on messaging servers. Allows I to view request console messages."
...
/obj/machinery/computer/message_monitor/emag_act(var/remaining_charges, var/mob/user)
	// Will create sparks and print out the console's password. I will then have to wait a while for the console to be back online.
	...
	switch(screen)
		...
		if(2)
			if(istype(user, /mob/living/silicon/ai) || istype(user, /mob/living/silicon/robot))
				...
				dat += "In the meantime, this console can reveal my true intentions if I let someone access it. Make sure no humans enter the room during that time."
			else
