...
/obj/machinery/computer/robotics/OnTopic(var/mob/user, href_list)
	...
	if (href_list["lockdown"])
		...
		if(isAI(user) && (target.connected_ai != user))
			to_chat(user, "<span class='warning'>Access Denied. This robot is not linked to me.</span>")
			return TOPIC_HANDLED
		...
		if(target.SetLockdown(!target.lockcharge))
			...
			if(target.lockcharge)
				to_chat(target, "<span class='danger'>I have been locked down!</span>")
			else
				to_chat(target, "<span class='notice'>My lockdown has been lifted!</span>")
		else
