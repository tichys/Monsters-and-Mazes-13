...
/obj/machinery/door/airlock/phoron/proc/PhoronBurn(temperature)
	...
	for(var/turf/simulated/wall/W in range(3,src))
		W.burn((temperature/4))//Added so that I can't set off a massive chain reaction with a small flame
...
*	An airlock wire dialog can be accessed by the normal way or by using wirecutters or a multitool on the door while the wire-panel is open. This would show the following wires, which I can either wirecut/mend or send a multitool pulse through. There are 9 wires.
*		one wire from the ID scanner. Sending a pulse through this flashes the red light on the door (if the door has power). If I cut this wire, the door will stop recognizing valid IDs. (If the door has 0000 access, it still opens and closes, though)
...
/obj/machinery/door/airlock/bumpopen(mob/living/user as mob) //Airlocks now zap I when I 'bump' them open when they're electrified. --NeoFite
	if(!issilicon(usr))
...
/obj/machinery/door/airlock/proc/isWireCut(var/wireIndex)
	// I can find the wires in the datum folder.
...
/obj/machinery/door/airlock/proc/cut_bolts(item, var/mob/user)
	...
	if(isWelder(item))
		...
		user.visible_message(
			...
			"<span class='warning'>I smash the bolt cover open!</span>"
			)
		...
		// I guess I can't cut bolts with that item. Never mind then.
		return 0

	if (src.lock_cut_state == BOLTS_FINE)
		user.visible_message(
			"<span class='notice'>\The [user] begins [cut_verb] through the bolt cover on [src].</span>",
			"<span class='notice'>I begin [cut_verb] through the bolt cover.</span>"
			)
		...
		if (do_after(user, cut_delay, src))
			user.visible_message(
				...
				"<span class='notice'>I remove the cover and expose the door bolts.</span>"
				)
			src.lock_cut_state = BOLTS_EXPOSED
		return 1
	...
	if (src.lock_cut_state == BOLTS_EXPOSED)
		user.visible_message(
			...
			"<span class='notice'>I begin [cut_verb] through the door bolts.</span>"
			)
		playsound(src, cut_sound, 100, 1)
		if (do_after(user, cut_delay, src))
			user.visible_message(
				"<span class='notice'>\The [user] severs the door bolts, unlocking [src].</span>",
				"<span class='notice'>I sever the door bolts, unlocking the door.</span>"
				)
...
/obj/machinery/door/airlock/attackby(var/obj/item/C, var/mob/user)
	...
	if(!brace && istype(C, /obj/item/weapon/airlock_brace))
		...
		if(!density)
			to_chat(user, "<span class='warning'>I must close \the [src] before installing \the [A]!</span>")
			return
		...
		if(do_after(user, 50, src) && density && A && user.unEquip(A, src))
			to_chat(user, "<span class='notice'>I successfully install \the [A].</span>")
			brace = A
			brace.airlock = src
			update_icon()
		return
	...
	if(!repairing && isWelder(C) && !( src.operating > 0 ) && src.density)
		...
		if (F.wielded)
			playsound(src, 'sound/weapons/smash.ogg', 100, 1)
			user.visible_message("<span class='danger'>[user] smashes \the [C] into the airlock's control panel! It explodes in a shower of sparks!</span>", "<span class='danger'>I smash \the [C] into the airlock's control panel! It explodes in a shower of sparks!</span>")
			src.health = 0
			src.set_broken()
		else
			..()
		...
		if(locked)
			...
			if(density)
				...
				if(F.wielded)
					...
					to_chat(user, "<span class='warning'>I need to be wielding \the [C] to do that.</span>")
			else
				var/obj/item/weapon/material/twohanded/fireaxe/F = C
				if(F.wielded)
					spawn(0)	close(1)
...
/obj/machinery/door/airlock/proc/lock(var/forced=0)
	...
	audible_message("I hear a click from the bottom of the door.", hearing_distance = 1)
	update_icon()
	return 1
