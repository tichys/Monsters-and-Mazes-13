		...
		if("unlock")
			...
			if(!req_access || istype(usr,/mob/living/silicon))
				...
				if(!istype(I, /obj/item/weapon/card/id))
					to_chat(usr, "<span class='warning'>[\src] flashes a yellow LED near the ID scanner. Did I remember to scan my ID or PDA?</span>")
					return TRUE
				if (check_access(I))
					locked = 0
					last_configurator = I.registered_name
				else
					to_chat(usr, "<span class='warning'>[\src] flashes a red LED near the ID scanner, indicating my access has been denied.</span>")
					return TRUE
		if("lock")
			if(!lockable)
				return TRUE
