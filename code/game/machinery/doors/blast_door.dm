...
/obj/machinery/door/blast/attackby(obj/item/weapon/C as obj, mob/user as mob)
	...
	if(isCrowbar(C) || (istype(C, /obj/item/weapon/material/twohanded/fireaxe) && C:wielded == 1))
		if(((stat & NOPOWER) || (stat & BROKEN)) && !( src.operating ))
			...
			to_chat(usr, "<span class='notice'>[src]'s motors resist my effort.</span>")
		return
	if(istype(C, /obj/item/stack/material) && C.get_material_name() == "plasteel")
		...
		if(P.amount < amt)
			to_chat(usr, "<span class='warning'>I don't have enough sheets to repair this! I need at least [amt] sheets.</span>")
			return
		to_chat(usr, "<span class='notice'>I begin repairing [src]...</span>")
		if(do_after(usr, 30, src))
			if(P.use(amt))
				to_chat(usr, "<span class='notice'>I have repaired \the [src]</span>")
				src.repair()
			else
				to_chat(usr, "<span class='warning'>I don't have enough sheets to repair this! I need at least [amt] sheets.</span>")
	check_force(C, user)
...
// SUBTYPE: Regular
// My classical blast door, found almost everywhere.
