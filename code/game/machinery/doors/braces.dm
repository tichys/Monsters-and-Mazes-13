...
/obj/item/weapon/airlock_brace/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W.GetIdCard(), /obj/item/weapon/card/id))
		if(!airlock)
			...
			update_access()
			if(check_access(C))
				to_chat(user, "I swipe \the [C] through \the [src].")
				if(do_after(user, 10, airlock))
					to_chat(user, "\The [src] clicks a few times and detaches itself from \the [airlock]!")
					unlock_brace(usr)
			else
				to_chat(user, "I swipe \the [C] through \the [src], but it does not react.")
		return
	...
	if (istype(W, /obj/item/weapon/crowbar/brace_jack))
		...
		to_chat(user, "I begin forcibly removing \the [src] with \the [C].")
		if(do_after(user, rand(150,300), airlock))
			to_chat(user, "I finish removing \the [src].")
			unlock_brace(user)
		return
	...
	if(isWelder(W))
		...
		if(C.remove_fuel(0,user))
			...
			if(cur_health == max_health)
				to_chat(user, "I repair some dents on \the [src]. It is in perfect condition now.")
			else
				to_chat(user, "I repair some dents on \the [src].")
