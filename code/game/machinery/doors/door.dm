...
/obj/machinery/door/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(istype(I, /obj/item/stack/material) && I.get_material_name() == src.get_material_name())
		...
		if (repairing)
			...
			if (!transfer)
				to_chat(user, "<span class='warning'>I must weld or remove \the [repairing] from \the [src] before I can add anything else.</span>")
		else
		...
		if (transfer)
			to_chat(user, "<span class='notice'>I fit [transfer] [stack.singular_name]\s to damaged and broken parts on \the [src].</span>")
	...
	if(repairing && isWelder(I))
		...
		if(welder.remove_fuel(0,user))
			to_chat(user, "<span class='notice'>I start to fix dents and weld \the [repairing] into place.</span>")
			playsound(src, 'sound/items/Welder.ogg', 100, 1)
			if(do_after(user, 5 * repairing.amount, src) && welder && welder.isOn())
				to_chat(user, "<span class='notice'>I finish repairing the damage to \the [src].</span>")
	...
	if(repairing && isCrowbar(I))
		to_chat(user, "<span class='notice'>I remove \the [repairing].</span>")
