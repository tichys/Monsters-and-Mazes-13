...
/obj/machinery/door/firedoor/attack_hand(mob/user as mob)
	...
	var/answer = alert(user, "Would I like to [density ? "open" : "close"] this [src.name]?[ alarmed && density ? "\nNote that by doing so, I acknowledge any damages from opening this\n[src.name] as being my own fault, and I will be held accountable under the law." : ""]",\
	"\The [src]", "Yes, [density ? "open" : "close"]", "No")
	if(answer == "No")
		return
	if(user.incapacitated() || (get_dist(src, user) > 1  && !issilicon(user)))
		to_chat(user, "Sorry, I must remain able bodied and close to \the [src] in order to use it.")
		return
...
/obj/machinery/door/firedoor/attackby(obj/item/weapon/C as obj, mob/user as mob)
	...
	if(density && isScrewdriver(C))
		...
		user.visible_message("<span class='danger'>[user] has [hatch_open ? "opened" : "closed"] \the [src] maintenance hatch.</span>",
									"I have [hatch_open ? "opened" : "closed"] the [src] maintenance hatch.")
		update_icon()
		return

	if(blocked && isCrowbar(C) && !repairing)
		if(!hatch_open)
			to_chat(user, "<span class='danger'>I must open the maintenance hatch first!</span>")
		else
			user.visible_message("<span class='danger'>[user] is removing the electronics from \the [src].</span>",
									"I start to remove the electronics from [src].")
			if(do_after(user,30,src))
				if(blocked && density && hatch_open)
					playsound(src.loc, 'sound/items/Crowbar.ogg', 100, 1)
					user.visible_message("<span class='danger'>[user] has removed the electronics from \the [src].</span>",
										"I have removed the electronics from [src].")
					deconstruct(user)
			else
				to_chat(user, "<span class='notice'>I must remain still to remove the electronics from \the [src].</span>")
		return
	...
	if(isCrowbar(C) || istype(C,/obj/item/weapon/material/twohanded/fireaxe))
		...
		user.visible_message("<span class='danger'>\The [user] starts to force \the [src] [density ? "open" : "closed"] with \a [C]!</span>",\
				"I start forcing \the [src] [density ? "open" : "closed"] with \the [C]!",\
				"I hear metal strain.")
		if(do_after(user,30,src))
			if(isCrowbar(C))
				if(stat & (BROKEN|NOPOWER) || !density)
					user.visible_message("<span class='danger'>\The [user] forces \the [src] [density ? "open" : "closed"] with \a [C]!</span>",\
					"I force \the [src] [density ? "open" : "closed"] with \the [C]!",\
					...
					user.visible_message("<span class='danger'>\The [user] forces \the [ blocked ? "welded" : "" ] [src] [density ? "open" : "closed"] with \a [C]!</span>",\
						"I force \the [ blocked ? "welded" : "" ] [src] [density ? "open" : "closed"] with \the [C]!",\
						"I hear metal strain and groan, and a door [density ? "opening" : "closing"].")
			if(density)
				spawn(0)
					open(1)
			else
			...
			to_chat(user, "<span class='notice'>I must remain still to interact with \the [src].</span>")
	return ..()
...
/*
	...
	glass = 1 //There is a glass window so I can see through the door
