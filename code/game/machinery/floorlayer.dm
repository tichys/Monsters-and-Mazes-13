...
/obj/machinery/floorlayer/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if(isWrench(W))
		...
		mode[m] = !mode[m]
		var/O = mode[m]
		user.visible_message("<span class='notice'>[usr] has set \the [src] [m] mode [!O?"off":"on"].</span>", "<span class='notice'>I set \the [src] [m] mode [!O?"off":"on"].</span>")
		return
	...
	if(isCrowbar(W))
		if(!length(contents))
			...
			if(E)
				to_chat(user, "<span class='notice'>I remove the [E] from /the [src].</span>")
				E.loc = src.loc
				T = null
		return
