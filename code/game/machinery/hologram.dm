...
/obj/machinery/hologram/holopad/attack_hand(var/mob/living/carbon/human/user) //Carn: Hologram requests.
	...
	if(incoming_connection&&caller_id)
		...
		end_call(user)
		return
	switch(alert(user,"Would I like to request an AI's presence or establish communications with another pad?", "Holopad","AI","Holocomms","Cancel"))
		if("AI")
			if(last_request + 200 < world.time) //don't spam the AI with requests I jerk!
		...
		if("Holocomms")
			if(user.loc != src.loc)
				...
				return
			if(last_request + 200 < world.time) //don't spam other people with requests either, I jerk!
				last_request = world.time
				...
				var/temppad = input(user, "Which holopad would I like to contact?", "holopad list") as null|anything in holopadlist
				targetpad = holopadlist["[temppad]"]
				if(targetpad==src)
					to_chat(user, "<span class='info'>Using such sophisticated technology, just to talk to myself seems a bit silly.</span>")
					return
				if(targetpad && targetpad.caller_id)
					to_chat(user, "<span class='info'>The pad flashes a busy sign. Maybe I should try again later..</span>")
					return
				if(targetpad)
...
/obj/machinery/hologram/holopad/proc/make_call(var/obj/machinery/hologram/holopad/targetpad, var/mob/living/carbon/user)
	...
	targetpad.sourcepad = src //This marks the holopad I am making the call from
	targetpad.caller_id = user //This marks I as the caller
...
/obj/machinery/hologram/holopad/proc/activate_holocall(mob/living/carbon/caller_id)
	if(caller_id)
		src.visible_message("A holographic image of [caller_id] flicks to life right before my eyes!")
...
/obj/machinery/hologram/holopad/proc/create_holo(mob/living/silicon/ai/A, mob/living/carbon/caller_id, turf/T = loc)
	...
	hologram.mouse_opacity = 0//So I can't click on it.
...
/mob/verb/hologram_test()
	...
	if(input)
		...
		log_debug("My icon should appear now.")
