...
/obj/machinery/media/jukebox
	...
	description_info = "Click the jukebox and then select a track on the interface. I can choose to play or stop the track, or set the volume. Use a wrench to attach or detach the jukebox to the floor. The room it is installed in must have power for it to operate!"
...
/obj/machinery/media/jukebox/interact(mob/user)
	if(!anchored)
		to_chat(usr, "<span class='warning'>I must secure \the [src] first.</span>")
		return
...
/obj/machinery/media/jukebox/tg_ui_interact(mob/user, ui_key = "main", datum/tgui/ui = null, force_open = 0, datum/tgui/master_ui = null, datum/ui_state/state = tg_default_state)
	...
	if(!ui)
		ui = new(user, src, ui_key, "jukebox", "My Media Library", 340, 440, master_ui, state)
		ui.open()
