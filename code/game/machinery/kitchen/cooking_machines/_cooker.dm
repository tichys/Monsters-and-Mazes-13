...
/obj/machinery/cooker/examine()
	...
	if(cooking_obj && Adjacent(usr))
		to_chat(usr, "I can see \a [cooking_obj] inside.")
...
/obj/machinery/cooker/attackby(var/obj/item/I, var/mob/user)
	...
	if(istype(G))
		...
		if(!isliving(G.affecting))
			to_chat(user, "<span class='warning'>I can't cook that.</span>")
			return
...
/obj/machinery/cooker/attack_hand(var/mob/user)
	...
	if(cooking_obj)
		to_chat(user, "<span class='notice'>I grab \the [cooking_obj] from \the [src].</span>")
	...
	if(output_options.len)
		...
		var/choice = input("What specific food do I wish to make with \the [src]?") as null|anything in output_options+"Default"
		if(!choice)
			return
		if(choice == "Default")
			selected_option = null
			to_chat(user, "<span class='notice'>I decide not to make anything specific with \the [src].</span>")
		else
			selected_option = choice
			to_chat(user, "<span class='notice'>I prepare \the [src] to make \a [selected_option].</span>")
