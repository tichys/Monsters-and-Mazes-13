...
/obj/machinery/microwave
	...
	var/global/list/datum/recipe/available_recipes // List of the recipes I can use
	var/global/list/acceptable_items // List of the items I can put in
	var/global/list/acceptable_reagents // List of the reagents I can put in
...
/obj/machinery/microwave/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(src.broken > 0)
		...
		if (contents.len >= max_n_of_items)
			to_chat(user, "<span class='warning'>This [src] is full of ingredients, I cannot put more.</span>")
			return 1
		...
		to_chat(user, "<span class='warning'>This is ridiculous. I can not fit \the [G.affecting] in this [src].</span>")
		return 1
	else if(isCrowbar(O))
		user.visible_message( \
			"<span class='notice'>\The [user] begins [src.anchored ? "securing" : "unsecuring"] the microwave.</span>", \
			"<span class='notice'>I attempt to [src.anchored ? "secure" : "unsecure"] the microwave.</span>"
			)
		if (do_after(user,20, src))
			user.visible_message( \
			"<span class='notice'>\The [user] [src.anchored ? "secures" : "unsecures"] the microwave.</span>", \
			"<span class='notice'>I [src.anchored ? "secure" : "unsecure"] the microwave.</span>"
			)
			src.anchored = !src.anchored
		else
			to_chat(user, "<span class='notice'>I decide not to do that.</span>")
	else

		to_chat(user, "<span class='warning'>I have no idea what I can cook with this [O].</span>")
	..()
	src.updateUsrDialog()
...
/obj/machinery/microwave/proc/muck_finish()
	...
	src.obj_flags = null //So I can't add condiments
	src.operating = 0 // Turn it off again aferwards
	src.updateUsrDialog()
	src.update_icon()
