...
/obj/machinery/smartfridge/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(isScrewdriver(O))
		...
		user.visible_message("[user] [panel_open ? "opens" : "closes"] the maintenance panel of \the [src].", "I [panel_open ? "open" : "close"] the maintenance panel of \the [src].")
		overlays.Cut()
		if(panel_open)
	...
	if(accept_check(O))
		if(!user.unEquip(O))
			return
		stock_item(O)
		user.visible_message("<span class='notice'>\The [user] has added \the [O] to \the [src].</span>", "<span class='notice'>I add \the [O] to \the [src].</span>")
		...
		if(plants_loaded)
			user.visible_message("<span class='notice'>\The [user] loads \the [src] with the contents of \the [P].</span>", "<span class='notice'>I load \the [src] with the contents of \the [P].</span>")
			if(P.contents.len > 0)
...
/obj/machinery/smartfridge/secure/emag_act(var/remaining_charges, var/mob/user)
	if(!emagged)
		emagged = 1
		locked = -1
		to_chat(user, "I short out the product lock on [src].")
		return 1
