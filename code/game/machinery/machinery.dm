...
/obj/machinery/Process()//If I dont use process or power why are I here
...
*/
	if (ishuman(user))
		...
		if(H.getBrainLoss() >= 55)
			...
			to_chat(user, "<span class='warning'>I momentarily forget how to use \the [src].</span>")
			return 1
...
/obj/machinery/proc/default_deconstruction_screwdriver(var/mob/user, var/obj/item/weapon/screwdriver/S)
	...
	to_chat(user, "<span class='notice'>I [panel_open ? "open" : "close"] the maintenance hatch of \the [src].</span>")
	update_icon()
	return 1
