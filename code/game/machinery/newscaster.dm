...
/obj/machinery/newscaster
	name = "newscaster"
	desc = "A standard newsfeed handler. All the news I absolutely have no use for, in one place!"
	icon = 'icons/obj/terminals.dmi'
	icon_state = "newscaster_normal"
	//var/list/datum/feed_channel/channel_list = list() //This list will contain the names of the feed channels. Each name will refer to a data region where the messages of the feed channels are stored.
	//OBSOLETE: We're now using a global news network
	var/screen = 0                  //Or maybe I'll make it into a list within a list afterwards... whichever I prefer, go fuck mineelves :3
	...
	if(istype(user, /mob/living/carbon/human) || istype(user,/mob/living/silicon) )
		...
		switch(screen)
			if(0)
				...
				dat+="<BR><HR>The newscaster recognises I as: <FONT COLOR='green'>[src.scanned_user]</FONT>"
			if(1)
			...
			if(7)
				...
				if(src.scanned_user in existing_authors)
					dat+="<FONT COLOR='maroon'>There already exists a Feed channel under my name.</FONT><BR>"
			...
			if(11)
				...
				dat+="stories it might contain at the time. I can lift a D-Notice if I have the required access at any time.</FONT><HR>"
				if(isemptylist(news_network.network_channels))
					dat+="<I>No feed channels found active...</I><BR>"
				else
					for(var/datum/feed_channel/CHANNEL in news_network.network_channels)
			...
			if(13)
				dat+="<B>[src.viewing_channel.channel_name]: </B><FONT SIZE=1>\[ created by: <FONT COLOR='maroon'>[src.viewing_channel.author]</FONT> \]</FONT><BR>"
				dat+="Channel messages listed below. If I deem them dangerous to the [station_name()], I can <A href='?src=\ref[src];toggle_d_notice=\ref[src.viewing_channel]'>Bestow a D-Notice upon the channel</A>.<HR>"
				if(src.viewing_channel.censored)
					dat+="<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the [station_name()], and marked with a [GLOB.using_map.company_name] D-Notice.<BR>"
			...
			if(19)
				...
				dat+="<BR><A href='?src=\ref[src];setScreen=[0]'>Return</A><BR>"
			if(20)
				dat+="<FONT COLOR='green'>Printing successful. Please receive my newspaper from the bottom of the machine.</FONT><BR><BR>"
...
/obj/machinery/newscaster/Topic(href, href_list)
	...
	if ((usr.contents.Find(src) || ((get_dist(src, usr) <= 1) && istype(src.loc, /turf))) || (istype(usr, /mob/living/silicon)))
		...
		if(href_list["set_channel_name"])
			...
			src.msg = sanitize(input(usr, "Write my Feed story", "Network Channel Handler", ""))
			src.updateUsrDialog()
			...
			src.msg = sanitize(input(usr, "Provide the a description of the Wanted person and any other details I deem important", "Network Security Handler", ""))
			src.updateUsrDialog()
				...
				if(choice=="Confirm")
					if(input_param==1)          //If input_param == 1 we're submitting a new wanted issue. At 2 we're just editing an existing one. See the else below
						...
						if(news_network.wanted_issue.is_admin_message)
							alert("The wanted issue has been distributed by a [GLOB.using_map.company_name] higherup. I cannot edit it.","Ok")
							return
			...
			if(news_network.wanted_issue.is_admin_message)
				alert("The wanted issue has been distributed by a [GLOB.using_map.company_name] higherup. I cannot take it down.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by a [GLOB.using_map.company_name] Officer. I cannot censor it.","Ok")
				return
			...
			if(MSG.is_admin_message)
				alert("This message was created by a [GLOB.using_map.company_name] Officer. I cannot censor its author.","Ok")
				return
			...
			if(FC.is_admin_channel)
				alert("This channel was created by a [GLOB.using_map.company_name] Officer. I cannot place a D-Notice upon it.","Ok")
				return
...
obj/item/weapon/newspaper/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/pen))
		if(src.scribble_page == src.curr_page)
			to_chat(user, "<FONT COLOR='blue'>There's already a scribble in this page... I wouldn't want to make things too cluttered, would you?</FONT>")
		else
