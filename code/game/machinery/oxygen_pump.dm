...
/obj/machinery/oxygen_pump
	...
	desc = "A wall mounted oxygen pump with a retractable face mask that I can pull over my face in case of emergencies."
	icon_state = "emerg"
...
/obj/machinery/oxygen_pump/attack_hand(mob/user as mob)
	if((stat & MAINT) && tank)
		user.visible_message("<span class='notice'>\The [user] removes \the [tank] from \the [src].</span>", "<span class='notice'>I remove \the [tank] from \the [src].</span>")
...
/obj/machinery/oxygen_pump/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isScrewdriver(W))
		stat ^= MAINT
		user.visible_message("<span class='notice'>\The [user] [stat & MAINT ? "opens" : "closes"] \the [src].</span>", "<span class='notice'>I [stat & MAINT ? "open" : "close"] \the [src].</span>")
		if(stat & MAINT)
	...
	if(istype(W, /obj/item/weapon/tank) && (stat & MAINT))
		if(tank)
			...
			if(!user.unEquip(W, src))
				return
			tank = W
			user.visible_message("<span class='notice'>\The [user] installs \the [tank] into \the [src].</span>", "<span class='notice'>I install \the [tank] into \the [src].</span>")
			src.add_fingerprint(user)
