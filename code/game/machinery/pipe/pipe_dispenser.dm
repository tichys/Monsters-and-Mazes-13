...
/obj/machinery/pipedispenser/attackby(var/obj/item/W as obj, var/mob/user as mob)
	if (istype(W, /obj/item/pipe) || istype(W, /obj/item/pipe_meter))
		if(!user.unEquip(W))
			return
		to_chat(usr, "<span class='notice'>I put \the [W] back into \the [src].</span>")
		add_fingerprint(usr)
		qdel(W)
		return
	else if(isWrench(W))
		...
		if (unwrenched==0)
			...
			to_chat(user, "<span class='notice'>I begin to unfasten \the [src] from the floor...</span>")
			if (do_after(user, 40, src))
				user.visible_message( \
					"<span class='notice'>\The [user] unfastens \the [src].</span>", \
					"<span class='notice'>I have unfastened \the [src]. Now it can be pulled somewhere else.</span>", \
					"I hear ratchet.")
				src.anchored = 0
				src.stat |= MAINT
				src.unwrenched = 1
			...
			playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
			to_chat(user, "<span class='notice'>I begin to fasten \the [src] to the floor...</span>")
			if (do_after(user, 20, src))
				user.visible_message( \
					"<span class='notice'>\The [user] fastens \the [src].</span>", \
					"<span class='notice'>I have fastened \the [src]. Now it can dispense pipes.</span>", \
					"I hear ratchet.")
				src.anchored = 1
				src.stat &= ~MAINT
				src.unwrenched = 0
...
/*
//Allow I to push disposal pipes into it (for those with density 1)
...
//Allow I to drag-drop disposal pipes into it
