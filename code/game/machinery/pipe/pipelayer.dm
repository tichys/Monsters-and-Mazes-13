...
/obj/machinery/pipelayer/attack_hand(mob/user as mob)
	if(!metal&&!on)
		...
		return
	on=!on
	user.visible_message("<span class='notice'>[user] has [!on?"de":""]activated \the [src].</span>", "<span class='notice'>I [!on?"de":""]activate \the [src].</span>")
	return
...
/obj/machinery/pipelayer/attackby(var/obj/item/W as obj, var/mob/user as mob)
	...
	if(isWrench(W))
		...
		user.visible_message("<span class='notice'>[user] has set \the [src] to manufacture [P_type_t].</span>", "<span class='notice'>I set \the [src] to manufacture [P_type_t].</span>")
		return

	if(isCrowbar(W))
		a_dis=!a_dis
		user.visible_message("<span class='notice'>[user] has [!a_dis?"de":""]activated auto-dismantling.</span>", "<span class='notice'>I [!a_dis?"de":""]activate auto-dismantling.</span>")
		return
	...
	if(istype(W, /obj/item/stack/material) && W.get_material_name() == DEFAULT_WALL_MATERIAL)
		...
		if(isnull(result))
			...
			user.visible_message("<span class='notice'>[user] has loaded metal into \the [src].</span>", "<span class='notice'>I load metal into \the [src]</span>")
	...
	if(isScrewdriver(W))
		if(metal)
			...
			if(m)
				...
				MM.amount = m
				user.visible_message("<span class='notice'>[user] removes [m] sheet\s of metal from the \the [src].</span>", "<span class='notice'>I remove [m] sheet\s of metal from \the [src]</span>")
		else
