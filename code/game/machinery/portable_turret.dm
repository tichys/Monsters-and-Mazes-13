...
/obj/machinery/porta_turret/proc/isLocked(mob/user)
	if(ailock && issilicon(user))
		to_chat(user, "<span class='notice'>There seems to be a firewall preventing I from accessing this device.</span>")
		return 1
...
/obj/machinery/porta_turret/attackby(obj/item/I, mob/user)
	if(stat & BROKEN)
		if(isCrowbar(I))
			//If the turret is destroyed, I can remove it with a crowbar to
		...
		if(enabled || raised)
			to_chat(user, "<span class='warning'>I cannot unsecure an active turret!</span>")
			return
		if(wrenching)
		...
		user.visible_message( \
				...
				"<span class='notice'>I begin [anchored ? "un" : ""]securing the turret.</span>" \
			)
...
/obj/machinery/porta_turret/proc/target(var/mob/living/target)
	...
	if(target)
		...
		set_dir(get_dir(src, target))	//even if I can't shoot, follow the target
		spawn()
			shootAt(target)
		return 1
	return
	...
	switch(build_step)
		if(0)	//first step
			if(isWrench(I) && !anchored)
				playsound(loc, 'sound/items/Ratchet.ogg', 100, 1)
				to_chat(user, "<span class='notice'>I secure the external bolts.</span>")
				anchored = 1
				build_step = 1
				return
				...
				to_chat(user, "<span class='notice'>I dismantle the turret construction.</span>")
		...
		if(1)
			if(istype(I, /obj/item/stack/material) && I.get_material_name() == DEFAULT_WALL_MATERIAL)
				...
				if(M.use(2))
					...
					to_chat(user, "<span class='warning'>I need two sheets of metal to continue construction.</span>")
				return

			else if(istype(I, /obj/item/weapon/wrench))
				playsound(loc, 'sound/items/Ratchet.ogg', 75, 1)
				to_chat(user, "<span class='notice'>I unfasten the external bolts.</span>")
				anchored = 0
				build_step = 0
				return
		...
		if(2)
			if(istype(I, /obj/item/weapon/wrench))
				...
				if(do_after(user, 20, src))
					...
					build_step = 1
					to_chat(user, "I remove the turret's interior metal armor.")
		...
		if(3)
			if(istype(I, /obj/item/weapon/gun/energy)) //the gun installation part
				...
				if(!user.unEquip(I))
					to_chat(user, "<span class='notice'>\the [I] is stuck to my hand, I cannot put it in \the [src]</span>")
					return
				installation = I.type //installation becomes I.type
				gun_charge = E.power_supply.charge //the gun's charge is stored in gun_charge
				to_chat(user, "<span class='notice'>I add [I] to the turret.</span>")
				...
				playsound(loc, 'sound/items/Ratchet.ogg', 100, 1)
				to_chat(user, "<span class='notice'>I remove the turret's metal armor bolts.</span>")
				build_step = 2
				return
		...
		if(6)
			if(istype(I, /obj/item/stack/material) && I.get_material_name() == DEFAULT_WALL_MATERIAL)
				...
				playsound(loc, 'sound/items/Screwdriver.ogg', 100, 1)
				build_step = 5
				to_chat(user, "<span class='notice'>I open the internal access hatch.</span>")
				return
		...
		if(7)
			if(isWelder(I))
				...
				playsound(loc, 'sound/items/Crowbar.ogg', 75, 1)
				to_chat(user, "<span class='notice'>I pry off the turret's exterior armor.</span>")
				new /obj/item/stack/material/steel(loc, 2)
				build_step = 6
				return

	if(istype(I, /obj/item/weapon/pen))	//I can rename turrets like bots!
...
/obj/machinery/porta_turret_construct/attack_hand(mob/user)
	switch(build_step)
		if(4)
			...
			installation = null
			gun_charge = 0
			to_chat(user, "<span class='notice'>I remove [Gun] from the turret frame.</span>")

		if(5)
			to_chat(user, "<span class='notice'>I remove the prox sensor from the turret frame.</span>")
			new /obj/item/device/assembly/prox_sensor(loc)
			build_step = 4
