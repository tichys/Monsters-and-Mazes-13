...
obj/machinery/recharger/attackby(obj/item/weapon/G as obj, mob/user as mob)
	...
	if(allowed)
		...
		if(!powered())
			to_chat(user, "<span class='warning'>The [name] blinks red as I try to insert the item!</span>")
			return
		if (istype(G, /obj/item/weapon/gun/energy/))
			var/obj/item/weapon/gun/energy/E = G
			if(E.self_recharge)
				to_chat(user, "<span class='notice'>I can't find a charging port on \the [E].</span>")
				return
		if(!G.get_cell())
		...
		if(charging)
			...
			return
		anchored = !anchored
		to_chat(user, "I [anchored ? "attached" : "detached"] the recharger.")
