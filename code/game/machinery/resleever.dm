	...
	if(resleeving)
		...
		if(remaining < timetosleeve)
			...
			if(remaining == 90) // 30 seconds left
				to_chat(occupant, "<span class='notice'>I feel a wash of sensation as my senses begin to flood my mind. I will come to soon.</span>")
		else
			remaining = 0
			resleeving = 0
			update_use_power(1)
...
/obj/machinery/resleever/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(default_deconstruction_screwdriver(user, W))
		if(occupant)
			to_chat(user, "<span class='warning'>I need to remove the occupant first!</span>")
			return
	...
	if(istype(W, /obj/item/organ/internal/stack))
		if(isnull(lace) && user.unEquip(W, src))
			to_chat(user, "<span class='notice'>I insert \the [W] into [src].</span>")
			lace = W
			if(lace.backup)
		...
		if(isnull(occupant))
			if(anchored)
				anchored = 0
				user.visible_message("[user] unsecures [src] from the floor.", "I unsecure [src] from the floor.")
			else
				anchored = 1
				user.visible_message("[user] secures [src] to the floor.", "I secure [src] to the floor.")
