...
/obj/machinery/space_heater/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/cell))
		...
		user.visible_message("<span class='notice'>[user] [panel_open ? "opens" : "closes"] the hatch on the [src].</span>", "<span class='notice'>I [panel_open ? "open" : "close"] the hatch on the [src].</span>")
		update_icon(1)
	...
	switch(href_list["op"])
		...
		if("cellremove")
			if(panel_open && cell && !usr.get_active_hand())
				usr.visible_message("<span class='notice'>\The usr] removes \the [cell] from \the [src].</span>", "<span class='notice'>I remove \the [cell] from \the [src].</span>")
				cell.update_icon()
		...
		if("cellinstall")
			if(panel_open && !cell)
				...
				if(istype(C))
					...
					power_change()
					usr.visible_message("<span class='notice'>[usr] inserts \the [C] into \the [src].</span>", "<span class='notice'>I insert \the [C] into \the [src].</span>")
