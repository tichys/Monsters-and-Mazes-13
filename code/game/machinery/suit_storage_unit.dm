...
/obj/machinery/suit_storage_unit/attack_hand(mob/user as mob)
	...
	if(panelopen) //The maintenance panel is open. Time for some shady stuff
		...
		dat+= "<font color ='grey'>The panel is ridden with controls, button and meters, labeled in strange signs and symbols that <BR>I cannot understand. Probably the manufactoring world's language.<BR> Among other things, a few controls catch my eye.</font><BR><BR>"
	...
	else if(isUV) //The thing is running its cauterisation cycle. I have to wait.
...
/obj/machinery/suit_storage_unit/proc/toggleUV(mob/user as mob)
	if(!panelopen)
		...
		if(issuperUV)
			to_chat(user, "<span class='notice'>I slide the dial back towards \"185nm\".</span>")
			issuperUV = 0
		else
			to_chat(user, "<span class='warning'>I crank the dial all the way up to \"15nm\".</span>")
			issuperUV = 1
		return
...
/obj/machinery/suit_storage_unit/proc/togglesafeties(mob/user as mob)
	if(!panelopen) //Needed check due to bugs
		...
		to_chat(user, "<span class='notice'>I push the button. The coloured LED next to it [safetieson ? "turns green" : "turns red"].</span>")
...
/obj/machinery/suit_storage_unit/proc/eject_occupant(mob/user as mob)
	...
	if (occupant.client)
		if(user != occupant)
			to_chat(occupant, "<span class='notice'>The machine kicks I out!</span>")
		if(user.loc != loc)
			to_chat(occupant, "<span class='notice'>I leave the not-so-cozy confines of the SSU.</span>")
...
/obj/machinery/suit_storage_unit/verb/move_inside()
	...
	if ( (occupant) || (helmet ) || (suit) )
		to_chat(usr, "<span class='warning'>It's too cluttered inside for I to fit in!</span>")
		return
...
/obj/machinery/suit_storage_unit/attackby(obj/item/I as obj, mob/user as mob)
	if(isScrewdriver(I))
		...
		to_chat(user, text("<span class='notice'>I [panelopen ? "open" : "close"] the unit's maintenance panel.</span>"))
		updateUsrDialog()
		update_icon()
		return
	if(isCrowbar(I))
		if((stat & NOPOWER) && !islocked && !isopen)
			to_chat(user, "<span class='warning'>I begin prying the unit open.</span>")
			if(do_after(user, 50, src))
				isopen = 1
				to_chat(user, "<span class='warning'>I pry the unit open.</span>")
				update_icon()
		else if(islocked)
			to_chat(user, "<span class='warning'>I can't pry the unit open, it's locked!</span>")
		return
...
#define TRY_INSERT_SUIT_PIECE(slot, path)\
	if(istype(I, /obj/item/##path)){\
		...
		to_chat(user, "I load the [I.name] into the storage compartment.");\
		##slot = I;\
		update_icon();\
		updateUsrDialog();\
		return\
...
//////////////////////////////REMINDER: Make it lock once I place some fucker inside.
...
/obj/machinery/suit_cycler/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(istype(I, /obj/item/grab))
		...
		if(I.icon_override == CUSTOM_ITEM_MOB)
			to_chat(user, "I cannot refit a customised voidsuit.")
			return
		if(!user.unEquip(I, src))
			return
		to_chat(user, "I fit \the [I] into the suit cycler.")
		helmet = I
...
/obj/machinery/suit_cycler/emag_act(var/remaining_charges, var/mob/user)
	...
	to_chat(user, "<span class='danger'>I run the sequencer across the interface, corrupting the operating protocols.</span>")
...
/obj/machinery/suit_cycler/attack_hand(mob/user as mob)
	...
	if(active)
		...
		dat += "<br><font color='red'><B>The [model_text ? "[model_text] " : ""]suit cycler is currently locked. Please contact my system administrator.</b></font>"
		if(allowed(user))
...
/obj/machinery/suit_cycler/Topic(href, href_list)
	if(href_list["eject_suit"])
		...
		if(allowed(usr))
			locked = !locked
			to_chat(usr, "I [locked ? "lock" : "unlock"] [src].")
		else
