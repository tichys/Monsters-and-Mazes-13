	...
	if(istype(user, /mob/living/carbon/human) || istype(user, /mob/living/silicon/ai))
		if(is_special_character(user))
			...
			if(user.gender == FEMALE)
				honorific = "Ms."
			dat += "<font color=red><i>Identity not found in operative database. What can the Syndicate do for I today, [honorific] [user.name]?</i></font><br>"
			if(!selfdestructing)
				dat += "<br><br><A href='?src=\ref[src];betraitor=1;traitormob=\ref[user]'>\"[pick("I want to switch teams.", "I want to work for you.", "Let me join you.", "I can be of use to me.", "I want me working for you, and here's why...", "Give me an objective.", "How's the 401k over at the Syndicate?")]\"</A><BR>"
	dat += temptext
...
/obj/machinery/syndicate_beacon/Topic(href, href_list)
	...
	if(href_list["betraitor"])
		...
		if(M.mind.special_role || jobban_isbanned(M, MODE_TRAITOR))
			temptext = "<i>We have no need for I at this time. Have a pleasant day.</i><br>"
			src.updateUsrDialog()
			return
		charges -= 1
		switch(rand(1,2))
			if(1)
				temptext = "<font color=red><i><b>Double-crosser. I planned to betray us from the start. Allow us to repay the favor in kind.</b></i></font>"
				src.updateUsrDialog()
...
/obj/machinery/power/singularity_beacon/proc/Activate(mob/user = null)
	...
	if(user)
		to_chat(user, "<span class='notice'>I activate the beacon.</span>")
...
/obj/machinery/power/singularity_beacon/proc/Deactivate(mob/user = null)
	...
	active = 0
	if(user)
		to_chat(user, "<span class='notice'>I deactivate the beacon.</span>")
...
/obj/machinery/power/singularity_beacon/attack_hand(var/mob/user as mob)
	if(anchored)
		...
		to_chat(user, "<span class='danger'>I need to screw the beacon to the floor first!</span>")
		return
...
/obj/machinery/power/singularity_beacon/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isScrewdriver(W))
		if(active)
			to_chat(user, "<span class='danger'>I need to deactivate the beacon first!</span>")
			return

		if(anchored)
			anchored = 0
			to_chat(user, "<span class='notice'>I unscrew the beacon from the floor.</span>")
			...
			if(!connect_to_network())
				...
				return
			anchored = 1
			to_chat(user, "<span class='notice'>I screw the beacon to the floor and attach the cable.</span>")
			return
	..()
	return
