...
/**
	...
	@param data:
		If specified:
				...
				4 -- AI can't track down this person. Useful for imitation broadcasts where I can't find the actual mob
...
/proc/Broadcast_Message(var/datum/radio_frequency/connection, var/mob/M,
  ...
  /* ###### Prepare the radio connection ###### */
	...
	if(ishuman(M))
		...
		switch(H.age)
			if(0 to 25)			age_gender = "Young"
			if(25 to 50)		age_gender = "Adult"
			if(50 to 75)		age_gender = "Middle-Aged"
			if(75 to INFINITY)	age_gender = "Elderly"
		switch(H.gender)
			if(MALE)	age_gender += " Man"
			if(FEMALE)	age_gender += " Woman"

	var/display_freq = freq

	var/list/obj/item/device/radio/radios = list()

	for (var/obj/item/device/radio/R in world) // world is kind of laggy if there's a lot of radios in the world, but /shrug
  ...
  /* ###### Begin formatting and sending the message ###### */
	if (length(heard_masked) || length(heard_normal) || length(heard_voice) || length(heard_garbled) || length(heard_gibberish))
	  ...
	  /* --- Some miscellaneous variables to format the string output --- */
		...
		quotedmsg = "\[[age_gender]\] [quotedmsg]"
  ...
  /* ###### Begin formatting and sending the message ###### */
	if (length(heard_normal) || length(heard_garbled) || length(heard_gibberish))
	 ...
	 /* ###### Send the message ###### */
		...
		if (length(heard_normal))
			var/rendered = "[part_a][source][age_gender][part_b]\"[text]\"[part_c]"
