...
/obj/machinery/telecomms/attackby(obj/item/P as obj, mob/user as mob)
	...
	// Using a multitool lets I access the receiver's interface
	if(isMultitool(P))
		attack_hand(user)
	...
	if(istype(P, /obj/item/stack/nanopaste))
		...
		if (integrity < 100)               								//Damaged, let's repair!
			if (T.use(1))
				...
				to_chat(usr, "I apply the Nanopaste to [src], repairing some of the damage.")
		else
	...
	switch(construct_op)
		if(0)
			if(isScrewdriver(P))
				to_chat(user, "I unfasten the bolts.")
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
				construct_op ++
		if(1)
			if(isScrewdriver(P))
				to_chat(user, "I fasten the bolts.")
				playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
				construct_op --
			if(isWrench(P))
				to_chat(user, "I dislodge the external plating.")
				playsound(src.loc, 'sound/items/Ratchet.ogg', 75, 1)
				construct_op ++
		if(2)
			if(isWrench(P))
				to_chat(user, "I secure the external plating.")
				playsound(src.loc, 'sound/items/Ratchet.ogg', 75, 1)
				construct_op --
			if(isWirecutter(P))
				playsound(src.loc, 'sound/items/Wirecutter.ogg', 50, 1)
				to_chat(user, "I remove the cables.")
				construct_op ++
		...
		if(3)
			if(isCoil(P))
				...
				if (A.use(5))
					to_chat(user, "<span class='notice'>I insert the cables.</span>")
					construct_op--
					stat &= ~BROKEN // the machine's not borked anymore!
				else
					to_chat(user, "<span class='warning'>I need five coils of wire for this.</span>")
			if(isCrowbar(P))
				to_chat(user, "I begin prying out the circuit board other components...")
				playsound(src.loc, 'sound/items/Crowbar.ogg', 50, 1)
				if(do_after(user,60, src))
					to_chat(user, "I finish prying out the components.")
...
/obj/machinery/telecomms/attack_hand(var/mob/user as mob)
	...
	// I need a multitool to use this, or be silicon
	if(!issilicon(user))
...
// Off-Site Relays
//
// I am able to send/receive signals from the station's z level (changeable in the STATION_Z #define) if
...
// Additional Options for certain machines. Use this when I want to add an option to a specific machine.
...
/obj/machinery/telecomms/relay/Options_Topic(href, href_list)
	...
	if(href_list["change_listening"])
		...
		//I need at least two receivers and two broadcasters for this to work, this includes the machine.
