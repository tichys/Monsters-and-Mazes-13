	...
	action(atom/target)
		...
		if(istype(target,/obj))
			...
			if(locate(/mob/living) in O)
				occupant_message("<span class='warning'>I can't load living things into the cargo compartment.</span>")
				return
			if(O.anchored)
			...
			occupant_message("I lift [target] and start to load it into cargo compartment.")
			...
			if(do_after_cooldown(target))
				if(T == chassis.loc && src == chassis.selected)
					...
					occupant_message("<span class='warning'>I must hold still while handling objects.</span>")
		...
		//attacking
			...
			if(chassis.occupant.a_intent == I_HURT)
				...
				M.updatehealth()
				occupant_message("<span class='warning'>I squeeze [target] with [src.name]. Something cracks.</span>")
				chassis.visible_message("<span class='warning'>[chassis] squeezes [target].</span>")
			else
				step_away(M,chassis)
				occupant_message("I push [target] out of the way.")
...
/obj/item/mecha_parts/mecha_equipment/tool/safety_clamp
	...
	action(atom/target)
		...
		if(istype(target,/obj))
			...
			if(!O.anchored)
				if(cargo_holder.cargo.len < cargo_holder.cargo_capacity)
					chassis.occupant_message("I lift [target] and start to load it into cargo compartment.")
					chassis.visible_message("[chassis] lifts [target] and starts to load it into cargo compartment.")
					set_ready_state(0)
					chassis.use_power(energy_drain)
					O.anchored = 1
					...
					if(do_after_cooldown(target))
						if(T == chassis.loc && src == chassis.selected)
							...
							chassis.occupant_message("<span class='warning'>I must hold still while handling objects.</span>")
							O.anchored = initial(O.anchored)
				else
			...
			if(M.stat>1) return
			if(chassis.occupant.a_intent == I_HURT)
				chassis.occupant_message("<span class='danger'>I obliterate [target] with [src.name], leaving blood and guts everywhere.</span>")
			...
			if(chassis.occupant.a_intent == I_DISARM)
				chassis.occupant_message("<span class='danger'>I tear [target]'s limbs off with [src.name].</span>")
				chassis.visible_message("<span class='danger'>[chassis] rips [target]'s arms off.</span>")
			else
				step_away(M,chassis)
				chassis.occupant_message("I smash into [target], sending them flying.")
...
/obj/item/mecha_parts/mecha_equipment/tool/passenger/destroy()
	for(var/atom/movable/AM in src)
		...
		to_chat(AM, "<span class='danger'>I tumble out of the destroyed [src.name]!</span>")
	return ..()
...
/obj/item/mecha_parts/mecha_equipment/tool/passenger/proc/move_inside(var/mob/user)
	...
	if(do_after(user, 40, src, needhand=0))
		...
		to_chat(user, "I stop entering the exosuit.")
...
/obj/item/mecha_parts/mecha_equipment/tool/passenger/verb/eject()
	...
	if(usr != occupant)
		return
	to_chat(occupant, "I climb out from \the [src].")
	go_out()
...
/obj/item/mecha_parts/mecha_equipment/tool/passenger/Topic(href,href_list)
	...
	if (href_list["toggle_lock"])
		...
		if (chassis)
			chassis.visible_message("The hatch on \the [chassis] [door_locked? "locks" : "unlocks"].", "I hear something latching.")
...
/obj/mecha/proc/move_inside_passenger()
	...
	if (!isturf(usr.loc))
		to_chat(usr, "<span class='danger'>I can't reach the passenger compartment from here.</span>")
		return
	...
	for(var/mob/living/carbon/slime/M in range(1,usr))
		if(M.Victim == usr)
			to_chat(usr, "<span class='danger'>I'm too busy getting my life sucked out of you.</span>")
			return
