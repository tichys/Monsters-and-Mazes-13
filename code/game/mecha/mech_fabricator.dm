	...
	if(materials[material] + amnt <= res_max_amount)
		if(stack && stack.amount >= 1)
			...
			while(materials[material] + amnt <= res_max_amount && stack.amount >= 1)
				...
				stack.use(1)
				count++
			to_chat(user, "I insert [count] [count==1 ? stack_singular : stack_plural] into the fabricator.")// 0 steel sheets, 1 steel sheet, 2 steel sheets, etc
...
/obj/machinery/mecha_part_fabricator/emag_act(var/remaining_charges, var/mob/user)
	switch(emagged)
		if(0)
			...
			sleep(30)
			visible_message("\icon[src] <b>[src]</b> beeps: \"User DB truncated. Please contact my [GLOB.using_map.company_name] system operator for future assistance.\"")
			req_access = null
			emagged = 1
			return 1
		if(0.5)
...
/obj/machinery/mecha_part_fabricator/proc/check_build()
	...
	if(D.build_path)
		...
		visible_message("\The [src] pings, indicating that \the [D] is complete.", "I hear a ping.")
		if(mat_efficiency != 1)
