...
/obj/mecha/relaymove(mob/user,direction)
	if(user != src.occupant) //While not "realistic", this piece is player friendly.
		...
		to_chat(user, "I climb out from [src]")
		return 0
	if(connected_port)
	...
	if(istype(user,/mob/living/carbon/human))
		...
		if(H.species.can_shred(user))
			if(!deflect_hit(is_melee=1))
				...
				to_chat(user, "<span class='danger'>I slash at the armored suit!</span>")
				visible_message("<span class='danger'>\The [user] slashes at [src.name]'s armor!</span>")
			else
				src.log_append_to_last("Armor saved.")
				playsound(src.loc, 'sound/weapons/slash.ogg', 50, 1, -1)
				to_chat(user, "<span class='danger'>My claws had no effect!</span>")
			...
			user.visible_message("<span class='danger'>\The [user] hits \the [src]. Nothing happens.</span>","<span class='danger'>I hit \the [src] with no visible effect.</span>")
			src.log_append_to_last("Armor saved.")
		return
...
/obj/mecha/attack_blob(mob/user as mob)
	...
	if(!prob(src.deflect_chance))
		...
		to_chat(user, "<span class='danger'>I smash at the armored suit!</span>")
		...
		src.log_append_to_last("Armor saved.")
		playsound(src.loc, 'sound/effects/blobattack.ogg', 50, 1, -1)
		to_chat(user, "<span class='warning'>My attack had no effect!</span>")
...
/obj/mecha/attackby(obj/item/weapon/W as obj, mob/user as mob)
		...
		spawn()
			if(E.can_attach(src))
				if(!user.unEquip(E))
					return
				E.attach(src)
				user.visible_message("[user] attaches [W] to [src]", "I attach [W] to [src]")
			else
				to_chat(user, "I were unable to attach [W] to [src]")
		return
	...
	if(id_card)
		...
		if(state==1)
			state = 2
			to_chat(user, "I undo the securing bolts.")
		else if(state==2)
			state = 1
			to_chat(user, "I tighten the securing bolts.")
		return
	else if(isCrowbar(W))
		if(state==2)
			state = 3
			to_chat(user, "I open the hatch to the power unit")
		else if(state==3)
			state=2
			to_chat(user, "I close the hatch to the power unit")
		return
	else if(isCoil(W))
		if(state == 3 && hasInternalDamage(MECHA_INT_SHORT_CIRCUIT))
			...
			if(CC.use(2))
				...
				to_chat(user, "I replace the fused wires.")
			else
		...
		if(hasInternalDamage(MECHA_INT_TEMP_CONTROL))
			...
			to_chat(user, "I repair the damaged temperature controller.")
		else if(state==3 && src.cell)
			src.cell.forceMove(src.loc)
			src.cell = null
			state = 4
			to_chat(user, "I unscrew and pry out the powercell.")
			src.log_message("Powercell removed.")
		else if(state==4 && src.cell)
			state=3
			to_chat(user, "I screw the cell in place.")
		return

	else if(isMultitool(W))
		if(state>=3 && src.occupant)
			to_chat(user, "I attempt to eject the pilot using the maintenance controls.")
			if(src.occupant.stat)
				src.go_out()
				src.log_message("[src.occupant] was ejected using the maintenance controls.")
			else
				to_chat(user, "<span class='warning'>My attempt is rejected.</span>")
				src.occupant_message("<span class='warning'>An attempt to eject I was made using the maintenance controls.</span>")
		...
		if(state==4)
			if(!src.cell)
				if(!user.unEquip(W, src))
					return
				to_chat(user, "I install the powercell")
				src.cell = W
...
//skytodo: //No idea what I want me to do here, mate.
...
/obj/mecha/verb/move_inside()
	...
	if (usr.buckled)
		to_chat(usr, "<span class='warning'>I can't climb into the exosuit while buckled!</span>")
		return
	...
	if(iscarbon(usr))
		...
		if(C.handcuffed)
			to_chat(usr, "<span class='danger'>Kinda hard to climb in while handcuffed don't I think?</span>")
			return
	if (src.occupant)
...
*/
	...
	for(var/mob/living/carbon/slime/M in range(1,usr))
		if(M.Victim == usr)
			to_chat(usr, "I'm too busy getting my life sucked out of you.")
			return
//	to_chat(usr, "I start climbing into [src.name]")
	...
	if(enter_after(40,usr))
		...
		to_chat(usr, "I stop entering the exosuit.")
	return
...
/obj/mecha/proc/go_out()
	...
	/*
		...
		//Can not have a pressure delta that would cause environment pressure > tank pressure
			...
			to_chat(occupant, "I were blown out of the mech!")
	*/
...
/obj/mecha/Topic(href, href_list)
	...
	if(href_list["select_equip"])
		...
		if(equip)
			src.selected = equip
			src.occupant_message("I switch to [equip]")
	...
	if(href_list["remove_passenger"] && state >= 1)
		...
		user.visible_message("<span class='notice'>\The [user] begins opening the hatch on \the [P]...</span>", "<span class='notice'>I begin opening the hatch on \the [P]...</span>")
		if (!do_after(user, 40, needhand=0))
			return

		user.visible_message("<span class='notice'>\The [user] opens the hatch on \the [P] and removes [occupant]!</span>", "<span class='notice'>I open the hatch on \the [P] and remove [occupant]!</span>")
		P.go_out()
	...
	if(href_list["dna_lock"])
		...
		if(istype(occupant, /mob/living/carbon/brain))
			occupant_message("I am a brain. No.")
			return
		if(src.occupant)
