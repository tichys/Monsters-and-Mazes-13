...
/obj/structure/foamedmetal/attack_hand(var/mob/user)
	if ((HULK in user.mutations) || (prob(75 - metal * 25)))
		user.visible_message("<span class='warning'>[user] smashes through the foamed metal.</span>", "<span class='notice'>I smash through the metal foam wall.</span>")
		qdel(src)
	else
		to_chat(user, "<span class='notice'>I hit the metal foam but bounce off it.</span>")
	return
...
/obj/structure/foamedmetal/attackby(var/obj/item/I, var/mob/user)
	...
	if(prob(I.force * 20 - metal * 25))
		user.visible_message("<span class='warning'>[user] smashes through the foamed metal.</span>", "<span class='notice'>I smash through the foamed metal with \the [I].</span>")
		qdel(src)
	else
		to_chat(user, "<span class='notice'>I hit the metal foam to no effect.</span>")
