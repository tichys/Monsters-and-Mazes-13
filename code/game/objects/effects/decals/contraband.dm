...
/obj/item/weapon/contraband/poster/afterattack(var/atom/A, var/mob/user, var/adjacent, var/clickparams)
	...
	if (!iswall(W) || !isturf(user.loc))
		to_chat(user, "<span class='warning'>I can't place this here!</span>")
		return

	var/placement_dir = get_dir(user, W)
	if (!(placement_dir in GLOB.cardinal))
		to_chat(user, "<span class='warning'>I must stand directly in front of the wall I wish to place that on.</span>")
		return
	...
	user.visible_message("<span class='notice'>\The [user] starts placing a poster on \the [W].</span>","<span class='notice'>I start placing the poster on \the [W].</span>")
	...
	if(do_after(user, 28, W) && iswall(W) && !ArePostersOnWall(W, P))
		user.visible_message("<span class='notice'>\The [user] has placed a poster on \the [W].</span>","<span class='notice'>I have placed the poster on \the [W].</span>")
	else
...
/obj/structure/sign/poster/attack_hand(mob/user as mob)
	...
	if(alert("Do I want to rip the poster from the wall?","I think...","Yes","No") == "Yes")
