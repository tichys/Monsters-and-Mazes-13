...
// to something, like a smoking beaker, so then I can just call start() and the steam
...
// The attach(atom/atom) proc is optional, and can be called to attach the effect
// to something, like the RCD, so then I can just call start() and the sparks
...
// in case I wanted a vent to always smoke north for example
/////////////////////////////////////////////
	...
	start()
		if (amount <= 2)
			...
			for(var/mob/M in viewers(1, location))
				if (prob (50 * amount))
					to_chat(M, "<span class='warning'>The explosion knocks I down.</span>")
					M.Weaken(rand(1,5))
			return
		else
			var/devst = -1
