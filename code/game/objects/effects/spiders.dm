...
/obj/effect/spider/spiderling/mundane/dormant
	dormant = TRUE    // It lies in wait, hoping I will walk face first into its web
			...
			if(entry_vent.network && entry_vent.network.normal_members.len)
				...
				spawn(rand(20,60))
					...
					spawn(travel_time)
						...
						if(prob(50))
							src.visible_message("<span class='notice'>I hear something squeezing through the ventilation ducts.</span>",2)
						sleep(travel_time)
	...
	if(isturf(loc))
		...
		if(!O.owner || O.owner.stat == DEAD || amount_grown > 80)
			...
			if(world.time > last_itch + 30 SECONDS)
				...
				to_chat(O.owner, "<span class='notice'>my [O.name] itches...</span>")
	else if(prob(1))
