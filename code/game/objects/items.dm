...
//	causeerrorheresoifixthis
	...
	//This flag is used to determine when items in someone's inventory cover others. IE helmets making it so I can't see glasses, etc.
	...
	var/slowdown_general = 0 // How much clothing is slowing I down. Negative values speeds I up. This is a genera##l slowdown, no matter equipment slot.
	var/slowdown_per_slot[slot_last] // How much clothing is slowing I down. This is an associative list: item slot - slowdown
	var/slowdown_accessory // How much an accessory will slow I down when attached to a worn article of clothing.
...
/obj/item/attack_hand(mob/user as mob)
	...
	if (hasorgans(user))
		...
		if(temp && !temp.is_usable())
			...
			return
		if(!temp)
			to_chat(user, "<span class='notice'>I try to use my hand, but realize it is no longer attached!</span>")
			return
...
//If I am making custom procs but would like to retain partial or complete functionality of this one, include a 'return ..()' to where I want this to happen.
//Set disable_warning to 1 if I wish it to not give me outputs.
	...
	switch(slot)
		...
		if(slot_belt, slot_wear_id)
			if(slot == slot_belt && (item_flags & ITEM_FLAG_IS_BELT))
				...
				if(!disable_warning)
					to_chat(H, "<span class='warning'>I need a jumpsuit before I can attach this [name].</span>")
				return 0
		...
		if(slot_s_store)
			if(!H.wear_suit && (slot_wear_suit in mob_equip))
				if(!disable_warning)
					to_chat(H, "<span class='warning'>I need a suit before I can attach this [name].</span>")
				return 0
			if(!H.wear_suit.allowed)
				if(!disable_warning)
					to_chat(usr, "<span class='warning'>I somehow have a suit with no defined allowed items for suit storage, stop that.</span>")
				return 0
		...
		if(slot_tie)
			if((!H.w_uniform && (slot_w_uniform in mob_equip)) && (!H.wear_suit && (slot_wear_suit in mob_equip)))
				if(!disable_warning)
					to_chat(H, "<span class='warning'>I need something I can attach \the [src] to.</span>")
				return 0
			if(H.w_uniform && (slot_w_uniform in mob_equip))
				var/obj/item/clothing/under/uniform = H.w_uniform
				if(uniform && !uniform.can_attach_accessory(src))
					if (!disable_warning)
						to_chat(H, "<span class='warning'>I cannot equip \the [src] to \the [uniform].</span>")
					return 0
				else return 1
			if(H.wear_suit && (slot_wear_suit in mob_equip))
				...
				if(suit && !suit.can_attach_accessory(src))
					if (!disable_warning)
						to_chat(H, "<span class='warning'>I cannot equip \the [src] to \the [suit].</span>")
					return 0
...
/obj/item/verb/verb_pickup()
	...
	if((!istype(usr, /mob/living/carbon)) || (istype(usr, /mob/living/carbon/brain)))//Is humanoid, and is not a brain
		to_chat(usr, "<span class='warning'>I can't pick things up!</span>")
		return
	if( usr.stat || usr.restrained() )//Is not asleep/dead and is not restrained
		to_chat(usr, "<span class='warning'>I can't pick things up!</span>")
		return
	if(src.anchored) //Object isn't anchored
		to_chat(usr, "<span class='warning'>I can't pick that up!</span>")
		return
	if(!usr.hand && usr.r_hand) //Right hand is not full
		to_chat(usr, "<span class='warning'>My right hand is full.</span>")
		return
	if(usr.hand && usr.l_hand) //Left hand is not full
		to_chat(usr, "<span class='warning'>My left hand is full.</span>")
		return
...
/obj/item/proc/eyestab(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	...
	if(istype(H))
		for(var/obj/item/protection in list(H.head, H.wear_mask, H.glasses))
			if(protection && (protection.body_parts_covered & EYES))
				// I can't stab someone in the eyes wearing a mask!
				to_chat(user, "<span class='warning'>I'm going to need to remove the eye covering first.</span>")
				return
	...
	//	M = user
		/*
		to_chat(M, "<span class='warning'>I stab myself in the eye.</span>")
	...
	if(istype(H))
		...
		if(H != user)
			...
			to_chat(M, "<span class='danger'>[user] stabs I in the eye with [src]!</span>")
			to_chat(user, "<span class='danger'>I stab [M] in the eye with [src]!</span>")
		else
...
modules/mob/mob_movement.dm if I move I will be zoomed out
modules/mob/living/carbon/human/life.dm if I die, I will be zoomed out.
*/
//Looking through a scope or binoculars should /not/ improve my periphereal vision. Still, increase viewsize a tiny bit so that sniping isn't as restricted to NSEW
/obj/item/proc/zoom(mob/user, var/tileoffset = 14,var/viewsize = 9) //tileoffset is client view offset in the direction the user is facing. viewsize is how far out this thing zooms. 7 is normal view
	...
	if(user.incapacitated(INCAPACITATION_DISABLED))
		to_chat(user, "<span class='warning'>I am unable to focus through the [devicename].</span>")
		return
	else if(!zoom && istype(H) && H.equipment_tint_total >= TINT_MODERATE)
		to_chat(user, "<span class='warning'>My visor gets in the way of looking through the [devicename].</span>")
		return
	else if(!zoom && user.get_active_hand() != src)
		to_chat(user, "<span class='warning'>I am too distracted to look through the [devicename], perhaps if it was in my active hand this might work better.</span>")
		return
