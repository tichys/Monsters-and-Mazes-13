...
/obj/item/blueprints/attack_self(mob/M as mob)
	if (!istype(M,/mob/living/carbon/human))
		to_chat(M, "This stack of blue paper means nothing to me.")//monkeys cannot into projecting
...
"}
	switch (get_area_type())
		if (AREA_SPACE)
			text += {"
<p>According the blueprints, I am now in <b>outer space</b>.  Hold my breath.</p>
<p><a href='?src=\ref[src];action=create_area'>Mark this place as new area.</a></p>
"}
		if (AREA_STATION)
			if (A.apc)
				text += {"
<p>According the blueprints, I am now in <b>\"[A.name]\"</b>.</p>
<p>I may <a href='?src=\ref[src];action=edit_area'>
move an amendment</a> to the drawing.</p>
<p>I can't erase this area, because it has an APC.</p>
"}
			else
				text += {"
<p>According the blueprints, I am now in <b>\"[A.name]\"</b>.</p>
...
/obj/item/blueprints/proc/delete_area()
	...
	if (get_area_type(A)!=AREA_STATION || A.apc) //let's just check this one last time, just in case
		interact()
		return
	to_chat(usr, "<span class='notice'>I scrub [A.name] off the blueprint.</span>")
