...
/obj/item/weapon/pen/crayon/afterattack(atom/target, mob/user as mob, proximity)
	...
	if(istype(target,/turf/simulated/floor))
		...
		switch(drawtype)
			...
			if("graffiti")
				to_chat(user, "I start drawing graffiti on the [target.name].")
			if("rune")
				to_chat(user, "I start drawing a rune on the [target.name].")
			if("arrow")
				drawtype = input("Choose the arrow.", "Crayon scribbles") in list("left", "right", "up", "down")
				to_chat(user, "I start drawing an arrow on the [target.name].")
...
/obj/item/weapon/pen/crayon/attack(mob/living/carbon/M as mob, mob/user as mob)
	if(istype(M) && M == user)
		to_chat(M, "I take a bite of the crayon and swallow it.")
		M.nutrition += 1
		M.reagents.add_reagent(/datum/reagent/crayon_dust,min(5,uses)/3)
		if(uses)
			uses -= 5
			if(uses <= 0)
				to_chat(M, "<span class='warning'>I ate my crayon!</span>")
				qdel(src)
	else
		..()
