...
/obj/item/device/boombox/proc/AdjustFrequency(var/obj/item/W, var/mob/user)
	...
	var/operation = input(user, "How do I wish to adjust the player head?", "Adjust player", options[1]) as null|anything in options
	if(!operation)
		return FALSE
	if(!MayAdjust(user))
		return FALSE
	...
	user.visible_message(SPAN_NOTICE("\The [user] adjusts \the [src]'s player head."), SPAN_NOTICE("I adjust \the [src]'s player head."))
...
/obj/item/device/boombox/proc/MayAdjust(var/mob/user)
	if(playing)
		to_chat(user, "<span class='warning'>I can only adjust \the [src] when it's not playing.</span>")
		return FALSE
	return TRUE
