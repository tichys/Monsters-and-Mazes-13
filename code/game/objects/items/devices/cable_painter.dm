obj/item/device/cable_painter
	...
	description_info = "Use this device to select a preferred cable color. Apply it to a bundle of cables on my person, or use it on installed cabling on the floor to paint it in my chosen color."
...
obj/item/device/cable_painter/attack_self(mob/user)
	var/new_color_selection = input("What color would I like to use?", "Choose a Color", color_selection) as null|anything in GLOB.possible_cable_colours
	if(new_color_selection && !user.incapacitated() && (src in user))
		color_selection = new_color_selection
		to_chat(user, "<span class='notice'>I change the paint mode to [lowertext(color_selection)].</span>")
...
/obj/item/device/cable_painter/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(istype(A, /obj/structure/cable))
		...
		to_chat(user, "<span class='notice'>I set \the [A]'s color to [lowertext(color_selection)].</span>")
	else if(isCoil(A))
