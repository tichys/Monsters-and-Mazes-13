...
/obj/item/device/dociler/attack_self(var/mob/user)
	...
	to_chat(user, "I set \the [src] to [mode] docile mode.")
...
/obj/item/device/dociler/attack(var/mob/living/L, var/mob/user)
	...
	to_chat(L, "<span class='notice'>I feel pain as \the [user] injects something into me. All of a sudden I feel as if [user] is the friendliest and nicest person you've ever know. I want to be friends with him and all his friends.</span>")
	if(mode == "somewhat")
	...
	var/name = input(user, "Would I like to rename \the [L]?", "Dociler", L.name) as text
	if(length(name))
		L.real_name = name
		L.SetName(name)
