		...
		if(istype(H))
			for(var/obj/item/clothing/C in list(H.head,H.wear_mask,H.glasses))
				if(istype(C) && (C.body_parts_covered & EYES))
					to_chat(user, "<span class='warning'>I'm going to need to remove [C] first.</span>")
					return

			var/obj/item/organ/vision
			if(!H.species.vision_organ || !H.should_have_organ(H.species.vision_organ))
				to_chat(user, "<span class='warning'>I can't find anything on [H] to direct [src] into!</span>")
				return
			...
			user.visible_message("<span class='notice'>\The [user] directs [src] into [M]'s [vision.name].</span>", \
								 "<span class='notice'>I direct [src] into [M]'s [vision.name].</span>")
...
/obj/item/device/flashlight/proc/inspect_vision(obj/item/organ/vision, mob/living/user)
	...
	if(H == user)	//can't look into my own eyes buster
		return
...
/obj/item/device/flashlight/flare/proc/activate(var/mob/user)
	...
	on = 1
	if(user)
		user.visible_message("<span class='notice'>[user] pulls the cord on \the [src], activating it.</span>", "<span class='notice'>I pull the cord on \the [src], activating it!</span>")
	update_damage()
	update_icon()
...
/obj/item/device/flashlight/flare/glowstick/activate(var/mob/user)
	if(on)
		return
	if(user)
		user.visible_message("<span class='notice'>[user] cracks and shakes \the [src].</span>", "<span class='notice'>I crack and shake \the [src], turning it on!</span>")
	START_PROCESSING(SSobj, src)
