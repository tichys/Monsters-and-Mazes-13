/obj/item/device/floor_painter
	...
	description_info = "Use this item in my hand to access a menu in which I may change the type of decal, applied direction, and color. Click any accessible tile on the floor to apply my choice."
	...
	description_antag = "This thing would be perfect for vandalism. Could I write my name in the halls?"
...
/obj/item/device/floor_painter/resolve_attackby(var/atom/A, var/mob/user, proximity, params)
	...
	if(color_picker)
		...
		to_chat(usr, "<span class='notice'>I set \the [src] to paint with <font color='[paint_colour]'>a new colour</font>.</span>")
		return
	...
	if(istype(D))
		var/choice
		if(!D.paintable)
			to_chat(user, "<span class='warning'>I can't paint this airlock type.</span>")
			return
		if(D.paintable == AIRLOCK_PAINTABLE)
			...
			choice = input(user, "What do I wish to paint?") as null|anything in list("Paint","Stripe")
	...
	if(config_error)
		to_chat(user, "<span class='warning'>\The [src] flashes an error light. I might need to reconfigure it.</span>")
		return

	if(F.decals && F.decals.len > 5 && painting_decal != /obj/effect/floor_decal/reset)
		to_chat(user, "<span class='warning'>\The [F] has been painted too much; I need to clear it off.</span>")
		return
...
/obj/item/device/floor_painter/attack_self(var/mob/user)
	var/choice = input("What do I wish to change?") as null|anything in list("Decal","Direction", "Colour", "Preset Colour", "Mode")
	if(choice == "Decal")
		choose_decal()
...
/obj/item/device/floor_painter/verb/choose_decal()
	...
	if(new_decal && !isnull(decals[new_decal]))
		decal = new_decal
		to_chat(usr, "<span class='notice'>I set \the [src] decal to '[decal]'.</span>")
...
/obj/item/device/floor_painter/verb/choose_direction()
	...
	if(new_dir && !isnull(paint_dirs[new_dir]))
		paint_dir = new_dir
		to_chat(usr, "<span class='notice'>I set \the [src] direction to '[paint_dir]'.</span>")
...
/obj/item/device/floor_painter/verb/toggle_mode()
	...
	if(color_picker)
		to_chat(usr, "<span class='notice'>I set \the [src] to color picker mode, scanning colors off objects.</span>")
	else
		to_chat(usr, "<span class='notice'>I set \the [src] to painting mode.</span>")