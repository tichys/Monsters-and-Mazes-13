...
/obj/item/device/multitool/hacktool/proc/attempt_hack(var/mob/user, var/atom/target)
	if(is_hacking)
		to_chat(user, "<span class='warning'>I am already hacking!</span>")
		return 1
	...
	to_chat(user, "<span class='notice'>I begin hacking \the [target]...</span>")
	is_hacking = 1
	...
	if(hack_result && in_hack_mode)
		to_chat(user, "<span class='notice'>My hacking attempt was succesful!</span>")
		...
		to_chat(user, "<span class='warning'>My hacking attempt failed!</span>")
	return 1
