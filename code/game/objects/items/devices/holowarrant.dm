...
/obj/item/device/holowarrant/examine(mob/user)
	...
	if(in_range(user, src) || isghost(user))
		...
		to_chat(user, "<span class='notice'>I have to be closer if I want to read it.</span>")
...
/obj/item/device/holowarrant/attack_self(mob/living/user as mob)
	...
	if(warrants.len == 0)
		...
		return
	var/temp
	temp = input(user, "Which warrant would I like to load?") as null|anything in warrants
	for(var/datum/computer_file/data/warrant/W in GLOB.all_warrants)
...
/obj/item/device/holowarrant/attackby(obj/item/weapon/W, mob/user)
	if(active)
		...
		if(I && check_access_list(I.GetAccess()))
			var/choice = alert(user, "Would I like to authorize this warrant?","Warrant authorization","Yes","No")
			if(choice == "Yes")
				active.fields["auth"] = "[I.registered_name] - [I.assignment ? I.assignment : "(Unknown)"]"
			user.visible_message("<span class='notice'>I swipe \the [I] through the [src].</span>", \
...
/obj/item/device/holowarrant/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	user.visible_message("<span class='notice'>[user] holds up a warrant projector and shows the contents to [M].</span>", \
			"<span class='notice'>I show the warrant to [M].</span>")
	M.examinate(src)
