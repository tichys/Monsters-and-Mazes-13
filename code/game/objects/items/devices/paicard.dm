...
/obj/item/device/paicard/Topic(href, href_list)
	...
	if(href_list["setdna"])
		...
		if(!istype(M, /mob/living/carbon))
			to_chat(usr, "<span class='notice'>I don't have any DNA, or my DNA is incompatible with this device.</span>")
		else
			var/datum/dna/dna = usr.dna
			pai.master = M.real_name
			pai.master_dna = dna.unique_enzymes
			to_chat(pai, "<span class='warning'>I have been bound to a new master.</span>")
	if(href_list["request"])
		src.looking_for_personality = 1
		paiController.findPAI(src, usr)
	if(href_list["wipe"])
		var/confirm = input("Are I CERTAIN I wish to delete the current personality? This action cannot be undone.", "Personality Wipe") in list("Yes", "No")
		if(confirm == "Yes")
			for(var/mob/M in src)
				to_chat(M, "<font color = #ff0000><h2>I feel myself slipping away from reality.</h2></font>")
				to_chat(M, "<font color = #ff4d4d><h3>Byte by byte I lose my sense of self.</h3></font>")
	...
	if(href_list["setlaws"])
		var/newlaws = sanitize(input("Enter any additional directives I would like my pAI personality to follow. Note that these directives will not override the personality's allegiance to its imprinted master. Conflicting directives will be ignored.", "pAI Directive Configuration", pai.pai_laws) as message)
		if(newlaws)
			pai.pai_laws = newlaws
			to_chat(pai, "My supplemental directives have been updated. My new directives are:")
