...
/obj/item/device/radio/verb/wind_charge()
	set name = "Wind to Charge"
	set category = "Object"
	set src in usr
	if(istype(src, /obj/item/device/radio/headset))		return  // headsets can't be winded up
	to_chat(usr, "I begin winding up \the [src]")
	if(do_after(usr, 30, usr))
		if(cell)
			cell.charge = cell.maxcharge
			to_chat(usr, "It should be fully charged now.")

/obj/item/device/radio/RightClick(mob/user)
	wind_charge(user)
...
/obj/item/device/radio/Topic(href, href_list)
	...
	if(href_list["remove_cell"])
		if(cell)
			...
			to_chat(user, "<span class='notice'>I remove [cell] from \the [src].</span>")
			cell = null
		return 1
...
/obj/item/device/radio/talk_into(mob/living/M as mob, message, channel, var/verb = "says", var/datum/language/speaking = null)
	...
	if(loc == M)
		playsound(loc, pick('sound/effects/radio1.ogg','sound/effects/radio2.ogg','sound/effects/radio3.ogg','sound/effects/radio4.ogg'), 20, 0, -1)
	...
	*/
	...
	var/displayname = M.name	// grab the display name (name I get when I hover over someone's icon)
  ...
  /* ###### Intercoms and station-bounced radios ###### */
	...
	signal.data["level"] = list(1,2,3,4,5)  // up to 10 z levels, increase if necessary, but you shouldn't

	return	Broadcast_Message(signal.data["connection"], signal.data["mob"],
							  signal.data["vmask"], signal.data["vmessage"],
							  signal.data["radio"], signal.data["message"],
							  signal.data["name"], signal.data["job"],
							  signal.data["realname"], signal.data["vname"], filter_type,
...
/obj/item/device/radio
	var/list/channel_tags = list(
		...
		list(ERT_FREQ, "Response Team", COMMS_COLOR_CENTCOMM),
		list(DTH_FREQ, "Special Ops", COMMS_COLOR_SYNDICATE)
	)

/obj/item/device/radio/proc/get_channel_info(var/freq)
...
/obj/item/device/radio/proc/receive_range(freq, level)
	...
	if (!freq) //recieved on main frequency
		...
		var/accept = (listening && (frequency == freq || internal_channels["[freq]"]))
		if (!accept)
			for (var/ch_name in channels)
				if (channels[ch_name])
					accept = 1
					break
		if (!accept)
			for (var/ch in internal_channels)
				if(ch == freq)
					accept = 1
					break
		if (!accept)
...
/obj/item/device/radio/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(!cell && power_usage && istype(W, /obj/item/weapon/cell/device) && user.unEquip(W, target = src))
		to_chat(user, "<span class='notice'>I put [W] in \the [src].</span>")
		cell = W
		return
	...
	if (href_list["mode"])
		...
		if(enable_subspace_transmission != subspace_transmission)
			...
			if(subspace_transmission == 0)//Simple as fuck, clears the channel list to prevent talking/listening over them if subspace transmission is disabled
				...
				recalculateChannels()
		. = 1
	if (href_list["shutup"]) // Toggle loudspeaker mode, AKA everyone around me hearing my radio.
