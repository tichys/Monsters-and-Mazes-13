...
/proc/medical_scan_action(atom/target, mob/living/user, obj/scanner, var/verbose)
	if (!user.IsAdvancedToolUser())
		to_chat(user, "<span class='warning'>I am not nimble enough to use this device.</span>")
		return
...
/obj/item/device/analyzer/verb/verbosity(mob/user)
	...
	if (!user.incapacitated())
		...
		to_chat(user, "I toggle advanced gas analysis [advanced_mode ? "on" : "off"].")
...
/obj/item/device/slime_scanner
	...
	desc = "Multipurpose organic life scanner. With spectral breath analyzer I can find out what snacks Ian had! Or what gasses alien life breathes."
	icon_state = "xenobio"
	item_state = "analyzer"
	slot_flags = SLOT_BELT
	w_class = ITEM_SIZE_SMALL
