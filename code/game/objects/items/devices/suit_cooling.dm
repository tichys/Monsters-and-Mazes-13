/obj/item/device/suit_cooling_unit
	...
	description_info = "I may wear this instead of my packpack to cool myself down. It is commonly used by IPCs, \
	...
	Remember to turn it on by clicking it when it's my in my hand before I put it on."
...
// That way I can't carry it in my hands while it's running to cool myself down.
...
/obj/item/device/suit_cooling_unit/attack_self(var/mob/user)
	if(cover_open && cell)
		...
		to_chat(user, "I remove \the [src.cell].")
		src.cell = null
		update_icon()
		return
...
/obj/item/device/suit_cooling_unit/proc/toggle(var/mob/user)
	if(on)
		...
		turn_on()
	to_chat(user, "<span class='notice'>I switch \the [src] [on ? "on" : "off"].</span>")

/obj/item/device/suit_cooling_unit/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isScrewdriver(W))
		if(cover_open)
			cover_open = 0
			to_chat(user, "I screw the panel into place.")
		else
			cover_open = 1
			to_chat(user, "I unscrew the panel.")
		update_icon()
		return
		...
		if(cover_open)
			if(cell)
				...
				if(!user.unEquip(W, src))
					return
				cell = W
				to_chat(user, "I insert the [cell].")
		update_icon()
		return
