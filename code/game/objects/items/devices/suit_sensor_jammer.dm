...
/obj/item/device/suit_sensor_jammer/attackby(obj/item/I as obj, mob/user as mob)
	if(isCrowbar(I))
		if(bcell)
			to_chat(user, "<span class='notice'>I remove \the [bcell].</span>")
			disable()
			bcell.dropInto(loc)
			bcell = null
		else
		...
		if(bcell)
			...
			I.forceMove(src)
			bcell = I
			to_chat(user, "<span class='notice'>I insert \the [bcell] into \the [src]..</span>")
		else
			to_chat(user, "<span class='warning'>I'm unable to insert the battery.</span>")
