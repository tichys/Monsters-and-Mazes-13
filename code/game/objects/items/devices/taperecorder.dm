...
/obj/item/device/taperecorder/verb/eject()
	...
	if(playing || recording)
		stop()
	to_chat(usr, "<span class='notice'>I remove [mytape] from [src].</span>")
...
/obj/item/device/taperecorder/verb/record()
	...
	if(mytape.ruined || emagged)
		...
		return
	if(recording)
		to_chat(usr, "<span class='notice'>I'm already recording!</span>")
		return
	if(playing)
		to_chat(usr, "<span class='notice'>I can't record when playing!</span>")
		return
...
/obj/item/device/taperecorder/verb/wipe_tape()
	...
	if(recording || playing)
		to_chat(usr, "<span class='notice'>I can't wipe the tape while playing or recording!</span>")
		return
	else
		if(mytape.storedinfo)	mytape.storedinfo.Cut()
		if(mytape.timestamp)	mytape.timestamp.Cut()
		mytape.used_capacity = 0
		to_chat(usr, "<span class='notice'>I wipe the tape.</span>")
		return
...
/obj/item/device/taperecorder/verb/playback_memory()
	...
	if(recording)
		...
		return
	if(playing)
		to_chat(usr, "<span class='notice'>I'm already playing!</span>")
		return
	playing = 1
	update_icon()
...
/obj/item/device/tape/attackby(obj/item/I, mob/user, params)
	if(ruined && isScrewdriver(I))
		...
		if(loc == user && !user.incapacitated())
			var/new_name = input(user, "What would I like to label the tape?", "Tape labeling") as null|text
			if(isnull(new_name)) return
			new_name = sanitizeSafe(new_name)
			if(new_name)
				SetName("tape - '[new_name]'")
				to_chat(user, "<span class='notice'>I label the tape '[new_name]'.</span>")
			else
				SetName("tape")
				to_chat(user, "<span class='notice'>I scratch off the label.</span>")
		return
	..()
