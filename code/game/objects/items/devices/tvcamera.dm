...
/obj/item/robot_parts/head/attackby(var/obj/item/device/assembly/S, mob/user as mob)
	...
	qdel(S)
	user.put_in_hands(A)
	to_chat(user, "<span class='notice'>I add the infrared sensor to the robot head.</span>")
	qdel(src)
	...
	switch(buildstep)
		if(0)
			if(istype(W, /obj/item/robot_parts/robot_component/camera))
				to_chat(user, "<span class='notice'>I add the camera module to [src]</span>")
				qdel(W)
		...
		if(1)
			if(istype(W, /obj/item/device/taperecorder))
				qdel(W)
				buildstep++
				to_chat(user, "<span class='notice'>I add the tape recorder to [src]</span>")
		...
		if(2)
			if(isCoil(W))
				...
				if(!C.use(3))
					to_chat(user, "<span class='notice'>I need three cable coils to wire the devices.</span>")
					..()
					return
				C.use(3)
				buildstep++
				to_chat(user, "<span class='notice'>I wire the assembly</span>")
				desc = "This TV camera assembly has wires sticking out"
				return
		if(3)
			if(isWirecutter(W))
				to_chat(user, "<span class='notice'> I trim the wires.</span>")
				buildstep++
		...
		if(4)
			if(istype(W, /obj/item/stack/material/steel))
				...
				buildstep++
				S.use(1)
				to_chat(user, "<span class='notice'>I encase the assembly.</span>")
