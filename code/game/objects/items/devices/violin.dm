...
/obj/item/device/violin
	name = "violin"
	desc = "A wooden musical instrument with four strings and a bow, how elegant."
...
/obj/item/device/violin/Topic(href, href_list, state = GLOB.physical_state)
	...
	if(href_list["newsong"])
		...
		if(href_list["repeat"]) //Changing this from a toggle to a number of repeats to avoid infinite loops.
			...
			var/newline = html_encode(input("Enter my line: ", "violin") as text|null)
			if(!newline)
				return
			...
			var/content = html_encode(input("Enter my line: ", "violin", song.lines[num]) as text|null)
			if(!content)
				return
