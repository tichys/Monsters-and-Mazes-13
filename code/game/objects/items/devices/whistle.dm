...
/obj/item/device/hailer/verb/set_message()
	...
	set category = "Object"
	set desc = "Alter the message shouted by my hailer."
	...
	to_chat(usr, "I configure the hailer to shout \"[use_message]\".")
...
/obj/item/device/hailer/emag_act(var/remaining_charges, var/mob/user)
	if(isnull(insults))
		to_chat(user, "<span class='danger'>I overload \the [src]'s voice synthesizer.</span>")
		insults = rand(1, 3)//to prevent dickflooding
		return 1
	else
		to_chat(user, "The hailer is fried. I can't even fit the sequencer into the input slot.")
