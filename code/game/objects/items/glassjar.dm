...
/obj/item/glass_jar/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(istype(A, /mob))
		...
		if(!accept)
			...
			return
		var/mob/L = A
		user.visible_message("<span class='notice'>[user] scoops [L] into \the [src].</span>", "<span class='notice'>I scoop [L] into \the [src].</span>")
		L.forceMove(src)
		contains = 2
		update_icon()
		return
		...
		user.visible_message("<span class='notice'>[user] scoops [S] into \the [src].</span>", "<span class='notice'>I scoop [S] into \the [src].</span>")
		S.forceMove(src)
	...
	switch(contains)
		if(1)
			for(var/obj/O in src)
				O.loc = user.loc
			to_chat(user, "<span class='notice'>I take money out of \the [src].</span>")
			contains = 0
			update_icon()
			return
		if(2)
			for(var/mob/M in src)
				M.loc = user.loc
				user.visible_message("<span class='notice'>[user] releases [M] from \the [src].</span>", "<span class='notice'>I release [M] from \the [src].</span>")
			contains = 0
			update_icon()
			return
		if(3)
			for(var/obj/effect/spider/spiderling/S in src)
				S.dropInto(user.loc)
				user.visible_message("<span class='notice'>[user] releases [S] from \the [src].</span>", "<span class='notice'>I release [S] from \the [src].</span>")
