...
/obj/item/clothing/head/helmet/space/void/attackby(var/obj/item/O, var/mob/user)
	if(istype(O,/obj/item/device/kit/suit))
		...
		to_chat(user, "I set about modifying the helmet into [src].")
...
/obj/item/clothing/suit/space/void/attackby(var/obj/item/O, var/mob/user)
	if(istype(O,/obj/item/device/kit/suit))
		...
		if(kit.new_mob_icon_file)
			icon_override = kit.new_mob_icon_file
		to_chat(user, "I set about modifying the suit into [src].")
		var/mob/living/carbon/human/H = user
		if(istype(H))
			species_restricted = list(H.species.get_bodytype(H))
		kit.use(1,user)
...
/obj/mecha/attackby(var/obj/item/weapon/W, var/mob/user)
	if(istype(W, /obj/item/device/kit/paint))
		if(occupant)
			to_chat(user, "I can't customize a mech while someone is piloting it - that would be unsafe!")
			return
...
/obj/item/device/kit/paint/durand/seraph
	...
	new_desc = "A Durand combat exosuit modelled after ancient Earth entertainment. My heart goes doki-doki just looking at it."
	new_icon = "old_durand"
