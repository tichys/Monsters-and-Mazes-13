...
/obj/item/robot_parts/chest/can_install(mob/user)
	var/success = TRUE
	if(!wires)
		to_chat(user, "<span class='warning'>I need to attach wires to it first!</span>")
		success = FALSE
	if(!cell)
		to_chat(user, "<span class='warning'>I need to attach a cell to it first!</span>")
		success = FALSE
	return success && ..()
...
/obj/item/robot_parts/head/can_install(mob/user)
	var/success = TRUE;
	if(!(flash1 && flash2))
		to_chat(user, "<span class='warning'>I need to attach a flash to it first!</span>")
		success = FALSE
	return success && ..();
...
/obj/item/robot_parts/robot_suit/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/stack/material) && W.get_material_name() == DEFAULT_WALL_MATERIAL && !parts[BP_L_ARM] && !parts[BP_R_ARM] && !parts[BP_L_LEG] && !parts[BP_R_LEG] && !parts[BP_CHEST] && !parts[BP_HEAD])
		...
		if (M.use(1))
			...
			to_chat(user, "<span class='notice'>I armed the robot frame.</span>")
			if (user.get_inactive_hand()==src)
				user.drop_from_inventory(src) //clears inactive hand; item will be deleted anyway.
				user.put_in_inactive_hand(B)
			qdel(src)
		else
			to_chat(user, "<span class='warning'>I need one sheet of metal to arm the robot frame.</span>")
	...
	if(istype(W, /obj/item/device/mmi) || istype(W, /obj/item/organ/internal/posibrain))
		...
		if(check_completion())
			if(!istype(loc,/turf))
				to_chat(user, "<span class='warning'>I can't put \the [W] in, the frame has to be standing on the ground to be perfectly precise.</span>")
				return
			if(!B)
			...
			if(O.mind && O.mind.special_role)
				O.mind.store_memory("In case I look at this after being borged, the objectives are only here until I find a way to make them not show up for you, as I can't simply delete them without screwing up round-end reporting. --NeoFite")
...
/obj/item/robot_parts/chest/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/cell))
		if(src.cell)
			to_chat(user, "<span class='warning'>I have already inserted a cell!</span>")
			return
		else
			if(!user.unEquip(W, src))
				return
	...
	if(isCoil(W))
		if(src.wires)
			to_chat(user, "<span class='warning'>I have already inserted wire!</span>")
			return
		else
...
/obj/item/robot_parts/head/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/device/flash))
		if(istype(user,/mob/living/silicon/robot))
			...
			if(current_module == W)
				to_chat(user, "<span class='warning'>How do I propose to do that?</span>")
				return
			else
				add_flashes(W,user)
		else
...
/obj/item/robot_parts/emag_act(var/remaining_charges, var/mob/user)
	if(sabotaged)
		...
		to_chat(user, "<span class='warning'>I short out the safeties.</span>")
		sabotaged = 1
		return 1
