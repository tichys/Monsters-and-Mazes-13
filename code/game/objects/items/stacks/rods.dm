...
/obj/item/stack/rods/attackby(obj/item/W as obj, mob/user as mob)
	if(isWelder(W))
		...
		if(WT.remove_fuel(0,user))
			...
			for (var/mob/M in viewers(src))
				M.show_message("<span class='notice'>[src] is shaped into metal by [user.name] with the weldingtool.</span>", 3, "<span class='notice'>I hear welding.</span>", 2)
	...
	if (istype(W, /obj/item/weapon/tape_roll))
		...
		user.visible_message("<span class='notice'>\The [user] constructs \a [new_splint] out of a [singular_name].</span>", \
				"<span class='notice'>I use make \a [new_splint] out of a [singular_name].</span>")
		src.use(1)
		return
...
/obj/item/stack/rods/attack_self(mob/user as mob)
	...
	if (locate(/obj/structure/grille, usr.loc))
		...
		to_chat(usr, "<span class='notice'>I assemble a grille</span>")
		in_use = 0
