	...
	if(!is_reinforced)
		if(istype(W,/obj/item/stack/cable_coil))
			...
			if (V.get_amount() < 1 || get_amount() < 1)
				to_chat(user, "<span class='warning'>I need one rod and one sheet of glass to make reinforced glass.</span>")
				return
...
/obj/item/stack/material/glass/proc/construct_window(mob/user as mob)
	...
	switch(input(title, "What would I like to construct?") as null|anything in construction_options)
		if("One Direction")
			if(!src)	return 1
			...
			for (var/obj/structure/window/win in user.loc)
				...
				if(!(win.dir in cardinal))
					to_chat(user, "<span class='warning'>Can't let I do that.</span>")
					return 1
