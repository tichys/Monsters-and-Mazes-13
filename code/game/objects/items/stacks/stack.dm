...
/obj/item/stack/proc/produce_recipe(datum/stack_recipe/recipe, var/quantity, mob/user)
	...
	if (!can_use(required))
		if (produced>1)
			to_chat(user, "<span class='warning'>I haven't got enough [src] to build \the [produced] [recipe.display_name()]\s!</span>")
		else
			to_chat(user, "<span class='warning'>I haven't got enough [src] to build \the [recipe.display_name()]!</span>")
		return
...
/obj/item/stack/proc/add_to_stacks(mob/user, check_hands)
	...
	for (var/obj/item/stack/item in stacks)
		...
		if (transfer)
			to_chat(user, "<span class='notice'>I add a new [item.singular_name] to the stack. It now contains [item.amount] [item.singular_name]\s.</span>")
		if(!amount)
			break
...
/obj/item/stack/attack_hand(mob/user as mob)
	if (user.get_inactive_hand() == src)
		var/N = input("How many stacks of [src] would I like to split off?", "Split stacks", 1) as num|null
		if(N)
