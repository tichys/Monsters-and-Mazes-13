...
/obj/item/stack/telecrystal/afterattack(var/obj/item/I as obj, mob/user as mob, proximity)
	...
	if(istype(I, /obj/item))
		if(I.hidden_uplink && I.hidden_uplink.active) //No metagaming by using this on every PDA around just to see if it gets used up.
			...
			use(amount)
			to_chat(user, "<span class='notice'>I slot \the [src] into \the [I] and charge its internal uplink.</span>")

/obj/item/stack/telecrystal/attack_self(var/mob/user)
	if(use(ceil(DEFAULT_TELECRYSTAL_AMOUNT/20)))
		user.visible_message("<span class='warning'>\The [user] crushes a crystal!</span>", "<span class='warning'>I crush \a [src]!</span>", "I hear the sound of a crystal breaking just before a sudden crack of electricity.")
