		...
		if(O.reagents)
			if(O.reagents.total_volume < 1)
				...
				if(O.reagents.has_reagent(/datum/reagent/acid/polyacid, 1))
					...
					src.desc = "A translucent balloon with some form of liquid sloshing around in it."
					to_chat(user, "<span class='notice'>I fill the balloon with the contents of [O].</span>")
...
/obj/item/toy/water_balloon/throw_impact(atom/hit_atom)
	if(src.reagents.total_volume >= 1)
		src.visible_message("<span class='warning'>\The [src] bursts!</span>","I hear a pop and a splash.")
...
/obj/item/toy/balloon/nanotrasen
	...
	desc = "Man, I love NanoTrasen soooo much. I use only NT products. I have NO idea."
	...
	attackby(obj/item/I as obj, mob/user as mob)
		if(istype(I, /obj/item/toy/ammo/crossbow))
			if(bullets <= 4)
				...
				qdel(I)
				bullets++
				to_chat(user, "<span class='notice'>I load the foam dart into the crossbow.</span>")
			else
...
// ******* Check
		...
		if (src.bullets > 0 && M.lying)
			...
			for(var/mob/O in viewers(M, null))
				if(O.client)
					O.show_message(text("<span class='danger'>\The [] casually lines up a shot with []'s head and pulls the trigger!</span>", user, M), 1, "<span class='warning'>I hear the sound of foam against skull</span>", 2)
			...
			for(var/mob/O in viewers(M, null))
				if (O.client)	O.show_message(text("<span class='danger'>\The [] casually lines up a shot with []'s head, pulls the trigger, then realizes they are out of ammo and drops to the floor in search of some!</span>", user, M), 1, "<span class='warning'>I hear someone fall</span>", 2)
			user.Weaken(5)
		return
...
/obj/item/toy/sword
	...
	attack_self(mob/user as mob)
		...
		if (src.active)
			to_chat(user, "<span class='notice'>I extend the plastic blade with a quick flick of my wrist.</span>")
...
/obj/item/toy/snappop
	...
	throw_impact(atom/hit_atom)
		...
		src.visible_message("<span class='warning'>The [src.name] explodes!</span>","<span class='warning'>I hear a snap!</span>")
...
/obj/item/toy/snappop/Crossed(H as mob|obj)
	if((ishuman(H))) //i guess carp and shit shouldn't set them off
		...
		if(!MOVING_DELIBERATELY(M))
			to_chat(M, "<span class='warning'>I step on the snap pop!</span>")
...
/obj/item/toy/bosunwhistle/attack_self(mob/user as mob)
	if(cooldown < world.time - 35)
		to_chat(user, "<span class='notice'>I blow on [src], creating an ear-splitting noise!</span>")
...
/obj/structure/plushie/attack_hand(mob/user)
	...
	if(user.a_intent == I_HELP)
		user.visible_message("<span class='notice'><b>\The [user]</b> hugs [src]!</span>","<span class='notice'>I hug [src]!</span>")
	else if (user.a_intent == I_HURT)
		user.visible_message("<span class='warning'><b>\The [user]</b> punches [src]!</span>","<span class='warning'>I punch [src]!</span>")
	else if (user.a_intent == I_GRAB)
		user.visible_message("<span class='warning'><b>\The [user]</b> attempts to strangle [src]!</span>","<span class='warning'>I attempt to strangle [src]!</span>")
	else
		user.visible_message("<span class='notice'><b>\The [user]</b> pokes the [src].</span>","<span class='notice'>I poke the [src].</span>")
		visible_message("[src] says, \"[phrase]\"")

/obj/structure/plushie/ian
	name = "plush corgi"
	desc = "A plushie of an adorable corgi! Don't I just want to hug it and squeeze it and call it \"Ian\"?"
	icon_state = "ianplushie"
	phrase = "Arf!"
...
/obj/structure/plushie/carp
	name = "plush carp"
	desc = "A plushie of an elated carp! Straight from the wilds of the Nyx frontier, now right here in my hands."
...
/obj/item/toy/plushie/mouse
	name = "mouse plush"
	desc = "A plushie of a delightful mouse! What was once considered a vile rodent is now my very best friend."
	icon_state = "mouseplushie"

/obj/item/toy/plushie/kitten
	name = "kitten plush"
	desc = "A plushie of a cute kitten! Watch as it purrs it's way right into my heart."
	icon_state = "kittenplushie"

/obj/item/toy/plushie/lizard
	name = "lizard plush"
	desc = "A plushie of a scaly lizard, how cute."
	icon_state = "lizardplushie"

/obj/item/toy/plushie/spider
	name = "spider plush"
	desc = "A plushie of a fuzzy spider! It has eight legs - all the better to hug I with."
...
/obj/item/weapon/inflatable_duck
	...
	desc = "No bother to sink or swim when I can just float!"
...
/obj/item/weapon/marshalling_wand
	...
	desc = "An illuminated, hand-held baton used by hangar personnel to visually signal shuttle pilots. The signal changes depending on my intent."
...
/obj/item/weapon/marshalling_wand/attack_self(mob/living/user as mob)
	...
	if (user.a_intent == I_HELP)
		user.visible_message("<span class='notice'>[user] beckons with \the [src], signalling forward motion.</span>",
							"<span class='notice'>I beckon with \the [src], signalling forward motion.</span>")
		...
		user.visible_message("<span class='notice'>[user] holds \the [src] above their head, signalling a stop.</span>",
							"<span class='notice'>I hold \the [src] above my head, signalling a stop.</span>")
	else if (user.a_intent == I_GRAB)
		var/WAND_TURN_DIRECTION
		if (user.l_hand == src) WAND_TURN_DIRECTION = "left"
		else if (user.r_hand == src) WAND_TURN_DIRECTION = "right"
		else return //how can I not be holding it in either hand?? black magic
		user.visible_message("<span class='notice'>[user] waves \the [src] to the [WAND_TURN_DIRECTION], signalling a turn.</span>",
							"<span class='notice'>I wave \the [src] to the [WAND_TURN_DIRECTION], signalling a turn.</span>")
	else if (user.a_intent == I_HURT)
		user.visible_message("<span class='warning'>[user] frantically waves \the [src] above their head!</span>",
							"<span class='warning'>I frantically wave \the [src] above my head!</span>")

/obj/item/toy/genesismodel
	name = "table-top SEV Mayflower model"
	desc = "This is a replica of the SEV Mayflower, in 1:250th scale, on a handsome wooden stand. Small lights blink on the hull and at the engine exhaust."
	icon = 'icons/obj/toy.dmi'
	icon_state = "genesis_model_figure"
