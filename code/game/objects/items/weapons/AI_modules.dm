...
/obj/item/weapon/aiModule/proc/install(var/obj/machinery/computer/C)
	if (istype(C, /obj/machinery/computer/aiupload))
		...
		if(comp.stat & BROKEN)
			...
			return
		if (!comp.current)
			to_chat(usr, "I haven't selected an AI to transmit laws to!")
			return
		...
		if(comp.stat & BROKEN)
			...
			return
		if (!comp.current)
			to_chat(usr, "I haven't selected a robot to transmit laws to!")
			return
		...
		if (comp.current.stat == 2 || comp.current.emagged)
			...
			src.transmitInstructions(comp.current, usr)
			to_chat(comp.current, "These are my laws now:")
...
/obj/item/weapon/aiModule/proc/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	...
	to_chat(target, "\The [sender] has uploaded a change to the laws I must follow, using \an [src]. From now on: ")
	target.show_laws()
...
/obj/item/weapon/aiModule/protectStation
	...
	desc = "A 'protect installation' AI module: 'Protect the installation against damage. Anyone I see harming the installation is no longer to be considered a crew member, and is a threat which must be neutralized.'."
...
/obj/item/weapon/aiModule/protectStation/addAdditionalLaws(var/mob/living/silicon/ai/target, var/mob/sender)
	var/law = "Protect the installation against damage. Anyone I see harming the [station_name()] is no longer to be considered a crew member, and is a threat which must be neutralized."
...
/obj/item/weapon/aiModule/freeform/attack_self(var/mob/user as mob)
	..()
	var/new_lawpos = input("Please enter the priority for my new law. Can only write to law sectors 15 and above.", "Law Priority (15+)", lawpos) as num
...
/obj/item/weapon/aiModule/reset/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	...
	to_chat(target, "[sender.real_name] attempted to reset my laws using a reset module.")
	target.show_laws()
...
/obj/item/weapon/aiModule/purge/transmitInstructions(var/mob/living/silicon/ai/target, var/mob/sender)
	...
	to_chat(target, "[sender.real_name] attempted to wipe my laws using a purge module.")
	target.show_laws()
...
/obj/item/weapon/aiModule/solgov // aka genesis default
