...
/obj/item/weapon/rcd
	...
	description_info = "On use, this device will toggle between various types of structures (or their removal). I can examine it to see its current mode. It must be loaded with compressed matter cartridges, which can be obtained from an autolathe. Click an adjacent tile to use the device."
	...
	description_antag = "RCDs can be incredibly dangerous in the wrong hands. Use them to swiftly block off corridors, or instantly breach the ship wherever I want."
...
/decl/hierarchy/rcd_mode/proc/work_message(var/atom/target, var/mob/user, var/rcd)
	...
	if(work_type)
		...
		message = "<span class='notice'>I begin constructing \a [initial(work.name)].</span>"
	else
		message = "<span class='notice'>I begin construction.</span>"
...
/decl/hierarchy/rcd_mode/deconstruction/work_message(var/atom/target, var/mob/user, var/rcd)
	user.visible_message("<span class='warning'>\The [user] is using \a [rcd] to deconstruct \the [target]!</span>", "<span class='warning'>I am deconstructing \the [target]!</span>")
