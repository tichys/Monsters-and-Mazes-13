...
/obj/item/weapon/card/id/syndicate/afterattack(var/obj/item/weapon/O as obj, mob/user as mob, proximity)
	...
	if(istype(O, /obj/item/weapon/card/id))
		...
		if(player_is_antag(user.mind))
			to_chat(user, "<span class='notice'>The microscanner activates as I pass it over the ID, copying its access.</span>")
...
/obj/item/weapon/card/id/syndicate/attack_self(mob/user as mob)
	...
	if(!registered_user && register_user(user))
		to_chat(user, "<span class='notice'>The microscanner marks I as its owner, preventing others from accessing its internals.</span>")
	if(registered_user == user)
		switch(alert("Would I like edit the ID, or show it?","Show or Edit?", "Edit","Show"))
			if("Edit")
				ui_interact(user)
			if("Show")
				..()
...
/obj/item/weapon/card/id/syndicate/Topic(href, href_list, var/datum/topic_state/state)
	...
	if(href_list["electronic_warfare"])
		...
		switch(href_list["set"])
			if("Age")
				var/new_age = input(user,"What age would I like to put on this card?","Agent Card Age", age) as null|num
			...
			if("Blood Type")
				...
				var/new_blood_type = sanitize(input(user,"What blood type would I like to be written on this card?","Agent Card Blood Type",default) as null|text)
			...
			if("DNA Hash")
				...
				var/new_dna_hash = sanitize(input(user,"What DNA hash would I like to be written on this card?","Agent Card DNA Hash",default) as null|text)
			...
			if("Fingerprint Hash")
				...
				var/new_fingerprint_hash = sanitize(input(user,"What fingerprint hash would I like to be written on this card?","Agent Card Fingerprint Hash",default) as null|text)
				if(!isnull(new_fingerprint_hash) && CanUseTopic(user, state))
					...
					. = 1
			if("Name")
				var/new_name = sanitizeName(input(user,"What name would I like to put on this card?","Agent Card Name", registered_name) as null|text, allow_numbers=TRUE)
			...
			if("Photo")
				...
				. = 1
			if("Sex")
				var/new_sex = sanitize(input(user,"What sex would I like to put on this card?","Agent Card Sex", sex) as null|text)
			...
			if("Factory Reset")
				if(alert("This will factory reset the card, including access and owner. Continue?", "Factory Reset", "No", "Yes") == "Yes" && CanUseTopic(user, state))
					...
					. = 1
			if("Branch")
				var/new_branch = sanitize(input(user,"What branch of service would I like to put on this card?","Agent Card Branch") as null|anything in mil_branches.spawn_branches())
				if(!isnull(new_branch) && CanUseTopic(user, state))
					...
					. = 1
			if("Rank")
				var/new_rank = sanitize(input(user,"What rank would I like to put on this card?","Agent Card Rank") as null|anything in mil_branches.spawn_ranks(military_branch.name))
