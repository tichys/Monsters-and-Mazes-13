...
/obj/item/clothing/mask/smokable
	name = "smokable item"
	desc = "I'm not sure what this is. I should probably ahelp it."
...
/obj/item/clothing/mask/smokable/New()
	..()
	atom_flags |= ATOM_FLAG_NO_REACT // so it doesn't react until I light it
...
/obj/item/clothing/mask/smokable/cigarette/attack(mob/living/carbon/human/H, mob/user, def_zone)
	if(lit && H == user && istype(H))
		...
		if(blocked)
			...
			return 1
		to_chat(H, "<span class='notice'>I take a drag on my [name].</span>")
		smoke(5)
		return 1
	return ..()
...
/obj/item/clothing/mask/smokable/cigarette/afterattack(obj/item/weapon/reagent_containers/glass/glass, mob/user as mob, proximity)
	...
	if(!proximity)
		return
	if(istype(glass)) //I can dip cigarettes into beakers
		if(!glass.is_open_container())
			to_chat(user, "<span class='notice'>I need to take the lid off first.</span>")
			return
...
/obj/item/clothing/mask/smokable/cigarette/cigar/cohiba
	...
	desc = "There's little more I could want from a cigar."
	icon_state = "cigar2off"
	icon_on = "cigar2on"
...
//tobacco sold seperately if you're too snobby to grow it myself.
...
//and if I am a savage I can just use a sheet of ordinary paper.
/obj/item/weapon/paper/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/weapon/reagent_containers/food/snacks/grown))
		...
		if(G.dry)
			...
			qdel(src)
			user.put_in_active_hand(R)
			to_chat(user, "<span class='notice'>I roll the [target.name] into a regular sheet of paper. How bold.</span>")
...
/obj/item/weapon/teleportation_scroll/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/weapon/reagent_containers/food/snacks/grown))
		...
		if(G.dry)
			...
			qdel(src)
			user.put_in_active_hand(R)
			to_chat(user, "<span class='notice'>I roll the [target.name] into the wizard's teleportation scroll. Not like he'll be needing it anymore.</span>")
...
/obj/item/paper/cig/filter/afterattack(atom/target, mob/user, proximity)
	...
	if(istype(target, /obj/item/clothing/mask/smokable/cigarette/rolled))
		...
		qdel(src)
		user.put_in_active_hand(R)
		to_chat(user, "<span class='notice'>I roll the filter into the rolled cigarette.</span>")
...
/obj/item/clothing/mask/smokable/pipe/die(var/nomessage = 0)
	...
	if(ismob(loc))
		var/mob/living/M = loc
		if (!nomessage)
			to_chat(M, "<span class='notice'>my [name] goes out, and I empty the ash.</span>")
...
/obj/item/clothing/mask/smokable/pipe/attack_self(mob/user as mob)
	if(lit == 1)
		...
		user.visible_message("<span class='notice'>[user] empties out [src].</span>", "<span class='notice'>I empty out [src].</span>")
...
/obj/item/clothing/mask/smokable/pipe/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W, /obj/item/weapon/reagent_containers/food/snacks))
		...
		if (!G.dry)
			to_chat(user, "<span class='notice'>[G] must be dried before I stuff it into [src].</span>")
			return
		if (smoketime)
...
/obj/item/clothing/mask/chewable/tobacco
	name = "wad"
	desc = "A chewy wad of terbecco. Cut in long strands and treated with syrups so it doesn't taste like a ash-tray when I stuff it into my face."
	throw_speed = 0.5
	icon_state = "chew"
...
//			butt.desc += " This one is \a [brand]."
		if(ismob(loc))
			var/mob/living/M = loc
			if (!nomessage)
				to_chat(M, "<span class='notice'>I spit out the [name].</span>")
		qdel(src)

/obj/item/clothing/mask/chewable/tobacco/lenni
	name = "chewing tobacco"
	desc = "A chewy wad of tobacco. Cut in long strands and treated with syrups so it tastes less like a ash-tray when I stuff it into my face."
	filling = list(/datum/reagent/tobacco = 2)

/obj/item/clothing/mask/chewable/tobacco/redlady
	name = "chewing tobacco"
	desc = "A chewy wad of fine tobacco. Cut in long strands and treated with syrups so it doesn't taste like a ash-tray when I stuff it into my face"
	filling = list(/datum/reagent/tobacco/fine = 2)
...
/obj/item/weapon/flame/lighter/proc/light_effects(mob/living/carbon/user)
	if(prob(95))
		...
		to_chat(user, "<span class='warning'>I burn myself while lighting the lighter.</span>")
		if (user.l_hand == src)
...
/obj/item/weapon/flame/lighter/zippo/shutoff_effects(mob/user)
	user.visible_message("<span class='rose'>I hear a quiet click, as [user] shuts off [src] without even looking at what they're doing.</span>")
...
/obj/item/weapon/flame/lighter/zippo/afterattack(obj/O, mob/user, proximity)
	...
	if (istype(O, /obj/structure/reagent_dispensers/fueltank) && !lit)
		...
		to_chat(user, "<span class='notice'>I refuel [src] from \the [O]</span>")
