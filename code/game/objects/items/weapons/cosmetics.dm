	...
	if(ishuman(A))
		...
		if(user.a_intent == I_HELP && target_zone == BP_HEAD)
			...
			if(H.lip_style)	//if they already have lipstick on
				to_chat(user, "<span class='notice'>I need to wipe off the old lipstick first!</span>")
				return
			if(H == user)
				...
				user.visible_message("<span class='warning'>[user] begins to do [H]'s lips with \the [src].</span>", \
									 "<span class='notice'>I begin to apply \the [src].</span>")
				if(do_after(user, 20, H) && do_after(H, 20, needhand = 0, progress = 0, incapacitation_flags = INCAPACITATION_NONE))	//user needs to keep their active hand, H does not.
					user.visible_message("<span class='notice'>[user] does [H]'s lips with \the [src].</span>", \
										 "<span class='notice'>I apply \the [src].</span>")
					H.lip_style = colour
					H.update_body()
...
//I can wipe off lipstick with paper! see code/modules/paperwork/paper.dm, paper/attack()
