...
/obj/item/weapon/defibrillator/attackby(obj/item/weapon/W, mob/user, params)
	if(W == paddles)
		...
		if(bcell)
			...
			W.forceMove(src)
			bcell = W
			to_chat(user, "<span class='notice'>I install a cell in \the [src].</span>")
			update_icon()
		...
		if(bcell)
			...
			bcell = null
			to_chat(user, "<span class='notice'>I remove the cell from \the [src].</span>")
			update_icon()
	else
		return ..()
...
/obj/item/weapon/defibrillator/verb/toggle_paddles()
	...
	if(!slot_check())
		to_chat(user, "<span class='warning'>I need to equip [src] before taking out [paddles].</span>")
	else
...
/obj/item/weapon/shockpaddles
	...
	var/safety = 1 //if I can zap people with the paddles on harm mode
...
/obj/item/weapon/shockpaddles/proc/can_use(mob/user, mob/M)
	...
	if(!wielded && !isrobot(user))
		to_chat(user, "<span class='warning'>I need to wield the paddles with both hands before I can use them on someone!</span>")
		return 0
	if(cooldown)
...
//Since harm-intent now skips the delay for deliberate placement, I have to be able to hit them in combat in order to shock people.
...
/obj/item/weapon/shockpaddles/proc/do_revive(mob/living/carbon/human/H, mob/living/user)
	if(H.ssd_check())
		to_chat(find_dead_player(H.ckey, 1), "<span class='notice'>Someone is attempting to resuscitate you. Re-enter my body if I want to be revived!</span>")

	//beginning to place the paddles on patient's chest to allow some time for people to move away to stop the process
	user.visible_message("<span class='warning'>\The [user] begins to place [src] on [H]'s chest.</span>", "<span class='warning'>I begin to place [src] on [H]'s chest...</span>")
	if(!user.do_skilled(3 SECONDS, SKILL_MEDICAL, H))
		return
	user.visible_message("<span class='notice'>\The [user] places [src] on [H]'s chest.</span>", "<span class='warning'>I place [src] on [H]'s chest.</span>")
...
/obj/item/weapon/shockpaddles/proc/lowskill_revive(mob/living/carbon/human/H, mob/living/user)
	if(prob(60))
		...
		user.visible_message("<span class='warning'><i>The paddles were misaligned! \The [user] shocks [H] with \the [src]!</i></span>", "<span class='warning'>The paddles were misaligned! I shock [H] with \the [src]!</span>")
		return 0
	if(prob(50))
		playsound(get_turf(src), 'sound/machines/defib_zap.ogg', 100, 1, -1)
		...
		user.visible_message("<span class='warning'><i>\The [user] shocks themselves with \the [src]!</i></span>", "<span class='warning'>I forget to move my hands away and shock myself with \the [src]!</span>")
		return 0
	return 1
...
/obj/item/weapon/shockpaddles/proc/do_electrocute(mob/living/carbon/human/H, mob/user, var/target_zone)
	...
	user.visible_message("<span class='danger'>\The [user] slaps [src] onto [H]'s [affecting.name].</span>", "<span class='danger'>I overcharge [src] and slap them onto [H]'s [affecting.name].</span>")

	//Just stop at awkwardly slapping electrodes on people if the safety is enabled
	if(safety)
		to_chat(user, "<span class='warning'>I can't do that while the safety is enabled.</span>")
		return
	...
	user.visible_message("<span class='danger'><i>\The [user] shocks [H] with \the [src]!</i></span>", "<span class='warning'>I shock [H] with \the [src]!</span>")
	playsound(get_turf(src), 'sound/machines/defib_zap.ogg', 100, 1, -1)
...
/obj/item/weapon/shockpaddles/emag_act(var/uses, var/mob/user, var/obj/item/weapon/defibrillator/base)
	...
	if(safety)
		safety = 0
		to_chat(user, "<span class='warning'>I silently disable \the [src]'s safety protocols with the cryptographic sequencer.</span>")
		burn_damage_amt *= 3
		base.update_icon()
		return 1
	else
		safety = 1
		to_chat(user, "<span class='notice'>I silently enable \the [src]'s safety protocols with the cryptographic sequencer.</span>")
...
/obj/item/weapon/shockpaddles/standalone/checked_use(var/charge_amt)
	SSradiation.radiate(src, charge_amt/12) //just a little bit of radiation. It's the price I pay for being powered by magic I guess
	return 1
