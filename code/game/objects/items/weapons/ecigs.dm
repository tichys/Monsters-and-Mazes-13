	...
	if(ishuman(loc))
		...
		if (!active || !ec_cartridge || !ec_cartridge.reagents.total_volume)//no cartridge
			if(!ec_cartridge.reagents.total_volume)
				to_chat(C, "<span class='notice'>There's no liquid left in \the [src], so I shut it down.</span>")
			Deactivate()
			return

		if (src == C.wear_mask && C.check_has_mouth()) //transfer, but only when not disabled
			idle = 0
			//here we'll reduce battery by usage, and check powerlevel - I only use batery while smoking
...
/obj/item/clothing/mask/smokable/ecig/attackby(var/obj/item/I, var/mob/user as mob)
	...
	if(istype(I, /obj/item/weapon/screwdriver))
		if(cigcell) //if contains powercell
			...
			cigcell = null
			to_chat(user, "<span class='notice'>I remove \the [cigcell] from \the [src].</span>")
	...
	if(istype(I, /obj/item/weapon/cell/device))
		if(!cigcell && user.unEquip(I))
			I.forceMove(src)
			cigcell = I
			to_chat(user, "<span class='notice'>I install \the [cigcell] into \the [src].</span>")
			update_icon()
		else
...
/obj/item/clothing/mask/smokable/ecig/attack_self(mob/user as mob)
	if (active)
		...
		if(cigcell)
			if (!ec_cartridge)
				to_chat(user, "<span class='notice'>I can't use \the [src] with no cartridge installed!</span> ")
				return
			else if(!ec_cartridge.reagents.total_volume)
				to_chat(user, "<span class='notice'>I can't use \the [src] with no liquid left!</span> ")
				return
...
/obj/item/clothing/mask/smokable/ecig/attack_hand(mob/user as mob)//eject cartridge
	if(user.get_inactive_hand() == src)//if being hold
		if (ec_cartridge)
			...
			to_chat(user, "<span class='notice'>I remove \the [ec_cartridge] from \the [src].</span> ")
			ec_cartridge = null
			update_icon()
	else
		..()
...
/obj/item/weapon/reagent_containers/ecig_cartridge/blanknico
	...
	desc = "A small metal cartridge which contains an atomizing coil and a solution to be atomized. The label says I can add whatever flavoring agents I want."
