...
/obj/effect/spresent/relaymove(mob/user as mob)
	if (user.stat)
		return
	to_chat(user, "<span class='warning'>I can't move.</span>")
...
/obj/item/weapon/wrapping_paper/attackby(obj/item/W as obj, mob/user as mob)
	...
	if (!( locate(/obj/structure/table, src.loc) ))
		to_chat(user, "<span class='warning'>I MUST put the paper on a table!</span>")
	if (W.w_class < ITEM_SIZE_HUGE)
		if(isWirecutter(user.l_hand) || isWirecutter(user.r_hand))
			...
			if (a_used == ITEM_SIZE_NO_CONTAINER)
				to_chat(user, "<span class='warning'>I can't wrap that!</span>")//no gift-wrapping lit welders
			...
			to_chat(user, "<span class='warning'>I need scissors!</span>")
	else
