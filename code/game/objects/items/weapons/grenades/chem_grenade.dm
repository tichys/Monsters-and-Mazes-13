...
//				detonator.loc=src.loc
		...
		if(stage > 1 && !active && clown_check(user))
			to_chat(user, "<span class='warning'>I prime \the [name]!</span>")
	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		...
		if(istype(W,/obj/item/device/assembly_holder) && (!stage || stage==1) && path != 2)
			...
			to_chat(user, "<span class='notice'>I add [W] to the metal casing.</span>")
			...
			if(stage == 1)
				path = 1
				if(beakers.len)
					to_chat(user, "<span class='notice'>I lock the assembly.</span>")
					SetName("grenade")
				else
//					to_chat(user, "<span class='warning'>I need to add at least one beaker before locking the assembly.</span>")
					to_chat(user, "<span class='notice'>I lock the empty assembly.</span>")
				...
				if(active && prob(95))
					to_chat(user, "<span class='warning'>I trigger the assembly!</span>")
					detonate()
					return
				else
					to_chat(user, "<span class='notice'>I unlock the assembly.</span>")
					playsound(src.loc, 'sound/items/Screwdriver.ogg', 25, -3)
					SetName("unsecured grenade with [beakers.len] containers[detonator?" and detonator":""]")
			...
			if(beakers.len == 2)
				...
				if(W.reagents.total_volume)
					if(!user.unEquip(W, src))
						return
					to_chat(user, "<span class='notice'>I add \the [W] to the assembly.</span>")
					beakers += W
					stage = 1
					SetName("unsecured grenade with [beakers.len] containers[detonator?" and detonator":""]")
				else
	...
	detonate()
		...
		if(istype(loc, /mob/living/carbon))		//drop dat grenade if it goes off in my hand
