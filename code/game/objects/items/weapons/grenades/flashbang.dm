...
//This really should be in mob not every check
	if(ishuman(M))
		...
		if (E && E.damage >= E.min_bruised_damage)
			to_chat(M, "<span class='danger'>My eyes start to burn badly!</span>")
			if(!banglet && !(istype(src , /obj/item/weapon/grenade/flashbang/clusterbang)))
				if (E.damage >= E.min_broken_damage)
					to_chat(M, "<span class='danger'>I can't see anything!</span>")
	if (M.ear_damage >= 15)
		to_chat(M, "<span class='danger'>My ears start to ring badly!</span>")
		if(!banglet && !(istype(src , /obj/item/weapon/grenade/flashbang/clusterbang)))
			if (prob(M.ear_damage - 10 + 5))
				to_chat(M, "<span class='danger'>I can't hear anything!</span>")
				M.sdisabilities |= DEAF
	else
		if (M.ear_damage >= 5)
			to_chat(M, "<span class='danger'>My ears start to ring!</span>")
	M.update_icons()
