	...
	if(istype(C))
		if(!C.handcuffed)
			...
			if(can_place(C, user))
				...
				to_chat(user, "<span class='danger'>I need to have a firm grip on [C] before I can put \the [src] on!</span>")
		else
...
/obj/item/weapon/handcuffs/proc/place_handcuffs(var/mob/living/carbon/target, var/mob/user)
	...
	if (!H.has_organ_for_slot(slot_handcuffed))
		to_chat(user, "<span class='danger'>\The [H] needs at least two wrists before I can cuff them together!</span>")
		return 0
...
/mob/living/carbon/human/RestrainedClickOn(var/atom/A)
	...
	H.visible_message("<span class='warning'>\The [H] chews on \his [O.name]!</span>", "<span class='warning'>I chew on my [O.name]!</span>")
