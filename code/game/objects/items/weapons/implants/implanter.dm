...
/obj/item/weapon/implanter/verb/remove_implant()
	...
	if(can_use(usr))
		...
		to_chat(usr, "<span class='notice'>I remove \the [imp] from \the [src].</span>")
		name = "implanter"
		imp = null
		update_icon()
		return
	else
		to_chat(usr, "<span class='notice'>I cannot do this in my current condition.</span>")
...
/obj/item/weapon/implanter/attackby(obj/item/weapon/I, mob/user)
	if(!imp && istype(I, /obj/item/weapon/implant) && user.unEquip(I,src))
		to_chat(usr, "<span class='notice'>I slide \the [I] into \the [src].</span>")
		imp = I
		update_icon()
	else
		..()
