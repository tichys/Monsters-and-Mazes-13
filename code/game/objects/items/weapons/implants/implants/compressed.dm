...
/obj/item/weapon/implanter/compressed/afterattack(obj/item/A, mob/user as mob, proximity)
	...
	if(istype(A) && imp)
		...
		if (c.scanned)
			...
			if (!istype(A,/obj/item/weapon/storage))
				to_chat(user, "<span class='warning'>The matter compressor safeties prevent I from doing that.</span>")
			return
...
/obj/item/weapon/implanter/compressed/attack_self(var/mob/user)
	...
	safe = !safe
	to_chat(user, "<span class='notice'>I [safe ? "enable" : "disable"] the matter compressor safety.</span>")
