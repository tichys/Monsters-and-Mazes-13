...
/obj/item/taperoll/attack_self(mob/user as mob)
	if(!start)
		start = get_turf(src)
		to_chat(usr, "<span class='notice'>I place the first end of \the [src].</span>")
		update_icon()
	else
		end = get_turf(src)
		...
		if(start == end)
			...
			if(!possible_dirs)
				start = null
				update_icon()
				to_chat(usr, "<span class='notice'>I can't place \the [src] here.</span>")
				return
			...
			start = null
			update_icon()
			to_chat(usr, "<span class='notice'>I finish placing \the [src].</span>")
			return
		...
		if (!can_place)
			start = null
			update_icon()
			to_chat(usr, "<span class='warning'>I can't run \the [src] through that!</span>")
			return
...
/obj/item/taperoll/afterattack(var/atom/A, mob/user as mob, proximity)
	...
	if (istype(A, /obj/machinery/door/airlock))
		...
		to_chat(user, "<span class='notice'>I finish placing \the [src].</span>")
	...
	if (istype(A, /turf/simulated/floor) ||istype(A, /turf/unsimulated/floor))
		...
		if(tape_roll_applications[F] & direction) // hazard_overlay in F.overlays wouldn't work.
			user.visible_message("\The [user] uses the adhesive of \the [src] to remove area markings from \the [F].", "I use the adhesive of \the [src] to remove area markings from \the [F].")
			F.overlays -= hazard_overlay
			tape_roll_applications[F] &= ~direction
		else
			user.visible_message("\The [user] applied \the [src] on \the [F] to create area markings.", "I apply \the [src] on \the [F] to create area markings.")
			F.overlays |= hazard_overlay
			tape_roll_applications[F] |= direction
		return
...
/obj/item/tape/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
	if(!lifted && ismob(mover))
		...
		if (!allowed(M))	//only select few learn art of not crumpling the tape
			to_chat(M, "<span class='warning'>I am not supposed to go past [src]...</span>")
...
/obj/item/tape/proc/breaktape(mob/user)
	if(user.a_intent == I_HELP)
		to_chat(user, "<span class='warning'>I refrain from breaking \the [src].</span>")
		return
	user.visible_message("<span class='notice'>\The [user] breaks \the [src]!</span>","<span class='notice'>I break \the [src].</span>")
