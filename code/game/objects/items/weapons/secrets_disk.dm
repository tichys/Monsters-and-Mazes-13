...
/obj/item/weapon/disk/secret_project/examine(var/user)
	...
	if(!locked)
		...
		I peer into its preview screen and see <span class='notice'>[subject]</span>.")
	else
		to_chat(user, "The disk is locked, I cannot see its contents.")
...
/obj/item/weapon/disk/secret_project/emag_act(var/remaining_charges, var/mob/user)
	...
	My sequencer can't break the code.</span>")
	return 0
...
/obj/item/weapon/disk/secret_project/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W,/obj/item/weapon/card/id))
		...
		if(check_access(ID))
			locked = !locked
			to_chat(user, "<span class='notice'>I swipe my card and [locked ? "lock":"unlock"] the disk.</span>")
		else
...
/obj/item/weapon/disk/secret_project/verb/change_codename()
	...
	if(!locked)
		var/input = sanitize(input(usr, "What would I like to change the project codename to?", "Classified Project Data Disk"))
		if(!input || input == "")
			return
