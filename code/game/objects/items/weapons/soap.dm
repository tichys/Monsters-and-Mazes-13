...
/obj/item/weapon/soap/afterattack(atom/target, mob/user as mob, proximity)
	...
	if(user.client && (target in user.client.screen))
		to_chat(user, "<span class='notice'>I need to take that [target.name] off before cleaning it.</span>")
		...
		to_chat(user, "<span class='notice'>I scrub \the [target.name] clean.</span>")
		var/turf/T = target
		T.clean(src, user)
	else if(istype(target,/obj/structure/sink))
		to_chat(user, "<span class='notice'>I wet \the [src] in the sink.</span>")
		wet()
	else
...
/obj/item/weapon/soap/attackby(var/obj/item/I, var/mob/user)
	if(istype(I, /obj/item/weapon/key))
		if(!key_data)
			to_chat(user, "<span class='notice'>I imprint \the [I] into \the [src].</span>")
