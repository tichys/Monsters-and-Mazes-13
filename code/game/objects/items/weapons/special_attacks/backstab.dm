...
target: Mob
user: mob/user
damage: Num. Defaults to 30 if not set. Can be zero, if I just want the backstab check itself. This is the flat damage done to SIMPLE MOBS. Humans recieve organ damage instead.
damtype: Expects a damage type macro. Can accept the damage strings, but it is recommended I use the macro instead.
...
/obj/item/weapon/proc/backstab(var/mob/living/target, mob/user, var/damage = 30, var/damage_type = BRUTE, var/damage_flags, var/target_zone = BP_CHEST, var/location_check = TRUE)
	...
	if(!iscarbon(target)) //No. I cannot backstab the borg.
		return FALSE
	...
	if( target.lying && ( target.dir in list(NORTH, EAST) ) )
		location_check = FALSE //Skip the  check for locations if the enemy is floored. \His back is waiting for you, seductively. It's ready to take my knife.
	...
	if(location_check)
		...
		if(!( sharp ))
			user.visible_message("<span class = 'danger'>\The [user] tries to stab deep into \The [target]'s back, but it dinks off, scraping \him instead!</span>", "<span class = 'warning'>\The [src] is too dull for a proper backstab!</span>", "<span class = 'notice'>I hear a soft dinking noise.</span>")
			return FALSE

		if(!( get_turf(user) == get_step(target, turn( target.dir, 180)) ) ) //I amn't behind them.
			return FALSE
		...
		if( get_turf(user) == get_turf(target) ) //To prevent people from stabbing people from Neckgrab. Still possible when they're lying face down, but you're fucked anyways.
			to_chat(user, "<span class = 'notice'>I am too close to [target] to stab them properly!</span>")
			return FALSE

		if( target.lying && ( target.dir in list(WEST, SOUTH) ) ) //Failed the above lying check. His back isn't exposed.
			to_chat(user, "<span class = 'notice'>I can't reach \The [target]'s back, flip them over!</span>")
			return FALSE

		if(target_zone in list(BP_L_FOOT, BP_R_FOOT) ) //No feetstabs.
			to_chat(user, "<span class = 'notice'>How do I expect to get a meaningful backstab on that floppy thing? </span>")
			return FALSE
	...
	if(damage >= 1) //Let's not do a damage check if it doesn't actually do damage.
		...
		if(ishuman(target))
			...
			if( !prob(target.run_armor_check(target_zone, "melee")) && !isnull(stabbed_part) && length(stabbed_part.internal_organs) )
				...
				to_chat(user, "<span class = 'danger'>I stab [target] in the back of \His [stabbed_part]!</span>")
				H.custom_pain("<span class = 'danger' font size='10'>I feel a stabbing pain in the back of my [stabbed_part]!</span>") //Only the stabber and stabbed should know how bad this is.
			...
			to_chat(user, "<span class = 'danger'>I stab [target] in the back!</span>")

	return TRUE //Returns a value in case I want to layer additional behavior on this.
