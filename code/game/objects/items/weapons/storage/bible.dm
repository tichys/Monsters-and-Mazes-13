...
/obj/item/weapon/storage/bible/afterattack(atom/A, mob/user as mob, proximity)
	...
	if(user.mind && (user.mind.assigned_role == "Chaplain"))
		if(A.reagents && A.reagents.has_reagent(/datum/reagent/water)) //blesses all the water in the holder
			to_chat(user, "<span class='notice'>I bless \the [A].</span>") // I wish it was this easy in nethack
...
/obj/item/weapon/storage/bible/verb/rename_bible()
	...
	set category = "Object"
	set desc = "Click to rename my bible."

	if(!renamed)
		var/input = sanitizeSafe(input("What do I want to rename my bible to? I can only do this once.", ,""), MAX_NAME_LEN)

		var/mob/M = usr
		if(src && input && !M.stat && in_range(M,src))
			SetName(input)
			to_chat(M, "I name my religious book [input].")
			renamed = 1
			return 1
...
/obj/item/weapon/storage/bible/verb/set_icon()
	...
	set category = "Object"
	set desc = "Click to change my book's icon."
	...
	if(!icon_changed)
		...
		for(var/i = 10; i >= 0; i -= 1)
			if(src && !M.stat && in_range(M,src))
				...
				if(i != 0)
					var/confirm = alert(M, "Is this what I want? Chances remaining: [i]", "Confirmation", "Yes", "No")
					if(confirm == "Yes")
						icon_changed = 1
						break
				if(i == 0)
