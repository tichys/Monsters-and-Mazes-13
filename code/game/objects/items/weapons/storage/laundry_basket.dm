...
/obj/item/weapon/storage/laundry_basket/attack_hand(mob/user as mob)
	if(ishuman(user))
		...
		if(!temp)
			to_chat(user, "<span class='warning'>I need two hands to pick this up!</span>")
			return

	if(user.get_inactive_hand())
		to_chat(user, "<span class='warning'>I need my other hand to be empty</span>")
		return
	return ..()

/obj/item/weapon/storage/laundry_basket/attack_self(mob/user as mob)
	var/turf/T = get_turf(user)
	to_chat(user, "<span class='notice'>I dump the [src]'s contents onto \the [T].</span>")
	return ..()
...
/obj/item/weapon/storage/laundry_basket/pickup(mob/user)
	...
	O.desc = "My second grip on the [name]."
	O.linked = src
