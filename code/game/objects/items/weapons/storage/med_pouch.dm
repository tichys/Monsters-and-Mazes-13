...
/obj/item/weapon/storage/med_pouch
	...
	\t7) Contact the medical team with my location.
...
/obj/item/weapon/storage/med_pouch/attack_self(mob/user as mob)
	if(!opened)
		user.visible_message("<span class='notice'>\The [user] tears open [src], breaking the vacuum seal!</span>", "<span class='notice'>I tear open [src], breaking the vacuum seal!</span>")
		opened = 1
...
/obj/item/weapon/storage/med_pouch/attackby(obj/item/W, mob/user)
	if(!opened)
		to_chat(user, "<span class='notice'>The [src] must be opened to insert objects.</span>")
	else
		..()
...
/obj/item/weapon/storage/med_pouch/trauma
	...
	/obj/item/weapon/reagent_containers/pill/pouch_pill/oxycodone
		)
	instructions = {"
	1) Tear open the emergency medical pack using the easy open tab at the top.\n\
	\t2) Carefully remove all items from the pouch and discard the pouch.\n\
	...
	\t4) Use bandages to stop bleeding if required.\n\
	\t5) Force the injured party to swallow all pills.\n\
	\t6) Contact the medical team with my location.
...
/obj/item/weapon/storage/med_pouch/oxyloss
	...
	\t5) Contact the medical team with my location.\n\
	\t6) Find a source of oxygen if possible.\n\
	\t7) Update the medical team with my new location.\n\
	8) Stay in place once they respond.\
		"}
...
/obj/item/weapon/reagent_containers/pill/pouch_pill/oxycodone
	chem_type = /datum/reagent/tramadol/oxycodone
