...
/obj/item/weapon/storage
	...
	var/use_to_pickup	//Set this to make it possible to use this item in an inverse way, so I can have the item in my hand and click items on the floor to pick them up.
...
//The stop_warning parameter will stop the insertion message from being displayed. It is intended for cases where I am inserting multiple items at once,
	...
	if(usr)
		...
		if(!prevent_warning)
			for(var/mob/M in viewers(usr, null))
				if (M == usr)
					to_chat(usr, "<span class='notice'>I put \the [W] into [src].</span>")
...
//This proc is called when I want to place an item into the storage item.
...
/obj/item/weapon/storage/attack_hand(mob/user as mob)
	...
	if (check_inaccessible(user)) /* misc checks on if the user can actually open the storage */
		return
...
/obj/item/weapon/storage/proc/check_inaccessible(mob/user as mob)
	return
...
/obj/item/proc/get_storage_cost()
	//If I want to prevent stuff above a certain w_class from being stored, use max_w_class
