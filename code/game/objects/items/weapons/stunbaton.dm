...
/obj/item/weapon/melee/baton/attackby(obj/item/weapon/W, mob/user)
	if(istype(W, /obj/item/weapon/cell/device))
		if(!bcell && user.unEquip(W))
			W.forceMove(src)
			bcell = W
			to_chat(user, "<span class='notice'>I install a cell into the [src].</span>")
			update_icon()
		else
...
/obj/item/weapon/melee/baton/attack(mob/M, mob/user)
	if(status && (CLUMSY in user.mutations) && prob(50))
		to_chat(user, "<span class='danger'>I accidentally hit myself with the [src]!</span>")
		user.Weaken(30)
...
/obj/item/weapon/melee/baton/robot/attack_self(mob/user)
	...
	if (R)
		return ..()
	else	// Stop pretending and get out of my cardborg suit, human.
		to_chat(user, "<span class='warning'>I don't seem to be able interacting with this by myself..</span>")
		add_fingerprint(user)
	return 0
