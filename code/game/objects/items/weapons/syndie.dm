...
/*Click it when closed to open, when open to bring up a prompt asking I if I want to close it or press the button.*/
...
/obj/item/weapon/syndie/c4detonator
	...
	var/pr_open = 0  /*Is the "What do I want to do?" prompt open?*/

/obj/item/weapon/syndie/c4detonator/attack_self(mob/user as mob)
	switch(src.icon_state)
		if("c-4detonator_0")
			src.icon_state = "c-4detonator_1"
			to_chat(user, "I flick open the lighter.")

		if("c-4detonator_1")
			if(!pr_open)
				pr_open = 1
				switch(alert(user, "What would I like to do?", "Lighter", "Press the button.", "Close the lighter."))
					if("Press the button.")
						to_chat(user, "<span class='warning'>I press the button.</span>")
					...
					if("Close the lighter.")
						...
						to_chat(user, "I close the lighter.")
				pr_open = 0
