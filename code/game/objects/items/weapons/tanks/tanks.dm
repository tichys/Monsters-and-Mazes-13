...
/obj/item/weapon/tank/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(isCoil(W))
		...
		if(C.use(1))
			wired = 1
			to_chat(user, "<span class='notice'>I attach the wires to the tank.</span>")
			update_icon(override = TRUE)

	if(isWirecutter(W))
		if(wired && proxyassembly.assembly)

			to_chat(user, "<span class='notice'>I carefully begin clipping the wires that attach to the tank.</span>")
			if(do_after(user, 100,src))
				wired = 0
				to_chat(user, "<span class='notice'>I cut the wire and remove the device.</span>")
				...
				to_chat(user, "<span class='danger'>I slip and bump the igniter!</span>")
				if(prob(85))
					proxyassembly.receive_signal()

		else if(wired)
			if(do_after(user, 10, src))
				to_chat(user, "<span class='notice'>I quickly clip the wire from the tank.</span>")
				wired = 0
				update_icon(override = TRUE)
	...
	if(istype(W, /obj/item/device/assembly_holder))
		if(wired)
			to_chat(user, "<span class='notice'>I begin attaching the assembly to \the [src].</span>")
			if(do_after(user, 50, src))
				to_chat(user, "<span class='notice'>I finish attaching the assembly to \the [src].</span>")
				...
				to_chat(user, "<span class='notice'>I stop attaching the assembly.</span>")
		else
			to_chat(user, "<span class='notice'>I need to wire the device up first.</span>")

	if(isWelder(W))
		var/obj/item/weapon/weldingtool/WT = W
		if(WT.remove_fuel(1,user))
			if(!valve_welded)
				to_chat(user, "<span class='notice'>I begin welding the \the [src] emergency pressure relief valve.</span>")
				if(do_after(user, 40,src))
					to_chat(user, "<span class='notice'>I carefully weld \the [src] emergency pressure relief valve shut.</span><span class='warning'> \The [src] may now rupture under pressure!</span>")
					valve_welded = 1
					leaking = 0
				else
					...
					if(WT.welding)
						to_chat(user, "<span class='danger'>I accidentally rake \the [W] across \the [src]!</span>")
...
/obj/item/weapon/tank/proc/toggle_valve(var/mob/user)
	if(istype(loc,/mob/living/carbon))
		...
		if(location.internal == src)
			...
			to_chat(user, "<span class='notice'>I close the tank release valve.</span>")
			...
			if(can_open_valve)
				...
				to_chat(user, "<span class='notice'>I open \the [src] valve.</span>")
				if (location.internals)
					location.internals.icon_state = "internal1"
			else
				to_chat(user, "<span class='warning'>I need something to connect to \the [src].</span>")
...
/obj/item/weapon/tank/proc/check_status()
	...
	if(pressure > TANK_FRAGMENT_PRESSURE)
		...
		if(integrity <= 0)
			...
			visible_message("\icon[src] <span class='danger'>\The [src] flies apart!</span>", "<span class='warning'>I hear a bang!</span>")
			T.hotspot_expose(air_contents.temperature, 70, 1)
		...
		if((integrity <= 19 || leaking) && !valve_welded)
			...
			if(!leaking)
				visible_message("\icon[src] <span class='warning'>\The [src] relief valve flips open with a hiss!</span>", "I hear hissing.")
...
/obj/item/device/tankassemblyproxy
	...
	desc = "Used as a stand in to trigger single tank assemblies... but I shouldn't see this."
...
/obj/item/device/tankassemblyproxy/receive_signal()	//This is mainly called by the sensor through sense() to the holder, and from the holder to here.
	tank.ignite()	//boom (or not boom if I made shijwtty mix)
...
/obj/item/weapon/tank/proc/assemble_bomb(W,user)	//Bomb assembly proc. This turns assembly+tank into a bomb
	...
	if(!M.unEquip(src))
		return					//Remove the tank from my character,in case I were holding it
