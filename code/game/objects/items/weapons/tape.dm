	...
	if(istype(H))
		if(user.zone_sel.selecting == BP_EYES)
			...
			if(H.wear_suit && istype(H.wear_suit, /obj/item/clothing/suit/space))
				if(H == user || do_mob(user, H, 10))	//Skip the time-check if patching my own suit, that's handled in attackby()
					playsound(src, 'sound/effects/tape.ogg',25)
					H.wear_suit.attackby(src, user)
			else
				to_chat(user, "<span class='warning'>\The [H] isn't wearing a spacesuit for I to reseal.</span>")
...
/obj/item/weapon/ducttape/attack_self(mob/user)
	...
	to_chat(user, "I remove \the [initial(name)] from [stuck].")
...
/obj/item/weapon/ducttape/afterattack(var/A, mob/user, flag, params)
	...
	if(target_turf != source_turf)
		...
		if(!(dir_offset in GLOB.cardinal))
			to_chat(user, "I cannot reach that from here.")// can only place stuck papers in cardinal directions, to
