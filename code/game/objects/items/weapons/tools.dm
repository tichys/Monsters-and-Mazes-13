...
/obj/item/weapon/wrench
	...
	description_info = "This versatile tool is used for dismantling machine frames, anchoring or unanchoring heavy objects like vending machines and emitters, and much more. In general, if I want something to move or stop moving entirely, I ought to use a wrench on it."
	description_fluff = "The classic open-end wrench (or spanner, if I prefer) hasn't changed significantly in shape in over 500 years, though these days they employ a bit of automated trickery to match various bolt sizes and configurations."
...
/obj/item/weapon/screwdriver
	name = "screwdriver"
	desc = "My archetypal flathead screwdriver, with a nice, heavy polymer handle."
	description_info = "This tool is used to expose or safely hide away cabling. It can open and shut the maintenance panels on vending machines, airlocks, and much more. I can also use it, in combination with a crowbar, to install or remove windows."
...
/obj/item/weapon/wirecutters
	...
	description_info = "This tool will cut wiring anywhere I see it - make sure to wear insulated gloves! When used on more complicated machines or airlocks, it can not only cut cables, but repair them, as well."
...
/obj/item/weapon/wirecutters/attack(mob/living/carbon/C as mob, mob/user as mob)
	if(istype(C) && user.a_intent == I_HELP && (C.handcuffed) && (istype(C.handcuffed, /obj/item/weapon/handcuffs/cable)))
		...
		"I cut \the [C]'s restraints with \the [src]!",\
...
/obj/item/weapon/weldingtool
	...
	description_antag = "I can use a welder to rapidly seal off doors, ventilation ducts, and scrubbers. It also makes for a devastating weapon. Modify it with a screwdriver and stick some metal rods on it, and you've got the beginnings of a flamethrower."
	obj_flags = OBJ_FLAG_CONDUCTIBLE
	slot_flags = SLOT_BELT
...
/obj/item/weapon/weldingtool/MouseDrop(atom/over)
	...
	if(istype(over, /obj/item/weapon/weldpack))
		...
		if(wp.welder)
			...
			wp.welder = src
			usr.visible_message("[usr] attaches \the [src] to \the [wp].", "I attach \the [src] to \the [wp].")
			wp.update_icon()
		return
...
/obj/item/weapon/weldingtool/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(isScrewdriver(W))
		status = !status
		if(status)
			to_chat(user, "<span class='notice'>I secure the welder.</span>")
		else
	...
	if(istype(W, /obj/item/weapon/welder_tank))
		...
		if(!user.unEquip(W, src))
			return
		tank = W
		user.visible_message("[user] slots \a [W] into \the [src].", "I slot \a [W] into \the [src].")
		update_icon()
		return
...
/obj/item/weapon/weldingtool/attack_hand(mob/user as mob)
	if(tank && user.get_inactive_hand() == src)
		if(!welding)
			if(tank.can_remove)
				user.visible_message("[user] removes \the [tank] from \the [src].", "I remove \the [tank] from \the [src].")
...
/obj/item/weapon/weldingtool/afterattack(obj/O as obj, mob/user as mob, proximity)
	...
	if (istype(O, /obj/structure/reagent_dispensers/fueltank) && get_dist(src,O) <= 1 && !src.welding)
		...
		to_chat(user, "<span class='notice'>I refuel \the [tank].</span>")
...
/obj/item/weapon/weldingtool/proc/remove_fuel(var/amount = 1, var/mob/M = null)
	...
	if(get_fuel() >= amount)
		...
		if(M)
			to_chat(M, "<span class='notice'>I need more [welding_resource] to complete this task.</span>")
		return 0
...
//Sets the welding state of the welding tool. If I see W.welding = 1 anywhere, please change it to W.setWelding(1)
...
/obj/item/weapon/weldingtool/proc/setWelding(var/set_welding, var/mob/M)
	...
	if(set_welding && !welding)
		if (get_fuel() > 0)
			if(M)
				to_chat(M, "<span class='notice'>I switch the [src] on.</span>")
			else if(T)
	...
	//Otherwise
		...
		STOP_PROCESSING(SSobj, src)
		if(M)
			to_chat(M, "<span class='notice'>I switch \the [src] off.</span>")
		else if(T)
	...
	if(istype(user, /mob/living/carbon/human))
		...
		switch(safety)
			if(FLASH_PROTECTION_MODERATE)
				to_chat(H, "<span class='warning'>My eyes sting a little.</span>")
				E.damage += rand(1, 2)
				if(E.damage > 12)
					H.eye_blurry += rand(3,6)
			if(FLASH_PROTECTION_NONE)
				to_chat(H, "<span class='warning'>My eyes burn.</span>")
				E.damage += rand(2, 4)
				if(E.damage > 10)
					E.damage += rand(4,10)
			if(FLASH_PROTECTION_REDUCED)
				to_chat(H, "<span class='danger'>My equipment intensifies the welder's glow. My eyes itch and burn severely.</span>")
				H.eye_blurry += rand(12,20)
				E.damage += rand(12, 16)
		if(safety<FLASH_PROTECTION_MAJOR)
			if(E.damage > 10)
				to_chat(user, "<span class='warning'>My eyes are really starting to hurt. This can't be good for me!</span>")

			if (E.damage >= E.min_broken_damage)
				to_chat(H, "<span class='danger'>I go blind!</span>")
...
/obj/item/weapon/welder_tank/afterattack(obj/O as obj, mob/user as mob, proximity)
	...
	if (istype(O, /obj/structure/reagent_dispensers/fueltank) && get_dist(src,O) <= 1)
		...
		to_chat(user, "<span class='notice'>I refuel \the [src].</span>")
		playsound(src.loc, 'sound/effects/refill.ogg', 50, 1, -6)
		return
...
/obj/item/weapon/weldingtool/experimental
	...
	icon_state = "welder_l"
	item_state = "welder"
	desc = "This welding tool feels heavier in my possession than is normal. There appears to be no external fuel port."
	w_class = ITEM_SIZE_LARGE
...
/obj/item/weapon/weldingtool/attack(mob/living/M, mob/living/user, target_zone)
	...
	if(ishuman(M))
		...
		if(!welding)
			to_chat(user, "<span class='warning'>I'll need to turn [src] on to patch the damage on [M]'s [S.name]!</span>")
			return 1
...
/obj/item/weapon/crowbar
	name = "crowbar"
	desc = "A heavy crowbar of solid steel, good and solid in my hand."
	...
	description_antag = "Need to bypass a bolted door? I can use a crowbar to pry the electronics out of an airlock, provided that it has no power and has been welded shut."
	icon = 'icons/obj/tools.dmi'
	icon_state = "crowbar"
	obj_flags = OBJ_FLAG_CONDUCTIBLE
	slot_flags = SLOT_BELT
...
/obj/item/weapon/combitool/attack_self(mob/user as mob)
	...
	if(!tool)
		to_chat(user, "I can't seem to find any fittings in \the [src].")
	else
		to_chat(user, "I switch \the [src] to the [tool.name] fitting.")
	return 1
