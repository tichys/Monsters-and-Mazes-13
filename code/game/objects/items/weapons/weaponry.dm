...
/obj/item/weapon/nullrod/attack(mob/M as mob, mob/living/user as mob) //Paste from old-code to decult with a null rod.
	...
	if(M.mind && LAZYLEN(M.mind.learned_spells))
		...
		to_chat(M, "<span class='danger'>I have been silenced!</span>")
		return

	if (!(istype(user, /mob/living/carbon/human) || ticker) && ticker.mode.name != "monkey")
		to_chat(user, "<span class='danger'>I don't have the dexterity to do this!</span>")
		return

	if ((CLUMSY in user.mutations) && prob(50))
		to_chat(user, "<span class='danger'>The rod slips out of my hand and hits my head.</span>")
...
/obj/item/weapon/nullrod/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(istype(A, /turf/simulated/wall/cult))
		...
		user.visible_message("<span class='notice'>\The [user] touches \the [A] with \the [src], and the enchantment affecting it fizzles away.</span>", "<span class='notice'>I touch \the [A] with \the [src], and the enchantment affecting it fizzles away.</span>")
...
/obj/effect/energy_net/post_buckle_mob(mob/living/M)
	if(buckled_mob)
		...
		to_chat(M,"<span class='warning'>I am free of the net!</span>")
...
/obj/effect/energy_net/attack_hand(var/mob/user)
	...
	to_chat(H,"<span class='danger'>I claw at the energy net.</span>")
...
/obj/effect/energy_net/proc/escape_net(mob/user as mob)
	visible_message(
		...
		"<span class='warning'>I attempt to free myself from \the [src]!</span>"
		)
