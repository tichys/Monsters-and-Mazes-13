/obj/item/weapon/weldpack
	...
	description_info = "This pack acts as a portable source of welding fuel. Use a welder on it to refill its tank - but make sure it's not lit! I can use this kit on a fuel tank or appropriate reagent dispenser to replenish its reserves."
	...
	description_antag = "In theory, I could hold an open flame to this pack and produce some pretty catastrophic results. The trick is getting out of the blast radius."
...
/obj/item/weapon/weldpack/attackby(obj/item/W as obj, mob/user as mob)
	if(isWelder(W))
		...
		if(T.welding & prob(50))
			...
			to_chat(user, "<span class='notice'>I refuel \the [W].</span>")
...
/obj/item/weapon/weldpack/afterattack(obj/O as obj, mob/user as mob, proximity)
	...
	if (istype(O, /obj/structure/reagent_dispensers/fueltank) && src.reagents.total_volume < max_fuel)
		...
		to_chat(user, "<span class='notice'>I crack the cap off the top of the pack and fill it back up again from the tank.</span>")
		playsound(src.loc, 'sound/effects/refill.ogg', 50, 1, -6)
		return
...
/obj/item/weapon/weldpack/attack_hand(mob/user as mob)
	if(welder && user.get_inactive_hand() == src)
		...
		user.visible_message("[user] removes \the [welder] from \the [src].", "I remove \the [welder] from \the [src].")
		welder = null
		update_icon()
	else
		..()
