...
/obj/structure/alien/egg/attack_ghost(var/mob/observer/ghost/user)
	...
	if(jobban_isbanned(user, MODE_XENOMORPH))
		to_chat(user, "<span class='danger'>I am banned from playing a Xenophage.</span>")
		return

	var/confirm = alert(user, "Are I sure I want to join as a Xenophage larva?", "Become Larva", "No", "Yes")
