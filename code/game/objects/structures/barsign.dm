/obj/structure/sign/double/barsign
	...
	description_info = "If my ID has bar access, I may swipe it on this sign to alter its display."
...
/obj/structure/sign/double/barsign/attackby(obj/item/I, mob/user)
	...
	if(istype(card))
		if(access_bar in card.GetAccess())
			var/sign_type = input(user, "What would I like to change the barsign to?") as null|anything in get_valid_states(0)
			if(!sign_type)
				return
			icon_state = sign_type
			to_chat(user, "<span class='notice'>I change the barsign.</span>")
		else
