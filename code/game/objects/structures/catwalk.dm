...
/obj/structure/catwalk/attackby(obj/item/C as obj, mob/user as mob)
	...
	if(isCrowbar(C) && plated_tile)
		...
		if(hatch_open)
			...
			to_chat(user, "<span class='notice'>I pry open \the [src]'s maintenance hatch.</span>")
		else
			playsound(src, 'sound/items/Deconstruct.ogg', 100, 2)
			to_chat(user, "<span class='notice'>I shut \the [src]'s maintenance hatch.</span>")
		update_icon()
		return
	if(istype(C, /obj/item/stack/tile/mono) && !plated_tile)
		...
		if(!ST.in_use)
			...
			if (!do_after(user, 10))
				ST.in_use = 0
				return
			to_chat(user, "<span class='notice'>I plate \the [src]</span>")
			name = "plated catwalk"
			ST.in_use = 0
