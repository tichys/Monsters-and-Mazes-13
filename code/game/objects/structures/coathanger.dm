...
/obj/structure/coatrack/attack_hand(mob/user as mob)
	user.visible_message("[user] takes [coat] off \the [src].", "I take [coat] off the \the [src]")
...
/obj/structure/coatrack/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (can_hang && !coat && user.unEquip(coat, src))
		user.visible_message("[user] hangs [W] on \the [src].", "I hang [W] on the \the [src]")
		coat = W
		update_icon()
	else
		to_chat(user, "<span class='notice'>I cannot hang [W] on [src]</span>")
		return ..()
