/obj/structure/closet
	...
	var/welded = 0
	var/large = 1
	var/wall_mounted = 0 //never solid (I can always pass over it)
	var/health = 100
...
// If I adjust any of the values below, please also update /proc/unit_test_weight_of_path(var/path)
	...
	if(src.opened)
		...
		if(istype(W, /obj/item/weapon/storage/laundry_basket) && W.contents.len)
			...
			user.visible_message("<span class='notice'>[user] empties \the [LB] into \the [src].</span>", \
								 "<span class='notice'>I empty \the [LB] into \the [src].</span>", \
								 "<span class='notice'>I hear rustling of clothes.</span>")
			return
		...
		if(emag_act(INFINITY, user, "<span class='danger'>The locker has been sliced open by [user] with \an [W]</span>!", "<span class='danger'>I hear metal being sliced and sparks flying.</span>"))
		...
		src.update_icon()
		user.visible_message("<span class='warning'>\The [src] has been [welded?"welded shut":"unwelded"] by \the [user].</span>", blind_message = "I hear welding.", range = 3)
...
/obj/structure/closet/proc/slice_into_parts(obj/item/weapon/weldingtool/WT, mob/user)
	...
	user.visible_message("<span class='notice'>\The [src] has been cut apart by [user] with \the [WT].</span>", \
						 "<span class='notice'>I have cut \the [src] apart with \the [WT].</span>", \
						 "I hear welding.")
	qdel(src)
...
/obj/structure/closet/MouseDrop_T(atom/movable/O as mob|obj, mob/user as mob)
	...
	if((!( istype(O, /atom/movable) ) || O.anchored || !Adjacent(user) || !Adjacent(O) || !user.Adjacent(O) || user.contents.Find(src)))
		return
	if(!isturf(user.loc)) // are I in a container/closet/pod/etc?
		return
	if(!src.opened)
		return
...
/obj/structure/closet/proc/req_breakout()
	if(opened)
		return 0 //Door's open... wait, why are I in it's contents then?
	if((setup & CLOSET_HAS_LOCK) && locked)
...
/obj/structure/closet/proc/mob_breakout(var/mob/living/escapee)
	...
	to_chat(escapee, "<span class='warning'>I lean on the back of \the [src] and start pushing the door open. (this will take about [breakout_time] minutes)</span>")
	...
	//Well then break it!
	breakout = 0
	to_chat(escapee, "<span class='warning'>I successfully break out!</span>")
...
/obj/structure/closet/proc/togglelock(var/mob/user, var/obj/item/weapon/card/id/id_card)
	...
	if(src.broken)
		...
		return FALSE
	if(user.loc == src)
		to_chat(user, "<span class='notice'>I can't reach the lock from inside.</span>")
		return FALSE
...
/obj/structure/closet/emag_act(var/remaining_charges, var/mob/user, var/emag_source, var/visual_feedback = "", var/audible_feedback = "")
	if(make_broken())
		...
		if(visual_feedback)
			...
			visible_message("<span class='warning'>\The [src] has been broken by \the [user] with \an [emag_source]!</span>", "I hear a faint electrical spark.")
		else
			visible_message("<span class='warning'>\The [src] sparks and breaks open!</span>", "I hear a faint electrical spark.")
		return 1
	else
		. = ..()
