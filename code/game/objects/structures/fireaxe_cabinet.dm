...
/obj/structure/fireaxecabinet/attack_generic(var/mob/user, var/damage, var/attack_verb, var/wallbreaker)
	...
	if(damage_threshold > damage)
		to_chat(user, "<span class='danger'>My strike is deflected by the reinforced glass!</span>")
		return
	if(shattered)
		return
	shattered = 1
	...
	if(istype(O, /obj/item/weapon/material/twohanded/fireaxe))
		if(open)
			if(fireaxe)
				...
				O.forceMove(src)
				fireaxe = O
				to_chat(user, "<span class='notice'>I place \the [fireaxe] into \the [src].</span>")
				update_icon()
			return
...
/obj/structure/fireaxecabinet/proc/toggle_open(var/mob/user)
	if(shattered)
		...
		user.setClickCooldown(10)
		open = !open
		to_chat(user, "<span class='notice'>I [open ? "open" : "close"] \the [src].</span>")
	update_icon()
...
/obj/structure/fireaxecabinet/proc/toggle_lock(var/mob/user)
	...
	if(shattered)
		...
		user.setClickCooldown(10)
		to_chat(user, "<span class='notice'>I begin [unlocked ? "enabling" : "disabling"] \the [src]'s maglock.</span>")
		...
		to_chat(user, "<span class = 'notice'>I [unlocked ? "disable" : "enable"] the maglock.</span>")
