...
/obj/structure/fitness/punchingbag/attack_hand(var/mob/living/carbon/human/user)
	...
	if(!synth && user.nutrition < 20)
		to_chat(user, "<span class='warning'>I need more energy to use the punching bag. Go eat something.</span>")
	else
		if(user.a_intent == I_HURT)
			...
			to_chat(user, "<span class='warning'>I [pick(hit_message)] \the [src].</span>")
...
/obj/structure/fitness/weightlifter/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isWrench(W))
		...
		to_chat(user, "I set the machine's weight level to [weight].")
...
/obj/structure/fitness/weightlifter/attack_hand(var/mob/living/carbon/human/user)
	...
	var/synth = user.isSynthetic()
	if(user.loc != src.loc)
		to_chat(user, "<span class='warning'>I must be on the weight machine to use it.</span>")
		return
	if(!synth && user.nutrition < 50)
		to_chat(user, "<span class='warning'>I need more energy to lift weights. Go eat something.</span>")
		return
	if(being_used)
		...
		if(do_after(user, 20 + (weight * 10)))
			...
			if(skill < weight)
				...
				user.visible_message("<span class='notice'>\The [user] fails to lift the weights[message].</span>", "<span class='notice'>I fail to lift the weights[message].</span>")
			else
				if(!synth)
					user.nutrition -= weight * 5
				message = success_message[min(1 + round(skill - weight), fail_message.len)]
				user.visible_message("<span class='notice'>\The [user] lift\s the weights [message].</span>", "<span class='notice'>I lift the weights [message].</span>")
			being_used = 0
		else
			to_chat(user, "<span class='notice'>Against my previous judgement, perhaps working out is not for you.</span>")
			being_used = 0
