...
/obj/structure/girder/bullet_act(var/obj/item/projectile/Proj)
	//Girders only provide partial cover. There's a chance that the projectiles will just pass through. (unless I am trying to shoot the girder)
	...
	if(isWrench(W) && state == 0)
		if(anchored && !reinf_material)
			...
			if(do_after(user, 40,src))
				if(!src) return
				to_chat(user, "<span class='notice'>I dissasembled the girder!</span>")
				dismantle()
		else if(!anchored)
			playsound(src.loc, 'sound/items/Ratchet.ogg', 100, 1)
			to_chat(user, "<span class='notice'>Now securing the girder...</span>")
			if(do_after(user, 40,src))
				to_chat(user, "<span class='notice'>I secured the girder!</span>")
				reset_girder()
		...
		if(state == 2)
			...
			if(do_after(user, 40,src))
				if(!src) return
				to_chat(user, "<span class='notice'>I unsecured the support struts!</span>")
				state = 1
		...
		if(do_after(user, 40,src))
			if(!src) return
			to_chat(user, "<span class='notice'>I removed the support struts!</span>")
		...
		if(do_after(user, 40,src))
			if(!src) return
			to_chat(user, "<span class='notice'>I dislodged the girder!</span>")
			icon_state = "displaced"
			anchored = 0
			health = 50
			cover = 25
...
/obj/structure/girder/proc/construct_wall(obj/item/stack/material/S, mob/user)
	...
	to_chat(user, "<span class='notice'>I begin adding the plating...</span>")

	if(!do_after(user,40,src) || !S.use(2))
		return 1 //once we've gotten this far don't call parent attackby()

	if(anchored)
		to_chat(user, "<span class='notice'>I added the plating!</span>")
	else
		to_chat(user, "<span class='notice'>I create a false wall! Push on it to open or close the passage.</span>")
		wall_fake = 1
...
/obj/structure/girder/proc/reinforce_with_material(obj/item/stack/material/S, mob/user) //if the verb is removed this can be renamed.
	...
	if(!istype(M) || M.integrity < 50)
		to_chat(user, "I cannot reinforce \the [src] with that; it is too soft.")
		return 0

	to_chat(user, "<span class='notice'>Now reinforcing...</span>")
	if (!do_after(user, 40,src) || !S.use(2))
		return 1 //don't call parent attackby() past this point
	to_chat(user, "<span class='notice'>I added reinforcement!</span>")
