/obj/structure/bed/roller/ironingboard
	name = "ironing board"
	desc = "An ironing board to unwrinkle my wrinkled clothing."
...
/obj/structure/bed/roller/ironingboard/attackby(var/obj/item/I, var/mob/user)
	...
	if(istype(I,/obj/item/clothing))
		...
		if(buckled_mob && ishuman(buckled_mob))
			...
			visible_message("<span class='danger'>[user] begins ironing [src.buckled_mob]'s [parsed]!</span>", "<span class='danger'>I begin ironing [buckled_mob]'s [parsed]!</span>")
			if(!do_after(user, 40, src))
				return
			visible_message("<span class='danger'>[user] irons [src.buckled_mob]'s [parsed]!</span>", "<span class='danger'>I iron [buckled_mob]'s [parsed]!</span>")
...
/obj/structure/bed/roller/ironingboard/attack_hand(var/mob/user)
	if(density) // check if it's deployed
		...
		if(cloth && user.put_in_hands(cloth))
			...
			return
		if(!buckled_mob)
			to_chat(user, "I fold the ironing table down.")
			set_density(0)
	else
		to_chat(user, "I deploy the ironing table.")
		set_density(1)
	update_icon()
