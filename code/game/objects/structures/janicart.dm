		...
		switch(href_list["take"])
			if("garbage")
				if(mybag)
					...
					to_chat(user, "<span class='notice'>I take [mybag] from [src].</span>")
					mybag = null
			if("mop")
				if(mymop)
					user.put_in_hands(mymop)
					to_chat(user, "<span class='notice'>I take [mymop] from [src].</span>")
					mymop = null
			if("spray")
				if(myspray)
			...
			if("sign")
				if(signs)
					...
					if(Sign)
						...
						to_chat(user, "<span class='notice'>I take \a [Sign] from [src].</span>")
						signs--
					else
...
/obj/structure/bed/chair/janicart/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/mop))
		if(reagents.total_volume > 1)
			...
			to_chat(user, "<span class='notice'>I wet [I] in the [callme].</span>")
			playsound(loc, 'sound/effects/slosh.ogg', 25, 1)
		else
			to_chat(user, "<span class='notice'>This [callme] is out of water!</span>")
	else if(istype(I, /obj/item/key))
		to_chat(user, "Hold [I] in one of my hands while I drive this [callme].")
	else if(istype(I, /obj/item/weapon/storage/bag/trash))
		if(!user.unEquip(I, src))
			return
		to_chat(user, "<span class='notice'>I hook the trashbag onto the [callme].</span>")
		mybag = I
...
/obj/structure/bed/chair/janicart/relaymove(mob/user, direction)
	...
	if(istype(user.l_hand, /obj/item/key) || istype(user.r_hand, /obj/item/key))
		...
		to_chat(user, "<span class='notice'>I'll need the keys in one of my hands to drive this [callme].</span>")
