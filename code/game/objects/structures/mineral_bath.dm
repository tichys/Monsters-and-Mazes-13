		...
		if((H.species.name == SPECIES_ADHERENT || H.species.name == SPECIES_GOLEM) && prob(10))
			for(var/limb_type in H.species.has_limbs)
				...
				if(!E)
					...
					to_chat(occupant, "<span class='notice'>I feel my [O.name] reform in the crystal bath.</span>")
					H.update_body()
					repaired_organ = TRUE
					break
			...
			for(var/thing in H.internal_organs)
				...
				if(BP_IS_CRYSTAL(I))
					if(I.damage > 0)
						...
						to_chat(H, "<span class='notice'>The mineral-rich bath mends my [I.name].</span>")
						repaired_organ = TRUE
						break
			...
			for(var/thing in H.organs)
				...
				if(BP_IS_ROBOTIC(E))
					if(E.brute_dam || E.burn_dam)
						...
						to_chat(H, "<span class='notice'>The mineral-rich bath mends my [E.name].</span>")
