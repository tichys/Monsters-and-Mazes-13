...
//	log_debug("Note: [note]")
	...
	var/soundfile
	/*BYOND loads resource files at compile time if they are ''. This means I can't really manipulate them dynamically.
...
/obj/structure/device/piano/Topic(href, href_list, state = GLOB.physical_state)
	...
	if(href_list["newsong"])
		...
		if(href_list["repeat"]) //Changing this from a toggle to a number of repeats to avoid infinite loops.
			...
			var/tempnum = input("How many times do I want to repeat this piece? (max:10)") as num|null
			if(tempnum > 10)
				tempnum = 10
			if(tempnum < 0)
				tempnum = 0
			...
			var/newline = html_encode(input("Enter my line: ", "Piano") as text|null)
			if(!newline)
				return
			...
			var/content = html_encode(input("Enter my line: ", "Piano", song.lines[num]) as text|null)
			if(!content)
				return
			...
			do
				...
				if(lentext(t) >= 3072)
					var/cont = input(usr, "My message is too long! Would I like to continue editing it?", "", "yes") in list("yes", "no")
					if(cont == "no")
						break
...
/obj/structure/device/piano/attackby(obj/item/O as obj, mob/user as mob)
	if(isWrench(O))
		if (anchored)
			...
			to_chat(user, "<span class='notice'>I begin to loosen \the [src]'s casters...</span>")
			if (do_after(user, 40, src))
				user.visible_message( \
					"[user] loosens \the [src]'s casters.", \
					"<span class='notice'>I have loosened \the [src]. Now it can be pulled somewhere else.</span>", \
					"I hear ratchet.")
				src.anchored = 0
		else
			playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
			to_chat(user, "<span class='notice'>I begin to tighten \the [src] to the floor...</span>")
			if (do_after(user, 20, src))
				user.visible_message( \
					"[user] tightens \the [src]'s casters.", \
					"<span class='notice'>I have tightened \the [src]'s casters. Now it can be played again</span>.", \
					"I hear ratchet.")
				src.anchored = 1
	else
		..()
