...
/obj/structure/pit/attackby(obj/item/W, mob/user)
	if( istype(W,/obj/item/weapon/shovel) )
		...
		if( do_after(user, 50) )
			...
			to_chat(user, "<span class='notice'>I stop shoveling.</span>")
		return
	if (!open && istype(W,/obj/item/stack/material/wood))
		if(locate(/obj/structure/gravemarker) in src.loc)
			...
			if( do_after(user, 50) )
				...
				to_chat(user, "<span class='notice'>I stop making a grave marker.</span>")
		return
	..()
...
/obj/structure/pit/Exited(mob/living/carbon/human/H)
	. = ..()
	H.update_buried(0)
...
/obj/structure/pit/Entered(mob/living/carbon/human/H)
	. = ..()
	H.update_buried(1)
...
/obj/structure/pit/proc/digout(mob/escapee)
	...
	to_chat(escapee, "<span class='warning'>I start digging my way out of \the [src] (this will take about [breakout_time] minute\s)</span>")
	...
	for(var/i in 1 to (6*breakout_time * 2)) //minutes * 6 * 5seconds * 2
		...
		if(!do_after(escapee, 50))
			to_chat(escapee, "<span class='warning'>I have stopped digging.</span>")
			return
		if(open)
			return
	...
	to_chat(escapee, "<span class='warning'>I successfuly dig myself out!</span>")
