/obj/structure/plasticflaps //HOW DO I CALL THOSE THINGS ANYWAY
...
/obj/structure/plasticflaps/attackby(obj/item/W, mob/user)
	if(isScrewdriver(W) && !anchored)
		user.visible_message("<span class='notice'>\The [user] begins deconstructing \the [src].</span>", "<span class='notice'>I start deconstructing \the [src].</span>")
		if(user.do_skilled(3 SECONDS, SKILL_CONSTRUCTION, src))
			user.visible_message("<span class='warning'>\The [user] deconstructs \the [src].</span>", "<span class='warning'>I deconstruct \the [src].</span>")
			qdel(src)
	else ..()
