...
/obj/structure/railing/verb/flip() // This will help push railing to remote places, such as open space turfs
	...
	if(!turf_is_crowded())
		to_chat(usr, "<span class='warning'>I can't flip \the [src] - something is in the way.</span>")
		return 0
...
/obj/structure/railing/attackby(var/obj/item/W, var/mob/user)
	...
	if(istype(W, /obj/item/grab) && get_dist(src,user)<2)
		...
		if(istype(G.affecting, /mob/living/carbon/human))
			...
			if(G.force_danger())
				...
				to_chat(user, "<span class='danger'>I need a better grip to do that!</span>")
			return
	...
	if(isWelder(W))
		...
		if(F.isOn())
			...
			if(do_after(user, 20, src))
				user.visible_message("<span class='notice'>\The [user] repairs some damage to \the [src].</span>", "<span class='notice'>I repair some damage to \the [src].</span>")
	...
	if(isScrewdriver(W))
		...
		if(do_after(user, 10, src))
			to_chat(user, (anchored ? "<span class='notice'>I have unfastened \the [src] from the floor.</span>" : "<span class='notice'>I have fastened \the [src] to the floor.</span>"))
			anchored = !anchored
			update_icon()
		return
...
/obj/structure/railing/do_climb(var/mob/living/user)
	...
	if(!turf_is_crowded())
		to_chat(user, "<span class='warning'>I can't climb there, the way is blocked.</span>")
		climbers -= user
		return
