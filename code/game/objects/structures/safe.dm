...
/obj/structure/safe/proc/check_unlocked(mob/user as mob, canhear)
	if(user && canhear)
		if(tumbler_1_pos == tumbler_1_open)
			to_chat(user, "<span class='notice'>I hear a [pick("tonk", "krunk", "plunk")] from [src].</span>")
		if(tumbler_2_pos == tumbler_2_open)
			to_chat(user, "<span class='notice'>I hear a [pick("tink", "krink", "plink")] from [src].</span>")
...
/obj/structure/safe/Topic(href, href_list)
	...
	if(href_list["open"])
		if(check_unlocked())
			...
			to_chat(user, "<span class='notice'>I can't [open ? "close" : "open"] [src], the lock is engaged!</span>")
			return
	...
	if(href_list["decrement"])
		...
		if(dial == tumbler_1_pos + 1 || dial == tumbler_1_pos - 71)
			...
			if(canhear)
				to_chat(user, "<span class='notice'>I hear a [pick("clack", "scrape", "clank")] from [src].</span>")
			if(tumbler_1_pos == tumbler_2_pos + 37 || tumbler_1_pos == tumbler_2_pos - 35)
				tumbler_2_pos = decrement(tumbler_2_pos)
				if(canhear)
					to_chat(user, "<span class='notice'>I hear a [pick("click", "chink", "clink")] from [src].</span>")
...
/obj/structure/safe/attackby(obj/item/I as obj, mob/user as mob)
	if(open)
		...
		if(istype(I, /obj/item/clothing/accessory/stethoscope))
			to_chat(user, "Hold [I] in one of my hands while I manipulate the dial.")
			return
