...
/obj/structure/skele_stand/attack_hand(mob/user)
	if(swag.len)
		var/obj/item/clothing/C = input("What piece of clothing do I want to remove?", "Skeleton undressing") as null|anything in list_values(swag)
		if(C)
			swag -= get_key_by_value(swag, C)
			user.put_in_hands(C)
			to_chat(user,"<span class='notice'>I take \the [C] off \the [src]</span>")
			update_icon()
	else
...
/obj/structure/skele_stand/attackby(obj/item/weapon/W, mob/user)
	if(istype(W,/obj/item/weapon/pen))
		var/nuname = sanitize(input(user,"What do I want to name this skeleton as?","Skeleton Christening",name) as text|null)
