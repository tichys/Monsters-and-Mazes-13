		...
		if(buckled_mob.buckled == src)
			if(buckled_mob != user)
				buckled_mob.visible_message(\
					...
					"<span class='notice'>[user.name] pulls I free from the gelatinous resin.</span>",\
				...
				buckled_mob.visible_message(\
					...
					"<span class='warning'>I struggle to break free from the gelatinous resin...</span>",\
					"<span class='notice'>I hear squelching...</span>")
...
/obj/structure/bed/nest/user_buckle_mob(mob/M as mob, mob/user as mob)
	...
	if(M == usr)
		...
		M.visible_message(
			...
			"<span class='warning'>[user.name] drenches I in a foul-smelling resin, trapping I in the [src]!</span>",
			"<span class='notice'>I hear squelching...</span>")
	buckle_mob(M)
	src.add_fingerprint(user)
	return
