...
/obj/structure/bed/chair/wheelchair/relaymove(mob/user, direction)
	...
	if(user.stat || user.stunned || user.weakened || user.paralysis || user.lying || user.restrained())
		if(user==pulling)
			pulling = null
			user.pulledby = null
			to_chat(user, "<span class='warning'>I lost my grip!</span>")
		return
	...
	if(pulling && (get_dir(src.loc, pulling.loc) == direction))
		to_chat(user, "<span class='warning'>I cannot go there.</span>")
		return
	if(pulling && buckled_mob && (buckled_mob == user))
		to_chat(user, "<span class='warning'>I cannot drive while being pushed.</span>")
		return
...
/obj/structure/bed/chair/wheelchair/Move()
	...
	if(buckled_mob)
		...
		if(!driving)
			...
			if (pulling && (get_dist(src, pulling) > 1))
				pulling.pulledby = null
				to_chat(pulling, "<span class='warning'>I lost my grip!</span>")
				pulling = null
		else
			if (occupant && (src.loc != occupant.loc))
...
/obj/structure/bed/chair/wheelchair/RightClick(var/mob/user)
	if(in_range(src, user))
		if(!ishuman(user))	return
		if(user == buckled_mob)
			to_chat(user, "<span class='warning'>I realize I am unable to push the wheelchair I sit in.</span>")
			return
		if(!pulling)
			pulling = user
			user.pulledby = src
			...
			to_chat(user, "I grip \the [name]'s handles.")
		else
			to_chat(usr, "I let go of \the [name]'s handles.")
			pulling.pulledby = null
			pulling = null
		return
