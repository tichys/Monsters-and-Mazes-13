...
// Mappers: I can use "Generate Instances from Icon-states"
...
/obj/structure/transit_tube_pod/relaymove(mob/mob, direction)
	if(istype(mob, /mob) && mob.client)
		// If the pod is not in a tube at all, I can get out at any time.
		if(!(locate(/obj/structure/transit_tube) in loc))
			mob.loc = loc
			mob.client.Move(get_step(loc, direction), direction)

			//if(moving && istype(loc, /turf/space))
				// Todo: If I get out of a moving pod in space, I should move as well.
				//  Same direction as pod? Direcion I moved? Halfway between?
