...
/obj/structure/undies_wardrobe
	...
	desc = "Holds item of clothing I shouldn't be showing off in the hallways."
...
/obj/structure/undies_wardrobe/attackby(var/obj/item/underwear/underwear, var/mob/user)
	if(istype(underwear))
		if(!user.unEquip(underwear))
			return
		qdel(underwear)
		user.visible_message("<span class='notice'>\The [user] inserts \their [underwear.name] into \the [src].</span>", "<span class='notice'>I insert my [underwear.name] into \the [src].</span>")

		var/id = user.GetIdCard()
		var/message
		if(id)
			message = "ID card detected. My underwear quota for this shift as been increased, if applicable."
		else
			message = "No ID card detected. Thank I for my contribution."
...
/obj/structure/undies_wardrobe/attack_hand(var/mob/user)
	if(!human_who_can_use_underwear(user))
		to_chat(user, "<span class='warning'>Sadly there's nothing in here for I to wear.</span>")
		return
	interact(user)
...
/obj/structure/undies_wardrobe/interact(var/mob/living/carbon/human/H)
	...
	dat += "I may claim [id ? length(GLOB.underwear.categories) - LAZYACCESS(amount_of_underwear_by_id_card, id) : 0] more article\s this shift.<br><br>"
...
/obj/structure/undies_wardrobe/Topic(href, href_list, state)
	...
	if(href_list["select_underwear"])
		...
		if(!UWC)
			return
		var/datum/category_item/underwear/UWI = input("Select my desired underwear:", "Choose underwear") as null|anything in exlude_none(UWC.items)
		if(!UWI)
			return
		...
		var/id = H.GetIdCard()
		if(!id)
			audible_message("No ID card detected. Unable to acquire my underwear quota for this shift.", WARDROBE_BLIND_MESSAGE(H))
			return

		var/current_quota = LAZYACCESS(amount_of_underwear_by_id_card, id)
		if(current_quota >= length(GLOB.underwear.categories))
			audible_message("I have already used up my underwear quota for this shift. Please return previously acquired items to increase it.", WARDROBE_BLIND_MESSAGE(H))
			return
