...
// If nothing has been built on the low wall, I can climb on it
...
/obj/structure/wall_frame/attackby(var/obj/item/weapon/W, var/mob/user)
	...
	//window placing begin
		...
		if(do_after(user,20,src))
			...
			if (ST.use(1))
				...
				to_chat(user, "<span class='notice'>I place the [WD] on [src].</span>")
	...
	if(isWrench(W))
		...
		if(do_after(user, 40,src))
			if(!src) return
			to_chat(user, "<span class='notice'>I dissasembled the low wall!</span>")
			dismantle()
