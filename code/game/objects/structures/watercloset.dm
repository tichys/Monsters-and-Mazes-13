...
/obj/structure/toilet/attack_hand(mob/living/user as mob)
	if(swirlie)
		usr.visible_message("<span class='danger'>[user] slams the toilet seat onto [swirlie.name]'s head!</span>", "<span class='notice'>I slam the toilet seat onto [swirlie.name]'s head!</span>", "I hear reverberating porcelain.")
	...
	if(cistern && !open)
		if(!contents.len)
			...
			if(ishuman(user))
				...
				I.loc = get_turf(src)
			to_chat(user, "<span class='notice'>I find \an [I] in the cistern.</span>")
			w_items -= I.w_class
			return
...
/obj/structure/toilet/attackby(obj/item/I as obj, mob/living/user as mob)
	if(isCrowbar(I))
		...
		if(do_after(user, 30, src))
			user.visible_message("<span class='notice'>[user] [cistern ? "replaces the lid on the cistern" : "lifts the lid off the cistern"]!</span>", "<span class='notice'>I [cistern ? "replace the lid on the cistern" : "lift the lid off the cistern"]!</span>", "I hear grinding porcelain.")
			cistern = !cistern
			update_icon()
			return
	...
	if(cistern && !istype(user,/mob/living/silicon/robot)) //STOP PUTTING YOUR MODULES IN THE TOILET.
		...
		if(!user.unEquip(I, src))
			return
		w_items += I.w_class
		to_chat(user, "I carefully place \the [I] into the cistern.")
		return
...
//add heat controls? when emagged, I can freeze to death in it?
...
/obj/machinery/shower/attackby(obj/item/I as obj, mob/user as mob)
	...
	if(isWrench(I))
		var/newtemp = input(user, "What setting would I like to set the temperature valve to?", "Water Temperature Valve") in temperature_settings
		to_chat(user, "<span class='notice'>I begin to adjust the temperature valve with \the [I].</span>")
		playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
		if(do_after(user, 50, src))
			watertemp = newtemp
			user.visible_message("<span class='notice'>\The [user] adjusts \the [src] with \the [I].</span>", "<span class='notice'>I adjust the shower with \the [I].</span>")
...
/obj/machinery/shower/proc/wash(atom/movable/O as obj|mob)
	...
	if(iscarbon(O))
		...
		if(M.touching)
			var/remove_amount = M.touching.maximum_volume * M.reagent_permeability() //take off my suit first
...
/obj/item/weapon/bikehorn/rubberducky
	name = "rubber ducky"
	desc = "Rubber ducky you're so fine, I make bathtime lots of fuuun. Rubber ducky I'm awfully fooooond of yooooouuuu~"	//thanks doohl
	icon = 'icons/obj/watercloset.dmi'
...
/obj/structure/sink/attackby(obj/item/O as obj, mob/living/user as mob)
	...
	if (istype(RG) && RG.is_open_container() && !istype(RG, /obj/item/weapon/reagent_containers/food/snacks/grown))
		...
		user.visible_message("<span class='notice'>[user] fills \the [RG] using \the [src].</span>","<span class='notice'>I fill \the [RG] using \the [src].</span>")
		return 1
		...
		to_chat(user, "<span class='notice'>I wet \the [O] in \the [src].</span>")
	...
	to_chat(usr, "<span class='notice'>I start washing \the [I].</span>")
	...
	if(do_after(usr, 4 SECONDS, src, needhand = FALSE))
		...
		if(istype(O, /obj/item/organ/external/head))
			...
			"<span class='notice'>I wash \A [I] using \The [src].</span>")
	else
		busy = FALSE
		to_chat(user, "<span class = 'notice'> I must stay still to wash \The [I]!</span>")
