...
/obj/structure/window/CanPass(atom/movable/mover, turf/target, height=0, air_group=0)
	if(istype(mover) && mover.checkpass(PASS_FLAG_GLASS))
		return 1
	if(is_full_window())
		return 0	//full tile window, I can't move into it!
...
/obj/structure/window/attack_hand(mob/user as mob)
	...
	if(HULK in user.mutations)
		...
		usr.visible_message("<span class='danger'>\The [usr] bangs against \the [src]!</span>",
							"<span class='danger'>I bang against \the [src]!</span>",
							"I hear a banging sound.")
	else
		playsound(src.loc, 'sound/effects/glassknock.ogg', 80, 1)
		usr.visible_message("[usr.name] knocks on the [src.name].",
							"I knock on the [src.name].",
...
/obj/structure/window/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(isScrewdriver(W))
		if(reinf && state >= 1)
			...
			to_chat(user, (state == 1 ? "<span class='notice'>I have unfastened the window from the frame.</span>" : "<span class='notice'>I have fastened the window to the frame.</span>"))
		else if(reinf && state == 0)
			set_anchored(!anchored)
			playsound(loc, 'sound/items/Screwdriver.ogg', 75, 1)
			to_chat(user, (anchored ? "<span class='notice'>I have fastened the frame to the floor.</span>" : "<span class='notice'>I have unfastened the frame from the floor.</span>"))
		else if(!reinf)
			set_anchored(!anchored)
			playsound(loc, 'sound/items/Screwdriver.ogg', 75, 1)
			to_chat(user, (anchored ? "<span class='notice'>I have fastened the window to the floor.</span>" : "<span class='notice'>I have unfastened the window.</span>"))
	else if(isCrowbar(W) && reinf && state <= 1)
		state = 1 - state
		playsound(loc, 'sound/items/Crowbar.ogg', 75, 1)
		to_chat(user, (state ? "<span class='notice'>I have pried the window into the frame.</span>" : "<span class='notice'>I have pried the window out of the frame.</span>"))
	else if(isWrench(W) && !anchored && (!state || !reinf))
		if(!glasstype)
			to_chat(user, "<span class='notice'>I'm not sure how to dismantle \the [src] properly.</span>")
		else
