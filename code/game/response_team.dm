...
/client/proc/response_team()
	...
	if(send_emergency_team)
		...
		return
	if(alert("Do I want to dispatch an Emergency Response Team?",,"Yes","No") != "Yes")
		return
	...
	if(security_state.current_security_level_is_lower_than(security_state.high_security_level)) // Allow admins to reconsider if the alert level is below High
		switch(alert("Current security level lower than [security_state.high_security_level.name]. Do I still want to dispatch a response team?",,"Yes","No"))
			if("No")
				return
	if(send_emergency_team)
		to_chat(usr, "<span class='danger'>Looks like somebody beat I to it!</span>")
		return
...
client/verb/JoinResponseTeam()
	...
	if(!MayRespawn(1))
		to_chat(usr, "<span class='warning'>I cannot join the response team at this time.</span>")
		return
	...
	if(isghost(usr) || isnewplayer(usr))
		...
		if(jobban_isbanned(usr, MODE_ERT) || jobban_isbanned(usr, "Security Officer"))
			to_chat(usr, "<span class='danger'>I am jobbanned from the emergency reponse team!</span>")
			return
		...
		to_chat(usr, "I need to be an observer or new player to use this.")
