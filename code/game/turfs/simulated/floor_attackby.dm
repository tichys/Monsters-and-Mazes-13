/turf/simulated/floor/attackby(obj/item/C as obj, mob/user as mob)
	...
	if(flooring)
		if(isCrowbar(C))
			if(broken || burnt)
				to_chat(user, "<span class='notice'>I remove the broken [flooring.descriptor].</span>")
				make_plating()
			else if(flooring.flags & TURF_IS_FRAGILE)
				to_chat(user, "<span class='danger'>I forcefully pry off the [flooring.descriptor], destroying them in the process.</span>")
				make_plating()
			else if(flooring.flags & TURF_REMOVE_CROWBAR)
				to_chat(user, "<span class='notice'>I lever off the [flooring.descriptor].</span>")
				make_plating(1)
			else
				return
			...
			if(broken || burnt)
				return
			to_chat(user, "<span class='notice'>I unscrew and remove the [flooring.descriptor].</span>")
			make_plating(1)
			playsound(src, 'sound/items/Screwdriver.ogg', 80, 1)
			return
		else if(isWrench(C) && (flooring.flags & TURF_REMOVE_WRENCH))
			to_chat(user, "<span class='notice'>I unwrench and remove the [flooring.descriptor].</span>")
			make_plating(1)
			playsound(src, 'sound/items/Ratchet.ogg', 80, 1)
			return
		else if(istype(C, /obj/item/weapon/shovel) && (flooring.flags & TURF_REMOVE_SHOVEL))
			to_chat(user, "<span class='notice'>I shovel off the [flooring.descriptor].</span>")
			make_plating(1)
			playsound(src, 'sound/items/Deconstruct.ogg', 80, 1)
			return
		else if(isCoil(C))
			to_chat(user, "<span class='warning'>I must remove the [flooring.descriptor] first.</span>")
			return
	else
		...
		if(istype(C, /obj/item/stack))
			...
			if(use_flooring.build_cost && S.get_amount() < use_flooring.build_cost)
				to_chat(user, "<span class='warning'>I require at least [use_flooring.build_cost] [S.name] to complete the [use_flooring.descriptor].</span>")
				return
		...
		// Repairs and Deconstruction.
			...
			if(welder.isOn() && (is_plating()))
				if(broken || burnt)
					if(welder.isOn())
						to_chat(user, "<span class='notice'>I fix some dents on the broken plating.</span>")
		...
		else if(istype(C, /obj/item/weapon/shovel) && diggable)
			visible_message("<span class='notice'>\The [user] starts digging a pit</span>")
			if(do_after(user, 40, src))
				visible_message("<span class='notice'>\The [user] digs a pit.</span>")
				new /obj/structure/pit(src)
			else
				visible_message("<span class='notice'>\The [user] stops digging the pit</span>")
	return ..()
