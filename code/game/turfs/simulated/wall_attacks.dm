...
/turf/simulated/wall/proc/fail_smash(var/mob/user)
	to_chat(user, "<span class='danger'>I smash against \the [src]!</span>")
	take_damage(rand(25,75))

/turf/simulated/wall/proc/success_smash(var/mob/user)
	to_chat(user, "<span class='danger'>I smash through \the [src]!</span>")
...
/turf/simulated/wall/proc/try_touch(var/mob/user, var/rotting)
	...
	if(rotting)
		if(reinf_material)
			...
			to_chat(user, "<span class='danger'>\The [material.display_name] crumbles under my touch!</span>")
			dismantle_wall()
			return 1
	...
	if(!can_open)
		to_chat(user, "<span class='notice'>I push \the [src], but nothing happens.</span>")
...
/turf/simulated/wall/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(locate(/obj/effect/overlay/wallrot) in src)
		if(isWelder(W))
			...
			if( WT.remove_fuel(0,user) )
				to_chat(user, "<span class='notice'>I burn away the fungi with \the [WT].</span>")
				playsound(src, 'sound/items/Welder.ogg', 10, 1)
				for(var/obj/effect/overlay/wallrot/WR in src)
					qdel(WR)
				return
		else if(!is_sharp(W) && W.force >= 10 || W.force >= 20)
			to_chat(user, "<span class='notice'>\The [src] crumbles away under the force of my [W.name].</span>")
	...
	if(thermite)
		if(isWelder(W))
			...
			to_chat(user, "<span class='notice'>I slash \the [src] with \the [EB]; the thermite ignites!</span>")
	...
	if(damage && istype(W, /obj/item/weapon/weldingtool))
		...
		if(WT.remove_fuel(0,user))
			to_chat(user, "<span class='notice'>I start repairing the damage to [src].</span>")
			playsound(src, 'sound/items/Welder.ogg', 100, 1)
			if(do_after(user, max(5, damage / 5), src) && WT && WT.isOn())
				to_chat(user, "<span class='notice'>I finish repairing the damage to [src].</span>")
				take_damage(-damage)
		else
	...
	if(isnull(construction_stage) || !reinf_material)
		...
		if(dismantle_verb)
			...
			to_chat(user, "<span class='notice'>I begin [dismantle_verb] through the outer plating.</span>")
			if(dismantle_sound)
	...
	//Reinforced dismantling.
		...
		switch(construction_stage)
			...
			if(5)
				if(isScrewdriver(W))
					to_chat(user, "<span class='notice'>I begin removing the support lines.</span>")
					playsound(src, 'sound/items/Screwdriver.ogg', 100, 1)
					if(!do_after(user,40,src) || !istype(src, /turf/simulated/wall) || construction_stage != 5)
						return
					construction_stage = 4
					update_icon()
					to_chat(user, "<span class='notice'>I remove the support lines.</span>")
					return
					...
					if(O.get_amount()>0)
						...
						update_icon()
						to_chat(user, "<span class='notice'>I replace the outer grille.</span>")
						return
			if(4)
				var/cut_cover
				if(istype(W,/obj/item/weapon/weldingtool))
			...
			if(1)
				...
				if(cut_cover)
					...
					update_icon()
					new /obj/item/stack/rods(src)
					to_chat(user, "<span class='notice'>The support rods drop out as I cut them loose from the frame.</span>")
					return
			if(0)
				if(isCrowbar(W))
