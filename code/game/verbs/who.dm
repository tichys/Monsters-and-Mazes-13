...
/client/verb/who()
	...
	msg += "Total Players: <b>[length(Lines)]</b>"
	to_chat(src, msg)
...
/client/verb/staffwho()
	...
	for(var/client/C in GLOB.admins)
		...
		if(check_rights(R_ADMIN,0,C))
			line += "<span class='player'>\t[C] is \an <b>["\improper[C.holder.rank]"]</b>"
		else
			line += "<span class='player'>\t[C] is \an ["\improper[C.holder.rank]"]"
		if(!C.is_afk())
			active_staff++
		if(can_investigate)
			if(C.is_afk())
	...
	var/tochat
	if(config.admin_irc)
		tochat += "<span class='info'>Adminhelps are also sent to IRC. If no admins are available in game try anyway and an admin on IRC may see it and respond.</span>"
	tochat += "<div class='lightbox'><span class='lightTitle'>Current Staff ([active_staff]/[total_staff]):</span>"
	tochat += jointext(msg,"</div>\n")
	to_chat(src, tochat)