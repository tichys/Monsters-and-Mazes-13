...
mob/proc/airflow_stun()
	...
	if(!(status_flags & CANSTUN) && !(status_flags & CANWEAKEN))
		...
		return 0
	if(buckled)
		to_chat(src, "<span class='notice'>Air suddenly rushes past me!</span>")
		return 0
	if(!lying)
		to_chat(src, "<span class='warning'>The sudden rush of air knocks I over!</span>")
	Weaken(5)
...
/atom/movable/Bump(atom/A)
	if(airflow_speed > 0 && airflow_dest)
		if(airborne_acceleration > 1)
			...
			to_chat(src, "<span class='notice'>I am pinned against [A] by airflow!</span>")
...
mob/airflow_hit(atom/A)
	for(var/mob/M in hearers(src))
		M.show_message("<span class='danger'>\The [src] slams into \a [A]!</span>",1,"<span class='danger'>I hear a loud slam!</span>",2)
...
//	for(var/mob/M in hearers(src))
//		M.show_message("<span class='danger'>[src] slams into [A]!</span>",1,"<span class='danger'>I hear a loud slam!</span>",2)
