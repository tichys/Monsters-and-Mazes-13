...
/pl_control
	...
	var/PHORON_HALLUCINATION_DESC = "Does being in phoron cause I to hallucinate?"

	var/N2O_HALLUCINATION = 1
	var/N2O_HALLUCINATION_NAME = "N2O Hallucination"
	var/N2O_HALLUCINATION_DESC = "Does being in sleeping gas cause I to hallucinate?"
...
/mob/living/carbon/human/pl_effects()
	...
	if(vsc.plc.SKIN_BURNS)
		if(!pl_head_protected() || !pl_suit_protected())
			burn_skin(0.75)
			if(prob(20)) to_chat(src, "<span class='danger'>My skin burns!</span>")
			updatehealth()
	...
	if(vsc.plc.GENETIC_CORRUPTION)
		if(rand(1,10000) < vsc.plc.GENETIC_CORRUPTION)
			randmutb(src)
			to_chat(src, "<span class='danger'>High levels of toxins cause I to spontaneously mutate!</span>")
			domutcheck(src,null)
...
/mob/living/carbon/human/proc/burn_eyes()
	...
	if(E && !E.phoron_guard)
		if(prob(20)) to_chat(src, "<span class='danger'>My eyes burn!</span>")
		E.damage += 2.5
		eye_blurry = min(eye_blurry+1.5,50)
		if (prob(max(0,E.damage - 15) + 1) &&!eye_blind)
			to_chat(src, "<span class='danger'>I am blinded!</span>")
			eye_blind += 20
