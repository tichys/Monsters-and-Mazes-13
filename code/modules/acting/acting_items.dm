...
/obj/machinery/acting/wardrobe/attack_hand(var/mob/user as mob)
	user.show_message("I push a button and watch patiently as the machine begins to hum.")
	if(active)
		active = 0
		spawn(30)
			new /obj/item/weapon/storage/backpack/chameleon/sydie_kit(src.loc)
			src.visible_message("\The [src] beeps, dispensing a small box onto the floor.", "I hear a beeping sound followed by a thumping noise of some kind.")
			active = 1

/obj/machinery/acting/changer
	name = "Quickee's Plastic Surgeon"
	desc = "For when I need to be someone else right now."
...
/obj/machinery/acting/changer/attack_hand(var/mob/user as mob)
	if(ishuman(user))
		...
		var/getName = sanitize(input(H, "Would I like to change my name to something else?", "Name change") as null|text, MAX_NAME_LEN)
		if(getName)
			H.real_name = getName
			H.SetName(getName)
