datum/admins/proc/DB_staffwarn_record(var/ckey, var/reason)
	...
	if(playerid == -1)
		to_chat(usr,"<font color='red'>I have attempted to set staffwarn on [ckey], but they haven't been seen yet. Staffwarn can only be set on existing players.</font>")
		return
...
//Either pass the mob I wish to ban in the 'banned_mob' attribute, or the banckey, banip and bancid variables. If both are passed, the mob takes priority! If a mob is not passed, banckey is the minimum that needs to be passed! banip and bancid are optional.
...
datum/admins/proc/DB_ban_unban(var/ckey, var/bantype, var/job = "")
	...
	if(ban_number == 0)
		to_chat(usr, "<span class='warning'>Database update failed due to no bans fitting the search criteria. If this is not a legacy ban I should contact the database admin.</span>")
		return
...
/datum/admins/proc/DB_ban_panel(var/playerckey = null, var/adminckey = null, var/playerip = null, var/playercid = null, var/dbbantype = null, var/match = null)
	...
	output += "<form method='GET' action='?src=\ref[src]'><b>Add custom ban:</b> (ONLY use this if I can't ban through any other method)"
