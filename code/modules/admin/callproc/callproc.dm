...
/datum/callproc/proc/callproc(hastarget, datum/target)
	...
	if(hastarget)
		if(!target)
			to_chat(usr, "My callproc target no longer exists.")
			clear()
			return
	...
	while(!done)
		...
		switch(input("Type of [arguments.len+1]\th variable", "argument [arguments.len+1]") as null|anything in list(
			...
			if("mob's area")
				...
				current = get_area(M)
				if(!current)
					switch(alert("\The [M] appears to not have an area; do I want to pass null instead?",, "Yes", "Cancel"))
						if("Yes")
							; // do nothing
						if("Cancel")
							return CANCEL
			...
			if("marked datum")
				...
				if(!current)
					switch(alert("I do not currently have a marked datum; do I want to pass null instead?",, "Yes", "Cancel"))
						if("Yes")
							; // do nothing
						if("Cancel")
							return CANCEL
...
/client/Click(atom/A)
	if(holder && holder.callproc && holder.callproc.waiting_for_click)
		if(alert("Do I want to select \the [A] as the [holder.callproc.arguments.len+1]\th argument?",, "Yes", "No") == "Yes")
...
/datum/callproc/proc/finalise()
	...
	feedback_add_details("admin_verb","APC") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
