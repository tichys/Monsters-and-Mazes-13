...
proc/admin_proc()
	...
	to_chat(usr, "I have enough rights!")

NOTE: It checks usr by default. Supply the "user" argument if I wish to check for a specific mob.
*/
/proc/check_rights(rights_required, show_msg=1, var/client/C = usr)
	...
	if(!C.holder)
		if(show_msg)
			to_chat(C, "<span class='warning'>Error: I am not an admin.</span>")
		return FALSE
	...
	if(rights_required)
		if(rights_required & C.holder.rights)
			...
			if(show_msg)
				to_chat(C, "<span class='warning'>Error: I do not have sufficient rights to do that. I require one of the following flags:[rights2text(rights_required," ")].</span>")
			return FALSE
	else
		return TRUE
...
/client/proc/stealth()
	...
	if(!holder)
		to_chat(src, "<span class='warning'>Error: I am not an admin.</span>")
		return

	holder.stealthy_ = holder.stealthy_ == STEALTH_OFF ? STEALTH_MANUAL : STEALTH_OFF
	if(holder.stealthy_)
		to_chat(src, "<span class='notice'>I am now stealthed.</span>")
	else
		to_chat(src, "<span class='notice'>I am no longer stealthed.</span>")
	log_and_message_admins("has turned stealth mode [holder.stealthy_ ? "ON" : "OFF"]")
	feedback_add_details("admin_verb","SM") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
