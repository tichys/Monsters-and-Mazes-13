/datum/admins/proc/capture_map_part(tx as null|num, ty as null|num, tz as null|num, range as null|num)
	...
	if(!check_rights(R_ADMIN|R_DEBUG|R_SERVER))
		to_chat(usr, "I am not allowed to use this command")
		return
	...
	if(locate(tx,ty,tz))
		var/ligths = 0
		if(alert("Do I want lighting to be included in capture?", "Map Capture", "No", "Yes") == "Yes")
			ligths = 1
...
/datum/admins/proc/capture_map(tz as null|num)
	...
	switch(alert("Are I sure? (This will cause masive lag!!!)", "Map Capture", "No", "Yes"))
		if("Yes")
