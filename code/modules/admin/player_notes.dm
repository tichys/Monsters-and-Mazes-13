...
/proc/notes_remove(var/ckey, var/start_index, var/end_index)
	...
	if(!ckey)
		notesfile.cd = "/"
		ckey = ckey(input(usr,"Who would I like to remove notes for?","Enter a ckey",null) as null|anything in notesfile.dir)
		if(!ckey)	return
	...
	if(start_index)
		...
		notesfile.cd = "/"
		if(alert(usr,"Are I sure I want to remove all their notes?","Confirmation","No","Yes - Remove all notes") == "Yes - Remove all notes")
