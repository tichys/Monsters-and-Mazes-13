...
/datum/admin_secret_item/admin_secret/move_shuttle/execute(var/mob/user)
	...
	if(!.)
		return
	var/confirm = alert(user, "This command directly moves a shuttle from one area to another. DO NOT USE THIS UNLESS I ARE DEBUGGING A SHUTTLE AND I KNOW WHAT I ARE DOING.", "Are I sure?", "Ok", "Cancel")
	if (confirm == "Cancel")
		return

	var/shuttle_tag = input(user, "Which shuttle do I want to move?") as null|anything in SSshuttle.shuttles
