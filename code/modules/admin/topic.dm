/datum/admins/Topic(href, href_list)
	...
	if(href_list["dbsearchckey"] || href_list["dbsearchadmin"])
		...
		if(task == "remove")
			if(alert("Are I sure I want to remove [adm_ckey]?","Message","Yes","Cancel") == "Yes")
				if(!D)	return
			...
			to_chat(C, "[key_name_admin(usr)] has set my admin rank to: [new_rank].")
			...
			to_chat(C, "[key_name_admin(usr)] has toggled my permission: [new_permission].")
		...
		if( ticker.mode.name == "blob" )
			alert("I can't call the shuttle during blob!")
			return
	...
	/////////////////////////////////////new ban stuff
		...
		var/key = Banlist["key"]
		if(alert(usr, "Are I sure I want to unban [key]?", "Confirmation", "Yes", "No") == "Yes")
...
//Regular jobs
	...
	//JOBBAN'S INNARDS
		...
		if(!check_rights(R_MOD,0) && !check_rights(R_ADMIN,0))
			to_chat(usr, "<span class='warning'>I do not have the appropriate permissions to add job bans!</span>")
			return
		...
		if(M != usr)																//we can jobban ourselves
			if(M.client && M.client.holder && (M.client.holder.rights & R_BAN))		//they can ban too. So we can't ban them
				alert("I cannot perform this action. I must be of a higher administrative rank!")
				return
			...
			switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
				if("Yes")
					if(!check_rights(R_MOD,0) && !check_rights(R_BAN, 0))
						to_chat(usr, "<span class='warning'> I Cannot issue temporary job-bans!</span>")
						return
					if(config.ban_legacy_system)
						to_chat(usr, "<span class='warning'>The server is using the legacy banning system, which does not support temporary job bans. Consider upgrading. Aborting ban.</span>")
						return
					...
					to_chat(M, "<span class='danger'>I have been jobbanned by [usr.client.ckey] from: [msg].</span>")
		...
		if(joblist.len) //at least 1 banned job exists in joblist so we have stuff to unban.
			...
			if(msg)
				...
				to_chat(M, "<span class='danger'>I have been un-jobbanned by [usr.client.ckey] from [msg].</span>")
				href_list["jobban2"] = 1 // lets it fall through and refresh
			return 1
		...
		if (ismob(M))
			...
			if(!reason)
				to_chat(M, "<span class='warning'>I have been kicked from the server</span>")
			else
				to_chat(M, "<span class='warning'>I have been kicked from the server: [reason]</span>")
		...
		if(t)
			if((alert("Do I want to unjobban [t]?","Unjobban confirmation", "Yes", "No") == "Yes") && t) //No more misclicks! Unless I do it twice.
		...
		if(!check_rights(R_MOD,0) && !check_rights(R_BAN, 0))
			to_chat(usr, "<span class='warning'>I do not have the appropriate permissions to add bans!</span>")
			return
		...
		var/dat = {"<B>What mode do I wish to play?</B><HR>"}
		...
		var/dat = {"<B>What game mode do I want to force secret to be? Use this if I want to change the game mode, but want the players to believe it's secret. This will only work if the current game mode is secret.</B><HR>"}
		for(var/mode in config.modes)
		...
		spawn(50)
			to_chat(M, "<span class='notice'>I have been sent to the Thunderdome.</span>")
	...
	// call dibs on IC messages (prays, emergency comms, faxes)
		...
		if(ismob(M))
			...
			for(var/client/X in GLOB.admins)
				if((R_ADMIN|R_MOD|R_MENTOR) & X.holder.rights)
					to_chat(X, take_msg)
			to_chat(M, "<span class='notice'><b>My message is being attended to by [usr.client]. Thanks for my patience!</b></span>")
		else
		...
		to_chat(H, "<span class='notice'>My prayers have been answered!! I received the <b>best cookie</b>!</span>")
		...
		if(alert(src.owner, "Are I sure I wish to hit [key_name(M)] with Blue Space Artillery?",  "Confirm Firing?" , "Yes" , "No") != "Yes")
			return
		...
		to_chat(M, "I have been hit by bluespace artillery!")
		...
		if(L.can_centcom_reply())
			...
			to_chat(src.owner, "I sent [input] to [L] via a secure channel.")
			log_admin("[src.owner] replied to [key_name(L)]'s Centcomm message with the message [input].")
			message_admins("[src.owner] replied to [key_name(L)]'s Centcom message with: \"[input]\"")
			if(!isAI(L))
				to_chat(L, "<span class='info'>I hear something crackle in my headset for a moment before a voice speaks.</span>")
			...
			to_chat(src.owner, "The person I am trying to contact does not have functional radio equipment.")
		...
		if(!istype(H.l_ear, /obj/item/device/radio/headset) && !istype(H.r_ear, /obj/item/device/radio/headset))
			to_chat(usr, "The person I am trying to contact is not wearing a headset")
			return
		...
		to_chat(src.owner, "I sent [input] to [H] via a secure channel.")
		log_admin("[src.owner] replied to [key_name(H)]'s illegal message with the message [input].")
		to_chat(H, "I hear something crackle in my headset for a moment before a voice speaks.  \"Please stand by for a message from my benefactor.  Message as follows, agent. <b>\"[input]\"</b>  Message ends.\"")
		...
		if(!paths)
			alert("The path list I sent is empty")
			return
		if(length(paths) > 5)
		...
		if ( where == "inhand" )	//Can only give when human or monkey
			if ( !( ishuman(usr) || issmall(usr) ) )
				...
				to_chat(usr, "My active hand is full. Spawning on floor.")
				where = "onfloor"
		...
		if ( where == "inmarked" )
			...
			if ( !marked_datum )
				...
				if ( !istype(marked_datum,/atom) )
					to_chat(usr, "The object I have marked cannot be used as a target. Target must be of type /atom. Abandoning spawn.")
					return
		...
		src.admincaster_feed_message.body = sanitize(input(usr, "Write my Feed story", "Network Channel Handler", ""))
		src.access_news_network()
		...
		src.admincaster_feed_message.body = sanitize(input(usr, "Provide the a description of the Wanted person and any other details I deem important", "Network Security Handler", ""))
		src.access_news_network()
		...
		src.admincaster_signature = sanitize(input(usr, "Provide my desired signature", "Network Identity Handler", ""))
		src.access_news_network()
