...
/client/proc/cmd_admin_pm_context(mob/M as mob in SSmobs.mob_list)
	...
	feedback_add_details("admin_verb","APMM") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_pm_panel()
	...
	feedback_add_details("admin_verb","APM") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_pm(var/client/C, var/msg = null, var/datum/ticket/ticket = null)
	if(prefs.muted & MUTE_ADMINHELP)
		to_chat(src, "<span class='warning'>Error: Private-Message: I am unable to use PM-s (muted).</span>")
		return
	...
	if(isnull(ticket)) // finally, accept that no ticket exists
		if(holder && sender_lite.ckey != receiver_lite.ckey)
			...
			to_chat(src, "<span class='notice'>I do not have an open ticket. Please use the adminhelp verb to open a ticket.</span>")
			return
	else if(ticket.status != TICKET_ASSIGNED && sender_lite.ckey == ticket.owner.ckey)
		to_chat(src, "<span class='notice'>My ticket is not open for conversation. Please wait for an administrator to receive my adminhelp.</span>")
		return
...
//		to_chat(src, "<span class='warning'>My message was not sent because it was more then 400 characters find my message below for ease of copy/pasting</span>")
