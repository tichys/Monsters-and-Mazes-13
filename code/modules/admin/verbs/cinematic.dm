/client/proc/cinematic(var/cinematic as anything in list("explosion",null))
	...
	if(alert("Are I sure I want to run [cinematic]?","Confirmation","Yes","No")=="No") return
	if(!ticker)	return
	switch(cinematic)
		if("explosion")
			if(alert("The game will be over. Are I really sure?", "Confirmation" ,"Continue", "Cancel") == "Cancel")
				return
