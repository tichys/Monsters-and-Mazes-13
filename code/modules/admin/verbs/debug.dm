/client/proc/Debug2()
	...
	feedback_add_details("admin_verb","DG2") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/Cell()
	...
	feedback_add_details("admin_verb","ASL") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/makepAI(var/turf/T in SSmobs.mob_list)
	...
	if(!choice)
		return 0
	if(!isghost(choice))
		var/confirm = input("[choice.key] isn't ghosting right now. Are I sure I want to yank them out of them out of their body and place them in this pAI?", "Spawn pAI Confirmation", "No") in list("Yes", "No")
		if(confirm != "Yes")
			return 0
	var/obj/item/device/paicard/card = new(T)
	var/mob/living/silicon/pai/pai = new(card)
	pai.SetName(sanitizeSafe(input(choice, "Enter my pAI name:", "pAI Name", "Personal AI") as text))
	...
	feedback_add_details("admin_verb","MPAI") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_slimeize(var/mob/M in SSmobs.mob_list)
	...
	if(ishuman(M))
		...
		spawn(10)
			M:slimeize()
			feedback_add_details("admin_verb","MKMET") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
	...
	if(M)
		if(M.mind in ticker.mode.cult)
			...
			if(alert("Spawn that person a tome?",,"Yes","No")=="Yes")
				to_chat(M, "<span class='warning'>I catch a glimpse of the Realm of Nar-Sie, The Geometer of Blood. I now see how flimsy the world is, I see that it should be open to the knowledge of Nar-Sie. A tome, a message from my new master, appears on the ground.</span>")
				new /obj/item/weapon/book/tome(M.loc)
			else
				to_chat(M, "<span class='warning'>I catch a glimpse of the Realm of Nar-Sie, The Geometer of Blood. I now see how flimsy the world is, I see that it should be open to the knowledge of Nar-Sie.</span>")
...
/client/proc/cmd_debug_del_all()
	...
	feedback_add_details("admin_verb","DELA") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_debug_make_powernets()
	...
	feedback_add_details("admin_verb","MPWN") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_debug_tog_aliens()
	...
	feedback_add_details("admin_verb","TAL") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_grantfullaccess(var/mob/M in SSmobs.mob_list)
	...
	if (istype(M, /mob/living/carbon/human))
		...
		alert("Invalid mob")
	feedback_add_details("admin_verb","GFA") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_assume_direct_control(var/mob/M in SSmobs.mob_list)
	...
	if(M.ckey)
		if(alert("This mob is being controlled by [M.ckey]. Are I sure I wish to assume control of it? [M.ckey] will be made a ghost.",,"Yes","No") != "Yes")
			return
		else
	...
	if(isghost(adminmob))
		qdel(adminmob)
	feedback_add_details("admin_verb","ADC") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/datum/admins/proc/cmd_admin_dress()
	...
	if(!reset_equipment)
		reset_equipment = alert("Do I wish to delete all current equipment first?", "Delete Equipment?","Yes", "No") == "Yes"
...
/client/proc/startSinglo()
	...
	if(alert("Are I sure? This will start up the engine. Should only be used during debug!",,"Yes","No") != "Yes")
		return
...
/client/proc/cmd_debug_mob_lists()
	...
	set desc = "For when I just gotta know"
...
/client/proc/cmd_analyse_health_context(mob/living/carbon/human/H as mob in GLOB.human_mob_list)
	...
	if(!ishuman(H))	return
	cmd_analyse_health(H)
	feedback_add_details("admin_verb","ANLS") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/view_initialization_log()  // for viewing the world setup logs
	set category = "Debug"
	set name = "View Initialization Log"

	var/html = "<head><title>Initialization Log</title></head>"
	
	for(var/x in global.initializationlog)
		html += x
	
	show_browser(usr, html, "window=initializationlog;size=430x600")