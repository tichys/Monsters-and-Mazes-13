/*
	...
	navigate the popup window to the data/logs/runtime/ folder from where my tgstation .dmb is located.
	(I may have to make this folder myself)

	OPTIONAL: 	I can select the little checkbox down the bottom to make dreamdeamon save the log everytime you
				start a world. Just remember to repeat these steps with a new name when I update to a new revision!

	Save it with the name of the revision The server uses (e.g. r3459.txt).
	Game Masters will now be able to grant access any runtime logs I have archived this way!
...
//I trust access. Also, it may be wise to ensure that they are not going to play in the current round.
...
/datum/admins/proc/view_txt_log()
	...
	if( fexists(path) )
		...
		return
	feedback_add_details("admin_verb","VTL") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
	return
...
/datum/admins/proc/view_atk_log()
	...
	if( fexists(path) )
		...
		return
	usr << run(file(path))
	feedback_add_details("admin_verb","SSAL") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
	return
