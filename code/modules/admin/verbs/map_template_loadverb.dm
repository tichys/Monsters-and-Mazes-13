/datum/admins/proc/map_template_load()
	set category = "Fun"
	set desc = "Pick a map template to load at my current location. I will be able to confirm bounds before committing."
	set name = "Map Template - Place"

	if (!check_rights(R_FUN)) return

	var/map = input(usr, "Choose a Map Template to place at my CURRENT LOCATION","Place Map Template") as null|anything in SSmapping.map_templates
	if(!map)
		return
...
/datum/admins/proc/map_template_load_new_z()
	...
	if (template.loaded && !(template.template_flags & TEMPLATE_FLAG_ALLOW_DUPLICATES))
		var/jesus_take_the_wheel = alert(usr, "That template has already been loaded and doesn't want to be loaded again. \
			Proceeding may unpredictably break things and cause runtimes.", "Confirm load", "Cancel load", "Do I see any cops around?") == "Do I see any cops around?"
