...
/client/proc/cmd_modify_ticker_variables()
	...
	if (ticker == null)
		...
		feedback_add_details("admin_verb","ETV") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/mod_list_add(var/list/L, atom/O, original_name, objectvar)
	...
	switch(alert("Would I like to associate a var with the list entry?",,"Yes","No"))
		if("Yes")
			L += var_value
...
/client/proc/mod_list(var/list/L, atom/O, original_name, objectvar)
	...
	if(L.len > 0)
		...
		catch {} // Builtin non-assoc lists (contents, etc.) will runtime if I try to get an assoc value of them
	...
	var/new_var
	switch(class) //Spits a runtime error if I try to modify an entry in the contents list. Dunno how to fix it, yet.
