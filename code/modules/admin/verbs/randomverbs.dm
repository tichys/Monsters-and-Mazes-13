/client/proc/cmd_admin_drop_everything(mob/M as mob in SSmobs.mob_list)
	...
	feedback_add_details("admin_verb","DEVR") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_prison(mob/M as mob in SSmobs.mob_list)
	...
	if (ismob(M))
		if(istype(M, /mob/living/silicon/ai))
			alert("The AI can't be sent to prison I jerk!", null, null, null, null, null)
			return
		...
		feedback_add_details("admin_verb","PRISON") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
	...
	if(usr)
		if (usr.client)
			if(usr.client.holder)
				to_chat(M, "<b>I hear a voice in my head... <i>[msg]</i></b>")
	log_and_message_staff(" - SubtleMessage -> [key_name_admin(M)] : [msg]")
	feedback_add_details("admin_verb","SMS") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_world_narrate() // Allows administrators to fluff events a little easier -- TLE
	...
	var/msg = sanitize(input("Message:", text("Enter the text I wish to appear to everyone:")) as text)
	...
	feedback_add_details("admin_verb","GLN") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_direct_narrate(var/mob/M)
	...
	var/msg = sanitize(input("Message:", text("Enter the text I wish to appear to my target:")) as text)
	...
	feedback_add_details("admin_verb","DIRN") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!

// Local narrate, narrates to everyone who can see where I am regardless of whether they are blind or deaf.
/client/proc/cmd_admin_local_narrate()
	set category = "Special Verbs"
	set name = "Local Narrate"
	set desc = "Narrate to everyone who can see the turf my mob is on."
...
/client/proc/cmd_admin_visible_narrate(var/atom/A)
	...
	if(!M)
		to_chat(src, "I must be in control of a mob to use this.")
		return
...
/client/proc/cmd_admin_godmode(mob/M as mob in SSmobs.mob_list)
	...
	feedback_add_details("admin_verb","GOD") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!

proc/cmd_admin_mute(mob/M as mob, mute_type)
	if(!usr || !usr.client)
		return
	if(!usr.client.holder)
		to_chat(usr, "<font color='red'>Error: cmd_admin_mute: I don't have permission to do this.</font>")
		return
	if(!M.client)
		to_chat(usr, "<font color='red'>Error: cmd_admin_mute: This mob doesn't have a client tied to it.</font>")
	if(M.client.holder)
		to_chat(usr, "<font color='red'>Error: cmd_admin_mute: I cannot mute an admin/mod.</font>")
	if(!M.client)		return
	...
	to_chat(M, "<span class = 'alert'>I have been [muteunmute] from [mute_string].</span>")
	feedback_add_details("admin_verb","MUTE") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_add_random_ai_law()
	...
	IonStorm(0)
	feedback_add_details("admin_verb","ION") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/get_ghosts(var/notify = 0,var/what = 2)
	...
	if(!any)
		if(notify)
			to_chat(src, "There doesn't appear to be any ghosts for I to select.")
		return
...
/client/proc/allow_character_respawn(var/selection in get_ghosts_by_key())
	...
	if(G.has_enabled_antagHUD == 1 && config.antag_hud_restricted)
		var/response = alert(src, "[selection] has enabled antagHUD. Are I sure I wish to allow them to respawn?","Ghost has used AntagHUD","No","Yes")
		if(response == "No") return
	else
		var/response = alert(src, "Are I sure I wish to allow [selection] to respawn?","Allow respawn","No","Yes")
		if(response == "No") return
	...
	G.show_message("<span class=notice><b>I may now respawn.  I should roleplay as if I learned nothing about the round during my time with the dead.</b></span>", 1)
...
/client/proc/toggle_antagHUD_restrictions()
	...
	if(config.antag_hud_restricted)
		for(var/mob/observer/ghost/g in get_ghosts())
			to_chat(g, "<span class='notice'><B>The administrator has lifted restrictions on joining the round if I use AntagHUD</B></span>")
		action = "lifted restrictions"
		config.antag_hud_restricted = 0
		to_chat(src, "<span class='notice'><B>AntagHUD restrictions have been lifted</B></span>")
	else
		for(var/mob/observer/ghost/g in get_ghosts())
			to_chat(g, "<span class='danger'>The administrator has placed restrictions on joining the round if I use AntagHUD</span>")
			to_chat(g, "<span class='danger'>My AntagHUD has been disabled, I may choose to re-enabled it but will be under restrictions</span>")
			g.antagHUD = 0
...
/*
If a guy was gibbed and I want to revive him, this is a good way to do so.
	...
	*/
	...
	if(!issilicon(new_character))//If they are not a cyborg/AI.
		if(!record_found && !player_is_antag(new_character.mind, only_offstation_roles = 1)) //If there are no records for them. If they have a record, this info is already in there. MODE people are not announced anyway.
			if(alert(new_character,"Would I like an active AI to announce this character?",,"No","Yes")=="Yes")
	...
	feedback_add_details("admin_verb","RSPCH") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_add_freeform_ai_law()
	...
	if(!holder)
		to_chat(src, "Only administrators may use this command.")
		return
	var/input = sanitize(input(usr, "Please enter anything I want the AI to do. Anything. Serious.", "What?", "") as text|null)
	if(!input)
		return
	...
	if(show_log == "Yes")
		command_announcement.Announce("Ion storm detected near the [station_name()]. Please check all AI-controlled equipment for errors.", "Anomaly Alert", new_sound = 'sound/AI/ionstorm.ogg')
	feedback_add_details("admin_verb","IONC") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_rejuvenate(mob/living/M as mob in SSmobs.mob_list)
	...
	feedback_add_details("admin_verb","REJU") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_delete(atom/O as obj|mob|turf in range(world.view))
	...
	if (alert(src, "Are I sure I want to delete:\n[O]\nat ([O.x], [O.y], [O.z])?", "Confirmation", "Yes", "No") == "Yes")
		log_admin("[key_name(usr)] deleted [O] at ([O.x],[O.y],[O.z])")
		message_admins("[key_name_admin(usr)] deleted [O] at ([O.x],[O.y],[O.z])", 1)
		feedback_add_details("admin_verb","DEL") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
		qdel(O)
...
/client/proc/cmd_admin_list_open_jobs()
	...
	feedback_add_details("admin_verb","LFS") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_explosion(atom/O as obj|mob|turf in range(world.view))
	...
	if ((devastation != -1) || (heavy != -1) || (light != -1) || (flash != -1))
		if ((devastation > 20) || (heavy > 20) || (light > 20))
			if (alert(src, "Are I sure I want to do this? It will laaag.", "Confirmation", "Yes", "No") == "No")
				return

		explosion(O, devastation, heavy, light, flash, shaped=shaped)
		log_admin("[key_name(usr)] created an explosion ([devastation],[heavy],[light],[flash]) at ([O.x],[O.y],[O.z])")
		message_admins("[key_name_admin(usr)] created an explosion ([devastation],[heavy],[light],[flash]) at ([O.x],[O.y],[O.z])", 1)
		feedback_add_details("admin_verb","EXPL") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
		return
	else
		return
...
/client/proc/cmd_admin_emp(atom/O as obj|mob|turf in range(world.view))
	...
	if (heavy || light)
		...
		feedback_add_details("admin_verb","EMP") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_gib(mob/M as mob in SSmobs.mob_list)
	...
	M.gib()
	feedback_add_details("admin_verb","GIB") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_gib_self()
	...
	if(confirm == "Yes")
		...
		feedback_add_details("admin_verb","GIBS") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!

/client/proc/update_world()
	// If I see anyone granting powers to specific keys like the code that was here,
	// I will both remove their SVN access and permanently ban them from The servers.
	return
...
/client/proc/cmd_admin_check_contents(mob/living/M as mob in SSmobs.mob_list)
	...
	for(var/t in L)
		to_chat(usr, "[t]")
	feedback_add_details("admin_verb","CC") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_remove_phoron()
	...
	if(!holder)
		to_chat(src, "Only administrators may use this command.")
		return
	feedback_add_details("admin_verb","STATM") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
// DEFERRED
	spawn(0)
...
/client/proc/toggle_view_range()
	...
	feedback_add_details("admin_verb","CVRA") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/admin_call_shuttle()
	...
	if(alert(src, "Are I sure?", "Confirm", "Yes", "No") != "Yes") return

	if(ticker.mode.auto_recall_shuttle)
		if(input("The evacuation will just be cancelled if I call it. Call anyway?") in list("Confirm", "Cancel") != "Confirm")
			return
	...
	feedback_add_details("admin_verb","CSHUT") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/admin_cancel_shuttle()
	...
	if(alert(src, "I sure?", "Confirm", "Yes", "No") != "Yes") return
	...
	feedback_add_details("admin_verb","CCSHUT") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/cmd_admin_attack_log(mob/M as mob in SSmobs.mob_list)
	...
	for(var/t in M.attack_logs_)
		to_chat(usr, t)
	feedback_add_details("admin_verb","ATTL") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!


/client/proc/everyone_random()
	set category = "Fun"
	set name = "Make Everyone Random"
	set desc = "Make everyone have a random appearance. I can only use this before rounds!"

	if(!check_rights(R_FUN))	return

	if (ticker && ticker.mode)
		to_chat(usr, "Nope I can't do this, the game's already started. This only works before rounds!")
		return
	...
	var/notifyplayers = alert(src, "Do I want to notify the players?", "Options", "Yes", "No", "Cancel")
	...
	to_chat(usr, "<i>Remember: I can always disable the randomness by using the verb again, assuming the round hasn't started yet</i>.")
	ticker.random_players = 1
	feedback_add_details("admin_verb","MER") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
...
/client/proc/toggle_random_events()
	...
	feedback_add_details("admin_verb","TRE") //If I am copy-pasting this, ensure the 2nd parameter is unique to the new proc!
