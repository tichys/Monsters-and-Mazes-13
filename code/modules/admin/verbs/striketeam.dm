...
/client/proc/strike_team()
	...
	set desc = "Spawns a strike team if I want to run an admin event."
	...
	if(alert("Do I want to send in a strike team? Once enabled, this is irreversible.",,"Yes","No")!="Yes")
		return

	alert("This 'mode' will go on until everyone is dead or the installation is destroyed. I may also admin-call the evac shuttle when appropriate. Spawned commandos have internals cameras which are viewable through a monitor inside the Spec. Ops. Office. Assigning the team's detailed task is recommended from there. While I will be able to manually pick the candidates from active ghosts, their assignment in the squad will be random.")

	choice = null
	while(!choice)
		choice = sanitize(input(src, "Please specify which mission the strike team shall undertake.", "Specify Mission", ""))
		if(!choice)
			if(alert("Error, no mission set. Do I want to exit the setup process?",,"Yes","No")=="Yes")
				return

	if(team.deployed)
		to_chat(usr, "Looks like someone beat I to it.")
		return
