...
/obj/machinery/atmospherics/portables_connector/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	if(!isWrench(W))
		return ..()
	if (connected_device)
		to_chat(user, "<span class='warning'>I cannot unwrench \the [src], dettach \the [connected_device] first.</span>")
		return 1
	...
	if ((int_air.return_pressure()-env_air.return_pressure()) > 2*ONE_ATMOSPHERE)
		to_chat(user, "<span class='warning'>I cannot unwrench \the [src], it too exerted due to internal pressure.</span>")
