...
/obj/machinery/atmospherics/unary/vent_pump/Process()
	...
	if((environment.temperature || air_contents.temperature) && pressure_delta > 0.5)
		...
		//JESUS FUCK.  THERE ARE LITERALLY 250 OF I MOTHERFUCKERS ON ZLEVEL ONE AND I DO THIS SHIT EVERY TICK WHEN VERY OFTEN THERE IS NO REASON TO
...
/obj/machinery/atmospherics/unary/vent_pump/attackby(obj/item/W, mob/user)
	if(isWelder(W))
		...
		if(!do_after(user, 20, src))
			to_chat(user, "<span class='notice'>I must remain close to finish this task.</span>")
			return 1
		...
		user.visible_message("<span class='notice'>\The [user] [welded ? "welds \the [src] shut" : "unwelds \the [src]"].</span>", \
			"<span class='notice'>I [welded ? "weld \the [src] shut" : "unweld \the [src]"].</span>", \
			"I hear welding.")
		return 1
...
/obj/machinery/atmospherics/unary/vent_pump/examine(mob/user)
	if(..(user, 1))
		...
		to_chat(user, "I am too far away to read the gauge.")
	if(welded)
...
/obj/machinery/atmospherics/unary/vent_pump/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	if (!(stat & NOPOWER) && use_power)
		to_chat(user, "<span class='warning'>I cannot unwrench \the [src], turn it off first.</span>")
		return 1
	var/turf/T = src.loc
	...
	if ((int_air.return_pressure()-env_air.return_pressure()) > 2*ONE_ATMOSPHERE)
		to_chat(user, "<span class='warning'>I cannot unwrench \the [src], it is too exerted due to internal pressure.</span>")
