...
//#define TOPIC_DEBUGGING 1
	...
	If I have any  questions about this stuff feel free to ask. ~Carn
	*/
...
//		to_chat(src, "ASSET JOB [href_list["asset_cache_confirm_arrival"]] ARRIVED.")
	...
	if(href_list["irc_msg"])
		if(!holder && received_irc_pm < world.time - 6000) //Worse they can do is spam IRC for 10 minutes
			to_chat(usr, "<span class='warning'>I am no longer able to use this, it's been more then 10 minutes since an admin on IRC has responded to me</span>")
			return
		if(mute_irc)
			to_chat(usr, "<span class='warning'I cannot use this as my client has been muted from sending messages to the admins on IRC</span>")
			return
...
/client/New(TopicData)
	...
	if(byond_version < DM_VERSION)
		to_chat(src, "<span class='warning'>I am running an older version of BYOND than the server and may experience issues.</span>")
		to_chat(src, "<span class='warning'>It is recommended that I update to at least [DM_VERSION] at http://www.byond.com/download/.</span>")
	...
	spawn(5) // And wait a half-second, since it sounds like I can do this too fast.
		if(src)
