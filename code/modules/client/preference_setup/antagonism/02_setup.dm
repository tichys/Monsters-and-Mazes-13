...
/datum/category_item/player_setup_item/antagonism/basic/content(var/mob/user)
	...
	if(!pref.uplink_sources.len)
		. += "<span class='warning'>I will not receive an uplink unless I add an uplink source!</span>"
	. +="<br>"
	. +="Exploitable information:<br>"
	if(jobban_isbanned(user, "Records"))
		. += "<b>I am banned from using character records.</b><br>"
	else
...
/datum/category_item/player_setup_item/antagonism/basic/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["exploitable_record"])
		var/exploitmsg = sanitize(input(user,"Set exploitable information about I here.","Exploitable Information", html_decode(pref.exploit_record)) as message|null, MAX_PAPER_MESSAGE_LEN, extra = 0)
