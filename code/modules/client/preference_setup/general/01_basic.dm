...
/datum/category_item/player_setup_item/general/basic/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["rename"])
		var/raw_name = input(user, "Choose my character's name:", "Character Name")  as text|null
		if (!isnull(raw_name) && CanUseTopic(user))
			...
			if(new_name)
				...
				to_chat(user, "<span class='warning'>Invalid name. My name should be at least 2 and at most [MAX_NAME_LEN] characters long. It may only contain the characters A-Z, a-z, -, ' and .</span>")
		...
		var/new_gender = input(user, "Choose my character's gender:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.gender) as null|anything in S.genders
		...
		for(var/spawntype in spawntypes())
			spawnkeys += spawntype
		var/choice = input(user, "Where would I like to spawn when late-joining?") as null|anything in spawnkeys
