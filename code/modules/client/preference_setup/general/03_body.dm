...
/datum/category_item/player_setup_item/general/body/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["random"])
		...
		var/new_b_type = input(user, "Choose my character's blood-type:", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in valid_bloodtypes
		...
		var/choice = input("Which species would I like to look at?") as null|anything in playable_species
		if(!choice) return
		...
		if(!has_flag(mob_species, HAS_HAIR_COLOR))
			return TOPIC_NOACTION
		var/new_hair = input(user, "Choose my character's hair colour:", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_hair, pref.g_hair, pref.b_hair)) as color|null
		...
		var/new_h_style = input(user, "Choose my character's hair style:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.h_style)  as null|anything in valid_hairstyles
		...
		if(!has_flag(mob_species, HAS_HAIR_COLOR))
			return TOPIC_NOACTION
		var/new_facial = input(user, "Choose my character's facial-hair colour:", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_facial, pref.g_facial, pref.b_facial)) as color|null
		...
		if(!has_flag(mob_species, HAS_EYE_COLOR))
			return TOPIC_NOACTION
		var/new_eyes = input(user, "Choose my character's eye colour:", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_eyes, pref.g_eyes, pref.b_eyes)) as color|null
		...
		if(!has_flag(mob_species, HAS_BASE_SKIN_COLOURS))
			return TOPIC_NOACTION
		var/new_s_base = input(user, "Choose my character's base colour:", CHARACTER_PREFERENCE_INPUT_TITLE) as null|anything in mob_species.base_skin_colours
		...
		if(!has_flag(mob_species, HAS_A_SKIN_TONE))
			return TOPIC_NOACTION
		var/new_s_tone = input(user, "Choose my character's skin-tone. Lower numbers are lighter, higher are darker. Range: 1 to [mob_species.max_skin_tone()]", CHARACTER_PREFERENCE_INPUT_TITLE, (-pref.s_tone) + 35) as num|null
		mob_species = all_species[pref.species]
		...
		if(!has_flag(mob_species, HAS_SKIN_COLOR))
			return TOPIC_NOACTION
		var/new_skin = input(user, "Choose my character's skin colour: ", CHARACTER_PREFERENCE_INPUT_TITLE, rgb(pref.r_skin, pref.g_skin, pref.b_skin)) as color|null
		...
		var/new_f_style = input(user, "Choose my character's facial-hair style:", CHARACTER_PREFERENCE_INPUT_TITLE, pref.f_style)  as null|anything in valid_facialhairstyles
		...
		var/organ_tag = input(user, "Which limb do I want to change?") as null|anything in limb_selection_list

		if(!organ_tag || !CanUseTopic(user)) return TOPIC_NOACTION

		var/limb = null
		var/second_limb = null // if I try to change the arm, the hand should also change
		var/third_limb = null  // if I try to unchange the hand, the arm should also change
		...
		var/new_state = input(user, "What state do I wish the limb to be in?") as null|anything in choice_options
		...
		switch(new_state)
			...
			if("Prosthesis")
				...
				if(!usable_manufacturers.len)
					return
				var/choice = input(user, "Which manufacturer do I wish to use for this limb?") as null|anything in usable_manufacturers
				if(!choice)
					return
		...
		var/organ_name = input(user, "Which internal function do I want to change?") as null|anything in list("Heart", "Eyes", "Lungs", "Liver", "Kidneys")
		...
		var/new_state = input(user, "What state do I wish the organ to be in?") as null|anything in organ_choices
		if(!new_state) return
...
/datum/category_item/player_setup_item/general/body/proc/SetSpecies(mob/user)
	...
	if(restricted)
		if(restricted == 2)
			dat += "<font color='red'><b>I cannot play as this species.</br><small>This species is not available as a player race.</small></b></font></br>"
