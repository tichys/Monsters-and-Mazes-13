...
/datum/category_item/player_setup_item/general/background/content(var/mob/user)
	...
	if(jobban_isbanned(user, "Records"))
		. += "<span class='danger'>I am banned from using character records.</span><br>"
	else
		. += "Medical Records:<br>"
...
/datum/category_item/player_setup_item/general/background/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["nt_relation"])
		var/new_relation = input(user, "Choose my relation to [GLOB.using_map.company_name]. Note that this represents what others can find out about my character by researching my background, not what my character actually thinks.", CHARACTER_PREFERENCE_INPUT_TITLE, pref.nanotrasen_relation)  as null|anything in COMPANY_ALIGNMENTS
		...
		var/choice = input(user, "Please choose my current citizenship.", CHARACTER_PREFERENCE_INPUT_TITLE, pref.citizenship) as null|anything in GLOB.using_map.citizenship_choices + list("None","Other")
		if(!choice || !CanUseTopic(user))
			return TOPIC_NOACTION
		if(choice == "Other")
			var/raw_choice = sanitize(input(user, "Please enter my current citizenship.", CHARACTER_PREFERENCE_INPUT_TITLE) as text|null, MAX_NAME_LEN)
			if(raw_choice && CanUseTopic(user))
