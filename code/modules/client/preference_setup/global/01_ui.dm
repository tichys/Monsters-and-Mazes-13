...
/datum/category_item/player_setup_item/player_global/ui/OnTopic(var/href,var/list/href_list, var/mob/user)
	if(href_list["select_style"])
		...
		if (user.client && user.client.byond_version < 511)
			version_message = "\nI need to be using byond version 511 or later to take advantage of this feature, my version of [user.client.byond_version] is too low"
		if (world.byond_version < 511)
			version_message += "\nThis server does not currently support client side fps. I can set now for when it does."
		var/new_fps = input(user, "Choose my desired fps.[version_message]\n(0 = synced with server tick rate (currently:[world.fps]))", "Global Preference") as num|null
...
/proc/can_select_ooc_color(var/mob/user) 
	...
	if(can_get_reward(user, "custom_ooc_color"))
		allowed = TRUE

	if(config.allow_admin_ooccolor && check_rights(R_ADMIN, 0, user))
		allowed = TRUE
