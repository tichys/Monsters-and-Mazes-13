...
/datum/matchmaker/proc/do_matchmaking()
	...
	for(var/mob/M in to_warn)
		to_chat(M,"<span class='warning'>I have new connections. Use \"See Relationship Info\" to view and finalize them.</span>")
...
/datum/relation/proc/valid_candidate(datum/mind/M)
	if(M == holder)	//no, I NEED other people
		return FALSE
...
/datum/relation/proc/sever()
	to_chat(holder.current,"<span class='warning'>My connection with [other.holder] is no more.</span>")
	to_chat(other.holder.current,"<span class='warning'>My connection with [holder] is no more.</span>")
	other.other = null
...
/datum/relation/proc/finalize()
	finalized = 1
	to_chat(holder.current,"<span class='warning'>I have finalized a connection with [other.holder].</span>")
	to_chat(other.holder.current,"<span class='warning'>[holder] has finalized a connection with you.</span>")
	if(other && other.finalized)
		to_chat(holder.current,"<span class='warning'>My connection with [other.holder] is now confirmed!</span>")
		to_chat(other.holder.current,"<span class='warning'>My connection with [holder] is now confirmed!</span>")
		...
		for(var/mob/living/carbon/human/M in candidates)
			...
			if(coworker && holder.assigned_job && other.holder.assigned_job)
				if((coworker.department_flag & holder.assigned_job.department_flag) || (coworker.department_flag & other.holder.assigned_job.department_flag))
					candidates[M] = 5	//coworkers are 5 times as likely to know about my relations
...
/mob/living/verb/see_relationship_info()
	...
	set desc = "See what connections between people I know of."
	set category = "IC"
	...
	for(var/datum/relation/R in relations)
		...
		if(R.other.info)
			dat += "<br><b>Things I know about them:</b><br>[R.other.info]<br>[R.other.holder.gen_relations_info]"
		dat += "<hr>"
...
/mob/living/Topic(href, href_list)
	...
	if(href_list["info_relation"])
		...
		if(istype(R))
			var/info = sanitize(input("What would I like the other party for this connection to know about my character?","Character info",R.info) as message|null)
			if(info)
				R.info = info
				see_relationship_info()
				return 1
	if(href_list["relations_close"])
		var/ok = "Close anyway"
		ok = alert("HEY! I have some non-finalized relationships. I can terminate them if they do not fit my character, or edit the info tidbit that the other party is given. THIS IS YOUR ONLY CHANCE to do so - after I close the window, they won't be editable.","Finalize relationships","Return to edit", "Close anyway")
		if(ok == "Close anyway")
			var/list/relations = matchmaker.get_relationships(mind)
			for(var/datum/relation/R in relations)
				R.finalize()
