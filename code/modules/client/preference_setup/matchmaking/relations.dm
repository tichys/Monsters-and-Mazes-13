...
/datum/category_item/player_setup_item/relations/content(mob/user)
	.=list()
	. += "Characters with enabled relations are paired up randomly after spawn. I can terminate relations when I first open relations info window, but after that it's final."
	. += "<hr>"
	. += "<br><b>What do they know about you?</b> This is the general info that all kinds of my connections would know. <a href='?src=\ref[src];relation_info=["general"]'>Edit</a>"
...
/datum/category_item/player_setup_item/relations/OnTopic(var/href,var/list/href_list, var/mob/user)
	...
	if(href_list["relation_info"])
		...
		var/info = sanitize(input("Character info", "What would I like the other party for this connection to know about my character?",pref.relations_info[R]) as message|null)
		if(info)
