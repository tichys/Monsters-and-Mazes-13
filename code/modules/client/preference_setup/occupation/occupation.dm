...
/datum/category_item/player_setup_item/occupation/content(mob/user, limit = 16, list/splitJobs, splitLimit = 1)
	...
	. += "<br>"
	. += "<table width='100%' cellpadding='1' cellspacing='0'><tr><td width='20%'>" // Table within a table for alignment, also allows me to easily add more columns.
...
/datum/category_item/player_setup_item/occupation/OnTopic(href, href_list, user)
	if(href_list["reset_jobs"])
		...
		var/choice = input(user, "Choose my branch of service.", CHARACTER_PREFERENCE_INPUT_TITLE, pref.char_branch) as null|anything in mil_branches.spawn_branches(preference_species())
		...
		if(current_branch)
			choice = input(user, "Choose my rank.", CHARACTER_PREFERENCE_INPUT_TITLE, pref.char_rank) as null|anything in mil_branches.spawn_ranks(pref.char_branch, preference_species())
	...
	//From the skills popup
		...
		if(job.department)
			...
			if(job.head_position)
				dat += "I am in charge of this department."

		dat += "I answer to <b>[job.supervisors]</b> normally."

		if(job.allowed_branches)
			dat += "I can be of following ranks:"
			for(var/T in job.allowed_branches)
				var/datum/mil_branch/B = mil_branches.get_branch_by_type(T)
				if(B)	dat += "<li>[B.name]: [job.get_ranks(B.name)]"
		dat += "<hr style='clear:left;'>"
		var/description = job.get_description_blurb()
		if(description)
			dat += html_encode(description)
		if(job.objective)
			dat += "<b>My Objective as [job.title]: </b> [job.objective]"
