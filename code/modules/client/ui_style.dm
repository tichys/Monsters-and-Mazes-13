...
/client/verb/change_ui()
	set name = "Change UI"
	set category = "OOC"
	set desc = "Configure my user interface"

	if(!ishuman(usr))
		to_chat(usr, "<span class='warning'>I must be human to use this verb.</span>")
		return

	var/UI_style_new = input(usr, "Select a style. White is recommended for customization") as null|anything in all_ui_styles
	if(!UI_style_new) return

	var/UI_style_alpha_new = input(usr, "Select a new alpha (transparency) parameter for my UI, between 50 and 255") as null|num
	if(!UI_style_alpha_new | !(UI_style_alpha_new <= 255 && UI_style_alpha_new >= 50)) return

	var/UI_style_color_new = input(usr, "Choose my UI color. Dark colors are not recommended!") as color|null
