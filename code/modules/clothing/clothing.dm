	...
	if(species_restricted && istype(M,/mob/living/carbon/human))
		...
		if(H.species)
			...
			if(!wearable && !(slot in list(slot_l_store, slot_r_store, slot_s_store)))
				if(!disable_warning)
					to_chat(H, "<span class='danger'>My species cannot wear [src].</span>")
				return 0
	return 1
...
/obj/item/clothing/proc/refit_for_species(var/target_species)
	...
	switch(target_species)
		if(SPECIES_HUMAN)	//humanoid bodytypes
			species_restricted = list(SPECIES_HUMAN, SPECIES_IPC) //humans/machines can wear each other's suits
		else
...
/obj/item/clothing/head/helmet/refit_for_species(var/target_species)
	...
	//Set species_restricted list
	if(target_species == SPECIES_HUMAN)
...
/obj/item/clothing/ears/earmuffs
	name = "earmuffs"
	desc = "Protects my hearing from loud noises, and quiet ones as well."
	icon_state = "earmuffs"
	item_state = "earmuffs"
...
/obj/item/clothing/ears/earmuffs/headphones/verb/togglemusic()
	...
	if(headphones_on)
		...
		item_state = "headphones_off"
		headphones_on = 0
		to_chat(usr, "<span class='notice'>I turn the music off.</span>")
	else
		icon_state = "headphones_on"
		item_state = "headphones_on"
		headphones_on = 1
		to_chat(usr, "<span class='notice'>I turn the music on.</span>")
...
/obj/item/clothing/gloves/attackby(obj/item/weapon/W, mob/user)
	if(istype(W, /obj/item/weapon/wirecutters) || istype(W, /obj/item/weapon/scalpel))
		...
		user.visible_message("<span class='warning'>\The [user] modifies \the [src] with \the [W].</span>","<span class='warning'>I modify \the [src] with \the [W].</span>")
...
/obj/item/clothing/gloves/mob_can_equip(mob/user)
	...
	if(istype(H.gloves, /obj/item/clothing/ring))
		ring = H.gloves
		if(!ring.undergloves)
			to_chat(user, "I am unable to wear \the [src] as \the [H.gloves] are in the way.")
			ring = null
			return 0
		if(!H.unEquip(ring, src))//Remove the ring (or other under-glove item in the hand slot?) so I can put on the gloves.
			ring = null
			return 0
	...
	if (ring)
		to_chat(user, "I slip \the [src] on over \the [ring].")
...
/obj/item/clothing/head/attack_self(mob/user)
	if(brightness_on)
		if(!isturf(user.loc))
			...
			return
		on = !on
		to_chat(user, "I [on ? "enable" : "disable"] the helmet light.")
...
/obj/item/clothing/head/proc/mob_wear_hat(var/mob/user)
	...
	if(!success)
		...
		to_chat(user, "<span class='warning'>I am already wearing a hat.</span>")
	else if(success == 1)
		to_chat(user, "<span class='notice'>I crawl under \the [src].</span>")
	return 1
...
/obj/item/clothing/mask/proc/adjust_mask(var/mob/user)
	...
	if(!user.incapacitated())
		if(!pull_mask)
			to_chat(usr, "<span class ='notice'>I cannot pull down my [src.name].</span>")
			return
		else
			...
			if (src.hanging)
				...
				to_chat(usr, "I pull [src] below my chin.")
			else
				...
				to_chat(usr, "I pull [src] up to cover my face.")
			update_clothing_icon()
			user.update_action_buttons()
...
/obj/item/clothing/shoes/proc/draw_knife()
	...
	set desc = "Pull out my boot knife."
	set category = "IC"
	set src in usr
	...
	if(usr.put_in_hands(holding))
		...
		to_chat(usr, "<span class='warning'>My need an empty, unbroken hand to do that.</span>")
...
/obj/item/clothing/under/proc/set_sensors(mob/user as mob)
	...
	if(get_dist(user, src) > 1)
		to_chat(user, "I have moved too far away.")
		return
	...
	if (src.loc == user)
		switch(sensor_mode)
			if(0)
				user.visible_message("[user] adjusts the tracking sensor on \his [src.name].", "I disable my suit's remote sensing equipment.")
			if(1)
				user.visible_message("[user] adjusts the tracking sensor on \his [src.name].", "My suit will now report whether I am live or dead.")
			if(2)
				user.visible_message("[user] adjusts the tracking sensor on \his [src.name].", "My suit will now report my vital lifesigns.")
			if(3)
				user.visible_message("[user] adjusts the tracking sensor on \his [src.name].", "My suit will now report my vital lifesigns as well as my coordinate position.")

	else if (ismob(src.loc))
		if(sensor_mode == 0)
			user.visible_message("<span class='warning'>[user] disables [src.loc]'s remote sensing equipment.</span>", "I disable [src.loc]'s remote sensing equipment.")
		else
			user.visible_message("[user] adjusts the tracking sensor on [src.loc]'s [src.name].", "I adjust [src.loc]'s sensors.")
	else
		user.visible_message("[user] adjusts the tracking sensor on [src]", "I adjust the sensor on [src].")
...
/obj/item/clothing/under/verb/rollsuit()
	...
	update_rolldown_status()
	if(rolled_down == -1)
		to_chat(usr, "<span class='notice'>I cannot roll down [src]!</span>")
...
/obj/item/clothing/under/verb/rollsleeves()
	...
	update_rollsleeves_status()
	if(rolled_sleeves == -1)
		to_chat(usr, "<span class='notice'>I cannot roll up my [src]'s sleeves!</span>")
		return
	if(rolled_down == 1)
		to_chat(usr, "<span class='notice'>I must roll up my [src] first!</span>")
		return
	...
	if(rolled_sleeves)
		...
		to_chat(usr, "<span class='notice'>I roll up my [src]'s sleeves.</span>")
	else
		body_parts_covered = initial(body_parts_covered)
		item_state_slots[slot_w_uniform_str] = "[worn_state]"
		to_chat(usr, "<span class='notice'>I roll down my [src]'s sleeves.</span>")
	update_clothing_icon()
