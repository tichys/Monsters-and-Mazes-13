...
/obj/item/clothing/glasses/emp_act(severity)
	if(electric)
		if(istype(src.loc, /mob/living/carbon/human))
			...
			to_chat(M, "<span class='danger'>my [name] malfunction[gender != PLURAL ? "s":""], blinding me!</span>")
			if(M.glasses == src)
				M.eye_blind = 2
				M.eye_blurry = 4
...
/obj/item/clothing/glasses/attack_self(mob/user)
	if(toggleable && !user.incapacitated())
		if(active)
			...
			tint = TINT_NONE
			to_chat(usr, "I deactivate the optical matrix on the [src].")
		else
			active = TRUE
			...
			tint = initial(tint)
			to_chat(usr, "I activate the optical matrix on the [src].")
...
/obj/item/clothing/glasses/eyepatch/verb/flip_patch()
	...
	to_chat (usr, "I change \the [src] to cover the [src.flipped ? "left" : "right"] eye.")
...
/obj/item/clothing/glasses/welding/verb/toggle()
	...
	if(!usr.incapacitated())
		if(src.up)
			...
			flash_protection = initial(flash_protection)
			tint = initial(tint)
			to_chat(usr, "I flip \the [src] down to protect my eyes.")
		else
			src.up = !src.up
			flags_inv &= ~HIDEEYES
			body_parts_covered &= ~EYES
			...
			flash_protection = FLASH_PROTECTION_NONE
			tint = TINT_NONE
			to_chat(usr, "I push \the [src] up out of my face.")
		update_clothing_icon()
		update_vision()
...
/obj/item/clothing/glasses/sunglasses/sechud/goggles //now just a more "military" set of HUDglasses for the Mayflower
	name = "HUD goggles"
...
/obj/item/clothing/glasses/sunglasses/sechud/toggle/attack_self(mob/user)
	if(toggleable && !user.incapacitated())
		...
		if(on)
			flash_protection = FLASH_PROTECTION_NONE
			src.hud = hud_holder
			to_chat(user, "I switch \the [src] to HUD mode.")
		else
			flash_protection = initial(flash_protection)
			src.hud = null
			to_chat(user, "I switch \the [src] to flash protection mode.")
		update_icon()
