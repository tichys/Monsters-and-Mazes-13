...
/obj/item/clothing/mask/ai/attack_self(var/mob/user)
	if(user.incapacitated())
		return
	active = !active
	to_chat(user, "<span class='notice'>I [active ? "" : "dis"]engage \the [src].</span>")
	if(active)
		engage_mask(user)
	else
		disengage_mask(user)
...
/obj/item/clothing/mask/ai/proc/engage_mask(var/mob/user)
	...
	eye.possess(user)
	to_chat(eye.owner, "<span class='notice'>I feel disorented for a moment as my mind connects to the camera network.</span>")

/obj/item/clothing/mask/ai/proc/disengage_mask(var/mob/user)
	if(user == eye.owner)
		to_chat(eye.owner, "<span class='notice'>I feel disorented for a moment as my mind disconnects from the camera network.</span>")
...
/obj/item/clothing/mask/rubber/turner
	...
	desc = "Premier of the Terran Colonial Confederation. Probably shouldn't wear this in front of my veteran uncle."
	icon_state = "turner"
