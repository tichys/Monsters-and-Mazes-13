...
/obj/item/clothing/mask/monitor/equipped()
	...
	if(istype(H) && H.wear_mask == src)
		canremove = 0
		to_chat(H, "<span class='notice'>\The [src] connects to my display output.</span>")
...
/obj/item/clothing/mask/monitor/mob_can_equip(var/mob/living/carbon/human/user, var/slot)
	...
	if(istype(user))
		...
		if(istype(E) && BP_IS_ROBOTIC(E))
			return 1
		to_chat(user, "<span class='warning'>I must have a robotic head to install this upgrade.</span>")
	return 0

/obj/item/clothing/mask/monitor/verb/set_monitor_state()
	set name = "Set Monitor State"
	set desc = "Choose an icon for my monitor."
	set category = "IC"
	...
	if(!istype(H) || H != usr)
		return
	if(H.wear_mask != src)
		to_chat(usr, "<span class='warning'>I have not installed \the [src] yet.</span>")
		return
