...
/obj/item/clothing/ring/engagement/attack_self(mob/user)
	user.visible_message("<span class='warning'>\The [user] gets down on one knee, presenting \the [src].</span>","<span class='warning'>I get down on one knee, presenting \the [src].</span>")
...
/obj/item/clothing/ring/reagent/equipped(var/mob/living/carbon/human/H)
	...
	if(istype(H) && H.gloves==src)
		to_chat(H, "<font color='blue'><b>I feel a prick as I slip on the ring.</b></font>")
...
/obj/item/clothing/ring/seal/signet/attack_self(mob/user)
	...
	nameset = 1
	to_chat(user, "<span class='notice'>I claim the [src] as my own!</span>")
