...
/obj/item/clothing/shoes/magboots/proc/set_slowdown()
	slowdown_per_slot[slot_shoes] = shoes? max(0, shoes.slowdown_per_slot[slot_shoes]): 0	//So I can't put on magboots to make I walk faster.
	if (magpulse)
...
/obj/item/clothing/shoes/magboots/attack_self(mob/user)
	if(magpulse)
		...
		to_chat(user, "I disable the mag-pulse traction system.")
	else
		item_flags |= ITEM_FLAG_NOSLIP
		magpulse = 1
		set_slowdown()
		...
		to_chat(user, "I enable the mag-pulse traction system.")
...
/obj/item/clothing/shoes/magboots/mob_can_equip(mob/user)
	...
	if(H.shoes)
		shoes = H.shoes
		if(shoes.overshoes)
			to_chat(user, "I am unable to wear \the [src] as \the [H.shoes] are in the way.")
			shoes = null
			return 0
		if(!H.unEquip(shoes, src))//Remove the old shoes so I can put on the magboots.
			shoes = null
			return 0
	...
	if (shoes)
		to_chat(user, "I slip \the [src] on over \the [shoes].")
	set_slowdown()
