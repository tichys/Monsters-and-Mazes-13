...
/obj/item/clothing/suit/space/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/stack/material))
		...
		if(istype(src.loc,/mob/living))
			to_chat(user, "<span class='warning'>How do I intend to patch a hardsuit while someone is wearing it?</span>")
			return
		...
		if(!WT.remove_fuel(5))
			to_chat(user, "<span class='warning'>I need more welding fuel to repair this suit.</span>")
			return
		...
		else if(user != loc || do_after(user, 30, src))		//Doing this in my own inventory is awkward.
