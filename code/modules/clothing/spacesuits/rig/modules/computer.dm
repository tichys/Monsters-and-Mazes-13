...
/obj/item/rig_module/ai_container/accepts_item(var/obj/item/input_device, var/mob/living/user)
	...
	if(istype(input_device,/obj/item/weapon/aicard))
		...
		if(integrated_ai)
			...
			// I'm using an empty card on an empty suit, idiot.
			if(!target_ai)
				return 0
	...
	if(ai_card)
		if(istype(ai_card, /obj/item/weapon/aicard))
			if(integrated_ai && !integrated_ai.stat)
				if(user)
					to_chat(user, "<span class='danger'>I cannot eject my currently stored AI. Purge it manually.</span>")
				return 0
			to_chat(user, "<span class='danger'>I purge the remaining scraps of data from my previous AI, freeing it for use.</span>")
			if(integrated_ai)
	...
	if(ai_mob)
		...
		if(ai_mob.key && ai_mob.client)
			...
			if(istype(ai, /obj/item/weapon/aicard))
				...
				ai_card = ai
				to_chat(ai_mob, "<span class='notice'>I have been transferred to \the [holder]'s [src.name].</span>")
				to_chat(user, "<span class='notice'>I load \the [ai_mob] into \the [holder]'s [src.name].</span>")
...
/obj/item/rig_module/datajack/accepts_item(var/obj/item/input_device, var/mob/living/user)
	...
	if(istype(input_device,/obj/item/weapon/disk/tech_disk))
		...
		if(disk.stored)
			if(load_data(disk.stored))
				...
				to_chat(user, "<span class='warning'>The disk is corrupt. It is useless to me.</span>")
		else
			to_chat(user, "<span class='warning'>The disk is blank. It is useless to me.</span>")
		return 1
	...
	// I fucking hate R&D code. This typecheck spam would be totally unnecessary in a sane setup.
		...
		if(!incoming_files || !incoming_files.known_tech || !incoming_files.known_tech.len)
			...
			// Maybe consider a way to drop all my data into a target repo in the future.
...
/obj/item/rig_module/power_sink/deactivate()
	...
	if(interfaced_with)
		if(holder && holder.wearer)
			to_chat(holder.wearer, "<span class = 'warning'>My power sink retracts as the module deactivates.</span>")
		drain_complete()
...
/obj/item/rig_module/power_sink/engage(atom/target)
	...
	to_chat(H, "<span class = 'danger'>I begin draining power from [target]!</span>")
...
/obj/item/rig_module/power_sink/Process()
	...
	if(!holder.cell)
		to_chat(H, "<span class = 'danger'>My power sink flashes an error; there is no cell in my rig.</span>")
		drain_complete(H)
		return

	if(!interfaced_with || !interfaced_with.Adjacent(H) || !(interfaced_with.loc == drain_loc))
		to_chat(H, "<span class = 'warning'>My power sink retracts into its casing.</span>")
		drain_complete(H)
		return

	if(holder.cell.fully_charged())
		to_chat(H, "<span class = 'warning'>My power sink flashes an amber light; my rig cell is full.</span>")
		drain_complete(H)
		return

	var/target_drained = interfaced_with.drain_power(0,0,max_draining_rate)
	if(target_drained <= 0)
		to_chat(H, "<span class = 'danger'>My power sink flashes a red light; there is no power left in [interfaced_with].</span>")
		drain_complete(H)
		return
