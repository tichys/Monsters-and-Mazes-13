	...
	if(istype(W,/obj/item/stack/nanopaste))
		...
		switch(damage)
			if(0)
				...
				return
			if(2)
				to_chat(user, "There is no damage that I am capable of mending with such crude tools.")
				return
		...
		damage = 1
		to_chat(user, "I mend some of damage to [src] with [W], but I will need more advanced tools to fix it completely.")
		cable.use(5)
		return
	..()
...
/obj/item/rig_module/proc/engage()
	...
	if(world.time < next_use)
		to_chat(usr, "<span class='warning'>I cannot use the [interface_name] again so soon.</span>")
		return 0
	...
	if(usr.lying || usr.stat || usr.stunned || usr.paralysis || usr.weakened)
		to_chat(usr, "<span class='warning'>I cannot use the suit in this state.</span>")
		return 0
