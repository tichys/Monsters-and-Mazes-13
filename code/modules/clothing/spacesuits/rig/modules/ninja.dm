...
/obj/item/rig_module/teleporter/engage(var/atom/target, var/notify_ai)
	...
	if(!istype(H.loc, /turf))
		to_chat(H, "<span class='warning'>I cannot teleport out of my current location.</span>")
		return 0
	...
	if(T.density)
		to_chat(H, "<span class='warning'>I cannot teleport into solid walls.</span>")
		return 0

	if(T.z in GLOB.using_map.admin_levels)
		to_chat(H, "<span class='warning'>I cannot use my teleporter on this Z-level.</span>")
		return 0

	if(T.contains_dense_objects())
		to_chat(H, "<span class='warning'>I cannot teleport to a location with solid objects.</span>")
		return 0

	if(T.z != H.z || get_dist(T, get_turf(H)) > world.view)
		to_chat(H, "<span class='warning'>I cannot teleport to such a distant object.</span>")
		return 0
...
/obj/item/rig_module/self_destruct/engage(var/skip_check = FALSE)
	...
	if(!skip_check)
		if(!usr || alert(usr, "Are I sure I want to push that button?", "Self-destruct", "No", "Yes") == "No")
			return
	...
	src.blink_mode = 1
	src.blink()
	holder.visible_message("<span class='notice'>\The [src.holder] begins beeping.</span>","<span class='notice'> I hear beeping.</span>")
	sleep(blink_time)
	src.blink_mode = 2
	holder.visible_message("<span class='warning'>\The [src.holder] beeps rapidly!</span>","<span class='warning'> I hear rapid beeping!</span>")
	sleep(blink_rapid_time)
	src.blink_mode = 3
	holder.visible_message("<span class='danger'>\The [src.holder] emits a shrill tone!</span>","<span class='danger'> I hear a shrill tone!</span>")
