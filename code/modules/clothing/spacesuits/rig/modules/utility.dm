...
/obj/item/rig_module/chem_dispenser/accepts_item(var/obj/item/input_item, var/mob/living/user)
	...
	if(total_transferred)
		to_chat(user, "<font color='blue'>I transfer [total_transferred] units into the suit reservoir.</font>")
	else
...
/obj/item/rig_module/chem_dispenser/engage(atom/target)
	...
	if(!charge_selected)
		to_chat(H, "<span class='danger'>I have not selected a chemical type.</span>")
		return 0
	...
	if(target_mob != H)
		to_chat(H, "<span class='danger'>I inject [target_mob] with [chems_to_use] unit\s of [charge.display_name].</span>")
	to_chat(target_mob, "<span class='danger'>I feel a rushing in my veins as [chems_to_use] unit\s of [charge.display_name] [chems_to_use == 1 ? "is" : "are"] injected.</span>")
...
/obj/item/rig_module/voice/engage()
	...
	var/choice= input("Would I like to toggle the synthesiser or set the name?") as null|anything in list("Enable","Disable","Set Name")
	...
	switch(choice)
		if("Enable")
			...
			to_chat(usr, "<font color='blue'>I enable the speech synthesiser.</font>")
		if("Disable")
			active = 0
			voice_holder.active = 0
			to_chat(usr, "<font color='blue'>I disable the speech synthesiser.</font>")
		if("Set Name")
			var/raw_choice = sanitize(input(usr, "Please enter a new name.")  as text|null, MAX_NAME_LEN)
			if(!raw_choice)
				return 0
			voice_holder.voice = raw_choice
			to_chat(usr, "<font color='blue'>I am now mimicking <B>[voice_holder.voice]</B>.</font>")
	return 1
