...
/obj/item/weapon/rig/proc/toggle_seals(var/mob/initiator,var/instant)
	...
	if(!seal_target && !suit_is_deployed())
		wearer.visible_message("<span class='danger'>[wearer]'s suit flashes an error light.</span>","<span class='danger'>My suit flashes an error light. It can't function properly without being fully deployed.</span>")
		failed_to_seal = 1
	...
	if(!failed_to_seal)
		...
		if(!instant)
			...
			if(seal_delay && !do_after(wearer,seal_delay, src))
				if(wearer) to_chat(wearer, "<span class='warning'>I must remain still while the suit is adjusting the components.</span>")
				failed_to_seal = 1
		...
		if(!wearer)
			...
			for(var/list/piece_data in list(list(wearer.shoes,boots,"boots",boot_type),list(wearer.gloves,gloves,"gloves",glove_type),list(wearer.head,helmet,"helmet",helm_type),list(wearer.wear_suit,chest,"chest",chest_type)))
				...
				if(!failed_to_seal && wearer.back == src && piece == compare_piece)
					...
					switch(msg_type)
						if("boots")
							to_chat(wearer, "<font color='blue'>\The [piece] [!seal_target ? "seal around my feet" : "relax their grip on my legs"].</font>")
							wearer.update_inv_shoes()
						if("gloves")
							to_chat(wearer, "<font color='blue'>\The [piece] [!seal_target ? "tighten around my fingers and wrists" : "become loose around my fingers"].</font>")
							wearer.update_inv_gloves()
						if("chest")
							to_chat(wearer, "<font color='blue'>\The [piece] [!seal_target ? "cinches tight again my chest" : "releases my chest"].</font>")
	...
	// Success!
	canremove = seal_target
	to_chat(wearer, "<font color='blue'><b>My entire suit [canremove ? "loosens as the components relax" : "tightens around me as the components lock into place"].</b></font>")
		...
		if(offline)
			...
			if(!canremove)
				if (offline_slowdown < 3)
					to_chat(wearer, "<span class='danger'>My suit beeps stridently, and suddenly goes dead.</span>")
				else
					to_chat(wearer, "<span class='danger'>My suit beeps stridently, and suddenly you're wearing a leaden mass of metal and plastic composites instead of a powered suit.</span>")
			if(offline_vision_restriction >= TINT_MODERATE)
				to_chat(wearer, "<span class='danger'>The suit optics flicker and die, leaving I with restricted vision.</span>")
			else if(offline_vision_restriction >= TINT_BLIND)
				to_chat(wearer, "<span class='danger'>The suit optics drop out completely, drowning I in darkness.</span>")
...
/obj/item/weapon/rig/proc/check_power_cost(var/mob/living/user, var/cost, var/use_unconcious, var/obj/item/rig_module/mod, var/user_is_ai)
	...
	if(!user_is_ai)
		...
		if(istype(H) && H.back != src)
			fail_msg = "<span class='warning'>I must be wearing \the [src] to do this.</span>"
	if(sealing)
		...
		fail_msg = "<span class='warning'>I am in no fit state to do that.</span>"
	else if(!cell)
...
/obj/item/weapon/rig/equipped(mob/living/carbon/human/M)
	...
	if(seal_delay > 0 && istype(M) && M.back == src)
		M.visible_message("<font color='blue'>[M] starts putting on \the [src]...</font>", "<font color='blue'>I start putting on \the [src]...</font>")
	...
	if(istype(M) && M.back == src)
		M.visible_message("<font color='blue'><b>[M] struggles into \the [src].</b></font>", "<font color='blue'><b>I struggle into \the [src].</b></font>")
		wearer = M
		wearer.wearing_rig = src
		update_icon()
	...
	if(use_obj)
		if(check_slot == use_obj && deploy_mode != ONLY_DEPLOY)
			...
			if(use_obj)
				...
				if(istype(holder))
					if(use_obj && check_slot == use_obj)
						to_chat(wearer, "<font color='blue'><b>my [use_obj.name] [use_obj.gender == PLURAL ? "retract" : "retracts"] swiftly.</b></font>")
						use_obj.canremove = 1
			...
			if(!wearer.equip_to_slot_if_possible(use_obj, equip_to, 0, 1))
				...
				if(check_slot)
					to_chat(initiator, "<span class='danger'>I am unable to deploy \the [piece] as \the [check_slot] [check_slot.gender == PLURAL ? "are" : "is"] in the way.</span>")
					return
			else
				to_chat(wearer, "<span class='notice'>my [use_obj.name] [use_obj.gender == PLURAL ? "deploy" : "deploys"] swiftly.</span>")
...
/obj/item/weapon/rig/proc/take_hit(damage, source, is_emp=0)
	...
	if(wearer)
		if(dam_module.damage >= 2)
			to_chat(wearer, "<span class='danger'>The [source] has disabled my [dam_module.interface_name]!</span>")
		else
			to_chat(wearer, "<span class='warning'>The [source] has damaged my [dam_module.interface_name]!</span>")
...
/obj/item/weapon/rig/proc/ai_can_move_suit(var/mob/user, var/check_user_module = 0, var/check_for_ai = 0)
	...
	if(check_user_module)
		...
		if(!istype(module) || module.damage >= 2)
			to_chat(user, "<span class='warning'>My host module is unable to interface with the suit.</span>")
			return 0

	if(offline || !cell || !cell.charge || locked_down)
		if(user) to_chat(user, "<span class='warning'>My host rig is unpowered and unresponsive.</span>")
		return 0
	if(!wearer || wearer.back != src)
		if(user) to_chat(user, "<span class='warning'>My host rig is not being worn.</span>")
		return 0
	if(!wearer.stat && !control_overridden && !ai_override_enabled)
		if(user) to_chat(user, "<span class='warning'>I am locked out of the suit servo controller.</span>")
		return 0
	return 1
...
// This returns the rig if I am contained inside one, but not if I am wearing it
/atom/proc/get_rig()
	if(loc)
		return loc.get_rig()
	return null
