	...
	if(open)
		...
		// Check if this is a hardsuit upgrade or a modification.
			...
			if(istype(src.loc,/mob/living/carbon/human))
				...
				if(H.back == src)
					to_chat(user, "<span class='danger'>I can't install a hardsuit module while the suit is being worn.</span>")
					return 1
			...
			var/to_remove = input("Which would I like to modify?") as null|anything in current_mounts
			if(!to_remove)
				return
			...
			switch(to_remove)
				...
				if("cell")
					...
					if(cell)
						to_chat(user, "I detach \the [cell] from \the [src]'s battery mount.")
				...
				if("system module")
					...
					var/removal_choice = input("Which module would I like to remove?") as null|anything in possible_removals
					if(!removal_choice)
						return

					var/obj/item/rig_module/removed = possible_removals[removal_choice]
					to_chat(user, "I detach \the [removed] from \the [src].")
			...
			if(malfunctioning || malfunction_delay)
				if(S.use(1))
					to_chat(user, "I pour some of \the [S] over \the [src]'s control circuitry and watch as the nanites do their work with impressive speed and precision.")
					malfunctioning = 0
					malfunction_delay = 0
				else
					to_chat(user, "\The [S] is empty!")
			else
				to_chat(user, "I don't see any use for \the [S].")
...
/obj/item/weapon/rig/emag_act(var/remaining_charges, var/mob/user)
	if(!subverted)
		...
		locked = 0
		subverted = 1
		to_chat(user, "<span class='danger'>I short out the access protocol for the suit.</span>")
		return 1
