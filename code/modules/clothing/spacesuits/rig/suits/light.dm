...
/obj/item/weapon/rig/light/ninja/verb/rename_suit()
	...
	set desc = "Rename my black voidsuit."
	set category = "Object"
	var/mob/M = usr
	if(!M.mind) return 0
	if(M.incapacitated()) return 0
	var/input = sanitizeSafe(input("What do I want to name my suit?", "Rename suit"), MAX_NAME_LEN)
...
/obj/item/weapon/rig/light/ninja/verb/rewrite_suit_desc()
	...
	set desc = "Give my voidsuit a custom description."
	set category = "Object"
	var/mob/M = usr
	if(!M.mind) return 0
	if(M.incapacitated()) return 0
	var/input = sanitizeSafe(input("Please describe my voidsuit in 128 letters or less.", "write description"), MAX_DESC_LEN)
	if(src && input && !M.incapacitated() && in_range(M,src))
		desc = input
