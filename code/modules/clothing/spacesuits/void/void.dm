...
/obj/item/clothing/suit/space/void/equipped(mob/M)
	...
	if(helmet)
		if(H.head)
			to_chat(M, "I am unable to deploy my suit's helmet as \the [H.head] is in the way.")
		else if (H.equip_to_slot_if_possible(helmet, slot_head))
			to_chat(M, "My suit's helmet deploys with a hiss.")
			helmet.canremove = 0

	if(tank)
		if(H.s_store) //In case someone finds a way.
			to_chat(M, "Alarmingly, the valve on my suit's installed tank fails to engage.")
		else if (H.equip_to_slot_if_possible(tank, slot_s_store))
			to_chat(M, "The valve on my suit's installed tank safely engages.")
			tank.canremove = 0
...
/obj/item/clothing/suit/space/void/verb/toggle_helmet()
	...
	if(H.head == helmet)
		to_chat(H, "<span class='notice'>I retract my suit helmet.</span>")
		helmet.canremove = 1
		H.drop_from_inventory(helmet, src)
	else
		if(H.head)
			to_chat(H, "<span class='danger'>I cannot deploy my helmet while wearing \the [H.head].</span>")
			return
		if(H.equip_to_slot_if_possible(helmet, slot_head))
			helmet.pickup(H)
			helmet.canremove = 0
			to_chat(H, "<span class='info'>I deploy my suit helmet, sealing I off from the world.</span>")
...
/obj/item/clothing/suit/space/void/verb/eject_tank()
	...
	to_chat(H, "<span class='info'>I press the emergency release, ejecting \the [tank] from my suit.</span>")
	tank.canremove = 1
...
/obj/item/clothing/suit/space/void/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(user.get_inventory_slot(src) == slot_wear_suit)
		to_chat(user, "<span class='warning'>I cannot modify \the [src] while it is being worn.</span>")
		return

	if(istype(W,/obj/item/weapon/screwdriver))
		if(helmet || boots || tank)
			var/choice = input("What component would I like to remove?") as null|anything in list(helmet,boots,tank)
			if(!choice) return

			if(choice == tank)	//No, a switch doesn't work here. Sorry. ~Techhead
				to_chat(user, "I pop \the [tank] out of \the [src]'s storage compartment.")
				tank.forceMove(get_turf(src))
				src.tank = null
			else if(choice == helmet)
				to_chat(user, "I detatch \the [helmet] from \the [src]'s helmet mount.")
				helmet.forceMove(get_turf(src))
				src.helmet = null
			else if(choice == boots)
				to_chat(user, "I detatch \the [boots] from \the [src]'s boot mounts.")
		...
		if(helmet)
			...
			if(!user.unEquip(W, src))
				return
			to_chat(user, "I attach \the [W] to \the [src]'s helmet mount.")
			src.helmet = W
		return
		...
		if(boots)
			...
			if(!user.unEquip(W, src))
				return
			to_chat(user, "I attach \the [W] to \the [src]'s boot mounts.")
			boots = W
		return
		...
		if(tank)
			...
			if(!user.unEquip(W, src))
				return
			to_chat(user, "I insert \the [W] into \the [src]'s storage compartment.")
			tank = W
		return
