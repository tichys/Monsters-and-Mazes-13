...
/obj/item/clothing/accessory/badge/attack_self(mob/user as mob)
	...
	if(!stored_name)
		to_chat(user, "I inspect my [src.name]. Everything seems to be in order and I give it a quick cleaning with my hand.")
	...
	if(isliving(user))
		if(stored_name)
			user.visible_message("<span class='notice'>[user] displays their [src.name].\nIt reads: [stored_name], [badge_string].</span>","<span class='notice'>I display my [src.name].\nIt reads: [stored_name], [badge_string].</span>")
		else
			user.visible_message("<span class='notice'>[user] displays their [src.name].\nIt reads: [badge_string].</span>","<span class='notice'>I display my [src.name]. It reads: [badge_string].</span>")

/obj/item/clothing/accessory/badge/attack(mob/living/carbon/human/M, mob/living/user)
	if(isliving(user))
		user.visible_message("<span class='danger'>[user] invades [M]'s personal space, thrusting \the [src] into their face insistently.</span>","<span class='danger'>I invade [M]'s personal space, thrusting \the [src] into their face insistently.</span>")
		if(stored_name)
...
/obj/item/clothing/accessory/badge/holo/emag_act(var/remaining_charges, var/mob/user)
	if (emagged)
		...
		emagged = 1
		to_chat(user, "<span class='danger'>I crack the holobadge security checks.</span>")
		return 1
...
/obj/item/clothing/accessory/badge/holo/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O, /obj/item/weapon/card/id) || istype(O, /obj/item/modular_computer))
		...
		if((badge_access in id_card.access) || emagged)
			...
			to_chat(user, "[src] rejects my ID, and flashes 'Insufficient access!'")
		return
	..()
