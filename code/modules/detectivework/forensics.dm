...
/atom/proc/add_fingerprint(mob/M, ignoregloves, obj/item/tool)
	...
	if(!ignoregloves && ishuman(M))
		...
		if (H.gloves && H.gloves.body_parts_covered & HANDS && H.gloves != src)
			if(istype(H.gloves, /obj/item/clothing/gloves)) //Don't add prints if I am wearing gloves.
...
/atom/proc/add_partial_print(full_print, bonus)
	if(!fingerprints[full_print])
		...
		switch(max(stringpercent(fingerprints[full_print]) - bonus,0))		//tells us how many stars are in the current prints.
			if(28 to 32)
				if(prob(1))
					fingerprints[full_print] = full_print 		// I rolled a one buddy.
				else
			...
			if(0 to 15)
				if(prob(5))
					fingerprints[full_print] = stars(full_print, rand(0,50)) 	// small chance I can smudge.
				else
...
/mob/examinate(atom/A as mob|obj|turf in view())
	...
	if(get_skill_value(SKILL_FORENSICS) >= SKILL_EXPERT && get_dist(src, A) <= (get_skill_value(SKILL_FORENSICS) - SKILL_ADEPT))
		if(A.suit_fibers && A.suit_fibers.len > 0)
			to_chat(src, "<span class='notice'>I notice some fibers embedded in \the [A]</span>")
		if(A.fingerprints && A.fingerprints.len > 0)
			to_chat(src, "<span class='notice'>I notice a partial print on \the [A]</span>")
		var/obj/item/clothing/O = A
		if(istype(O) && O.gunshot_residue)
			to_chat(src, "<span class='notice'>I notice a faint acrid smell coming from \the [A]</span>")
		//Noticing wiped blood is a bit harder
		if((get_skill_value(SKILL_FORENSICS) >= SKILL_PROF) && A.blood_DNA)
			to_chat(src, "<span class='warning'>I notice faint blood traces on \The [A]</span>")
