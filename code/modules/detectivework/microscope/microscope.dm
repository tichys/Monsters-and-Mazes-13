...
/obj/machinery/microscope/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(istype(W, /obj/item/weapon/forensics/swab)|| istype(W, /obj/item/weapon/sample/fibers) || istype(W, /obj/item/weapon/sample/print))
		to_chat(user, "<span class='notice'>I insert \the [W] into the microscope.</span>")
...
/obj/machinery/microscope/attack_hand(mob/user)
	...
	to_chat(user, "<span class='notice'>The microscope whirrs as I examine \the [sample].</span>")

	if(!user.do_skilled(25, SKILL_FORENSICS, src) || !sample)
		to_chat(user, "<span class='notice'>I stop examining \the [sample].</span>")
		return

	if(!user.skill_check(SKILL_FORENSICS, SKILL_ADEPT))
		to_chat(user, "<span class='warning'>I can't figure out what it means...</span>")
		return
...
/obj/machinery/microscope/proc/remove_sample(var/mob/living/remover)
	...
	if(!sample)
		...
		return
	to_chat(remover, "<span class='notice'>I remove \the [sample] from \the [src].</span>")
