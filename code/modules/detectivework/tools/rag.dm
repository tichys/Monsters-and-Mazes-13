...
/obj/item/weapon/reagent_containers/glass/rag/attack_self(mob/user as mob)
	if(on_fire && user.unEquip(src))
		user.visible_message("<span class='warning'>\The [user] stamps out [src].</span>", "<span class='warning'>I stamp out [src].</span>")
		extinguish()
	else
	...
	if(!on_fire && istype(W, /obj/item/weapon/flame))
		...
		if(F.lit)
			...
			if(on_fire)
				...
				to_chat(user, "<span class='warning'>I manage to singe [src], but fail to light it.</span>")
...
/obj/item/weapon/reagent_containers/glass/rag/proc/remove_contents(mob/user, atom/trans_dest = null)
	...
	if(reagents.total_volume)
		...
		user.visible_message("<span class='danger'>\The [user] begins to wring out [src] over [target_text].</span>", "<span class='notice'>I begin to wring out [src] over [target_text].</span>")
		...
		if(do_after(user, reagents.total_volume*5, progress = 0)) //50 for a fully soaked rag
			...
			user.visible_message("<span class='danger'>\The [user] wrings out [src] over [target_text].</span>", "<span class='notice'>I finish to wringing out [src].</span>")
			update_name()
		...
		if(on_fire)
			...
			if(user.zone_sel.selecting == BP_MOUTH)
				...
				user.visible_message(
					...
					"<span class='warning'>I smother [target] with [src]!</span>",
					"I hear some struggling and muffled cries of surprise"
					)
...
/obj/item/weapon/reagent_containers/glass/rag/afterattack(atom/A as obj|turf|area, mob/user as mob, proximity)
	...
	if(istype(A, /obj/structure/reagent_dispensers))
		...
		if(A.reagents && A.reagents.trans_to_obj(src, reagents.maximum_volume))
			user.visible_message("<span class='notice'>\The [user] soaks [src] using [A].</span>", "<span class='notice'>I soak [src] using [A].</span>")
			update_name()
		return
