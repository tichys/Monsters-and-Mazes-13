...
/obj/item/weapon/sample/proc/merge_evidence(var/obj/item/weapon/sample/supplied, var/mob/user)
	...
	to_chat(user, "<span class='notice'>I transfer the contents of \the [supplied] into \the [src].</span>")
	return 1
...
/obj/item/weapon/sample/print/merge_evidence(var/obj/item/weapon/sample/supplied, var/mob/user)
	...
	SetName("[initial(name)] (combined)")
	to_chat(user, "<span class='notice'>I overlay \the [src] and \the [supplied], combining the print records.</span>")
	return 1
...
/obj/item/weapon/sample/print/attack_self(var/mob/user)
	...
	to_chat(user, "<span class='notice'>I firmly press my fingertips onto the card.</span>")
...
/obj/item/weapon/forensics/sample_kit/proc/take_sample(var/mob/user, var/atom/supplied)
	...
	to_chat(user, "<span class='notice'>I transfer [S.evidence.len] [S.evidence.len > 1 ? "[evidence_type]s" : "[evidence_type]"] to \the [S].</span>")
...
/obj/item/weapon/forensics/sample_kit/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(user.skill_check(SKILL_FORENSICS, SKILL_ADEPT) && can_take_sample(user, A))
		...
		to_chat(user, "<span class='warning'>I am unable to locate any [evidence_type]s on \the [A].</span>")
		. = ..()
