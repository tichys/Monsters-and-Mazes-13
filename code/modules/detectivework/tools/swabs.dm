...
/obj/item/weapon/forensics/swab/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(!choices.len)
		...
		choice = input("What kind of evidence are I looking for?","Evidence Collection") as null|anything in choices
	...
	if(sample_type)
		user.visible_message("\The [user] swabs \the [A] for a sample.", "I swab \the [A] for a sample.")
