...
/obj/machinery/atm/attackby(obj/item/I as obj, mob/user as mob)
	if(istype(I, /obj/item/weapon/card))
		if(emagged > 0)
			...
			return
		if(stat & NOPOWER)
			to_chat(user, "I try to insert my card into [src], but nothing happens.")
			return
		...
		if(emagged > 0)
			...
			if(ticks_left_locked_down > 0)
				...
				if(!account_security_level)
					t += "To log in to my savings account, press 'submit' with ID clearly displayed. If I wish to log into another account, please enter the account number into the field below or insert a registered ID card into the slot above and then press 'submit'.<BR>"
		...
		switch(href_list["choice"])
			...
			if("insert_card")
				if(!held_card)
					...
					if(emagged > 0)
						to_chat(usr, "\icon[src] <span class='warning'>The ATM card reader rejected my ID because this machine has been sabotaged!</span>")
					else
