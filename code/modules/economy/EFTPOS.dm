...
/obj/item/device/eftpos/New()
	...
	spawn(0)
		...
		/*
		R.info += "<b>When first setting up my EFTPOS device:</b>"
		R.info += "1. Memorise my EFTPOS command code (provided with all EFTPOS devices).<br>"
		R.info += "2. Confirm that my EFTPOS device is connected to my local accounts database. For additional assistance with this step, contact NanoTrasen IT Support<br>"
		R.info += "3. Confirm that my EFTPOS device has been linked to the account that I wish to recieve funds for all transactions processed on this device.<br>"
		R.info += "<b>When starting a new transaction with my EFTPOS device:</b>"
		...
		R.info += "3. Lock the transaction, it is now ready for my customer.<br>"
		R.info += "4. If at this stage I wish to modify or cancel my transaction, I may simply reset (unlock) my EFTPOS device.<br>"
		R.info += "5. Give my EFTPOS device to the customer, they must authenticate the transaction by swiping their ID card and entering their PIN number.<br>"
		R.info += "6. If done correctly, the transaction will be logged to both accounts with the reference I have entered, the terminal ID of my EFTPOS device and the money transferred across accounts.<br>"
		*/
		//Temptative new manual:
		R.info += "<b>First EFTPOS setup:</b><br>"
		R.info += "1. Memorise my EFTPOS command code (provided with all EFTPOS devices).<br>"
		R.info += "2. Connect the EFTPOS to the account in which I want to receive the funds.<br><br>"
		R.info += "<b>When starting a new transaction:</b><br>"
		R.info += "1. Enter the amount of money I want to charge and a purpose message for the new transaction.<br>"
		R.info += "2. Lock the new transaction. If I want to modify or cancel the transaction, I simply have to reset my EFTPOS device.<br>"
		R.info += "3. Give the EFTPOS device to my customer, he/she must finish the transaction by swiping their ID card or a charge card with enough funds.<br>"
		R.info += "4. If everything is done correctly, the money will be transferred. To unlock the device I will have to reset the EFTPOS device.<br>"
...
/obj/item/device/eftpos/proc/scan_card(var/obj/item/weapon/card/I, var/obj/item/ID_container)
	if (istype(I, /obj/item/weapon/card/id))
				...
				if(!linked_account.suspended)
					...
					if(D)
						if(!D.suspended)
							...
							to_chat(usr, "\icon[src]<span class='warning'>My account has been suspended.</span>")
					else
		...
		if(transaction_locked)
			if(transaction_paid)
				to_chat(usr, "\icon[src]<span class='info'>I stealthily swipe \the [I] through \the [src].</span>")
				transaction_locked = 0
				transaction_paid = 0
			else
				usr.visible_message("<span class='info'>\The [usr] swipes a card through \the [src].</span>")
