...
/decl/emote
	...
	var/emote_message_1p               // First person message ('I do a flip!')
	var/emote_message_3p               // Third person message ('Urist McShitter does a flip!')
	var/emote_message_impaired         // Deaf/blind message ('I hear someone flipping out.', 'I see someone opening and closing their mouth')

	var/emote_message_1p_target        // 'I do a flip at Urist McTarget!'
	...
	var/playing = FALSE				   // Don't spam emotes
...
/decl/emote/proc/do_emote(var/atom/user, var/extra_params)
	...
	if(ismob(user) && check_restraints)
		var/mob/M = user
		if(M.restrained())
			to_chat(user, "<span class='warning'>I am restrained and cannot do that.</span>")
			playing = FALSE
			return
