...
/mob/proc/emote(var/act, var/m_type, var/message)
	...
	if(usr == src) //client-called emote
		...
		if(!can_emote(m_type))
			to_chat(src, "<span class='warning'>I cannot currently [m_type == AUDIBLE_MESSAGE ? "audibly" : "visually"] emote!</span>")
			return
	...
	if(m_type != use_emote.message_type && use_emote.conscious && stat != CONSCIOUS)
		to_chat(src, "<span class='warning'>I cannot currently [use_emote.message_type == AUDIBLE_MESSAGE ? "audibly" : "visually"] emote!</span>")
		return
...
/mob/proc/custom_emote(var/m_type = VISIBLE_MESSAGE, var/message = null)
	...
	if((usr && stat) || (!use_me && usr == src))
		to_chat(src, "I am unable to emote.")
		return
