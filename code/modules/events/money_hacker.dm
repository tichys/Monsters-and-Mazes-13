...
/datum/event/money_hacker/announce()
	...
	if(MS)
		// Hide the account number for now since it's all I need to access a standard-security account. Change when that's no longer the case.
...
/datum/event/money_hacker/end()
	...
	if(affected_account && !affected_account.suspended)
		...
		T.source_terminal = pick("","[pick("Biesel","New Gibson")] GalaxyNet Terminal #[rand(111,999)]","my mums place","nantrasen high CommanD")
