...
/obj/item/weapon/board/interact(mob/user as mob)
	if(user.is_physically_disabled() || (!isAI(user) && !user.Adjacent(src))) //can't see if I amnt conscious. If I am not an AI I can't see it unless I am next to it, either.
...
/obj/item/weapon/board/Topic(href, href_list)
	...
	if(!usr.incapacitated()) //I can't move pieces if I can't move
		if(href_list["select"])
...
/obj/item/weapon/reagent_containers/food/snacks/checker/pawn
	name = "pawn"
	desc = "How many pawns will die in my war?"
