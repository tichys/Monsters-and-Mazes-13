...
/obj/item/weapon/deck/holder
	name = "card box"
	desc = "A small leather case to show how classy I am compared to everyone else."
...
/obj/item/weapon/deck/attackby(obj/O as obj, mob/user as mob)
	if(istype(O,/obj/item/weapon/hand))
		...
		for(var/datum/playingcard/P in H.cards)
			cards += P
		qdel(O)
		to_chat(user, "I place my cards on the bottom of \the [src].")
		return
	..()
...
/obj/item/weapon/deck/verb/deal_card()
	...
	var/mob/living/M = input("Who do I wish to deal a card?") as null|anything in players
	if(!usr || !src || !M) return
...
/obj/item/weapon/deck/MouseDrop(atom/over)
	if(!usr || !over) return
	if(!Adjacent(usr) || !over.Adjacent(usr)) return // should stop me from dragging through windows
...
/obj/item/weapon/hand/verb/discard()
	...
	for(var/datum/playingcard/P in cards)
		to_discard[P.name] = P
	var/discarding = input("Which card do I wish to put down?") as null|anything in to_discard
