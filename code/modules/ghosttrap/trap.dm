	...
	if(islist(ban_checks))
		for(var/bantype in ban_checks)
			if(jobban_isbanned(candidate, "[bantype]"))
				if(feedback)
					to_chat(candidate, "I am banned from one or more required roles and hence cannot enter play as \a [object].")
				return 0
	return 1
...
/datum/ghosttrap/proc/welcome_candidate(var/mob/target)
	to_chat(target, "<b>I am a positronic brain, brought into existence on [station_name()].</b>")
	to_chat(target, "<b>As a synthetic intelligence, I answer to all crewmembers, as well as the AI.</b>")
	to_chat(target, "<b>Remember, the purpose of my existence is to serve the crew and the [station_name()]. Above all else, do no harm.</b>")
...
/datum/ghosttrap/plant/welcome_candidate(var/mob/target)
	to_chat(target, "<span class='alium'><B>I awaken slowly, stirring into sluggish motion as the air caresses you.</B></span>")
	// This is a hack, replace with some kind of species blurb proc.
	if(istype(target,/mob/living/carbon/alien/diona))
		to_chat(target, "<B>I am \a [target], one of a race of drifting interstellar plantlike creatures that sometimes share their seeds with human traders.</B>")
		to_chat(target, "<B>Too much darkness will send I into shock and starve you, but light will help I heal.</B>")
/*****************
* Cortical Borer *
*****************/
...
/datum/ghosttrap/borer/welcome_candidate(var/mob/target)
	to_chat(target, "<span class='notice'>I am a cortical borer!</span> I am a brain slug that worms its way \
	into the head of its victim. Use stealth, persuasion and my powers of mind control to keep you, \
	my host and my eventual spawn safe and warm.")
	to_chat(target, "I can speak to my victim with <b>say</b>, to other borers with <b>say [target.get_language_prefix()]x</b>, and use my Abilities tab to access powers.")
/********************
* Maintenance Drone *
*********************/
...
/datum/ghosttrap/cult/welcome_candidate(var/mob/target)
	...
	if(istype(S))
		if(S.is_evil)
			...
			to_chat(target, "<b>Remember, I serve the one who summoned I first, and the cult second.</b>")
		else
			to_chat(target, "<b>This soultone has been purified. I do not belong to the cult.</b>")
			to_chat(target, "<b>Remember, I only serve the one who summoned you.</b>")
