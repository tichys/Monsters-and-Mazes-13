...
/obj/machinery/beehive/attackby(var/obj/item/I, var/mob/user)
	if(isCrowbar(I))
		closed = !closed
		user.visible_message("<span class='notice'>\The [user] [closed ? "closes" : "opens"] \the [src].</span>", "<span class='notice'>I [closed ? "close" : "open"] \the [src].</span>")
		update_icon()
		return
	else if(isWrench(I))
		anchored = !anchored
		user.visible_message("<span class='notice'>\The [user] [anchored ? "wrenches" : "unwrenches"] \the [src].</span>", "<span class='notice'>I [anchored ? "wrench" : "unwrench"] \the [src].</span>")
		return
	else if(istype(I, /obj/item/bee_smoker))
		if(closed)
			to_chat(user, "<span class='notice'>I need to open \the [src] with a crowbar before smoking the bees.</span>")
			return
		user.visible_message("<span class='notice'>\The [user] smokes the bees in \the [src].</span>", "<span class='notice'>I smoke the bees in \the [src].</span>")
		smoked = 30
		update_icon()
		return
	else if(istype(I, /obj/item/honey_frame))
		if(closed)
			to_chat(user, "<span class='notice'>I need to open \the [src] with a crowbar before inserting \the [I].</span>")
			return
		if(frames >= maxFrames)
		...
		if(H.honey)
			...
			return
		++frames
		user.visible_message("<span class='notice'>\The [user] loads \the [I] into \the [src].</span>", "<span class='notice'>I load \the [I] into \the [src].</span>")
		update_icon()
		qdel(I)
		return
		...
		if(!B.full && !smoked)
			...
			return
		if(closed)
			to_chat(user, "<span class='notice'>I need to open \the [src] with a crowbar before moving the bees.</span>")
			return
		if(B.full)
			user.visible_message("<span class='notice'>\The [user] puts the queen and the bees from \the [I] into \the [src].</span>", "<span class='notice'>I put the queen and the bees from \the [I] into \the [src].</span>")
			bee_count = 20
			B.empty()
		else
			user.visible_message("<span class='notice'>\The [user] puts bees and larvae from \the [src] into \the [I].</span>", "<span class='notice'>I put bees and larvae from \the [src] into \the [I].</span>")
			bee_count /= 2
			B.fill()
		update_icon()
		return
		...
		if(bee_count)
			to_chat(user, "<span class='notice'>I can't dismantle \the [src] with these bees inside.</span>")
			return
		to_chat(user, "<span class='notice'>I start dismantling \the [src]...</span>")
		playsound(loc, 'sound/items/Screwdriver.ogg', 50, 1)
		if(do_after(user, 30, src))
			user.visible_message("<span class='notice'>\The [user] dismantles \the [src].</span>", "<span class='notice'>I dismantle \the [src].</span>")
...
/obj/machinery/beehive/attack_hand(var/mob/user)
	if(!closed)
		if(honeycombs < 100)
			...
			return
		if(!smoked && bee_count)
			to_chat(user, "<span class='notice'>The bees won't let I take the honeycombs out like this, smoke them first.</span>")
			return
		user.visible_message("<span class='notice'>\The [user] starts taking the honeycombs out of \the [src].</span>", "<span class='notice'>I start taking the honeycombs out of \the [src]...</span>")
		while(honeycombs >= 100 && do_after(user, 30, src))
			...
			update_icon()
		if(honeycombs < 100)
			to_chat(user, "<span class='notice'>I take all filled honeycombs out.</span>")
		return
...
/obj/machinery/honey_extractor/attackby(var/obj/item/I, var/mob/user)
	if(processing)
		...
		if(!H.honey)
			...
			return
		user.visible_message("<span class='notice'>\The [user] loads \the [H] into \the [src] and turns it on.</span>", "<span class='notice'>I load \the [H] into \the [src] and turn it on.</span>")
		processing = H.honey
		...
		honey -= transferred
		user.visible_message("<span class='notice'>\The [user] collects honey from \the [src] into \the [G].</span>", "<span class='notice'>I collect [transferred] units of honey from \the [src] into \the [G].</span>")
		return 1
...
/obj/item/beehive_assembly
	...
	desc = "Contains everything I need to build a beehive."
	icon = 'icons/obj/apiary_bees_etc.dmi'
	icon_state = "apiary"

/obj/item/beehive_assembly/attack_self(var/mob/user)
	to_chat(user, "<span class='notice'>I start assembling \the [src]...</span>")
	if(do_after(user, 30, src))
		user.visible_message("<span class='notice'>\The [user] constructs a beehive.</span>", "<span class='notice'>I construct a beehive.</span>")
...
/obj/structure/closet/crate/hydroponics/beekeeping
	...
	desc = "All I need to set up my own beehive."
