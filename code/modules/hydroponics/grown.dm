	...
	if(seed)
		if(seed.get_trait(TRAIT_PRODUCES_POWER) && isCoil(W))
			...
			if(C.use(5))
				...
				to_chat(user, "<span class='notice'>I add some cable to the [src.name] and slide it inside the battery casing.</span>")
			...
			if(seed.kitchen_tag == "pumpkin") // Ugggh these checks are awful.
				user.show_message("<span class='notice'>I carve a face into [src]!</span>", 1)
				new /obj/item/clothing/head/pumpkinhead (user.loc)
				qdel(src)
				return
			else if(seed.chems)
				if(istype(W,/obj/item/weapon/material/hatchet) && !isnull(seed.chems[/datum/reagent/woodpulp]))
					user.show_message("<span class='notice'>I make planks out of \the [src]!</span>", 1)
					...
					for(var/i=0,i<2,i++)
						...
						for (var/obj/item/stack/material/wood/G in user.loc)
							...
							if(G.amount>=G.max_amount)
								continue
							G.attackby(NG, user)
						to_chat(user, "I add the newly-formed wood to the stack. It now contains [NG.amount] planks.")
					qdel(src)
					return
				else if(!isnull(seed.chems[/datum/reagent/drink/juice/potato]))
					to_chat(user, "I slice \the [src] into sticks.")
...
/obj/item/weapon/reagent_containers/food/snacks/grown/attack_self(mob/user as mob)
	...
	if(seed.kitchen_tag == "grass")
		user.show_message("<span class='notice'>I make a grass tile out of \the [src]!</span>", 1)
		var/flesh_colour = seed.get_trait(TRAIT_FLESH_COLOUR)
		if(!flesh_colour) flesh_colour = seed.get_trait(TRAIT_PRODUCT_COLOUR)
		for(var/i=0,i<2,i++)
			...
			for (var/obj/item/stack/tile/grass/NG in user.loc)
				...
				if(NG.amount>=NG.max_amount)
					continue
				NG.attackby(G, user)
			to_chat(user, "I add the newly-formed grass to the stack. It now contains [G.amount] tiles.")
		qdel(src)
		return

	if(seed.get_trait(TRAIT_SPREAD) > 0)
		to_chat(user, "<span class='notice'>I plant the [src.name].</span>")
