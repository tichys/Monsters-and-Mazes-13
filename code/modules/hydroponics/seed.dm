...
/datum/seed/proc/do_thorns(var/mob/living/carbon/human/target, var/obj/item/fruit, var/target_limb)
	...
	if(blocked >= 100 || (target.species && target.species.species_flags & (SPECIES_FLAG_NO_EMBED|SPECIES_FLAG_NO_MINOR_CUT)))
		to_chat(target, "<span class='danger'>\The [fruit]'s thorns scratch against the armour on my [affecting.name]!</span>")
		return
	...
	if(get_trait(TRAIT_CARNIVOROUS) >= 2)
		if(affecting)
			to_chat(target, "<span class='danger'>\The [fruit]'s thorns pierce my [affecting.name] greedily!</span>")
		else
			to_chat(target, "<span class='danger'>\The [fruit]'s thorns pierce my flesh greedily!</span>")
		damage = max(5, round(15*get_trait(TRAIT_POTENCY)/100, 1))
		has_edge = prob(get_trait(TRAIT_POTENCY)/2)
	else
		if(affecting)
			to_chat(target, "<span class='danger'>\The [fruit]'s thorns dig deeply into my [affecting.name]!</span>")
		else
			to_chat(target, "<span class='danger'>\The [fruit]'s thorns dig deeply into my flesh!</span>")
...
/datum/seed/proc/do_sting(var/mob/living/carbon/human/target, var/obj/item/fruit)
	...
	if(chems && chems.len && target.reagents)
		...
		if(affecting)
			to_chat(target, "<span class='danger'>I am stung by \the [fruit] in my [affecting.name]!</span>")
			for(var/rid in chems)
				var/injecting = min(5,max(1,get_trait(TRAIT_POTENCY)/5))
				target.reagents.add_reagent(rid,injecting)
		else
			to_chat(target, "<span class='danger'>Sharp spines scrape against my armour!</span>")
...
/datum/seed/proc/harvest(var/mob/user,var/yield_mod,var/harvest_sample,var/force_amount)
	...
	if(!force_amount && get_trait(TRAIT_YIELD) == 0 && !harvest_sample)
		if(istype(user)) to_chat(user, "<span class='danger'>I fail to harvest anything useful.</span>")
	else
		if(istype(user)) to_chat(user, "I [harvest_sample ? "take a sample" : "harvest"] from the [display_name].")
