	...
	if(buckled_mob)
		...
		if(prob(chance) && (buckled_mob != user))
			buckled_mob.visible_message(\
				...
				"<span class='notice'>\The [user] frees I from \the [src].</span>",\
			...
			user.visible_message(
				...
				"<span class='warning'>I attempt to get free from [src].</span>")
			...
			if(do_after(user, breakouttime, incapacitation_flags = INCAPACITATION_DEFAULT & ~INCAPACITATION_RESTRAINED))
				...
				user.visible_message(
					...
					"<span class='warning'>I successfully escape [src]!</span>")
		...
		if(istype(victim, /mob/living/carbon/human))
			if(((istype(H.shoes, /obj/item/clothing/shoes/magboots) && (H.shoes.item_flags & ITEM_FLAG_NOSLIP)) || (H.species.species_flags & SPECIES_FLAG_NO_SLIP)) && victim.loc != src.loc)
				if(prob(90))
					...
					can_grab = 0
		if(can_grab)
			victim.visible_message("<span class='danger'>Tendrils lash out from \the [src] and drag \the [victim] in!</span>", "<span class='danger'>Tendrils lash out from \the [src] and drag I in!</span>")
			...
			if(victim.loc == src.loc)
				...
				to_chat(victim, "<span class='danger'>The tendrils [pick("wind", "tangle", "tighten", "coil", "knot", "snag", "twist", "constrict", "squeeze", "clench", "tense")] around me!</span>")
			Process(0)
