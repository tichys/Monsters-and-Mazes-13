...
/obj/machinery/portable_atmospherics/hydroponics/attack_ghost(var/mob/observer/ghost/user)
	...
	var/response = alert(user, "Are I sure I want to harvest this [seed.display_name]?", "Living plant request", "Yes", "No")
	if(response == "Yes")
		harvest()
...
/obj/machinery/portable_atmospherics/hydroponics/proc/harvest(var/mob/user)
	...
	if(closed_system)
		if(user) to_chat(user, "I can't harvest from the plant while the lid is shut.")
		return FALSE
...
/obj/machinery/portable_atmospherics/hydroponics/proc/remove_dead(var/mob/user, var/silent)
	...
	if(!silent) to_chat(user, "I remove the dead plant.")
	lastproduce = 0
	check_health()
	return TRUE
...
/obj/machinery/portable_atmospherics/hydroponics/verb/setlight()
	...
	if(ishuman(usr) || istype(usr, /mob/living/silicon/robot))
		...
		if(new_light)
			...
			to_chat(usr, "I set the tray to a light level of [tray_light] lumens.")
	return
...
/obj/machinery/portable_atmospherics/hydroponics/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(isWirecutter(O) || istype(O, /obj/item/weapon/scalpel))
		...
		if(weedlevel > 0)
			user.visible_message("<span class='notice'>[user] starts uprooting the weeds.</span>", "<span class='notice'>I remove the weeds from the [src].</span>")
			weedlevel = 0
			if(seed)
		...
		anchored = !anchored
		to_chat(user, "I [anchored ? "wrench" : "unwrench"] \the [src].")
...
/obj/machinery/portable_atmospherics/hydroponics/proc/plant_seed(var/mob/user, var/obj/item/seeds/S)
	...
	if(!S.seed)
		to_chat(user, "The packet seems to be empty. I throw it away.")
		qdel(S)
		return
...
/obj/machinery/portable_atmospherics/hydroponics/proc/close_lid(var/mob/living/user)
	...
	to_chat(user, "I [closed_system ? "close" : "open"] the tray's lid.")
	update_icon()
