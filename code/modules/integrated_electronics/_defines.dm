...
/obj/item/integrated_circuit/verb/rename_component()
	...
	var/mob/M = usr
	var/input = sanitizeSafe(input("What do I want to name the circuit?", "Rename", src.name) as null|text, MAX_NAME_LEN)
...
/obj/item/integrated_circuit/Topic(href, href_list, state = GLOB.physical_state)
	...
	if(href_list["wire"] && is_in_open_assembly())
		if(istype(held_item, /obj/item/device/integrated_electronics/wirer))
			...
			to_chat(usr, "<span class='warning'>I can't do a whole lot without the proper tools.</span>")
		. = 1
		...
		if(istype(held_item, /obj/item/weapon/screwdriver))
			...
			to_chat(usr, "<span class='notice'>I pop \the [src] out of the case, and slide it out.</span>")
		else
			to_chat(usr, "<span class='warning'>I need a screwdriver to remove components.</span>")
