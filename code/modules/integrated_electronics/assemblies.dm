...
/obj/item/device/electronic_assembly/verb/rename()
	...
	set category = "Object"
	set desc = "Rename my circuit, useful to stay organized."

	var/mob/M = usr
	if(!CanInteract(M, GLOB.physical_state))
		return

	var/input = sanitizeSafe(input("What do I want to name this?", "Rename", src.name) as null|text, MAX_NAME_LEN)
...
/obj/item/device/electronic_assembly/attackby(var/obj/item/I, var/mob/user)
	if(istype(I, /obj/item/integrated_circuit))
		...
		if(add_circuit(I, user))
			to_chat(user, "<span class='notice'>I slide \the [I] inside \the [src].</span>")
		...
		if(applied_shell)
			to_chat(user, "<span class='warning'>I cannot open the assembly while it has a shell attached.</span>")
			return 0
		playsound(get_turf(src), 'sound/items/Crowbar.ogg', 50, 1)
		opened = !opened
		to_chat(user, "<span class='notice'>I [opened ? "opened" : "closed"] \the [src].</span>")
		update_icon()
	else if(istype(I, /obj/item/electronic_assembly_shell))
		if(opened)
			to_chat(user, "<span class='warning'>I cannot attach a shell while the assembly is open.</span>")
			return 0
		if(!user.unEquip(I))
			return 0
		...
		if(opened)
			...
			to_chat(user, "<span class='warning'>\The [src] isn't opened, so I can't fiddle with the internal components.  \
...
/obj/item/device/electronic_assembly/proc/add_circuit(var/obj/item/integrated_circuit/IC, var/mob/user)
	if(!opened)
		to_chat(user, "<span class='warning'>\The [src] isn't opened, so I can't put anything inside.  Try using a crowbar.</span>")
		return FALSE
	...
	if((total_part_size + IC.size) > max_components)
		to_chat(user, "<span class='warning'>I can't seem to add the '[IC.name]', as there's insufficient space.</span>")
		return FALSE
	if((total_complexity + IC.complexity) > max_complexity)
		to_chat(user, "<span class='warning'>I can't seem to add the '[IC.name]', since this setup's too complicated for the case.</span>")
		return FALSE
