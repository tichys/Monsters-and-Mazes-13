...
/obj/item/integrated_circuit/input/button/OnICTopic(href_list, user)
	if(href_list["press"])
		to_chat(user, "<span class='notice'>I press the button labeled '[src.name]'.</span>")
		activate_pin(1)
...
/obj/item/integrated_circuit/input/toggle_button/OnICTopic(href_list, user)
	if(href_list["toggle"])
		...
		activate_pin(1)
		to_chat(user, "<span class='notice'>I toggle the button labeled '[src.name]' [get_pin_data(IC_OUTPUT, 1) ? "on" : "off"].</span>")
...
/obj/item/integrated_circuit/input/adjacent_locator
	...
	extended_desc = "The first pin requires a ref to a kind of object that I want the locator to acquire.  This means that it will \
...
/obj/item/integrated_circuit/input/access_scanner/emag_act(var/remaining_charges, var/mob/user)
	if(!emagged && remaining_charges > 0)
		emagged = TRUE
		to_chat(user, "<span class='warning'>I scramble the board's access protection logic.</span>")
		return 1
	return ..()
...
/obj/item/integrated_circuit/input/access_scanner/attackby(var/obj/item/weapon/card/id/id_card, var/mob/user)
	...
	if(user.unEquip(id_card))
		...
		scanned_access.data = json_encode(contained_id.GetAccess())
		user.visible_message("<span class='notice'>\The [user] installs an id card into the board.</span>", "<span class='notice'>I install the id card into the board.</span>")

/obj/item/integrated_circuit/input/access_scanner/attack_self(var/mob/user)
	if(contained_id)
		user.visible_message("<span class='notice'>\The [user] removes an id card from the board.</span>", "<span class='notice'>I remove the id card from the board.</span>")
		contained_id.dropInto(loc)
