...
/obj/machinery/integrated_circuit_printer/attackby(var/obj/item/O, var/mob/user)
	if(istype(O,/obj/item/stack/material))
		...
		if(stack.material.name == DEFAULT_WALL_MATERIAL)
			...
			if(do_after(usr, 16, src))
				if(stack.use(num))
					to_chat(user, "<span class='notice'>I add [num] sheet\s to \the [src].</span>")
					metal += num
					updateUsrDialog()
					return 1
	...
	if(istype(O,/obj/item/integrated_circuit))
		to_chat(user, "<span class='notice'>I insert the circuit into [src]. </span>")
		qdel(O)
...
/obj/machinery/integrated_circuit_printer/Topic(href, href_list)
	...
	if(href_list["mode"])
		...
		if(metal - cost < 0)
			to_chat(usr, "<span class='warning'>I need [cost] metal to build that!.</span>")
			return 1
		metal -= cost
