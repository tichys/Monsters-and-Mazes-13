...
/obj/item/integrated_circuit/manipulation/injector/do_work()
	...
	if(AM.can_be_injected_by(assembly))
		if(isliving(AM))
			...
			to_chat(AM, "<span class='notice'>I feel a tiny prick!</span>")
...
/obj/item/integrated_circuit/manipulation/grenade/attackby(var/obj/item/weapon/grenade/G, var/mob/user)
	if(istype(G))
		if(attached_grenade)
			...
			user.visible_message("<span class='warning'>\The [user] attaches \a [G] to \the [src]!</span>", "<span class='notice'>I attach \the [G] to \the [src].</span>")
			attach_grenade(G)
	else
		..()
...
/obj/item/integrated_circuit/manipulation/grenade/attack_self(var/mob/user)
	if(attached_grenade)
		user.visible_message("<span class='warning'>\The [user] removes \an [attached_grenade] from \the [src]!</span>", "<span class='notice'>I remove \the [attached_grenade] from \the [src].</span>")
...
/obj/item/integrated_circuit/manipulation/ai/verb/open_menu()
	...
	set desc = "With this I can press buttons on the assembly I am attached to."
	set category = "Object"
	set src = usr.loc
