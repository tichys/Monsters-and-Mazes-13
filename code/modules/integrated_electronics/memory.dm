	...
	switch(type_to_use)
		if("string")
			...
			if(istext(new_data) && CanInteract(user, GLOB.physical_state))
				...
				to_chat(user, "<span class='notice'>I set \the [src]'s memory to [O.display_data()].</span>")
		if("number")
			accepting_refs = 0
...
/obj/item/integrated_circuit/memory/constant/afterattack(atom/target, mob/living/user, proximity)
	if(accepting_refs && proximity)
		...
		to_chat(user, "<span class='notice'>I set \the [src]'s memory to a reference to [O.display_data()].  The ref scanner is \
		now off.</span>")
		accepting_refs = 0
