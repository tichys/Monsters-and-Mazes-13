...
/obj/item/device/integrated_electronics/wirer/proc/wire(var/datum/integrated_io/io, mob/user)
	if(mode == WIRE)
		selected_io = io
		to_chat(user, "<span class='notice'>I attach a data wire to \the [selected_io.holder]'s [selected_io.name] data channel.</span>")
		mode = WIRING
		update_icon()
	else if(mode == WIRING)
		if(selected_io && selected_io.link_io(io, user))
			to_chat(user, "<span class='notice'>I connect \the [selected_io.holder]'s [selected_io.name] to \the [io.holder]'s [io.name].</span>")
			mode = WIRE
			update_icon()
		...
		if(!io.linked.len)
			...
			selected_io = null
			return
		to_chat(user, "<span class='notice'>I prepare to detach a data wire from \the [selected_io.holder]'s [selected_io.name] data channel.</span>")
		mode = UNWIRING
		update_icon()
		return
		...
		if(io == selected_io)
			to_chat(user, "<span class='warning'>I can't wire a pin into each other, so unwiring \the [selected_io.holder] from \
			the same pin is rather moot.</span>")
			return
		if(selected_io in io.linked)
			io.linked.Remove(selected_io)
			selected_io.linked.Remove(io)
			to_chat(user, "<span class='notice'>I disconnect \the [selected_io.holder]'s [selected_io.name] from \
...
/obj/item/device/integrated_electronics/wirer/attack_self(mob/user)
	switch(mode)
		...
		if(WIRING)
			if(selected_io)
				to_chat(user, "<span class='notice'>I decide not to wire the data channel.</span>")
			selected_io = null
			mode = WIRE
		if(UNWIRE)
			mode = WIRE
		if(UNWIRING)
			if(selected_io)
				to_chat(user, "<span class='notice'>I decide not to disconnect the data channel.</span>")
			selected_io = null
			mode = UNWIRE
	update_icon()
	to_chat(user, "<span class='notice'>I set \the [src] to [mode].</span>")
#undef WIRE
#undef WIRING
#undef UNWIRE
#undef UNWIRING
...
/obj/item/device/integrated_electronics/debugger
	...
	obj_flags = OBJ_FLAG_CONDUCTIBLE
	w_class = ITEM_SIZE_SMALL
	description_info = "Ref scanning is done by click-drag-dropping the debugger unto an adjacent object that I wish to scan."
	...
	switch(type_to_use)
		if("string")
			...
			if(istext(new_data) && CanInteract(user, GLOB.physical_state))
				...
				to_chat(user, "<span class='notice'>I set \the [src]'s memory to \"[new_data]\".</span>")
		if("number")
			accepting_refs = 0
		...
		if("ref")
			accepting_refs = 1
			to_chat(user, "<span class='notice'>I turn \the [src]'s ref scanner on.  Slide it across \
			an object for a ref of that object to save it in memory.</span>")
		if("null")
			data_to_write = null
			to_chat(user, "<span class='notice'>I set \the [src]'s memory to absolutely nothing.</span>")
...
/obj/item/device/integrated_electronics/debugger/MouseDrop(var/atom/over_object)
	...
	to_chat(usr, "<span class='notice'>I set \the [src]'s memory to a reference to \the [over_object.name]. The ref scanner is now off.</span>")
	accepting_refs = 0
...
/obj/item/device/integrated_electronics/debugger/proc/write_data(var/datum/integrated_io/io, mob/user)
	if(io.io_type == DATA_CHANNEL)
		...
		if(isweakref(data_to_write))
			...
			data_to_show = A.name
		to_chat(user, "<span class='notice'>I write '[data_to_show ? data_to_show : "NULL"]' to the '[io]' pin of \the [io.holder].</span>")
	else if(io.io_type == PULSE_CHANNEL)
		io.holder.check_then_do_work(io)
		to_chat(user, "<span class='notice'>I pulse \the [io.holder]'s [io].</span>")
...
/obj/item/device/integrated_electronics/analyzer/afterattack(var/obj/item/device/electronic_assembly/assembly, var/mob/user)
	...
	user.visible_message("<span class='notify'>\The [user] begins to scan \the [assembly].</span>", "<span class='notify'>I begin to scan \the [assembly].</span>")
