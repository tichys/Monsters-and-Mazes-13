...
/obj/item/weapon/book/manual/hydroponics_pod_people
	...
	dat = {"<html>
				...
				<p>
				<ol>
					<li>Take a syringe of blood from the body I wish to turn into a Diona.</li>
...
/obj/item/weapon/book/manual/research_and_development
	...
	dat = {"<html>
				...
				So I want to further SCIENCE? Good man/woman/thing! However, SCIENCE is a complicated process even though it's quite easy. For the most part, it's a three step process:
				<ol>
				...
				Those are the basic steps to furthering science. What do I do science with, however? Well, I have four major tools: R&D Console, the Destructive Analyzer, the Protolathe, and the Circuit Imprinter.

				<h2>The R&D Console</h2>
				The R&D console is the cornerstone of any research lab. It is the central system from which the Destructive Analyzer, Protolathe, and Circuit Imprinter (my R&D systems) are controlled. More on those systems in their own sections.
				On its own, the R&D console acts as a database for all my technological gains and new devices I discover. So long as the R&D console remains intact, you'll retain all that SCIENCE you've discovered. Protect it though,
				because if it gets damaged, you'll lose my data!
				In addition to this important purpose, the R&D console has a disk menu that lets I transfer data from the database onto disk or from the disk into the database.
				It also has a settings menu that lets I re-sync with nearby R&D devices (if they've become disconnected), lock the console from the unworthy,
				...
				This is the source of all technology. Whenever I put a handheld object in it, it analyzes it and determines what sort of technological advancements I can discover from it. If the technology of the object is equal or higher then my current knowledge,
				I can destroy the object to further those sciences.
				Some devices (notably, some devices made from the protolathe and circuit imprinter) aren't 100% reliable when I first discover them. If these devices break down, I can put them into the Destructive Analyzer and improve their reliability rather than further science.
				...
				The downside is that, again, not all devices I make are 100% reliable when I first discover them.
				...
				and any advancements you've made with the device's source technologies. To be able to improve the reliability of a device, I have to use the device until it breaks beyond repair. Once that happens, I can analyze it in a Destructive Analyzer.
				...
				Many machines produced from circuit boards inserted into a machine frames require a variety of parts to construct. These are parts like capacitors, batteries, matter bins, and so forth. As my knowledge of science improves, more advanced versions are unlocked.
				If I use these parts when constructing something, its attributes may be improved.
				For example, if I use an advanced matter bin when constructing an autolathe (rather than a regular one), it'll hold more materials. Experiment around with stock parts of various qualities to see how they affect the end results! Be warned, however:
...
/obj/item/weapon/book/manual/excavation
	...
	dat = {"<html>
				...
				and get it to the site the first time. I learn quick that time is money, when you've got a shipful of bandits searching for I the next valley over,
				...
				<ul>
					...
					<li><b>Measuring tape</b> - don't leave home without it, I can use it to measure the depth a rock face has been excavated to.</li>
					<li><b>GPS locator</b> - knowing where I am is the first step to not be lost.</li>
					<li><b>Core sampler</b> - use this to take core samples from rock faces, which I can then run to the lab for analysis.</li>
					...
					<li><b>Alden-Saraspova counter</b> - uses a patented application of Fourier Transform analysis to determine the difference between background and
						exotic radiation. Use it to determine how far I am from anomalous energy sources.</li>
					...
					the best kind of protection I can hope for from the errors our forebears may have unleashed.</li>
				...
				<ul>
					...
					<li><b>Check the obvious areas first</b> - even if you're pressed for time, these ones are the generally easiest to search, and the most likely targets
						of my rivals.</li>
					<li><b>Do some prospecting</b> - the earth mother isn't in the habit of displaying her secrets to the world (although sometimes I get lucky).
						Drop a shaft and clear away a bit of surface rock here and there, I never know what might be lurking below the surface.</li>
					<li><b>Tips on unearthing a deposit</b> - How do I know when you're golden? Look for telltale white strata that looks strange or out of place, or if
						something has broken under my pick while you're digging. My depth scanner is my best friend, but even it can't distinguish between
				...
				I have found some unusual strata, but it's not all peaches from here. No archaeologist ever managed to pull a bone from the earth without doing thorough
				...
				<ul>
					...
					<li><b>Don't be afraid to be cautious</b> - It's slower sometimes, but the extra time will be worth the payoff when I find an Exolitic relic.</li>
				...
				<ul>
					...
					<li><b>Choose stasis field</b> - Chemical analysis on a core sample from the rock face will tell I which field is necessary to extract the find safely.</li>
					<li><b>Setup field gen</b> - Bolt it down, choose the field, check the charge, and activate it. If I forget it, you'll wish I hadn't when that priceless
...
/obj/item/weapon/book/manual/mass_spectrometry
	...
	dat = {"<html>
				...
				Modern mass spectrometry requires constant attention from the diligent researcher in order to be successful. There are many different elements to juggle,
					and later chapters will delve into them. For the spectrometry assistant, the first thing I need to know is that the scanner wavelength is automatically
...
/obj/item/weapon/book/manual/materials_chemistry_analysis
	...
	dat = {"<html>
				...
				a new element that defies classification. I defy I to classify it without reviving this once great field!
			"}
