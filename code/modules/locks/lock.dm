...
/datum/lock/proc/pick_lock(var/obj/item/I, var/mob/user)
	...
	if(prob(20*(unlock_power/getComplexity())))
		to_chat(user, "<span class='notice'>I pick open \the [holder]'s lock!</span>")
		unlock(lock_data)
		return 1
	else if(prob(5 * unlock_power))
		to_chat(user, "<span class='warning'>I accidently break \the [holder]'s lock with my [I]!</span>")
		status |= LOCK_BROKEN
	else
		to_chat(user, "<span class='warning'>I fail to pick open \the [holder].</span>")
	return 0