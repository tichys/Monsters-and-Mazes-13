...
/obj/item/weapon/material/lock_construct/attackby(var/obj/item/I, var/mob/user)
	if(istype(I,/obj/item/weapon/key))
		...
		if(!K.key_data)
			to_chat(user, "<span class='notice'>I fashion \the [I] to unlock \the [src]</span>")
	...
	if(istype(I,/obj/item/weapon/material/lock_construct))
		...
		to_chat(user, "<span class='notice'>I copy the lock from \the [L] to \the [src], making them identical.</span>")
		return
	..()
