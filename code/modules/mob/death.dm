...
/mob/proc/death(gibbed,deathmessage="seizes up and falls limp...", show_dead_message = "I have died.")
	...
	if(healths)
		...
		if("health7" in icon_states(healths.icon))
			...
			log_debug("[src] ([src.type]) died but does not have a valid health7 icon_state (using health6 instead). report this error to Ccomp5950 or my nearest Developer")
	...
	if(config.respawn_delay && config.abandon_allowed)
		to_chat(src, "<span class='deadsay'>I may respawn in [config.respawn_delay] minutes.</span>")
	...
	if(client)  // the following is ported from r4407
		...
		for(var/mob/living/carbon/human/H in GLOB.player_list)
			if(!H.stat)
				...
				break
		if(!cancel)
			world.auto_restart_world("Everyone has died.")
	return 1
