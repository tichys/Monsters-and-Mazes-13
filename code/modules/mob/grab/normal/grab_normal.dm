...
/datum/grab/normal/on_hit_harm(var/obj/item/grab/G)
	...
	if(!O.dislocated)
		...
		to_chat(assailant, "<span class='warning'>I can't dislocate [affecting]'s [O.joint]!</span>")
		return 0
...
/datum/grab/normal/proc/attack_eye(var/obj/item/grab/G)
	...
	for(var/obj/item/protection in list(target.head, target.wear_mask, target.glasses))
		if(protection && (protection.body_parts_covered & EYES))
			to_chat(attacker, "<span class='danger'>I'm going to need to remove the eye covering first.</span>")
			return
	if(!target.has_eyes())
		to_chat(attacker, "<span class='danger'>I cannot locate any eyes on [target]!</span>")
		return
	...
	switch(G.last_target)
		if(BP_MOUTH)
			if(!G.affecting.check_has_mouth())
				to_chat(G.assailant, "<span class='danger'>I cannot locate a mouth on [G.affecting]!</span>")
				return 0
		if(BP_EYES)
			if(!G.affecting.has_eyes())
				to_chat(G.assailant, "<span class='danger'>I cannot locate any eyes on [G.affecting]!</span>")
				return 0
	return 1
