...
/mob/proc/hear_say(var/message, var/verb = "says", var/datum/language/language = null, var/alt_name = "",var/italics = 0, var/mob/speaker = null, var/sound/speech_sound, var/sound_vol)
	...
	//non-verbal languages are garbled if I can't see the speaker. Yes, this includes if they are inside a closet.
	...
	if(is_deaf())
		if(!language || !(language.flags & INNATE)) // INNATE is the flag for audible-emote-language, so we don't want to show an "x talks but I cannot hear them" message if it's set
			if(speaker == src)
				to_chat(src, "<span class='warning'>I cannot hear myself speak!</span>")
			else if(!is_blind())
				to_chat(src, "<span class='name'>[speaker_name]</span>[alt_name] talks but I cannot hear \him.")
	else
		if(language)
			var/nverb = null
...
/mob/proc/hear_radio(var/message, var/verb="says", var/datum/language/language=null, var/part_a, var/part_b, var/part_c, var/mob/speaker = null, var/hard_to_hear = 0, var/vname ="")
	...
	if(last_radio_sound + 0.5 SECOND > world.time)
		playsound(loc, pick('sound/effects/radio1.ogg','sound/effects/radio2.ogg','sound/effects/radio3.ogg','sound/effects/radio4.ogg'), 10, 0, -1, falloff = -3)
	...
	if(sdisabilities & DEAF || ear_deaf)
		...
		if(istype(H) && H.has_headset_in_ears() && prob(20))
			to_chat(src, "<span class='warning'>I feel my headset vibrate but can hear nothing from it!</span>")
	else
...
/mob/proc/hear_sleep(var/message)
	...
	if(prob(15))
		...
		heard = "<span class = 'game_say'>...I hear something about...[heardword]</span>"

	else
		heard = "<span class = 'game_say'>...<i>I almost hear someone talking</i>...</span>"
