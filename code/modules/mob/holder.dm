...
/mob/living/proc/get_scooped(var/mob/living/carbon/human/grabber, var/self_grab)
	...
	if(self_grab)
		if(!grabber.equip_to_slot_if_possible(H, slot_back, del_on_fail=0, disable_warning=1))
			to_chat(src, "<span class='warning'>I can't climb onto [grabber]!</span>")
			return

		to_chat(grabber, "<span class='notice'>\The [src] clambers onto me!</span>")
		to_chat(src, "<span class='notice'>I climb up onto \the [grabber]!</span>")
	else
		if(!grabber.put_in_hands(H))
			to_chat(grabber, "<span class='warning'>My hands are full!</span>")
			return

		to_chat(grabber, "<span class='notice'>I scoop up \the [src]!</span>")
		to_chat(src, "<span class='notice'>\The [grabber] scoops I up!</span>")
