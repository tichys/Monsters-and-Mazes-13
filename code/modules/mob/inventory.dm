...
//set disable_warning to disable the 'I am unable to equip that' warning.
...
/mob/proc/equip_to_slot_if_possible(obj/item/W as obj, slot, del_on_fail = 0, disable_warning = 0, redraw_mob = 1, force = 0)
	...
	if(!W.mob_can_equip(src, slot, disable_warning, force))
		if(del_on_fail)
			...
			if(!disable_warning)
				to_chat(src, "<span class='warning'>I am unable to equip that.</span>")//Only print if del_on_fail is false
...
//This is an UNSAFE proc. It merely handles the actual job of equipping. All the checks on whether I can or can't eqip need to be done before! Use mob_can_equip() for that task.
//In most cases I will want to use equip_to_slot_if_possible()
...
//These procs handle putting s tuff in my hand. It's probably best to use these rather than setting l_hand = ...etc
...
//Puts the item into my l_hand if possible and calls all necessary triggers/updates. returns 1 on success.
/mob/proc/put_in_l_hand(var/obj/item/W)
	if(lying || !istype(W))
		return 0
	return 1

//Puts the item into my r_hand if possible and calls all necessary triggers/updates. returns 1 on success.
...
//This is probably the main one I need to know :)
...
/*
	...
	It's probably okay to use it if I am transferring the item between slots on the same mob,
...
//This differs from remove_from_mob() in that it checks if the item can be unequipped first. Use drop_from_inventory if I don't want to check.
