...
/mob/living/bot/attackby(var/obj/item/O, var/mob/user)
	if(O.GetIdCard())
		...
		if(!locked)
			...
			to_chat(user, "<span class='notice'>I need to unlock the controls first.</span>")
		return
	else if(isWelder(O))
		if(health < maxHealth)
			if(open)
				health = min(maxHealth, health + 10)
				user.visible_message("<span class='notice'>\The [user] repairs \the [src].</span>","<span class='notice'>I repair \the [src].</span>")
			else
