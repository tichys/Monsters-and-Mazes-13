...
/mob/living/bot/farmbot/emag_act(var/remaining_charges, var/mob/user)
	...
	if(!emagged)
		if(user)
			to_chat(user, "<span class='notice'>I short out [src]'s plant identifier circuits.</span>")
			ignore_list |= user
		emagged = 2
		return 1
...
/obj/item/weapon/farmbot_arm_assembly/attack_hand(mob/user as mob)
	return //it's a converted watertank, no I cannot pick it up and put it in my backpack
