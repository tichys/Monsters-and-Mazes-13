	...
	for(var/mob/living/carbon/human/H in view(7, src)) // Time to find a patient!
		if(confirmTarget(H))
			...
			if(last_newpatient_speak + 300 < world.time)
				var/message = pick("Hey, [H.name]! Hold on, I'm coming.", "Wait [H.name]! I want to help!", "[H.name], I appear to be injured!")
				say(message)
...
/mob/living/bot/medbot/attackby(var/obj/item/O, var/mob/user)
	if(istype(O, /obj/item/weapon/reagent_containers/glass))
		...
		if(!user.unEquip(O, src))
			return
		reagent_glass = O
		to_chat(user, "<span class='notice'>I insert [O].</span>")
		return
	else
		..()
			...
			if("eject")
				if(reagent_glass)
					if(!locked)
						...
						to_chat(user, "<span class='notice'>I cannot eject the beaker because the panel is locked.</span>")
			if("togglevoice")
				if(!locked || issilicon(user))
					vocal = !vocal
			if("declaretreatment")
