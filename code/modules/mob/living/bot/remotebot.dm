...
/mob/living/bot/remotebot/attackby(var/obj/item/I, var/mob/living/user)
	if(istype(I, /obj/item/device/bot_controller) && !controller)
		...
		to_chat(user, "<span class='notice'>I link \the [src] to \the [I].</span>")
...
/obj/item/device/bot_kit
	name = "Remote-Bot Kit"
	desc = "The cover says 'control my own cardboard nuclear powered robot. Comes with real plutonium!"
	icon = 'icons/obj/storage.dmi'
	icon_state = "remotebot"

/obj/item/device/bot_kit/attack_self(var/mob/living/user)
	to_chat(user, "I quickly dismantle the box and retrieve the controller and the remote bot itself.")
