...
/mob/living/bot/secbot/emag_act(var/remaining_charges, var/mob/user)
	...
	if(!emagged)
		if(user)
			to_chat(user, "<span class='notice'>I short out [src]'s threat identificator.</span>")
			ignore_list |= user
		emagged = 2
		return 1
...
/mob/living/bot/secbot/proc/begin_arrest(mob/target, var/threat)
	...
	say("Down on the floor, [suspect_name]! I have [SECBOT_WAIT_TIME] seconds to comply.")
...
/mob/living/bot/secbot/startPatrol()
	if(!locked) // Stop running away when we set I up
		return
	..()
...
/obj/item/weapon/secbot_assembly/attackby(var/obj/item/O, var/mob/user)
	...
	if(isWelder(O) && !build_step)
		...
		if(WT.remove_fuel(0, user))
			...
			to_chat(user, "I weld a hole in \the [src].")

	else if(isprox(O) && (build_step == 1))
		build_step = 2
		to_chat(user, "I add \the [O] to [src].")
