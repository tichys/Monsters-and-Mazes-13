...
/mob/living/carbon/alien/diona/proc/handle_tray_interaction(var/obj/machinery/portable_atmospherics/hydroponics/tray)
	...
	if(tray.dead)
		if(tray.remove_dead(src, silent = TRUE))
			...
			visible_message("<span class='notice'><b>\The [src]</b> chews up the dead plant, clearing \the [tray] out.</span>", "<span class='notice'>I devour the dead plant, clearing \the [tray].</span>")
		return

	if(tray.harvest)
		if(tray.harvest(src))
			visible_message("<span class='notice'><b>\The [src]</b> harvests from \the [tray].</span>", "<span class='notice'>I harvest the contents of \the [tray].</span>")
		return
	...
	if(tray.weedlevel || tray.pestlevel)
		...
		tray.weedlevel = 0
		tray.pestlevel = 0
		visible_message("<span class='notice'><b>\The [src]</b> begins rooting through \the [tray], ripping out pests and weeds, and eating them noisily.</span>","<span class='notice'>I begin rooting through \the [tray], ripping out pests and weeds, and eating them noisily.</span>")
		return

	if(tray.nutrilevel < 10)
		var/nutrition_cost = (10-tray.nutrilevel) * 5
		if(nutrition >= nutrition_cost)
			visible_message("<span class='notice'><b>\The [src]</b> secretes a trickle of green liquid, refilling [tray].</span>","<span class='notice'>I secrete some nutrients into \the [tray].</span>")
			tray.nutrilevel = 10
			nutrition -= (10-tray.nutrilevel) * 5
		else
			to_chat(src, "<span class='notice'>I haven't eaten enough to refill \the [tray]'s nutrients.</span>")
		return

	if(tray.waterlevel < 100)
		var/nutrition_cost = Floor((100-tray.nutrilevel)/10) * 5
		if(nutrition >= nutrition_cost)
			visible_message("<span class='notice'><b>\The [src]</b> secretes a trickle of clear liquid, refilling [tray].</span>","<span class='notice'>I secrete some water into \the [tray].</span>")
			tray.waterlevel = 100
		else
			to_chat(src, "<span class='notice'>I haven't eaten enough to refill \the [tray]'s water.</span>")
		return

	visible_message("<span class='notice'><b>\The [src]</b> rolls around in \the [tray] for a bit.</span>","<span class='notice'>I roll around in \the [tray] for a bit.</span>")
