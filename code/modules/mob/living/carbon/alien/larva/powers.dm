/mob/living/carbon/alien/larva/proc/check_can_infest(var/mob/living/M)
	if(!src)
		return 0
	if(!istype(loc, /turf))
		to_chat(src, "<span class='danger'>I cannot infest a target in my current position.</span>")
		return 0
	if(incapacitated())
		to_chat(src, "<span class='danger'>I cannot infest a target in my current state.</span>")
		return 0
	if(!M)
		return 1
	if(!M.lying)
...
/mob/living/carbon/alien/larva/verb/attach_host()
	...
	var/mob/living/carbon/human/H = input(src,"Who do I wish to infest?") as null|anything in choices
	...
	to_chat(src, "<span class='danger'>I burrow deeply into \the [H]'s [E.name]!</span>")
...
/mob/living/carbon/alien/larva/verb/release_host()
	...
	if(!loc || !loc.loc)
		to_chat(src, "I am not inside a host.")
		return
	...
	to_chat(src, "<span class='danger'>I begin writhing my way free of \the [H]'s flesh...</span>")
...
/mob/living/carbon/alien/larva/proc/leave_host()
	...
	if(affected)
		to_chat(src, "<span class='danger'>I crawl out of \the [H]'s [affected.name] and plop to the ground.</span>")
	else
		to_chat(src, "<span class='danger'>I plop to the ground.</span>")
