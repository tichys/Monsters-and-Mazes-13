...
/obj/item/device/mmi/attackby(var/obj/item/O as obj, var/mob/user as mob)
	if(istype(O,/obj/item/organ/internal/brain) && !brainmob) //Time to stick a brain in it --NEO
		...
		if(B.damage >= B.max_damage)
			...
			to_chat(user, "<span class='notice'>This brain is completely useless to me.</span>")
			return
	...
	if((istype(O,/obj/item/weapon/card/id)||istype(O,/obj/item/modular_computer)) && brainmob)
		if(allowed(user))
			locked = !locked
			to_chat(user, "<span class='notice'>I [locked ? "lock" : "unlock"] the brain holder.</span>")
		else
...
/obj/item/device/mmi/attack_self(mob/user as mob)
	if(!brainmob)
		to_chat(user, "<span class='warning'>I upend the MMI, but there's nothing in it.</span>")
	else if(locked)
		to_chat(user, "<span class='warning'>I upend the MMI, but the brain is clamped into place.</span>")
	else
		to_chat(user, "<span class='notice'>I upend the MMI, spilling the brain onto the floor.</span>")
