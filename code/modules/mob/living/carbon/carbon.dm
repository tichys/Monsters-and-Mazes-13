...
/mob/living/carbon/relaymove(var/mob/living/user, direction)
	if((user in src.stomach_contents) && istype(user))
		if(user.last_special <= world.time)
			...
			src.visible_message("<span class='danger'>I hear something rumbling inside [src]'s stomach...</span>")
...
/mob/living/carbon/attack_hand(mob/M as mob)
	...
	if (ishuman(M))
		...
		if(temp && !temp.is_usable())
			to_chat(H, "<span class='warning'>I can't use my [temp.name]</span>")
			return
...
/mob/living/carbon/electrocute_act(var/shock_damage, var/obj/source, var/siemens_coeff = 1.0, var/def_zone = null)
	...
	if (shock_damage > 15)
		...
		src.visible_message(
			...
			"<span class='warning'>I feel a shock course through my body.</span>", \
			"<span class='warning'>I hear a zapping sound.</span>" \
		)
	...
	switch(shock_damage)
		...
		if(26 to 25)
			Weaken(5)
		if(31 to INFINITY)
			Weaken(10) //This should work for now, more is really silly and makes I lay there forever
...
/mob/living/carbon/swap_hand()
	...
	if(hud_used.l_hand_hud_object && hud_used.r_hand_hud_object)
		if(hand)	//This being 1 means the left hand is in use
			hud_used.switch_hand_object.icon_state = "left"
			hud_used.l_hand_hud_object.icon_state = "l_hand_active"
			hud_used.r_hand_hud_object.icon_state = "r_hand_inactive"
		else
			hud_used.switch_hand_object.icon_state = "right"
	...
	if(!is_asystole())
		if (on_fire)
			...
			if (M.on_fire)
				...
				"<span class='warning'>I try to pat out [src]'s flames, but to no avail! Put myself out first!</span>")
			else
				M.visible_message("<span class='warning'>[M] tries to pat out [src]'s flames!</span>",
				"<span class='warning'>I try to pat out [src]'s flames! Hot!</span>")
				if(do_mob(M, src, 15))
					src.fire_stacks -= 0.5
					...
					if (M.on_fire)
						...
						"<span class='danger'>The fire spreads to I as well!</span>")
					else
						...
						if (src.fire_stacks <= 0)
							...
							"<span class='warning'>I successfully pat out [src]'s flames.</span>")
							src.ExtinguishMob()
							src.fire_stacks = 0
		else
			var/t_him = "it"
			...
			if(show_ssd && !client && !teleop)
				...
				"<span class='notice'>I shake [src], but they do not respond... Maybe they have S.S.D?</span>")
...
/mob/living/carbon/verb/mob_sleep()
	...
	if(usr.sleeping)
		...
		return
	if(alert(src,"I sure I want to sleep for a while?","Sleep","Yes","No") == "Yes")
...
/mob/living/carbon/slip(var/slipped_on,stun_duration=8)
	if(buckled)
		return 0
	stop_pulling()
	to_chat(src, "<span class='warning'>I slipped on [slipped_on]!</span>")
