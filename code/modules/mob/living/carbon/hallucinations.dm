...
/datum/hallucination/talking/start()
	...
	for(var/mob/living/talker in oview(holder))
		...
		if(prob(80))
			...
			var/list/phrases = list("[prob(50) ? "Hey, " : ""][pick(names)]!","[prob(50) ? "Hey, " : ""][pick(names)]?","Get out[add]!","Go away[add].","What are I doing[add]?","Where's my ID[add]?")
			if(holder.hallucination_power > 50)
				phrases += list("What did I come here for[add]?","Don't touch me[add].","I'm not getting out of here[add].", "I am a failure, [pick(names)].","Just kill myself already, [pick(names)].","Put on some clothes[add].","Take off my clothes[add].")
...
/datum/hallucination/spiderbabies/start()
	if(istype(holder,/mob/living/carbon/human))
		...
		to_chat(H,"<span class='warning'>I feel something [pick("moving","squirming","skittering")] inside of my [O.name]!</span>")
...
/datum/hallucination/telepahy/start()
	to_chat(holder,"<span class = 'notice'>I expand my mind outwards.</span>")
...
/mob/living/carbon/human/proc/fakeremotesay()
	...
	if(stat)
		to_chat(usr, "<span class = 'warning'>I'm not in any state to use my powers right now!'</span>")
		return

	if(chem_effects[CE_MIND] > 0)
		to_chat(usr, "<span class = 'warning'>Chemicals in my blood prevent I from using my power!'</span>")
	...
	var/msg = sanitize(input(usr, "What do I wish to transmit"))
	show_message("<span class = 'notice'>I project my mind into [target.name]: \"[msg]\"</span>")
	if(!stat && prob(20))
		say(msg)
...
/datum/hallucination/fakeattack/hypo/start()
	to_chat(holder, "<span class='notice'>I feel a tiny prick!</span>")