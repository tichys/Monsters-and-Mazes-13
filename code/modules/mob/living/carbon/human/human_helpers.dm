...
/mob/living/carbon/human/can_eat(var/food, var/feedback = 1)
	...
	if(feedback)
		if(status[1] == HUMAN_EATING_NBP_MOUTH)
			to_chat(src, "Where do I intend to put \the [food]? I don't have a mouth!")
...
/mob/living/carbon/human/can_force_feed(var/feeder, var/food, var/feedback = 1)
	...
	if(feedback)
		if(status[1] == HUMAN_EATING_NBP_MOUTH)
			to_chat(feeder, "Where do I intend to put \the [food]? \The [src] doesn't have a mouth!")
		else if(status[1] == HUMAN_EATING_BLOCKED_MOUTH)
...
/mob/living/carbon/human/proc/sonar_ping()
	set name = "Listen In"
	set desc = "Allows I to listen in to movement and noises around you."
	set category = "IC"

	if(incapacitated())
		to_chat(src, "<span class='warning'>I need to recover before I can use this ability.</span>")
		return
	if(world.time < next_sonar_ping)
		to_chat(src, "<span class='warning'>I need another moment to focus.</span>")
		return
	if(is_deaf() || is_below_sound_pressure(get_turf(src)))
		to_chat(src, "<span class='warning'>I am for all intents and purposes currently deaf!</span>")
		return
	next_sonar_ping += 10 SECONDS
	var/heard_something = FALSE
	to_chat(src, "<span class='notice'>I take a moment to listen in to my environment...</span>")
	...
	if(!heard_something)
		to_chat(src, "<span class='notice'>I hear no movement but my own.</span>")
...
/mob/living/carbon/human/proc/add_cloaking_source(var/datum/cloaking_source)
	...
	if(!has_uncloaked && LAZYLEN(cloaking_sources) == 1)
		update_icons()
		src.visible_message("<span class='warning'>\The [src] seems to disappear before my eyes!</span>", "<span class='notice'>I feel completely invisible.</span>")
		return TRUE
	return FALSE

#define CLOAK_APPEAR_OTHER "<span class='warning'>\The [src] appears from thin air!</span>"
#define CLOAK_APPEAR_SELF "<span class='notice'>I have re-appeared.</span>"
