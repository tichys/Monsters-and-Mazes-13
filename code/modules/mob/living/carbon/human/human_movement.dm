/mob/living/carbon/human/movement_delay()
	...
	if(embedded_flag || (stomach_contents && stomach_contents.len))
		handle_embedded_and_stomach_objects() //Moving with objects stuck in I can cause bad times.
	...
	if(can_feel_pain())
		if(get_shock() >= 10) tally += (get_shock() / 10) //pain shouldn't slow I down if I can't even feel it
	...
	if(aiming && aiming.aiming_at) tally += 5 // Iron sights make I slower, it's a well-known fact.
...
/mob/living/carbon/human/Allow_Spacemove(var/check_drift = 0)
	...
	if(thrust && thrust.on)
		if(prob(skill_fail_chance(SKILL_EVA, 10, SKILL_ADEPT)))
			to_chat(src, "<span class='warning'>I fumble with [thrust] controls!</span>")
...
/mob/living/carbon/human/proc/handle_exertion()
	...
	if(lac_chance && prob(skill_fail_chance(SKILL_HAULING, lac_chance)))
		...
		switch(rand(1,20))
			if(1)
				visible_message("<span class='notice'>\The [src] is sweating heavily!</span>", "<span class='notice'>I am sweating heavily!</span>")
			if(2)
				visible_message("<span class='notice'>\The [src] looks out of breath!</span>", "<span class='notice'>I am out of breath!</span>")
