...
/mob/living/carbon/human/proc/handle_organs()
	...
	for(var/obj/item/organ/external/E in bad_external_organs)
		...
		if(!E.need_process())
			...
			//Moving around with fractured ribs won't do I any good
				if (prob(10) && !stat && can_feel_pain() && chem_effects[CE_PAINKILLER] < 50 && E.is_broken() && E.internal_organs.len)
					custom_pain("Pain jolts through my broken [E.encased ? E.encased : E.name], staggering me!", 50, affecting = E)
					unequip_item(loc)
					Stun(2)
...
/mob/living/carbon/human/proc/handle_stance()
	...
	// Can't fall if nothing pulls I down
	...
	// Canes and crutches help I stand (if the latter is ever added)
	// One cane mitigates a broken leg+foot, or a missing foot.
	// Two canes are needed for a lost leg. If I am missing both legs, canes aren't gonna help you.
...
/mob/living/carbon/human/proc/handle_grasp()
	...
	// I should not be able to pick anything up, but stranger things have happened.
	if(l_hand)
	...
	if(affected)
		switch(affected.body_part)
			if(FOOT_LEFT, FOOT_RIGHT)
				to_chat(src, "<span class='warning'>I lose my footing as my [affected.name] spasms!</span>")
			if(LEG_LEFT, LEG_RIGHT)
				to_chat(src, "<span class='warning'>my [affected.name] buckles from the shock!</span>")
			else
				return
	Weaken(5)
	...
	if(BP_IS_ROBOTIC(affected))
		...
		if(affected.can_feel_pain())
			...
			visible_message(
				...
				null,
				"I hear someone [emote_scream_alt]!"
			)
			custom_pain("The sharp pain in my [affected.name] forces I to drop [thing]!", 30)
		else
