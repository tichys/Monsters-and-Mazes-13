...
/mob/living/carbon/human/handle_regular_hud_updates()
	...
	if(stat < DEAD)
		...
		var/obj/screen/fullscreen/aesthetic = screens["aesthetic-static"]
		if(hurtdamage)
			var/severity = 0
			switch(hurtdamage)
			...
			if(severity >= 5) // disorienting blur
				aesthetic.icon_state = "4 heavy"
				aesthetic.alpha = 255
		else
			aesthetic.icon_state = initial(aesthetic.icon_state)  // uses initial() so we dont have to change these if we change the default icon state
			aesthetic.alpha = initial(aesthetic.alpha)
