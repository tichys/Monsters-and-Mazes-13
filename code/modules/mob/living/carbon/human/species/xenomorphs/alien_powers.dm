...
/mob/living/carbon/human/proc/check_alien_ability(var/cost,var/needs_foundation,var/needs_organ)
	...
	if(!istype(P))
		to_chat(src, "<span class='danger'>My plasma vessel has been removed!</span>")
		return

	if(needs_organ)
		var/obj/item/organ/internal/I = internal_organs_by_name[needs_organ]
		if(!I)
			to_chat(src, "<span class='danger'>my [needs_organ] has been removed!</span>")
			return
		else if(!I.is_usable())
			to_chat(src, "<span class='danger'>my [needs_organ] is too damaged to function!</span>")
			return

	if(P.stored_plasma < cost)
		to_chat(src, "<span class='warning'>I don't have enough phoron stored to do that.</span>")
		return 0
	...
	if(needs_foundation)
		...
		if(T)
			...
			if(!(istype(T,/turf/space)))
				has_foundation = 1
		if(!has_foundation)
			to_chat(src, "<span class='warning'>I need a solid foundation to do that on.</span>")
			return 0
...
/mob/living/carbon/human/proc/lay_egg()
	...
	if(!config.alien_eggs_allowed)
		to_chat(src, "I begin to lay an egg, but hesitate. I suspect it isn't allowed.")
...
/mob/living/carbon/human/proc/neurotoxin(mob/target as mob in oview())
	...
	if(stat || paralysis || stunned || weakened || lying || restrained() || buckled)
		to_chat(src, "I cannot spit neurotoxin in my current state.")
		return

	visible_message("<span class='warning'>[src] spits neurotoxin at [target]!</span>", "<span class='alium'>I spit neurotoxin at [target].</span>")
...
/mob/living/carbon/human/proc/resin() // -- TLE
	...
	var/choice = input("Choose what I wish to shape.","Resin building") as null|anything in list("resin door","resin wall","resin membrane","resin nest") //would do it through typesof but then the player choice would have the type path and we don't want the internal workings to be exposed ICly - Urist
	if(!choice)
		return
	...
	visible_message("<span class='warning'><B>[src] vomits up a thick purple substance and begins to shape it!</B></span>", "<span class='alium'>I shape a [choice].</span>")
	switch(choice)
		if("resin door")
...
mob/living/carbon/human/proc/xeno_infest(mob/living/carbon/human/M as mob in oview())
	...
	to_chat(M, "<span class='danger'>A hideous lump of alien mass strains my ribcage as it settles within!</span>")
...
/mob/living/carbon/human/proc/pry_open(obj/machinery/door/A in filter_list(oview(1), /obj/machinery/door))
	...
	set desc = "Pry open an airlock with my claws."
	set category = "Abilities"
