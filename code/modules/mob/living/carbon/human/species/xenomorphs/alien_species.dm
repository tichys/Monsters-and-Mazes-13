...
/datum/species/xenos/hug(var/mob/living/carbon/human/H,var/mob/living/target)
	H.visible_message("<span class='notice'>[H] caresses [target] with countless prickling, needle-like legs.</span>", \
					"<span class='notice'>I caress [target] with countless prickling, needle-like legs.</span>")
...
/datum/species/xenos/proc/regenerate(var/mob/living/carbon/human/H)
	...
	if (H.getBruteLoss() || H.getFireLoss() || H.getOxyLoss() || H.getToxLoss())
		...
		if (prob(5))
			to_chat(H, "<span class='alium'>I feel a soothing sensation come over you...</span>")
		return 1
	...
	for(var/obj/item/organ/external/E in H.bad_external_organs)
		if (E.status & ORGAN_BROKEN)
			if (prob(mend_prob))
				if (E.mend_fracture())
					to_chat(H, "<span class='alium'>I feel something mend itself inside my [E.name].</span>")
			return 1
