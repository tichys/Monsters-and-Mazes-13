/mob/living/carbon/process_resist()
	...
	if(on_fire && !buckled)
		...
		visible_message(
			...
			"<span class='notice'>I stop, drop, and roll!</span>"
			)
		sleep(30)
		if(fire_stacks <= 0)
			visible_message(
				"<span class='danger'>[src] has successfully extinguished themselves!</span>",
				"<span class='notice'>I extinguish myself.</span>"
				)
			ExtinguishMob()
		return TRUE
...
/mob/living/carbon/proc/escape_handcuffs()
	...
	//A default in case I am somehow handcuffed with something that isn't an obj/item/weapon/handcuffs type
	var/breakouttime = 1200
	var/displaytime = 2 //Minutes to display in the "this will take X minutes."
	//If I am handcuffed with actual handcuffs... Well what do I know, maybe someone will want to handcuff I with toilet paper in the future...
	if(istype(HC))
	...
	visible_message(
		...
		"<span class='warning'>I attempt to remove \the [HC]. (This will take around [displaytime] minutes and I need to stand still)</span>"
		)
	...
	if(do_after(src, breakouttime, incapacitation_flags = INCAPACITATION_DEFAULT & ~INCAPACITATION_RESTRAINED))
		...
		visible_message(
			...
			"<span class='notice'>I successfully remove \the [handcuffed].</span>"
			)
...
/mob/living/carbon/proc/break_handcuffs()
	visible_message(
		...
		"<span class='warning'>I attempt to break my [handcuffed.name]. (This will take around 5 seconds and I need to stand still)</span>"
		)
	...
	if(do_after(src, 5 SECONDS, incapacitation_flags = INCAPACITATION_DEFAULT & ~INCAPACITATION_RESTRAINED))
		...
		visible_message(
			...
			"<span class='warning'>I successfully break my [handcuffed.name].</span>"
			)
...
/mob/living/carbon/escape_buckle()
	...
	if(!restrained())
		...
		visible_message(
			...
			"<span class='warning'>I attempt to unbuckle myself. (This will take around 2 minutes and I need to stand still)</span>"
			)
		...
		if(do_after(usr, 2 MINUTES, incapacitation_flags = INCAPACITATION_DEFAULT & ~(INCAPACITATION_RESTRAINED | INCAPACITATION_BUCKLED_FULLY)))
			...
			visible_message("<span class='danger'>\The [usr] manages to unbuckle themself!</span>",
							"<span class='notice'>I successfully unbuckle myself.</span>")
