...
/mob/living/deity/death()
	...
	if(.)
		for(var/m in minions)
			...
			to_chat(M.current, "<font size='3'><span class='danger'>My connection has been severed! \The [src] is no more!</span></font>")
...
/mob/living/deity/proc/set_form(var/type)
	form = new type(src)
	to_chat(src, "<span class='notice'>I undergo a transformation into my new form!</span>")
	spawn(1)
		SetName(form.name)
		var/newname = sanitize(input(src, "Choose a name for my new form.", "Name change", form.name) as text, MAX_NAME_LEN)
		if(newname)
