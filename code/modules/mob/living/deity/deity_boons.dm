/mob/living/deity/proc/set_boon(var/datum/boon)
	if(current_boon)
		qdel(current_boon)
	current_boon = boon
	to_chat(src,"<span class='notice'>I now have the boon [boon]</span>")
...
/mob/living/deity/proc/grant_boon(var/mob/living/L)
	if(istype(current_boon, /spell) && !grant_spell(L, current_boon))
		...
		if(L.equip_to_appropriate_slot(I))
			origin_text = "on my body"
		else if(L.put_in_any_hand_if_possible(I))
			origin_text = "in my hands"
		else
	...
	to_chat(L, "<span class='cult'>\The [src] grants I a boon of [current_boon]!</span>")
	to_chat(src, "<span class='notice'>I give \the [L] a boon of [current_boon].</span>")
...
/mob/living/deity/proc/grant_spell(var/mob/living/target, var/spell/spell)
	...
	to_chat(target, "<span class='notice'>I feel a surge of power as I learn the art of [current_boon].</span>")
	return 1
