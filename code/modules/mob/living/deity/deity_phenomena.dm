...
/mob/living/deity/proc/silence(var/amount)
	if(!silenced)
		to_chat(src, "<span class='warning'>I have been silenced! My phenomenas are disabled!</span>")
		var/obj/screen/intent/deity/SD = hud_used.action_intent
		SD.color = "#ff0000"
	silenced += amount
	for(var/phenom in phenomenas) //Also make it so that I don't do cooldowns.
...
/mob/living/deity/Life()
	...
	if(. && silenced)
		if(!--silenced)
			to_chat(src, "<span class='notice'>I am no longer silenced.</span>")
			var/obj/screen/intent/deity/SD = hud_used.action_intent
			SD.color = null
