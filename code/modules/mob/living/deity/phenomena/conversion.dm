/datum/phenomena/conversion
	name = "Conversion"
	desc = "Ask a non-follower to convert to my cult. This is completely voluntary. Requires the subject to be close to an altar."
	cost = 20
...
/datum/phenomena/conversion/activate(var/mob/living/L)
	to_chat(src,"<span class='notice'>I give \the [L] a chance to willingly convert. May they choose wisely.</span>")
	var/choice = alert(L, "I feel a weak power enter my mind attempting to convert it.", "Conversion", "Allow Conversion", "Deny Conversion")
	if(choice == "Allow Conversion")
		GLOB.godcult.add_antagonist_mind(L.mind,1, "Servant of [linked]", "I willingly give my mind to it, may it bring I fortune", specific_god=linked)
	else
		to_chat(L, "<span class='warning'>With little difficulty I force the intrusion out of my mind. May it stay that way.</span>")
...
/datum/phenomena/forced_conversion/activate(var/mob/living/L)
	var/obj/structure/deity/altar/A = locate() in get_turf(L)
	A.set_target(L)
	to_chat(linked, "<span class='notice'>I imbue \the [A] with my power, setting forth to force \the [L] to my will.</span>")