		...
		if (istype(AM, /mob/living))
			...
			for(var/mob/living/M in range(tmob, 1))
				if(tmob.pinned.len ||  ((M.pulling == tmob && ( tmob.restrained() && !( M.restrained() ) && M.stat == 0)) || locate(/obj/item/grab, tmob.grabbed_by.len)) )
					if ( !(world.time % 5) )
						to_chat(src, "<span class='warning'>[tmob] is restrained, I cannot push past</span>")
					now_pushing = 0
					return
				if( tmob.pulling == M && ( M.restrained() && !( tmob.restrained() ) && tmob.stat == 0) )
					if ( !(world.time % 5) )
						to_chat(src, "<span class='warning'>[tmob] is restraining [M], I cannot push past</span>")
					now_pushing = 0
					return
...
/mob/living/verb/succumb()
	...
	if ((src.health < src.maxHealth/2)) // Health below half of maxhealth.
		...
		updatehealth()
		to_chat(src, "<span class='notice'>I have given up life and succumbed to death.</span>")
	else
		to_chat(src, "<span class='notice'>It is not my time yet.</span>")
...
/mob/living/proc/escape_inventory(obj/item/weapon/holder/H)
	...
	if(istype(M))
		...
		to_chat(M, "<span class='warning'>\The [H] wriggles out of my grip!</span>")
		to_chat(src, "<span class='warning'>I wriggle out of \the [M]'s grip!</span>")
		...
		to_chat(src, "<span class='warning'>I extricate myself from \the [holster].</span>")
		...
		if(istype(H.loc, /obj/machinery/cooker))
			...
			C.cooking_obj = null
			C.check_cooking_obj()
		to_chat(src, "<span class='warning'>I struggle free of \the [H.loc].</span>")
		H.forceMove(get_turf(H))
...
/mob/living/proc/escape_buckle()
	if(buckled)
		if(buckled.can_buckle)
			...
			to_chat(usr, "<span class='warning'>I can't seem to escape from \the [buckled]!</span>")
			return
...
/mob/living/proc/cannot_use_vents()
	if(mob_size > MOB_SMALL)
		return "I can't fit into that vent."
	return null
...
/mob/living/can_be_possessed_by(var/mob/observer/ghost/possessor)
	...
	if(jobban_isbanned(possessor, "Animal"))
		to_chat(possessor, "<span class='warning'>I am banned from animal roles.</span>")
		return 0
...
/mob/living/proc/do_possession(var/mob/observer/ghost/possessor)
	...
	if(round_is_spooky(6)) // Six or more active cultists.
		to_chat(src, "<span class='notice'>I reach out with tendrils of ectoplasm and invade the mind of \the [src]...</span>")
		to_chat(src, "<b>I have assumed direct control of \the [src].</b>")
		to_chat(src, "<span class='notice'>Due to the spookiness of the round, I have taken control of the poor animal as an invading, possessing spirit - roleplay accordingly.</span>")
		src.universal_speak = 1
		src.universal_understand = 1
		//src.cultify() // Maybe another time.
		return

	to_chat(src, "<b>I am now \the [src]!</b>")
