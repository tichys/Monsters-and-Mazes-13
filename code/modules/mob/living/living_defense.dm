...
/mob/living/bullet_act(var/obj/item/projectile/P, var/def_zone)
	...
	if(!P.nodamage)
		...
		if(damage >= 20 && (P.damage_type == BRUTE || P.damage_type == PAIN)) // stopping power of the bullet
			
			if(def_zone == BP_HEAD && prob(damage-absorb))
				Stun(4)
				Weaken(4)
				visible_message("<span class='danger'>[src] falls under the power of the shot!</span>",
								"<span class='danger'>The power of the shot knocks me over!</span>")
			
			if(def_zone == BP_L_LEG || def_zone == BP_R_LEG) // legshots always drop you
				Stun(6)
				Weaken(6)
				visible_message("<span class='danger'>[src] falls as their leg is shot out!</span>",
								"<span class='danger'>My leg got shot out!</span>")

		if(def_zone == BP_HEAD && damage >= 20 && P.damage_type == BRUTE) // headshot
			var/braindamage = damage*2-absorb
			if(braindamage < 0)		braindamage = 0
			adjustBrainLoss(braindamage)
			if(braindamage >= 50 && prob(80))  // the bullet actually went through
				death(deathmessage="slumps for a moment and then falls limp...")
...
/mob/living/hitby(atom/movable/AM as mob|obj,var/speed = THROWFORCE_SPEED_DIVISOR)//Standardization and logging -Sieve
	...
	if(istype(AM,/obj/))
		...
		if(O.throw_source && momentum >= THROWNOBJ_KNOCKBACK_SPEED)
			...
			visible_message("<span class='warning'>\The [src] staggers under the impact!</span>","<span class='warning'>I stagger under the impact!</span>")
