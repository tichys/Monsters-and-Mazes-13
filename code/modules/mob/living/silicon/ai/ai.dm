...
/mob/living/silicon/ai
	...
	var/hacked_apcs_hidden = 0					// Whether the hacked APCs belonging to this AI am hidden, reduces CPU generation from APCs.
...
/mob/living/silicon/ai/proc/on_mob_init()
	to_chat(src, "<B>I am playing the [station_name()]'s AI. The AI cannot move, but can interact with many objects while viewing them (through cameras).</B>")
	to_chat(src, "<B>To look at other areas, click on myself to get a camera menu.</B>")
	to_chat(src, "<B>While observing through a camera, I can use most (networked) devices which I can see, such as computers, APCs, intercoms, doors, etc.</B>")
	to_chat(src, "To use something, simply click on it.")
	to_chat(src, "Use say [get_language_prefix()]b to speak to my cyborgs through binary. Use say :h to speak from an active holopad.")
	...
	if (GLOB.malf && !(mind in GLOB.malf.current_antagonists))
		show_laws()
		to_chat(src, "<b>These laws may be changed by other players, or by I being the traitor.</b>")
...
/mob/living/silicon/ai/proc/ai_call_shuttle()
	...
	var/confirm = alert("Are I sure I want to evacuate?", "Confirm Evacuation", "Yes", "No")
...
/mob/living/silicon/ai/proc/ai_recall_shuttle()
	...
	var/confirm = alert("Are I sure I want to cancel the evacuation?", "Confirm Cancel", "Yes", "No")
	if(check_unable(AI_CHECK_WIRELESS))
		return
...
/mob/living/silicon/ai/proc/ai_hologram_change()
	...
	var/input
	if(alert("Would I like to select a hologram based on a crew member or switch to unique avatar?",,"Crew Member","Unique")=="Crew Member")
...
/mob/living/silicon/ai/proc/control_integrated_radio()
	...
	set desc = "Allows I to change settings of my radio."
	set category = "Silicon Commands"
...
/mob/living/silicon/ai/proc/toggle_hologram_movement()
	...
	set category = "Silicon Commands"
	set desc = "Toggles hologram movement based on moving with my virtual eye."

	hologram_follow = !hologram_follow
	to_chat(usr, "<span class='info'>My hologram will now [hologram_follow ? "follow" : "no longer follow"] you.</span>")

/mob/living/silicon/ai/proc/check_unable(var/flags = 0, var/feedback = 1)
	if(stat == DEAD)
		if(feedback) to_chat(src, "<span class='warning'>I am dead!</span>")
		return 1

	if(!has_power())
		if(feedback) to_chat(src, "<span class='warning'>I lack power!</span>")
		return 1

	if(self_shutdown)
		if(feedback) to_chat(src, "<span class='warning'>I am offline!</span>")
		return 1
