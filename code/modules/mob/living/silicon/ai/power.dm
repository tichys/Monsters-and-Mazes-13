...
/mob/living/silicon/ai/proc/ai_power_override()
	...
	set desc = "Allows I to enable or disable power override, which lets I function without external power, at the cost of quickly expending my internal battery charge."

	power_override_active = !power_override_active

	if(power_override_active)
		to_chat(src, "I have enabled power override. Should I lose power I will remain normally operational, but my backup capacitor will run out much faster.")
	else
		to_chat(src, "I have disabled power override. Should I lose power I will enter diagnostics and low power mode, which will prolong the time for which I can remain operational.")

// This verb allows the AI to disable or enable the power override mode.
/mob/living/silicon/ai/proc/ai_shutdown()
	set category = "Silicon Commands"
	set name = "Shutdown"
	set desc = "Allows I to shut myself down, sacrificing most functions for considerably reduced power usage."
	...
	var/confirm = alert("Are I sure that I want to shut myself down? I can reboot myself later by using the \"Shutdown\" command again. This will put I into reduced power usage mode, at the cost of losing most functions.", "Confirm Shutdown", "Yes", "No")

	if(confirm == "Yes")
		to_chat(src, "<span class='notice'>Shutting down. Minimal power mode: Enabled. I may reboot myself by using the \"Shutdown\" command again.</span>")
		self_shutdown = 1
		return
