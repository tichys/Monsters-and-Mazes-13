...
/datum/paiController/Topic(href, href_list[])
	...
	if(href_list["new"])
		...
		switch(option)
			if("name")
				t = sanitizeSafe(input("Enter a name for my pAI", "pAI Name", candidate.name) as text, MAX_NAME_LEN)
				if(t)
					candidate.name = t
			if("desc")
				t = input("Enter a description for my pAI", "pAI Description", candidate.description) as message
				if(t)
					candidate.description = sanitize(t)
			if("role")
				t = input("Enter a role for my pAI", "pAI Role", candidate.role) as text
				if(t)
...
/datum/paiController/proc/recruitWindow(var/mob/M as mob, allowSubmit = 1)
	...
	<body>
		...
		<p class="top">Please configure my pAI personality's options. Remember, what I enter here could determine whether or not the user requesting a personality chooses me!</p>
		...
		<table>
			...
			<tr class="d1">
				<td>What I plan to call myself. Suggestions: Any character name that would be suitable for a living character OR an AI.</td>
			</tr>
			<tr class="d0">
			...
			<tr class="d1">
				<td>What sort of pAI I typically play; my mannerisms, my quirks, etc. This can be as sparse or as detailed as I like.</td>
			</tr>
			<tr class="d0">
			...
			<tr class="d1">
				<td>Anything you'd like to address specifically to the player reading this in an OOC manner. \"I prefer more serious RP.\", \"I'm still learning the interface!\", etc. Feel free to leave this blank if I want.</td>
			</tr>
		</table>
		<br>
		<table>
...
/datum/paiController/proc/question(var/client/C)
	spawn(0)
		...
		var/response = alert(C, "[inquirer] is requesting a pAI personality. Would I like to play as a personal AI?", "pAI Request", "Yes", "No", "Never for this round")
