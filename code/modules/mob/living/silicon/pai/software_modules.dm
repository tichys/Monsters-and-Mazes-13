...
/datum/pai_software/directives
	...
	Topic(href, href_list)
		...
		if(href_list["getdna"])
			...
			// Check the carrier
			var/answer = input(M, "[P] is requesting a DNA sample from you. Will I allow it to confirm my identity?", "[P] Check DNA", "No") in list("Yes", "No")
			if(answer == "Yes")
		...
		if(href_list["jack"])
			...
			for(var/mob/M in viewers(T))
				M.show_message("<span class='warning'>A port on [P] opens to reveal [P.cable], which promptly falls to the floor.</span>", 3,
				               "<span class='warning'>I hear the soft click of something light and hard falling to the ground.</span>", 2)
			return 1
...
/datum/pai_software/translator
	...
	ram_cost = 35
	id = "translator"
	var/list/languages = list(LANGUAGE_SIIK_MAAS, LANGUAGE_EAL, LANGUAGE_INDEPENDENT, LANGUAGE_SPACER, LANGUAGE_LUNAR)
