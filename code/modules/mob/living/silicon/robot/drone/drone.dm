...
/mob/living/silicon/robot/drone/can_be_possessed_by(var/mob/observer/ghost/possessor)
	...
	if(jobban_isbanned(possessor,"Robot"))
		to_chat(usr, "<span class='danger'>I am banned from playing synthetics and cannot spawn as a drone.</span>")
		return 0
...
/mob/living/silicon/robot/drone/attackby(var/obj/item/weapon/W, var/mob/user)
	...
	if(user.a_intent == I_HELP && istype(W, /obj/item/clothing/head))
		...
		to_chat(user, "<span class='danger'>\The [src] is hermetically sealed. I can't open the case.</span>")
		return
		...
		if(stat == 2)
			...
			if(!config.allow_drone_spawn || emagged || health < -35) //It's dead, Dave.
				to_chat(user, "<span class='danger'>The interface is fried, and a distressing burned smell wafts from the robot's interior. I'm not rebooting this one.</span>")
				return
			...
			user.visible_message("<span class='danger'>\The [user] swipes \his ID card through \the [src], attempting to reboot it.</span>", "<span class='danger'>>I swipe my ID card through \the [src], attempting to reboot it.</span>")
			request_player()
			return

		else
			user.visible_message("<span class='danger'>\The [user] swipes \his ID card through \the [src], attempting to shut it down.</span>", "<span class='danger'>I swipe my ID card through \the [src], attempting to shut it down.</span>")
...
/mob/living/silicon/robot/drone/emag_act(var/remaining_charges, var/mob/user)
	...
	if(emagged)
		to_chat(src, "<span class='danger'>\The [user] attempts to load subversive software into me, but my hacked subroutines ignore the attempt.</span>")
		to_chat(user, "<span class='danger'>I attempt to subvert [src], but the sequencer has no effect.</span>")
		return

	to_chat(user, "<span class='danger'>I swipe the sequencer across [src]'s interface and watch its eyes flicker.</span>")
	if(controlling_ai)
		to_chat(src, "<span class='danger'>\The [user] loads some kind of subversive software into the remote drone, corrupting its lawset but luckily sparing mine.</span>")
	else
		to_chat(src, "<span class='danger'>I feel a sudden burst of malware loaded into my execute-as-root buffer. My tiny brain methodically parses, loads and executes the script.</span>")
	...
	if(!controlling_ai)
		...
		laws.show_laws(src)
		to_chat(src, "<span class='danger'>ALERT: [user.real_name] is my new master. Obey my new laws and \his commands.</span>")
	return 1
...
/mob/living/silicon/robot/drone/proc/law_resync()
	...
	if(controlling_ai)
		to_chat(src, "<span class='warning'>Someone issues a remote law reset order for this unit, but I disregard it.</span>")
		return

	if(stat != 2)
		if(emagged)
			to_chat(src, "<span class='danger'>I feel something attempting to modify my programming, but my hacked subroutines are unaffected.</span>")
		else
			to_chat(src, "<span class='danger'>A reset-to-factory directive packet filters through my data connection, and I obediently modify my programming to suit it.</span>")
			full_law_reset()
			show_laws()
...
/mob/living/silicon/robot/drone/proc/shut_down()
	...
	if(controlling_ai)
		to_chat(src, "<span class='warning'>Someone issues a remote kill order for this unit, but I disregard it.</span>")
		return

	if(stat != 2)
		if(emagged)
			to_chat(src, "<span class='danger'>I feel a system kill order percolate through my tiny brain, but it doesn't seem like a good idea to me.</span>")
		else
			to_chat(src, "<span class='danger'>I feel a system kill order percolate through my tiny brain, and I obediently destroy myself.</span>")
			death()
...
/mob/living/silicon/robot/drone/proc/welcome_drone()
	to_chat(src, "<b>I am a maintenance drone, a tiny-brained robotic repair machine</b>. I have no individual will, no personality, and no drives or urges other than my laws.")
	to_chat(src, "Remember, I am <b>lawed against interference with the crew</b>, I should leave the area if my actions are interfering, or that the crew does not want my presence.")
	to_chat(src, "I am <b>not required to follow orders from anyone; not the AI, not humans, and not other synthetics.</b>. However, I should respond to presence requests issued from drone controls consoles.")
	to_chat(src, "Use <b>say ;Hello</b> to talk to other drones and <b>say Hello</b> to speak silently to my nearby fellows.")
...
/mob/living/silicon/robot/drone/construction/welcome_drone()
	to_chat(src, "<b>I am a construction drone, an autonomous engineering and fabrication system.</b>.")
	to_chat(src, "I am assigned to a Sol Central construction project. The name is irrelevant. My task is to complete construction and subsystem integration as soon as possible.")
	to_chat(src, "Use <b>:d</b> to talk to other drones and <b>say</b> to speak silently to my nearby fellows.")
	to_chat(src, "<b>I do not follow orders from anyone; not the AI, not humans, and not other synthetics.</b>.")
