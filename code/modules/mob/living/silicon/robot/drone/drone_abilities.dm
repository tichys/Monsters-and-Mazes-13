...
/mob/living/silicon/robot/drone/verb/set_mail_tag()
	...
	set desc = "Tag myself for delivery through the disposals system."
	...
	to_chat(src, "<span class='notice'>I configure my internal beacon, tagging myself for delivery to '[new_tag]'.</span>")
	mail_destination = new_tag

	//Auto flush if we use this verb inside a disposal chute.
	var/obj/machinery/disposal/D = src.loc
	if(istype(D))
		to_chat(src, "<span class='notice'>\The [D] acknowledges my signal.</span>")
