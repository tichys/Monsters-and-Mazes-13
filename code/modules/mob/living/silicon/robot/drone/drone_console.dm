...
/obj/machinery/computer/drone_control/Topic(href, href_list)
	...
	if (href_list["setarea"])
		...
		to_chat(usr, "<span class='notice'>I set the area selector to [drone_call_area].</span>")

	else if (href_list["ping"])

		to_chat(usr, "<span class='notice'>I issue a maintenance request for all active drones, highlighting [drone_call_area].</span>")
		for(var/mob/living/silicon/robot/drone/D in world)
		...
		if(D.stat != 2)
			to_chat(usr, "<span class='danger'>I issue a law synchronization directive for the drone.</span>")
			D.law_resync()
		...
		if(D.stat != 2)
			to_chat(usr, "<span class='danger'>I issue a kill command for the unfortunate drone.</span>")
		...
		to_chat(usr, "<span class='notice'>I [dronefab.produce_drones ? "enable" : "disable"] drone production in the nearby fabricator.</span>")
