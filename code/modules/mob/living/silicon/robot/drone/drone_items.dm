...
/obj/item/weapon/gripper
	...
	description_info = "Click an item to pick it up with my gripper. Use it as I would normally use anything in my hand. The Drop Item verb will allow I to release the item."
...
/obj/item/weapon/gripper/no_use //Used when I want to hold and put items in other things, but not able to 'use' the item
...
/obj/item/weapon/gripper/verb/drop_item()
	...
	set name = "Drop Item"
	set desc = "Release an item from my magnetic gripper."
	...
	to_chat(src.loc, "<span class='warning'>I drop \the [wrapped].</span>")
	...
	if(wrapped) //Already have an item.
		...
		if(grab)
			...
			if(istype(I.loc, /obj/item/weapon/storage))
				...
				I.forceMove(src)
			to_chat(user, "<span class='notice'>I collect \the [I].</span>")
			wrapped = I
			return
		else
			to_chat(user, "<span class='danger'>My gripper cannot hold \the [target].</span>")
		...
		if(A.opened)
			if(A.cell)
				...
				user.visible_message("<span class='danger'>[user] removes the power cell from [A]!</span>", "I remove the power cell.")
...
/obj/item/weapon/matter_decompiler
	...
	desc = "Eating trash, bits of glass, or other debris will replenish my stores."
	icon = 'icons/obj/device.dmi'
...
/obj/item/weapon/matter_decompiler/afterattack(atom/target as mob|obj|turf|area, mob/living/user as mob|obj, proximity, params)
	...
	for(var/mob/M in T)
		if(istype(M,/mob/living/simple_animal/lizard) || istype(M,/mob/living/simple_animal/mouse))
			src.loc.visible_message("<span class='danger'>[src.loc] sucks [M] into its decompiler. There's a horrible crunching noise.</span>","<span class='danger'>It's a bit of a struggle, but I manage to suck [M] into my decompiler. It makes a series of visceral crunching noises.</span>")
			...
			to_chat(D, "<span class='danger'>I begin decompiling [M].</span>")

			if(!do_after(D,50,M))
				to_chat(D, "<span class='danger'>I need to remain still while decompiling such a large object.</span>")
				return

			if(!M || !D) return

			to_chat(D, "<span class='danger'>I carefully and thoroughly decompile [M], storing as much of its resources as I can within myself.</span>")
			qdel(M)
	...
	if(grabbed_something)
		to_chat(user, "<span class='notice'>I deploy my decompiler and clear out the contents of \the [T].</span>")
	else
		to_chat(user, "<span class='danger'>Nothing on \the [T] is useful to me.</span>")
	return
