...
/proc/try_drone_spawn(var/mob/user, var/obj/machinery/drone_fabricator/fabricator)
	...
	if(jobban_isbanned(user,"Robot"))
		to_chat(user, "<span class='danger'>I am banned from playing synthetics and cannot spawn as a drone.</span>")
		return

	if(config.use_age_restriction_for_jobs && isnum(user.client.player_age))
		if(user.client.player_age <= 3)
			to_chat(user, "<span class='danger'> My account is not old enough to play as a maintenance drone.</span>")
			return
	...
	if(!fabricator)
		...
		var/choice = input(user,"Which fabricator do I wish to use?") as null|anything in all_fabricators
