...
/mob/living/silicon/robot/drone/attack_ai(var/mob/living/silicon/ai/user)
	...
	if(stat != 2 || client || key)
		to_chat(user, "<span class='warning'>I cannot take control of an autonomous, active drone.</span>")
		return
...
/mob/living/silicon/robot/drone/proc/assume_control(var/mob/living/silicon/ai/user)
	...
	if(user.mind)
		...
		key = user.key
	updatename()
	to_chat(src, "<span class='notice'><b>I have shunted my primary control loop into \a [initial(name)].</b> Use the <b>Release Control</b> verb to return to my core.</span>")
