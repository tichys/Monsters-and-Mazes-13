...
//	log_debug(life.dm line 35: cyborg use_power() called at tick [controller_iteration]")
	...
	if ( cell && is_component_functioning("power cell") && src.cell.charge > 0 )
		...
		if (src.has_power)
			to_chat(src, "<span class='warning'>I am now running on emergency backup power.</span>")
		src.has_power = 0
...
/mob/living/silicon/robot/handle_vision()
	...
	if (src.stat == DEAD || (XRAY in mutations) || (src.sight_mode & BORGXRAY))
		...
		set_sight(sight&(~SEE_TURFS)&(~SEE_MOBS)&(~SEE_OBJS))
		set_see_in_dark(8) 			 // see_in_dark means I can FAINTLY see in the dark, humans have a range of 3 or so, tajaran have it at 8
		set_see_invisible(SEE_INVISIBLE_LIVING) // This is normal vision (25), setting it lower for normal vision means I don't "see" things like darkness since darkness
