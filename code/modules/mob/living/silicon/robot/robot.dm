	...
	if(mmi)//Safety for when a cyborg gets dust()ed. Or there is no MMI inside.
		if(mind)
			...
			if(mmi.brainmob)
				...
				to_chat(src, "<span class='danger'>Oops! Something went very wrong, my MMI was unable to receive my mind. I have been ghosted. Please make a bug report so we can fix this bug.</span>")
				ghostize()
...
/mob/living/silicon/robot/verb/Namepick()
	...
	spawn(0)
		var/newname
		newname = sanitizeName(input(src,"I am a robot. Enter a name, or leave blank for the default name.", "Name change","") as text, MAX_NAME_LEN, allow_numbers = 1)
		if (newname)
			custom_name = newname
...
/mob/living/silicon/robot/verb/toggle_panel_lock()
	...
	if(!opened && has_power && do_after(usr, 60) && !opened && has_power)
		to_chat(src, "I [locked ? "un" : ""]lock my panel.")
		locked = !locked
...
/mob/living/silicon/robot/verb/toggle_lights()
	...
	lights_on = !lights_on
	to_chat(usr, "I [lights_on ? "enable" : "disable"] my integrated light.")
	update_robot_light()
...
/mob/living/silicon/robot/verb/toggle_component()
	...
	var/toggle = input(src, "Which component do I want to toggle?", "Toggle Component") as null|anything in installed_components
	if(!toggle)
		return
...
/mob/living/silicon/robot/proc/show_jetpack_pressure()
	// if I have a jetpack, show the internal tank pressure
...
/mob/living/silicon/robot/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(isWelder(W))
		if (src == user)
			to_chat(user, "<span class='warning'>I lack the reach to be able to repair myself.</span>")
			return
		...
		if(opened)
			if(cell)
				user.visible_message("<span class='notice'>\The [user] begins clasping shut \the [src]'s maintenance hatch.</span>", "<span class='notice'>I begin closing up \the [src].</span>")
				if(do_after(user, 50, src))
					to_chat(user, "<span class='notice'>I close \the [src]'s maintenance hatch.</span>")
					opened = 0
					update_icon()
				...
				user.visible_message("<span class='notice'>\The [user] begins ripping [mmi] from [src].</span>", "<span class='notice'>I jam the crowbar into the robot and begin levering [mmi].</span>")
				if(do_after(user, 50, src))
					to_chat(user, "<span class='notice'>I damage some parts of the chassis, but eventually manage to rip out [mmi]!</span>")
				...
				var/remove = input(user, "Which component do I want to pry out?", "Remove Component") as null|anything in removable_components
				if(!remove)
					return
			...
			if(locked)
				...
				user.visible_message("<span class='notice'>\The [user] begins prying open \the [src]'s maintenance hatch.</span>", "<span class='notice'>I start opening \the [src]'s maintenance hatch.</span>")
				if(do_after(user, 50, src))
					to_chat(user, "<span class='notice'>I open \the [src]'s maintenance hatch.</span>")
					opened = 1
					update_icon()
	...
	// If the robot is having something inserted which will remain inside it, self-inserting must be handled before exiting to avoid logic errors. Use the handle_selfinsert proc.
		...
		if(!user.unEquip(W, src))
			return
		if(storage)
			to_chat(user, "I replace \the [storage] with \the [W]")
			storage.forceMove(get_turf(src))
			storage = null
		else
			to_chat(user, "I install \the [W]")
		storage = W
		...
		if(silicon_radio)//sanityyyyyy
			silicon_radio.attackby(W,user)//GTFO, I have my own procs
		else
			to_chat(user, "Unable to locate a radio.")
		...
		if(!opened)
			to_chat(usr, "I must access the borgs internals!")
			...
			if(U.action(src))
				if(!user.unEquip(U, src))
					return
				to_chat(usr, "I apply the upgrade to [src]!")
				handle_selfinsert(W, user)
			else
...
/mob/living/silicon/robot/proc/ResetSecurityCodes()
	...
	set desc = "Scrambles my security and identification codes and resets my current buffers.  Unlocks I and but permenantly severs I from my AI and the robotics console and will deactivate my camera system."
...
/mob/living/silicon/robot/proc/choose_icon(var/triesleft, var/list/module_sprites)
	...
	if(module_sprites.len == 1 || !client)
		...
		icontype = input("Select an icon! [triesleft ? "I have [triesleft] more chance\s." : "This is my last try."]", "Robot Icon", icontype, null) in module_sprites
	icon_state = module_sprites[icontype]
	update_icon()

	if (module_sprites.len > 1 && triesleft >= 1 && client)
		icon_selection_tries--
		var/choice = input("Look at my icon - is this what I want?") in list("Yes","No")
		if(choice=="No")
...
/mob/living/silicon/robot/emag_act(var/remaining_charges, var/mob/user)
	...
	if(opened)//Cover is open
		if(emagged)	return//Prevents the X has hit Y with Z message also I cant emag them twice
		if(wiresexposed)
			to_chat(user, "I must close the panel first")
			return
		else
			sleep(6)
			if(prob(50))
				...
				spawn()
					...
					sleep(5)
					to_chat(src, "<span class='danger'>Would I like to send a report to NanoTraSoft? Y/N</span>")
					sleep(10)
				...
				to_chat(user, "I fail to hack [src]'s interface.")
				to_chat(src, "Hack attempt detected.")
			return 1
		return
