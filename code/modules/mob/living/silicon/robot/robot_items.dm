...
/obj/item/weapon/portable_destructive_analyzer
	...
	desc = "Similar to the stationary version, this rather unwieldy device allows me to break down objects in the name of science."
...
/obj/item/weapon/portable_destructive_analyzer/attack_self(user as mob)
	var/response = alert(user, 	"Analyzing the item inside will *DESTROY* the item for good.\n\
							...
							"What would I like to do?", "Analyze", "Sync", "Eject")
	if(response == "Analyze")
		if(loaded_item)
			var/confirm = alert(user, "This will destroy the item inside forever.  Are I sure?","Confirm Analyze","Yes","No")
			if(confirm == "Yes" && !QDELETED(loaded_item)) //This is pretty copypasta-y
				to_chat(user, "I activate the analyzer's microlaser, analyzing \the [loaded_item] and breaking it down.")
	...
	if(response == "Sync")
		...
		if(success)
			to_chat(user, "I connect to the research server, push my data upstream to it, then pull the resulting merged data from the master branch.")
...
/obj/item/weapon/portable_destructive_analyzer/afterattack(var/atom/target, var/mob/living/user, proximity)
	...
	if(istype(target,/obj/item))
		if(loaded_item)
			to_chat(user, "My [src] already has something inside.  Analyze or eject it first.")
			return
...
/obj/effect/party_light
	name = "party light"
	desc = "This is probably bad for my eyes."
...
/obj/item/weapon/card/robot //This is not a child of id cards, as to avoid dumb typechecks on computers.
	...
	allowing I to lock and unlock other robots' panels."
...
/obj/item/weapon/pen/robopen/attack_self(mob/user as mob)
	...
	var/choice = input("Would I like to change colour or mode?") as null|anything in list("Colour","Mode")
	if(!choice) return
	...
	switch(choice)
		...
		if("Colour")
			var/newcolour = input("Which colour would I like to use?") as null|anything in list("black","blue","red","green","yellow")
...
/obj/item/weapon/pen/robopen/proc/RenamePaper(mob/user as mob,obj/paper as obj)
	if ( !user || !paper )
		return
	var/n_name = sanitizeSafe(input(user, "What would I like to label the paper?", "Paper Labelling", null)  as text, 32)
	if ( !user || !paper )
		return
...
//TODO: Add prewritten forms to dispense when I work out a good way to store the strings.
...
/obj/item/weapon/inflatable_dispenser/attack_self()
	mode = !mode
	to_chat(usr, "I set \the [src] to deploy [mode ? "doors" : "walls"].")
...
/obj/item/weapon/inflatable_dispenser/proc/try_deploy_inflatable(var/turf/T, var/mob/living/user)
	...
	to_chat(user, "I deploy the inflatable [mode ? "door" : "wall"]!")
...
/obj/item/weapon/inflatable_dispenser/proc/pick_up(var/obj/A, var/mob/living/user)
	...
	to_chat(user, "I fail to pick up \the [A] with \the [src]")
	return
