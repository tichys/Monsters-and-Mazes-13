	...
	if(host)
		...
		if(!stat && !host.stat)
			...
			if(host.reagents.has_reagent(/datum/reagent/sugar))
				if(!docile)
					if(controlling)
						to_chat(host, "<span class='notice'>I feel the soporific flow of sugar in my host's blood, lulling I into docility.</span>")
					else
						to_chat(src, "<span class='notice'>I feel the soporific flow of sugar in my host's blood, lulling I into docility.</span>")
					docile = 1
			else
				if(docile)
					if(controlling)
						to_chat(host, "<span class='notice'>I shake off my lethargy as the sugar leaves my host's blood.</span>")
					else
						to_chat(src, "<span class='notice'>I shake off my lethargy as the sugar leaves my host's blood.</span>")
					docile = 0
			...
			if(controlling)
				...
				if(docile)
					to_chat(host, "<span class='notice'>I am feeling far too docile to continue controlling my host...</span>")
