...
/obj/item/device/soulstone/attackby(var/obj/item/I, var/mob/user)
	...
	if(is_evil && istype(I, /obj/item/weapon/nullrod))
		to_chat(user, "<span class='notice'>I cleanse \the [src] of taint, purging its shackles to its creator..</span>")
		is_evil = 0
		return
	if(I.force > 10)
		if(!smashing)
			to_chat(user, "<span class='notice'>\The [src] looks fragile. Are I sure I want to smash it? If so, hit it again.</span>")
			smashing = 1
			spawn(20)
				smashing = 0
			return
		user.visible_message("<span class='warning'>\The [user] hits \the [src] with \the [I], and it breaks.[shade.client ? " I hear a terrible scream!" : ""]</span>", "<span class='warning'>I hit \the [src] with \the [I], and it breaks.[shade.client ? " I hear a terrible scream!" : ""]</span>", shade.client ? "I hear a scream." : null)
		set_full(SOULSTONE_CRACKED)

/obj/item/device/soulstone/attack(var/mob/living/simple_animal/M, var/mob/user)
	if(M == shade)
		to_chat(user, "<span class='notice'>I recapture \the [M].</span>")
		M.forceMove(src)
		return
	if(full == SOULSTONE_ESSENCE)
...
/obj/item/device/soulstone/attack_self(var/mob/user)
	...
	if(!shade.key) // No key = hasn't been used
		to_chat(user, "<span class='notice'>I cut my finger and let the blood drip on \the [src].</span>")
		user.pay_for_rune(1)
		...
		var/choice = alert("Would I like to invoke the spirit within?",,"Yes","No")
		if(choice == "Yes")
			shade.forceMove(get_turf(src))
			to_chat(user, "<span class='notice'>I summon \the [shade].</span>")
		if(choice == "No")
			return
...
/obj/structure/constructshell/attackby(var/obj/item/I, var/mob/user)
	if(istype(I, /obj/item/device/soulstone))
		...
		if(!S.shade.client)
			to_chat(user, "<span class='notice'>\The [I] has essence, but no soul. Activate it in my hand to find a soul for it first.</span>")
			return
		if(S.shade.loc != S)
			to_chat(user, "<span class='notice'>Recapture the shade back into \the [I] first.</span>")
			return
		var/construct = alert(user, "Please choose which type of construct I wish to create.",,"Artificer", "Wraith", "Juggernaut")
		var/ctype
		switch(construct)
			if("Artificer")
