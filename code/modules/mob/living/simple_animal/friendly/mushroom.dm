...
/mob/living/simple_animal/mushroom/verb/spawn_spores()
	...
	set desc = "Spread my spores!"
	set src = usr

	if(stat == 2)
		to_chat(usr, "<span class='danger'>I am dead; it is too late for that.</span>")
		return

	if(!seed)
		to_chat(usr, "<span class='danger'>I am sterile!</span>")
		return

	if(world.time < harvest_time + min_explode_time)
		to_chat(usr, "<span class='danger'>I am not mature enough for that.</span>")
		return
