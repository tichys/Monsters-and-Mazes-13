...
/mob/living/simple_animal/spiderbot/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O, /obj/item/device/mmi) || istype(O, /obj/item/organ/internal/posibrain))
		...
		if(!user.unEquip(O, src))
			return
		to_chat(user, "<span class='notice'>I install \the [O] in \the [src]!</span>")
	...
	if(isWelder(O))
		...
		if (WT.remove_fuel(0))
			...
			to_chat(user, "<span class='danger'>I need more welding fuel for this task!</span>")
			return
	else if(istype(O, /obj/item/weapon/card/id)||istype(O, /obj/item/modular_computer))
		if (!mmi)
			to_chat(user, "<span class='danger'>There's no reason to swipe my ID - \the [src] has no brain to remove.</span>")
			return 0

		var/obj/item/weapon/card/id/id_card = O.GetIdCard()

		if(id_card && (access_robotics in id_card.access))
			to_chat(user, "<span class='notice'>I swipe my access card and pop the brain out of \the [src].</span>")
			eject_brain()
			if(held_item)
			...
			to_chat(user, "<span class='danger'>I swipe my card with no effect.</span>")
			return 0
...
/mob/living/simple_animal/spiderbot/emag_act(var/remaining_charges, var/mob/user)
	if (emagged)
		...
		spawn(100)	to_chat(src, "<span class='danger'>My cell seems to be outputting a lot of power...</span>")
		spawn(200)	to_chat(src, "<span class='danger'>Internal heat sensors are spiking! Something is badly wrong with my cell!</span>")
...
/mob/living/simple_animal/spiderbot/verb/drop_held_item()
	...
	if(istype(held_item, /obj/item/weapon/grenade))
		visible_message("<span class='danger'>\The [src] launches \the [held_item]!</span>", \
			"<span class='danger'>I launch \the [held_item]!</span>", \
			"I hear a skittering noise and a thump!")
	...
	visible_message("<span class='notice'>\The [src] drops \the [held_item].</span>", \
		"<span class='notice'>I drop \the [held_item].</span>", \
		"I hear a skittering noise and a soft thump.")
...
/mob/living/simple_animal/spiderbot/verb/get_item()
	...
	set category = "Spiderbot"
	set desc = "Allows I to take a nearby small item."
		...
		for(var/obj/item/I in view(1, src))
			if(selection == I)
				...
				visible_message("<span class='notice'>\The [src] scoops up \the [held_item].</span>", \
					"<span class='notice'>I grab \the [held_item].</span>", \
					"I hear a skittering noise and a clink.")
				return held_item
