...
/mob/proc/update_Login_details()
	...
	if(config.log_access)
		...
		if(is_multikeying && !client.warned_about_multikeying)
			...
			spawn(1 SECOND)
				to_chat(src, "<b>WARNING:</b> It would seem that I am sharing connection or computer with another player. If I haven't done so already, please contact the staff via the Adminhelp verb to resolve this situation. Failure to do so may result in administrative action. I have been warned.")
...
/mob/Login()
	...
	winset(src, null, "mainwindow.macro=macro hotkey_toggle.is-checked=false input.focus=true input.background-color=#252525")
