...
/mob/proc/buckled()
	...
	// where one might be fully restrained (like an elecrical chair), or merely secured (shuttle chair, keeping I safe but not otherwise restrained from acting)
	if(!buckled)
		return UNBUCKLED
...
/mob/verb/memory()
	...
	if(mind)
		...
		to_chat(src, "The game appears to have misplaced my mind datum, so we can't show I my notes.")
...
/mob/proc/update_flavor_text()
	...
	if(usr != src)
		to_chat(usr, "No.")
	var/msg = sanitize(input(usr,"Set the flavor text in my 'examine' verb. Can also be used for OOC notes about my character.","Flavor Text",html_decode(flavor_text)) as message|null, extra = 0)
...
/mob/proc/warn_flavor_changed()
	if(flavor_text && flavor_text != "") // don't spam people that don't use it!
		...
		to_chat(src, "<span class='alert'>My flavor text is likely out of date! <a href='byond://?src=\ref[src];flavor_change=1'>Change</a></span>")
...
/client/verb/changes()
	...
	getFiles(
		'html/88x31.png',
		'html/icons/bug-minus.png',
		'html/icons/burn-exclamation.png',
		'html/icons/chevron.png',
		'html/icons/chevron-expand.png',
		'html/icons/cross-circle.png',
		'html/icons/hard-hat-exclamation.png',
		'html/icons/image-minus.png',
		'html/icons/image-plus.png',
		'html/icons/map-pencil.png',
		'html/icons/music-minus.png',
		'html/icons/music-plus.png',
		'html/icons/tick-circle.png',
		'html/icons/scales.png',
		'html/icons/spell-check.png',
		'html/icons/wrench-screwdriver.png',
		'html/changelog.css',
		'html/changelog.html'
		)
...
/mob/proc/start_pulling(var/atom/movable/AM)
	...
	if(ismob(AM))
		...
		// If my size is larger than theirs and I have some
		// kind of mob pull value AT ALL, I will be able to pull
...
/mob/proc/SetStunned(amount) //if I REALLY need to set stun to a set amount without the whole "can't go below current stunned"
	if(status_flags & CANSTUN)
...
/mob/proc/yank_out_object()
	...
	var/obj/item/weapon/selection = input("What do I want to yank out?", "Embedded objects") in valid_objects

	if(self)
		to_chat(src, "<span class='warning'>I attempt to get a good grip on [selection] in my body.</span>")
	else
		to_chat(U, "<span class='warning'>I attempt to get a good grip on [selection] in [S]'s body.</span>")
	...
	if(self)
		visible_message("<span class='warning'><b>[src] rips [selection] out of their body.</b></span>","<span class='warning'><b>I rip [selection] out of my body.</b></span>")
	else
		visible_message("<span class='warning'><b>[usr] rips [selection] out of [src]'s body.</b></span>","<span class='warning'><b>[usr] rips [selection] out of my body.</b></span>")
	valid_objects = get_visible_implants(0)
	...
	if(ishuman(src))
		...
		if(prob(selection.w_class * 5) && affected.sever_artery()) //I'M SO ANEMIC I COULD JUST -DIE-.
			H.custom_pain("Something tears wetly in my [affected] as [selection] is pulled free!", 50, affecting = affected)
...
/mob/verb/face_direction()
	...
	if(!facing_dir)
		to_chat(usr, "I am now not facing anything.")
	else
		to_chat(usr, "I am now facing [dir2text(facing_dir)].")
...
/mob/proc/toggle_antag_pool()
	...
	set desc = "Toggles whether or not I will be considered a candidate by an add-antag vote."
	set category = "OOC"
	if(isghostmind(src.mind) || isnewplayer(src))
		if(ticker && ticker.looking_for_antags)
			if(src.mind in ticker.antag_pool)
				...
				to_chat(usr, "I have left the antag pool.")
			else
				ticker.antag_pool += src.mind
				to_chat(usr, "I have joined the antag pool. Make sure I have the needed role set to high!")
		else
			to_chat(usr, "The game is not currently looking for antags.")
	else
		to_chat(usr, "I must be observing or in the lobby to join the antag pool.")
...
/mob/ShiftRightClick(mob/user)
	if(/mob/proc/yank_out_object in verbs)
		user.yank_out_object(src)
