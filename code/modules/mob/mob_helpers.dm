// fun if I want to typecast humans/monkeys/etc without writing long path-filled lines.
/proc/isxenomorph(A)
...
/proc/get_zone_with_miss_chance(zone, var/mob/target, var/miss_chance_mod = 0, var/ranged_attack=0)
	...
	if(!ranged_attack)
		...
		if(target.a_intent == I_HELP)
			return zone
		// I cannot miss if my target is prone or restrained
		if(target.buckled || target.lying)
			return zone
		// if my target is being grabbed aggressively by someone I cannot miss either
...
/mob/living/carbon/human/jittery_damage()
	...
	if(src.jitteriness >= 400 && prob(5)) //Kills people if they have high jitters.
		if(prob(1))
			...
			to_chat(src, "<span class='danger'>Something explodes in my heart.</span>")
			admin_victim_log(src, "has taken <b>lethal heart damage</b> at jitteriness level [src.jitteriness].")
		else
			L.take_internal_damage(1, 0)
			to_chat(src, "<span class='danger'>The jitters are killing me! I feel my heart beating out of my chest.</span>")
