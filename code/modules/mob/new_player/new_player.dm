...
/mob/new_player
	var/ready = 0
	var/spawning = 0//Referenced when I want to delete the new_player later on in the code.
...
/mob/new_player/proc/new_player_panel_proc()
	...
	if(!ticker || ticker.current_state <= GAME_STATE_PREGAME)
		if(ready)
			output += "<p>\[ <span class='linkOn'><b>Locked In</b></span> \]</p>"
		else
			output += "<p>\[ <a href='byond://?src=\ref[src];ready=1'>Lock In</a> \]</p>"
	...
	if(statpanel("Lobby") && ticker)
		...
		if(ticker.current_state == GAME_STATE_PREGAME)
			...
			for(var/mob/new_player/player in GLOB.player_list)
				...
				stat("[player.key]", (player.ready)?("(Locked in[highjob])"):(null))
				totalPlayers++
...
/mob/new_player/Topic(href, href_list[])
	...
	if(href_list["ready"]) // locked in, no escape now
		ready = 1
	...
	if(href_list["observe"])
		...
		if(!config.respawn_delay || client.holder || alert(src,"Are I sure I wish to observe? I will have to wait [config.respawn_delay] minute\s before being able to respawn!","Player Setup","Yes","No") == "Yes")
			if(!client)	return 1
...
/mob/new_player/proc/create_character(var/turf/spawn_turf)
	...
	if(client.prefs.disabilities)
		// Set defer to 1 if I add more crap here so it only recalculates struc_enzymes once. - N3X
...
/mob/new_player/proc/check_species_allowed(datum/species/S, var/show_alert=1)
	if(!(S.spawn_flags & SPECIES_CAN_JOIN) && !has_admin_rights())
		if(show_alert)
			to_chat(src, alert("My current species, [client.prefs.species], is not available for play."))
		return 0
	return 1
