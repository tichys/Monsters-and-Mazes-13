		...
		switch(polltype)
			...
			if("TEXT")
				...
				if(!voted)	//Only make this a form if we have not voted yet
					...
					output += "<font size='2'>Please provide feedback below. I can use any letters of the English alphabet, numbers and the symbols: . , ! ? : ; -</font><br>"
			...
			if("MULTICHOICE")
				...
				var/output = "<div align='center'><B>Player poll</B>"
				output +="<hr>"
				output += "<b>Question: [pollquestion]</b><br>I can select up to [multiplechoiceoptions] options. If I select more, the first [multiplechoiceoptions] will be saved.<br>"
				output += "<font size='2'>Poll runs from <b>[pollstarttime]</b> until <b>[pollendtime]</b></font><p>"
...
/mob/new_player/proc/log_text_poll_reply(var/pollid = -1, var/replytext = "")
	...
	if(dbcon.IsConnected())
		...
		if(!text_pass)
			to_chat(usr, "The text I entered was blank, contained illegal characters or was too long. Please correct the text and submit again.")
			return
