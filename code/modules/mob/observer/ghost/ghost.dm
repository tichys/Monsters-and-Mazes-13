...
/mob/observer/ghost
	...
	var/bootime = 0
	var/started_as_observer //This variable is set to 1 when I enter the game as an observer.
							//If I died in the game and are a ghost - this will remain as null.
...
/mob/living/verb/ghost()
	set category = "OOC"
	set name = "Ghost"
	set desc = "Relinquish my life and enter the land of the dead."
	...
	if(stat == DEAD)
		...
		if(src.client && src.client.holder)
			response = alert(src, "I have the ability to Admin-Ghost. The regular Ghost verb will announce my presence to dead chat. Both variants will allow I to return to my body using 'aghost'.\n\nWhat do I wish to do?", "Are I sure I want to ghost?", "Ghost", "Admin Ghost", "Stay in body")
			if(response == "Admin Ghost")
				if(!src.client)
					return
				src.client.admin_ghost()
		else if(config.respawn_delay)
			response = alert(src, "Are I -sure- I want to ghost?\n(I am alive. If I ghost, I won't be able to play this round for another [config.respawn_delay] minute\s! I can't change my mind so choose wisely!)", "Are I sure I want to ghost?", "Ghost", "Stay in body")
		else
			response = alert(src, "Are I -sure- I want to ghost?\n(I am alive. If I ghost, I won't be able to return to this body! I can't change my mind so choose wisely!)", "Are I sure I want to ghost?", "Ghost", "Stay in body")
		if(response != "Ghost")
			return
		resting = 1
		...
		var/mob/observer/ghost/ghost = ghostize(0)	//0 parameter is so we can never re-enter our body, "Charlie, I can never come baaaack~" :3
...
/mob/observer/ghost/verb/reenter_corpse()
	...
	if(mind.current.key && copytext(mind.current.key,1,2)!="@")	//makes sure we don't accidentally kick any clients
		to_chat(src, "<span class='warning'>Another consciousness is in my body... it is resisting me.</span>")
		return
	stop_following()
...
/mob/observer/ghost/verb/toggle_medHUD()
	...
	set desc = "Toggles Medical HUD allowing I to see how everyone is doing"
	if(!client)
		return
	if(medHUD)
		medHUD = 0
...
/mob/observer/ghost/verb/toggle_antagHUD()
	...
	set desc = "Toggles AntagHUD allowing I to see who is the antagonist"
	...
	if(config.antag_hud_restricted && !M.has_enabled_antagHUD && (!client.holder || mentor))
		var/response = alert(src, "If I turn this on, I will not be able to take any part in the round.","Are I sure I want to turn this feature on?","Yes","No")
...
/mob/observer/ghost/verb/dead_tele(A in area_repository.get_areas_by_z_level())
	...
	if(!area_turfs.len)
		to_chat(src, "<span class='warning'>This area has been entirely made into sacred grounds, I cannot enter it while I am in this plane of existence!</span>")
		return
...
/mob/observer/ghost/move_to_turf(var/atom/movable/am, var/old_loc, var/new_loc)
	...
	if(check_is_holy_turf(T))
		to_chat(src, "<span class='warning'>I cannot follow something standing on holy grounds!</span>")
		return
	..()

/mob/observer/ghost/memory()
	set hidden = 1
	to_chat(src, "<span class='warning'>I am dead! I have no mind to store memory!</span>")
...
/mob/observer/ghost/verb/become_mouse()
	...
	if(!T || (T.z in GLOB.using_map.admin_levels))
		to_chat(src, "<span class='warning'>I may not spawn as a mouse on this Z-level.</span>")
		return

	var/response = alert(src, "Are I -sure- I want to become a mouse?","Are I sure I want to squeek?","Squeek!","Nope!")
	...
	if(host)
		...
		to_chat(host, "<span class='info'>I am now a mouse. Try to avoid interaction with players, and do not give hints away that I am more than a simple rodent.</span>")
...
/mob/observer/ghost/verb/toggle_anonsay()
	...
	set desc = "Toggles showing my key in dead chat."

	src.anonsay = !src.anonsay
	if(anonsay)
		to_chat(src, "<span class='info'>My key won't be shown when I speak in dead chat.</span>")
	else
		to_chat(src, "<span class='info'>My key will be publicly visible again.</span>")
...
/mob/observer/ghost/verb/toggle_ghostsee()
	...
	set desc = "Toggles my ability to see things only ghosts can see, like other ghosts"
	set category = "Ghost"
	ghostvision = !(ghostvision)
	updateghostsight()
	to_chat(src, "I [(ghostvision?"now":"no longer")] have ghost vision.")
...
/mob/observer/ghost/verb/toggle_darkness()
	...
	updateghostsight()
	to_chat(src, "I [(seedarkness?"now":"no longer")] see darkness.")
...
/mob/observer/ghost/MayRespawn(var/feedback = 0, var/respawn_time = 0)
	...
	if(mind && mind.current && mind.current.stat < DEAD && can_reenter_corpse == CORPSE_CAN_REENTER)
		if(feedback)
			to_chat(src, "<span class='warning'>My non-dead body prevents me from respawning.</span>")
		return 0
	if(config.antag_hud_restricted && has_enabled_antagHUD == 1)
		if(feedback)
			to_chat(src, "<span class='warning'>antagHUD restrictions prevent I from respawning.</span>")
		return 0
	...
	if(!client.holder && respawn_time && timeofdeath && timedifference < respawn_time MINUTES)
		...
		to_chat(src, "<span class='warning'>I must have been dead for [respawn_time] minute\s to respawn. I have [timedifference_text] left.</span>")
		return 0
...
/mob/observer/ghost/verb/respawn()
	...
	if (!(ticker && ticker.mode))
		to_chat(usr, "<span class='notice'><B>I may not attempt to respawn yet.</B></span>")
		return
