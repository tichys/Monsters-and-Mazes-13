...
/datum/nano_module/skill_ui/antag/Topic(href, href_list)
	...
	if(href_list["add_skill"])
		...
		var/choice = input(usr, "Which skill would I like to add?", "Add Skill") as null|anything in choices
	...
	if(href_list["submit"])
		if(!can_choose())
			return 1
		if(alert(usr, "Are I sure I want to commit this selection? I won't be able to change it again.", "Warning", "Yes", "No")=="No")
			return 1
		if(!can_choose())
			return 1
		commit()
