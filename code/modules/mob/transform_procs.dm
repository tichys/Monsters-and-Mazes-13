/mob/living/carbon/human/proc/monkeyize()
	...
	to_chat(src, "<B>I am now [species.name]. </B>")
	qdel(animation)
		...
		if (!loc_landmark)
			for(var/obj/effect/landmark/tripai in landmarks_list)
				if (tripai.name == "tripai")
					...
					loc_landmark = tripai
		if (!loc_landmark)
			to_chat(O, "Oh god sorry we can't find an unoccupied AI spawn location, so we're spawning I on top of someone.")
			for(var/obj/effect/landmark/start/sloc in landmarks_list)
				if (sloc.name == "AI")
					loc_landmark = sloc
...
/mob/living/carbon/human/proc/zombify()
	ChangeToHusk()
	mutations |= CLUMSY
	src.visible_message("<span class='danger'>\The [src]'s skin decays before my very eyes!</span>", "<span class='danger'>My entire body is ripe with pain as it is consumed down to flesh and bones. I ... hunger. Not only for flesh, but to spread this gift.</span>")
	if (src.mind)
