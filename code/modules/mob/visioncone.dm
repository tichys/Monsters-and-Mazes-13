...
mob/living/InCone(mob/center = usr, dir = NORTH)
	. = ..()
	for(var/obj/item/grab/G in center)//TG doesn't have the grab item. But if you're porting it and I do then uncomment this.
		if(src == G.affecting)
			return 0
		else
			return .
...
mob/living/carbon/human/update_vision_cone()
	...
	if(src.client)
		...
		if(fov.alpha != 0)
			...
			//Optional items can be made invisible too. Uncomment this part if I wish to items to be invisible.
			//var/obj/item/O
...
//Making these generic procs so I can call them anywhere.
