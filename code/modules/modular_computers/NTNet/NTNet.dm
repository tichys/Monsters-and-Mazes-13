...
/datum/ntnet/proc/rename_email(mob/user, old_login, desired_name, domain)
	...
	if(find_email_by_name(new_login))
		to_chat(user, "My email could not be updated: the new username is invalid.")
		return
	account.login = new_login
	to_chat(user, "My email account address has been changed to <b>[new_login]</b>. This information has also been placed into my notes.")
	add_log("Email address changed for [user]: [old_login] changed to [new_login]")
	if(user.mind)
		user.mind.initial_email_login["login"] = new_login
		user.mind.store_memory("My email account address has been changed to [new_login].")
	if(issilicon(user))
...
/datum/ntnet/proc/create_email(mob/user, desired_name, domain)
	...
	if(find_email_by_name(login))
		to_chat(user, "I were not assigned an email address.")
		user.mind.store_memory("I were not assigned an email address.")
	else
		var/datum/computer_file/data/email_account/EA = new/datum/computer_file/data/email_account()
		EA.password = GenerateKey()
		EA.login = login
		to_chat(user, "My email account address is <b>[EA.login]</b> and the password is <b>[EA.password]</b>. This information has also been placed into my notes.")
		if(user.mind)
			user.mind.initial_email_login["login"] = EA.login
			user.mind.initial_email_login["password"] = EA.password
			user.mind.store_memory("My email account address is [EA.login] and the password is [EA.password].")
		if(issilicon(user))
			var/mob/living/silicon/S = user
			var/datum/nano_module/email_client/my_client = S.get_subsystem_from_path(/datum/nano_module/email_client)
			if(my_client)
