...
1. Basic information
Program based computers will allow I to do multiple things from single computer. Each computer will have programs, with more being downloadable from NTNet (stationwide network with wireless coverage)
...
lift ALL locks on ALL installed programs, and allow download of programs even if my ID doesn't have access to them. Computers will have hard drives that can store files.
...
which will be related to how good hard drive I buy when purchasing the laptop. For storing more files USB-style drives will be buildable with Protolathe in research.
...
Basic - Software in this bundle is automagically preinstalled in every new computer
	...
	File Browser - Allows I to browse all files stored on the computer. Allows renaming/deleting of files.
	TXT Editor - Allows I editing data files in text editor mode.
	NanoPrint - Allows I to operate NanoPrinter hardware to print text files.
...
*REDACTED* - Can be downloaded from SyndiCorp servers, only via emagged devices. These files are very large and limited to laptops/consoles only.
	...
	AIHACK - Hacks an AI, allowing I to upload/remove/modify a law even without relevant circuit board. The AI is alerted once the hack starts, and it takes a while for it to complete. Does not work on AIs with zeroth law.
...
6. Security
Laptops will be password-lockable. If password is set a MD5 hash of it is stored and password is required every time I turn on the laptop.
...
If decrypted the password is cleared, so I can keep using my favorite password without people ever actually revealing it (for meta prevention reasons mostly).
