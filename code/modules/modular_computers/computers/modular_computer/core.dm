...
/obj/item/modular_computer/emag_act(var/remaining_charges, var/mob/user)
	if(computer_emagged)
		...
		computer_emagged = 1
		to_chat(user, "I emag \the [src]. It's screen briefly shows a \"OVERRIDE ACCEPTED: New software downloads available.\" message.")
		return 1
...
/obj/item/modular_computer/proc/turn_on(var/mob/user)
	...
	if(damage > broken_damage)
		if(issynth)
			to_chat(user, "I send an activation signal to \the [src], but it responds with an error code. It must be damaged.")
		else
			to_chat(user, "I press the power button, but the computer fails to boot up, displaying variety of errors before shutting down again.")
		return
	if(processor_unit && (apc_power(0) || battery_power(0))) // Battery-run and charged or non-battery but powered by APC.
		if(issynth)
			to_chat(user, "I send an activation signal to \the [src], turning it on")
		else
			to_chat(user, "I press the power button and start up \the [src]")
		enable_computer(user)

	else // Unpowered
		if(issynth)
			to_chat(user, "I send an activation signal to \the [src] but it does not respond")
		else
			to_chat(user, "I press the power button but \the [src] does not respond")
...
/obj/item/modular_computer/proc/run_program(prog)
	...
	if(P.requires_ntnet && !get_ntnet_status(P.requires_ntnet_feature)) // The program requires NTNet connection, but we are not connected to NTNet.
		to_chat(user, "<span class='danger'>\The [src]'s screen shows \"NETWORK ERROR - Unable to connect to NTNet. Please retry. If problem persists contact my system administrator.\" warning.</span>")
		return
