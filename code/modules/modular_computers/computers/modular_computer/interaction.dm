...
/obj/item/modular_computer/verb/emergency_shutdown()
	...
	if(!Adjacent(usr))
		to_chat(usr, "<span class='warning'>I can't reach it.</span>")
		return
	...
	if(enabled)
		...
		update_icon()
		shutdown_computer()
		to_chat(usr, "I press a hard-reset button on \the [src]. It displays a brief debug screen before shutting down.")
		spawn(2 SECONDS)
			bsod = 0
			update_icon()
...
/obj/item/modular_computer/verb/remove_pen()
	...
	if(istype(stored_pen))
		to_chat(usr, "<span class='notice'>I remove [stored_pen] from [src].</span>")
...
/obj/item/modular_computer/proc/proc_eject_id(mob/user)
	...
	to_chat(user, "I remove [card_slot.stored_card] from [src].")
...
/obj/item/modular_computer/attack_ghost(var/mob/observer/ghost/user)
	if(enabled)
		...
		var/response = alert(user, "This computer is turned off. Would I like to turn it on?", "Admin Override", "Yes", "No")
		if(response == "Yes")
			turn_on(user)
...
/obj/item/modular_computer/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	if(istype(W, /obj/item/weapon/card/id)) // ID Card, try to insert it.
		...
		if(!card_slot)
			to_chat(user, "I try to insert [I] into [src], but it does not have an ID card slot installed.")
			return

		if(card_slot.stored_card)
			to_chat(user, "I try to insert [I] into [src], but its ID card slot is occupied.")
			return
	...
	if(isWelder(W))
		...
		to_chat(user, "I begin repairing damage to \the [src]...")
		if(WT.remove_fuel(round(damage/75)) && do_after(usr, damage/10))
			damage = 0
			to_chat(user, "I repair \the [src].")
		return
	...
	if(isScrewdriver(W))
		...
		var/choice = input(usr, "Which component do I want to uninstall?", "Computer maintenance", null) as null|anything in component_names
