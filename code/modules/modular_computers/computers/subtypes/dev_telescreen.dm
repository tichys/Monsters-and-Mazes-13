...
/obj/item/modular_computer/telescreen/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	if(isCrowbar(W))
		if(anchored)
			...
			pixel_x = 0
			pixel_y = 0
			to_chat(user, "I unsecure \the [src].")
		else
			var/choice = input(user, "Where do I want to place \the [src]?", "Offset selection") in list("North", "South", "West", "East", "This tile", "Cancel")
			var/valid = FALSE
			switch(choice)
				if("North")
					valid = TRUE
