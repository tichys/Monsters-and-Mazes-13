				...
				if(program)
					if(is_autenthicated(user) && program.computer_emagged && !issilicon(usr) && ntn_comm)
						if(centcomm_message_cooldown)
							...
							return
						var/input = sanitize(input(usr, "Please choose a message to transmit to \[ABNORMAL ROUTING CORDINATES\] via quantum entanglement.  Please be aware that this process is very expensive, and abuse will lead to... termination. Transmission does not guarantee a response. There is a 30 second delay before I may send another message, be clear, full and concise.", "To abort, send an empty message.", "") as null|text)
						if(!input || !can_still_topic())
							return 1
		...
		if("evac")
			...
			if(is_autenthicated(user))
				...
				if (selected_evac_option.needs_syscontrol && !ntn_cont)
					return
				var/confirm = alert("Are I sure I want to [selected_evac_option.option_desc]?", name, "No", "Yes")
		...
		if("setalert")
			...
			if(is_autenthicated(user) && !issilicon(usr) && ntn_cont && ntn_comm)
				...
				if(target_level && security_state.can_switch_to(target_level))
					var/confirm = alert("Are I sure I want to change the alert level to [target_level.name]?", name, "No", "Yes")
					if(confirm == "Yes" && can_still_topic())
						if(security_state.set_security_level(target_level))
							feedback_inc(target_level.type,1)
			else
				to_chat(usr, "I press the button, but a red light flashes and nothing happens.") //This should never happen
