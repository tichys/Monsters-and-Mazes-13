...
/datum/nano_module/deck_management/Topic(href, href_list)
	...
	if(!get_default_access(user))
		return 1 //No program access if I don't have the right access.
	if(text2num(href_list["warning"])) //Gives the user a chance to avoid losing unsaved reports.
		if(alert(user, "Are I sure I want to do this? Data may be lost.",, "Yes.", "No.") == "No.")
	...
	if(href_list["report"])
		...
		if(set_shuttle(user, shuttle_name, 0) && set_mission(mission_ID))
			...
			if(href_list["flight_plan"])
				...
				if(selected_mission.stage in list(SHUTTLE_MISSION_PLANNED, SHUTTLE_MISSION_QUEUED))
					return 1 //Hold my horses until the mission is started on these reports.
	...
	if(href_list["summon_crew"])
		...
		if(alert(user, "Would I like to choose a custom gathering point, or just use [place]?", "Announcement Creation", "Default", "Custom") == "Custom")
			...
			if(A)
				place = A
		if(alert(user, "This will make a radio announcement summoning all mission crew to the [place]. Are I sure I want to do this?",, "Yes.", "No.") == "No.")
			return 1
		var/datum/shuttle_log/my_log = SSshuttle.shuttle_logs[selected_shuttle]
