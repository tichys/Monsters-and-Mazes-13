/datum/computer_file/program/email_client
	...
	extended_desc = "This program may be used to log in into my email account."
...
// Persistency. Unless I log out, or unless my password changes, this will pre-fill the login data when restarting the program
...
/datum/nano_module/email_client/proc/relayed_process(var/netspeed)
	...
	if(download_progress >= downloading.size)
		...
		if(!istype(MC) || !MC.hard_drive || !MC.hard_drive.check_functionality())
			error = "Error uploading file. Are I using a functional and NTOSv2-compliant device?"
			downloading = null
			download_progress = 0
			return 1
...
/datum/nano_module/email_client/Topic(href, href_list)
	...
	if(href_list["edit_title"])
		var/newtitle = sanitize(input(user,"Enter title for my message:", "Message title", msg_title), 100)
		if(newtitle)
			msg_title = newtitle
		return 1
	...
	if(href_list["edit_body"])
		...
		var/newtext = sanitize(replacetext(input(usr, "Enter my message. I may use most tags from paper formatting", "Message Editor", oldtext) as message|null, "\n", "\[br\]"), 20000)
		if(newtext)
			msg_body = newtext
		return 1
	...
	if(href_list["changepassword"])
		var/oldpassword = sanitize(input(user,"Please enter my old password:", "Password Change"), 100)
		if(!oldpassword)
			return 1
		var/newpassword1 = sanitize(input(user,"Please enter my new password:", "Password Change"), 100)
		if(!newpassword1)
			return 1
		var/newpassword2 = sanitize(input(user,"Please re-enter my new password:", "Password Change"), 100)
		if(!newpassword2)
			return 1
		...
		error = "My password has been successfully changed!"
		return 1
	...
	if(href_list["save"])
		...
		if(!istype(MC) || !MC.hard_drive || !MC.hard_drive.check_functionality())
			error = "Error exporting file. Are I using a functional and NTOS-compliant device?"
			return 1
	...
	if(href_list["downloadattachment"])
		...
		var/obj/item/modular_computer/MC = nano_host()
		if(!istype(MC) || !MC.hard_drive || !MC.hard_drive.check_functionality())
			error = "Error downloading file. Are I using a functional and NTOSv2-compliant device?"
			return 1
