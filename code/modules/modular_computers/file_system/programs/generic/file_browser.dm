...
/datum/computer_file/program/filemanager/Topic(href, href_list)
	...
	if(href_list["PRG_edit"])
		...
		var/newtext = sanitize(replacetext(input(usr, "Editing file [open_file]. I may use most tags used in paper formatting:", "Text Editor", oldtext) as message|null, "\n", "\[br\]"), MAX_TEXTFILE_LENGTH)
		if(!newtext)
			return
		...
		if(F)
			...
			if(!HDD.store_file(F))
				error = "I/O error: Unable to overwrite file. Hard drive is probably full. I may want to backup my changes before closing this window:<br><br>[html_decode(F.stored_data)]<br><br>"
	...
	if(href_list["PRG_printfile"])
		...
		if(!computer.nano_printer)
			error = "Missing Hardware: My computer does not have required hardware to complete this operation."
			return 1
