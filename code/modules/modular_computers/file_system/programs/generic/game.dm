...
/datum/computer_file/program/game
	...
	extended_desc = "Fun for the whole family! Probably not an AAA title, but at least I can download it on the corporate network.."		// A nice description.
...
// ui_interact handles transfer of data to NanoUI. Keep in mind that data I pass from here is actually sent to the client. In other words, don't send anything I don't want a client
...
// Three helper procs i've created. These are unique to this particular nano module. If I am creating my own nano module, you'll most likely create similar procs too.
/datum/nano_module/arcade_classic/proc/enemy_play()
	if((enemy_mana < 5) && prob(60))
		var/steal = rand(2, 3)
		player_mana -= steal
		enemy_mana += steal
		information += "[enemy_name] steals [steal] of my power!"
...
/datum/nano_module/arcade_classic/proc/check_gameover()
	if((player_health <= 0) || player_mana <= 0)
		if(enemy_health <= 0)
			information += "I have defeated [enemy_name], but I have died in the fight!"
		else
			information += "I have been defeated by [enemy_name]!"
		gameover = 1
		return TRUE
	else if(enemy_health <= 0)
		gameover = 1
		information += "Congratulations! I have defeated [enemy_name]!"
		return TRUE
	return FALSE
...
/datum/nano_module/arcade_classic/Topic(href, href_list)
	if(..())		// Always begin my Topic() calls with a parent call!
		return 1
	...
	if(href_list["attack"])
		var/damage = rand(2, 6)
		information = "I attack for [damage] damage."
		enemy_health -= damage
		enemy_play()
		check_gameover()
		return 1
	if(href_list["heal"])
		...
		var/cost = rand(1, 3)
		information = "I heal myself for [healfor] damage, using [cost] energy in the process."
	...
	if(href_list["regain_mana"])
		var/regen = rand(4, 7)
		information = "I rest of a while, regaining [regen] energy."
		player_mana += regen
		enemy_play()
		check_gameover()
		return 1