...
/datum/nano_module/library/ui_interact(mob/user, ui_key = "main", var/datum/nanoui/ui = null, var/force_open = 1, var/datum/topic_state/state = GLOB.default_state)
	...
	if(error_message)
		...
		if(!dbcon_old.IsConnected())
			error_message = "Unable to contact External Archive. Please contact my system administrator for assistance."
		else
	...
	if(href_list["uploadbook"])
		...
		if(choice == "Yes")
			...
			if(!query.Execute())
				...
				error_message = "Network Error: Unable to upload to the Archive. Contact my system Administrator for assistance."
				return 1
			else
