...
/datum/nano_module/program/reports/proc/save_report(mob/user, save_as)
	...
	if(!program.computer.hard_drive.try_store_file(selected_report))
		to_chat(user, "Error storing file. Please check my hard drive.")
...
/datum/nano_module/program/reports/proc/load_report(mob/user)
	...
	var/choice = input(user, "Which report would I like to load?", "Loading Report") as null|anything in choices
	if(choice in choices)
		var/datum/computer_file/report/chosen_report = choices[choice]
		var/editing = alert(user, "Would I like to view or edit the report", "Loading Report", "View", "Edit")
		if(editing == "View")
			if(!chosen_report.verify_access(get_access(user)))
				to_chat(user, "<span class='warning'>I lack access to view this report.</span>")
				return
			can_view_only = 1
		else
			if(!chosen_report.verify_access_edit(get_access(user)))
				to_chat(user, "<span class='warning'>I lack access to edit this report.</span>")
				return
			can_view_only = 0
...
/datum/nano_module/program/reports/Topic(href, href_list)
	...
	if(text2num(href_list["warning"])) //Gives the user a chance to avoid losing unsaved reports.
		if(alert(user, "Are I sure I want to leave this page? Unsubmitted data will be lost.",, "Yes.", "No.") == "No.")
	...
	if(href_list["submit"])
		...
		if(selected_report.submit(user))
			...
			if(alert(user, "Would I like to save a copy?","Save Report", "Yes.", "No.") == "Yes.")
				save_report(user)
		return 1
	...
	if(href_list["export"])
		...
		to_chat(user, (program.computer.hard_drive.store_file(file) ? "The report has been exported as [file.filename].[file.filetype]" : "Error storing file. Please check my hard drive."))
		return 1
