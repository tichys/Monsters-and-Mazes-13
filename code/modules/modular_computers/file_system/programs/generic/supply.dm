...
/datum/nano_module/supply/Topic(href, href_list)
	...
	if(href_list["order"])
		...
		var/reason = sanitize(input(user,"Reason:","Why do I require this item?","") as null|text,,0)
		if(!reason)
			return 1
		...
		if(can_print() && alert(user, "Would I like to print a confirmation receipt?", "Print receipt?", "Yes", "No") == "Yes")
			print_order(O, user)
		return 1
