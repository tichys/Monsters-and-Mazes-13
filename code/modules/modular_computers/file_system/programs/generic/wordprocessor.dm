...
/datum/computer_file/program/wordprocessor/Topic(href, href_list)
	...
	if(href_list["PRG_openfile"])
		. = 1
		if(is_edited)
			if(alert("Would I like to save my changes first?",,"Yes","No") == "Yes")
				save_file(open_file)
		browsing = 0
	...
	if(href_list["PRG_printfile"])
		...
		if(!computer.nano_printer)
			error = "Missing Hardware: My computer does not have the required hardware to complete this operation."
			return 1
