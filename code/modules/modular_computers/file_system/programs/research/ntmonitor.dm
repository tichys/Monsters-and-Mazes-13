...
/datum/nano_module/computer_ntnetmonitor/Topic(href, href_list, state)
	...
	if(href_list["toggleWireless"])
		...
		if(!user)
			return 1
		var/response = alert(user, "Really disable NTNet wireless? If my computer is connected wirelessly I won't be able to turn it back on! This will affect all connected wireless devices.", "NTNet shutdown", "Yes", "No")
		if(response == "Yes")
	...
	if(href_list["ban_nid"])
		if(!ntnet_global)
			return 1
		var/nid = input(user,"Enter NID of device which I want to block from the network:", "Enter NID") as null|num
	...
	if(href_list["unban_nid"])
		if(!ntnet_global)
			return 1
		var/nid = input(user,"Enter NID of device which I want to unblock from the network:", "Enter NID") as null|num
		if(nid && CanUseTopic(user, state))
			ntnet_global.banned_nids -= nid
		return 1