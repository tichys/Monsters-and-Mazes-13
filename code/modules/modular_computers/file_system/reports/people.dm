...
/datum/report_field/people/proc/send_email(mob/user)
	...
	var/attach_report = (alert(user, "Do I wish to attach [owner.display_name()]?","Document Email", "Yes.", "No.") == "Yes.") ? 1 : 0
	if(alert(user, "Are I sure I want to send this email?","Document Email", "Yes.", "No.") == "No.")
		return
...
//Lets I select one person on the manifest.
...
/datum/report_field/people/list_from_manifest/ask_value(mob/user)
	var/alert = alert(user, "Would I like to add or remove a name?", "Form Input", "Add", "Remove")
	var/list/formatted_manifest = list()
	switch(alert)
		if("Add")
			var/list/full_manifest = flat_nano_crew_manifest()
