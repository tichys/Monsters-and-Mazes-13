...
I must have access to have edit access.
...
//Looking up fields. Names might not be unique unless I ensure otherwise.
...
/datum/computer_file/report/proc/submit(mob/user)
	...
	for(var/datum/report_field/field in fields)
		if(field.required && !field.get_value())
			to_chat(user, "<span class='notice'>I am missing a required field!</span>")
			return 0
	creator = user.name
