...
/obj/item/weapon/computer_hardware/nano_printer/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/paper))
		if(stored_paper >= max_paper)
			to_chat(user, "I try to add \the [W] into \the [src], but its paper bin is full.")
			return

		to_chat(user, "I insert \the [W] into [src].")
		qdel(W)
		stored_paper++
		...
		if(B.pages.len == 0) //if all its papers have been put into the printer, delete bundle
			...
			B.update_icon()
		to_chat(user, "I add [num_of_pages_added] papers from \the [W] into \the [src].")
	return
