...
/mob/proc/zMove(direction)
	...
	if(!can_ztravel())
		to_chat(src, "<span class='warning'>I lack means of travel in that direction.</span>")
		return

	var/turf/start = loc
	if(!istype(start))
		to_chat(src, "<span class='notice'>I am unable to move from here.</span>")
		return 0
	...
	if(!destination.CanZPass(src, direction))
		to_chat(src, "<span class='warning'>I bump against \the [destination].</span>")
		return 0

	var/area/area = get_area(src)
	if(direction == UP && area.has_gravity() && !can_overcome_gravity())
		to_chat(src, "<span class='warning'>Gravity stops me from moving upward.</span>")
		return 0
	...
	if(direction == UP && area.has_gravity() && can_fall(FALSE, destination))
		to_chat(src, "<span class='warning'>I see nothing to hold on to.</span>")
		return 0
...
/atom/movable/proc/handle_fall_effect(var/turf/landing)
	if(istype(landing, /turf/simulated/open))
		visible_message("\The [src] falls from the deck above through \the [landing]!", "I hear a whoosh of displaced air.")
	else
		visible_message("\The [src] falls from the deck above and slams into \the [landing]!", "I hear something slam into the deck.")
		if(fall_damage())
