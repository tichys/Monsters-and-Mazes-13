...
/obj/item/organ/external/head/removed()
	...
	for(var/M in markings)
		...
		overlays |= mark_s //So when it's not on my body, it has icons
		mob_icon.Blend(mark_s, mark_style.layer_blend) //So when it's on my body, it has icons
...
/obj/item/organ/external/update_icon(var/regenerate = 0)
	...
	for(var/M in markings)
		...
		overlays |= mark_s //So when it's not on my body, it has icons
		mob_icon.Blend(mark_s, ICON_OVERLAY) //So when it's on my body, it has icons
		icon_cache_key += "[M][markings[M]["color"]]"
