...
/obj/item/organ/external/head/proc/write_on(var/mob/penman, var/style)
	...
	if(graffiti)
		...
		penman.visible_message("<span class='warning'>[penman] begins writing something on [head_name]!</span>", "I begin writing something on [head_name].")
		...
		if(do_after(penman, 3 SECONDS, target))
			...
			penman.visible_message("<span class='warning'>[penman] writes something on [head_name]!</span>", "I write something on [head_name].")
