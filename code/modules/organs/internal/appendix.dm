	...
	if(inflamed && owner)
		...
		if(prob(5))
			if(owner.can_feel_pain())
				owner.custom_pain("I feel a stinging pain in my abdomen!")
				if(owner.can_feel_pain())
		...
		if(inflamed > 400)
			if(prob(1))
				...
				if (owner.nutrition > 100)
					...
					to_chat(owner, "<span class='danger'>I gag as I want to throw up, but there's nothing in my stomach!</span>")
					owner.Weaken(10)
		if(inflamed > 600)
			if(prob(1))
				if(owner.can_feel_pain())
