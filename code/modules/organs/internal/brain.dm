...
/obj/item/organ/internal/brain/proc/transfer_identity(var/mob/living/carbon/H)
	...
	to_chat(brainmob, "<span class='notice'>I feel slightly disoriented. That's normal when you're just \a [initial(src.name)].</span>")
	callHook("debrain", list(brainmob))

/obj/item/organ/internal/brain/examine(mob/user) // -- TLE
	. = ..(user)
	if(brainmob && brainmob.client)//if thar be a brain inside... the brain.
		to_chat(user, "I can feel the small spark of life still left in this one.")
	else
	...
	if(owner)
		if(damage > max_damage / 2 && healed_threshold)
			spawn()
				...
				alert(owner, "I have taken massive brain damage! I will not be able to remember the events leading up to my injury.", "Brain Damaged")
				if(ishuman(owner))
					owner.clearmoods()  // I can't remember my mood
			healed_threshold = 0
		...
		if(owner.should_have_organ(BP_HEART))
			...
			// No heart? I am going to have a very bad time. Not 100% lethal because heart transplants should be a thing.
			...
			switch(blood_volume)
				...
				if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
					if(prob(1))
						to_chat(owner, "<span class='warning'>I feel [pick("dizzy","woozy","faint")]...</span>")
				...
				if(BLOOD_VOLUME_BAD to BLOOD_VOLUME_OKAY)
					...
					if(!owner.paralysis && prob(10))
						...
						to_chat(owner, "<span class='warning'>I feel extremely [pick("dizzy","woozy","faint")]...</span>")
				if(BLOOD_VOLUME_SURVIVE to BLOOD_VOLUME_BAD)
					...
					if(!owner.paralysis && prob(15))
						owner.Paralyse(3,5)
						to_chat(owner, "<span class='warning'>I feel extremely [pick("dizzy","woozy","faint")]...</span>")
				if(-(INFINITY) to BLOOD_VOLUME_SURVIVE) // Also see heart.dm, being below this point puts I into cardiac arrest.
					owner.eye_blurry = max(owner.eye_blurry,6)
...
/obj/item/organ/internal/brain/proc/handle_disabilities()
	...
	if((owner.disabilities & EPILEPSY) && prob(1))
		to_chat(owner, "<span class='warning'>I have a seizure!</span>")
...
/obj/item/organ/internal/brain/proc/handle_damage_effects()
	...
	if(damage >= 0.5*max_damage && prob(1) && owner.get_active_hand())
		to_chat(owner, "<span class='danger'>My hand won't respond properly, and I drop what I am holding!</span>")
		owner.unequip_item()
	if(damage >= 0.6*max_damage)
		owner.slurring = max(owner.slurring, 2)
	if(is_broken())
		if(!owner.lying)
			to_chat(owner, "<span class='danger'>I black out!</span>")
		owner.Paralyse(10)
