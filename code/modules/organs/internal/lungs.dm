		...
		if(prob(2))
			if(active_breathing)
				owner.visible_message(
					...
					"<span class='warning'>I cough up blood!</span>",
		...
		if(prob(4))
			if(active_breathing)
				owner.visible_message(
					...
					"<span class='danger'>I can't breathe!</span>",
					"I hear someone gasp for air!",
				)
			else
				to_chat(owner, "<span class='danger'>I'm having trouble getting enough [breath_type]!</span>")
...
/obj/item/organ/internal/lungs/proc/rupture()
	...
	if(istype(parent))
		owner.custom_pain("I feel a stabbing pain in my [parent.name]!", 50, affecting = parent)
	bruise()
...
/obj/item/organ/internal/lungs/proc/handle_breath(datum/gas_mixture/breath, var/forced)
	...
	// Presumably if I breathe it I have a specialized metabolism for it, so we drop/ignore breath_type. Also avoids
...
/obj/item/organ/internal/lungs/proc/handle_temperature_effects(datum/gas_mixture/breath)
	...
	if((breath.temperature < species.cold_level_1 || breath.temperature > species.heat_level_1) && !(COLD_RESISTANCE in owner.mutations))
		...
		if(breath.temperature <= species.cold_level_1)
			if(prob(20))
				to_chat(owner, "<span class='danger'>I feel my face freezing and icicles forming in my lungs!</span>")
			...
			if(prob(20))
				to_chat(owner, "<span class='danger'>I feel my face burning and a searing heat in my lungs!</span>")
		...
		//breathing in hot/cold air also heats/cools I a bit
