...
/obj/item/organ/internal/brain/adherent/attack_self(var/mob/user)
	...
	if(. && istype(species, /datum/species/adherent))
		...
		to_chat(owner, "<span class='notice'>I am now designated <b>[owner.real_name]</b>.</span>")
...
/obj/item/organ/internal/powered/Process()
	...
	if(owner)
		...
		if(active && !(cell && cell.use(maintenance_cost)))
			active = FALSE
			to_chat(owner, "<span class='danger'>my [name] [gender == PLURAL ? "are" : "is"] out of power!</span>")
...
/obj/item/organ/internal/powered/attack_self(var/mob/user)
	...
	if(.)
		...
		if(is_broken())
			...
			active = !active
			to_chat(owner, "<span class='notice'>I am [active ? "now" : "no longer"] using my [name] to [use_descriptor].</span>")
		refresh_action_button()
...
/obj/item/organ/internal/powered/jets
	...
	use_descriptor = "adjust my vector"
	organ_tag = BP_JETS
