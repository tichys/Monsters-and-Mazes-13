...
/obj/item/organ/internal/cell/Process()
	...
	if(drop)
		if(standing)
			to_chat(owner, "<span class='warning'>I don't have enough energy to function!</span>")
		owner.Weaken(2)
...
/obj/item/organ/internal/cell/attackby(obj/item/weapon/W, mob/user)
	if(isScrewdriver(W))
		if(open)
			open = 0
			to_chat(user, "<span class='notice'>I screw the battery panel in place.</span>")
		else
			open = 1
			to_chat(user, "<span class='notice'>I unscrew the battery panel.</span>")

	if(isCrowbar(W))
		if(open)
			if(cell)
				user.put_in_hands(cell)
				to_chat(user, "<span class='notice'>I remove \the [cell] from \the [src].</span>")
				cell = null
	...
	if (istype(W, /obj/item/weapon/cell))
		if(open)
			if(cell)
				...
				cell = W
				to_chat(user, "<span class = 'notice'>I insert \the [cell].</span>")
...
/obj/item/organ/internal/mmi_holder/proc/transfer_and_delete()
	if(stored_mmi)
		...
		if(persistantMind)
			...
			var/response = input(find_dead_player(ownerckey, 1), "My [initial(stored_mmi.name)] has been removed from my body. Do I wish to return to life?", "Robotic Rebirth") as anything in list("Yes", "No")
			if(response == "Yes")
				persistantMind.transfer_to(stored_mmi.brainmob)
	qdel(src)
