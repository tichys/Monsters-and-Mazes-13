...
/obj/item/organ/internal/posibrain/attack_self(mob/user as mob)
	if(brainmob && !brainmob.key && searching == 0)
		...
		to_chat(user, "<span class='notice'>I carefully locate the manual activation switch and start the positronic brain's boot process.</span>")
...
/obj/item/organ/internal/posibrain/proc/reset_search() //We give the players sixty seconds to decide, then reset the timer.
	...
	for (var/mob/M in viewers(T))
		M.show_message("<span class='notice'>The positronic brain buzzes quietly, and the golden lights fade away. Perhaps I could try again?</span>")
...
/obj/item/organ/internal/posibrain/attack_ghost(var/mob/observer/ghost/user)
	...
	if(!G.assess_candidate(user))
		return
	var/response = alert(user, "Are I sure I wish to possess this [src]?", "Possess [src]", "Yes", "No")
	if(response == "Yes")
