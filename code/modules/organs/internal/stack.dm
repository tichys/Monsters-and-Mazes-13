/mob/living/carbon/human/proc/create_stack()
	...
	to_chat(src, "<span class='notice'>I feel a faint sense of vertigo as my neural lace boots.</span>")
...
/obj/item/organ/internal/stack/replaced()
	...
	if(owner && !backup_inviable())
		...
		var/response = input(find_dead_player(ownerckey, 1), "My neural backup has been placed into a new body. Do I wish to return to life?", "Resleeving") as anything in list("Yes", "No")
...
/obj/item/organ/internal/stack/proc/overwrite()
	if(owner.mind && owner.ckey) //Someone is already in this body!
		...
		if(prob(66))
			to_chat(owner, "<span class='danger'>I fight off the invading tendrils of another mind, holding onto my own body!</span>")
			return
	...
	to_chat(owner, "<span class='notice'>Consciousness slowly creeps over I as my new body awakens.</span>")
