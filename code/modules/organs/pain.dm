...
mob/living/carbon/human/proc/handle_pain()
	...
	if(damaged_organ && chem_effects[CE_PAINKILLER] < maxdam)
		...
		switch(maxdam)
			...
			if(11 to 90)
				msg = "My [damaged_organ.name] [burning ? "burns" : "hurts"] badly!"
			if(91 to 10000)
				msg = "OH GOD! my [damaged_organ.name] is [burning ? "on fire" : "hurting terribly"]!"
	...
	if(prob(1))
		switch(getToxLoss())
			...
			if(17 to 35)
				custom_pain("My body stings.", getToxLoss())
			if(35 to 60)
				custom_pain("My body stings strongly.", getToxLoss())
			if(60 to 100)
				custom_pain("My whole body hurts badly.", getToxLoss())
			if(100 to INFINITY)
				custom_pain("My body aches all over, it's driving I mad.", getToxLoss())