...
No actual turfs are moved, I would need exploration shuttles or teleports to move atoms between different sectors/ships.
Unless stated otherwise, I just need to place any of things below somewhere on the map and they'll handle the rest.
...
1. Make /obj/effect/overmap/sector/[whatever]
	If I want explorations shuttles be able to dock here, remember to set waypoints lists
...
If this zlevel (or any of connected ones for multiz) doesn't have this object, I won't be able to travel there by ovemap means.
### HOW TO USE
1. Create subtype for my ship/sector. Use /ship one for ships.
2. Put it anywhere on the ship/sector map. It will do the rest on its own during init.
If my thing is multiz, only one is needed per multiz sector/ship.
...
If I want exploration shuttles (look below) to be able to dock here, set up waypoints lists.
...
### WHAT IT DOES
Lets I steer ship around on overmap.
Lets I use autopilot.
### HOW TO USE
...
### WHAT IT DOES
Lets use set thrust limits for engines of my ship.
Lets I shutdown/restart the engines.
Lets I check status of engines.
### HOW TO USE
Just place it anywhere on the ship.
...
### WHAT IT DOES
Lets my ship move on the map at all.
### HOW TO USE
...
### WHAT IT DOES
Lets I control shuttles that can change destinations and visit other sectors/ships.
### HOW TO USE
1. Define starting shuttle landmark.
2. Define a /datum/shuttle/autodock/overmap for my shuttle. Same as normal shuttle, aside from 'range' var - how many squares on overmap it can travel on its own.
3. Place console anywhere on the ship/sector. Set shuttle_tag to shuttle's name.
4. Use. I can select destinations if you're in range (on same tile by defualt) on the map and sector has waypoints lists defined
*/