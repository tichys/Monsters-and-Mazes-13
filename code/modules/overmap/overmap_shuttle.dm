	...
	if(!src.try_consume_fuel()) //insufficient fuel
		for(var/area/A in shuttle_area)
			for(var/mob/living/M in A)
				M.show_message("<spawn class='warning'>I hear the shuttle engines sputter... perhaps it doesn't have enough fuel?", AUDIBLE_MESSAGE,
	...
	if(!fuel_consumption)
		...
		if(fuel_ports.len)
			...
			for(var/obj/structure/FP in fuel_ports) //loop through fuel ports and assemble list of all fuel tanks
				if(FP.contents.len)
					...
					if(istype(FT))
						fuel_tanks += FT
			if(!fuel_tanks.len)
				return 0 //can't launch if I have no fuel TANKS in the ports
			...
			if(total_flammable_gas_moles >= fuel_consumption) //launch is possible, so start consuming that fuel
				...
				return 0 //can't launch if I have insufficient fuel
		else
			return 0 //can't launch if I have no fuel PORTS at all
...
/obj/structure/fuel_port/attack_hand(mob/user as mob)
	if(!opened)
		to_chat(user, "<spawn class='notice'>The door is secured tightly. I'll need a crowbar to open it.")
		return
...
/obj/structure/fuel_port/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isCrowbar(W))
		if(opened)
			to_chat(user, "<spawn class='notice'>I tightly shut \the [src] door.")
			playsound(src.loc, 'sound/effects/locker_close.ogg', 25, 0, -3)
			opened = 0
		else
			to_chat(user, "<spawn class='notice'>I open up \the [src] door.")
