...
/obj/item/weapon/paper/admin
	...
	desc = "If I see this, something has gone horribly wrong."
...
/obj/item/weapon/paper/admin/Topic(href, href_list)
	if(href_list["write"])
		...
		var/t =  sanitize(input("Enter what I want to write:", "Write", null, null) as message, free_space, extra = 0)
	...
	if(href_list["confirm"])
		switch(alert("Are I sure I want to send the fax as is?",, "Yes", "No"))
			if("Yes")
				if(headerOn)
					info = header + info
				if(footerOn)
...
/obj/item/weapon/paper/admin/get_signature()
	return input(usr, "Enter the name I wish to sign the paper with (will prompt for multiple entries, in order of entry)", "Signature") as text|null