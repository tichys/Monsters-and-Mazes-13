...
/obj/item/weapon/paper/examine(mob/user)
	...
	if(in_range(user, src) || isghost(user))
		...
		to_chat(user, "<span class='notice'>I have to go closer if I want to read it.</span>")
...
/obj/item/weapon/paper/verb/rename()
	...
	if((CLUMSY in usr.mutations) && prob(50))
		to_chat(usr, "<span class='warning'>I cut myself on the paper.</span>")
		return
	var/n_name = sanitizeSafe(input(usr, "What would I like to label the paper?", "Paper Labelling", null)  as text, MAX_NAME_LEN)
...
/obj/item/weapon/paper/attack(mob/living/carbon/M as mob, mob/living/carbon/user as mob)
	if(user.zone_sel.selecting == BP_EYES)
		user.visible_message("<span class='notice'>I show the paper to [M]. </span>", \
...
/obj/item/weapon/paper/proc/burnpaper(obj/item/weapon/flame/P, mob/user)
	...
	if(P.lit && !user.restrained())
		...
		"<span class='[class]'>I hold \the [P] up to \the [src], burning it slowly.</span>")

		spawn(20)
			if(get_dist(src, user) < 2 && user.get_active_hand() == P && P.lit)
				user.visible_message("<span class='[class]'>[user] burns right through \the [src], turning it to ash. It flutters through the air before settling on the floor in a heap.</span>", \
				"<span class='[class]'>I burn right through \the [src], turning it to ash. It flutters through the air before settling on the floor in a heap.</span>")

				new /obj/effect/decal/cleanable/ash(get_turf(src))
				qdel(src)

			else
				to_chat(user, "<span class='warning'>I must hold \the [P] steady to burn \the [src].</span>")
...
/obj/item/weapon/paper/Topic(href, href_list)
	...
	if(href_list["write"])
		...
		//var/t = strip_html_simple(input(usr, "What text do I wish to add to " + (id=="end" ? "the end of the paper" : "field "+id) + "?", "[name]", null),8192) as message

		if(free_space <= 0)
			to_chat(usr, "<span class='info'>There isn't enough space left on \the [src] to write anything.</span>")
			return

		var/t =  sanitize(input("Enter what I want to write:", "Write", null, null) as message, free_space, extra = 0, trim = 0)
		...
		if(fields > 50)//large amount of fields creates a heavy load on the server, see updateinfolinks() and addtofield()
			to_chat(usr, "<span class='warning'>Too many fields. Sorry, I can't do this.</span>")
...
/obj/item/weapon/paper/attackby(obj/item/weapon/P as obj, mob/user as mob)
	...
	if(istype(P, /obj/item/weapon/paper) || istype(P, /obj/item/weapon/photo))
		...
		to_chat(user, "<span class='notice'>I clip the [P.name] to [(src.name == "paper") ? "the paper" : src.name].</span>")
		...
		to_chat(user, "<span class='notice'>I stamp the paper with my [P.name].</span>")
...
/obj/item/weapon/paper/exodus_cmo
	...
	info = "<I><center>To the incoming CMO of Exodus:</I></center><BR><BR>I wish I and my crew well. Do take note:<BR><BR><BR>The Medical Emergency Red Phone system has proven itself well. Take care to keep the phones in their designated places as they have been optimised for broadcast. The two handheld green radios (I have left one in this office, and one near the Emergency Entrance) are free to be used. The system has proven effective at alerting Medbay of important details, especially during power outages.<BR><BR>I think I may have left the toilet cubicle doors shut. It might be a good idea to open them so the staff and patients know they are not engaged.<BR><BR>The new syringe gun has been stored in secondary storage. I tend to prefer it stored in my office, but 'guidelines' are 'guidelines'.<BR><BR>Also in secondary storage is the grenade equipment crate. I've just realised I've left it open - I may wish to shut it.<BR><BR>There were a few problems with their installation, but the Medbay Quarantine shutters should now be working again  - they lock down the Emergency and Main entrances to prevent travel in and out. Pray I shan't have to use them.<BR><BR>The new version of the Medical Diagnostics Manual arrived. I distributed them to the shelf in the staff break room, and one on the table in the corner of this room.<BR><BR>The exam/triage room has the walking canes in it. I'm not sure why we'd need them - but there I have it.<BR><BR>Emergency Cryo bags are beside the emergency entrance, along with a kit.<BR><BR>Spare paper cups for the reception are on the left side of the reception desk.<BR><BR>I've fed Runtime. She should be fine.<BR><BR><BR><center>That should be all. Good luck!</center>"
