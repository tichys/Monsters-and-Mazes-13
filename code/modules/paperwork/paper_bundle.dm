...
/obj/item/weapon/paper_bundle/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	// merging bundles
		...
		to_chat(user, "<span class='notice'>I add \the [W.name] to [(src.name == "paper bundle") ? "the paper bundle" : src.name].</span>")
		qdel(W)
	else
...
/obj/item/weapon/paper_bundle/proc/insert_sheet_at(mob/user, var/index, obj/item/weapon/sheet)
	...
	to_chat(user, "<span class='notice'>I add [sheet_name] to [bundle_name].</span>")
...
/obj/item/weapon/paper_bundle/Topic(href, href_list)
	...
	if((src in usr.contents) || (istype(src.loc, /obj/item/weapon/folder) && (src.loc in usr.contents)))
		...
		if(href_list["remove"])
			...
			to_chat(usr, "<span class='notice'>I remove the [W.name] from the bundle.</span>")
		...
		to_chat(usr, "<span class='notice'>I need to hold it in hands!</span>")
...
/obj/item/weapon/paper_bundle/verb/rename()
	...
	var/n_name = sanitizeSafe(input(usr, "What would I like to label the bundle?", "Bundle Labelling", null)  as text, MAX_NAME_LEN)
...
/obj/item/weapon/paper_bundle/verb/remove_all()
	...
	to_chat(usr, "<span class='notice'>I loosen the bundle.</span>")
