	...
	if((user == usr && (!( usr.restrained() ) && (!( usr.stat ) && (usr.contents.Find(src) || in_range(src, usr))))))
		if(!istype(usr, /mob/living/carbon/slime) && !istype(usr, /mob/living/simple_animal))
			if( !usr.get_active_hand() )		//if active hand is empty
				...
				to_chat(user, "<span class='notice'>I pick up the [src].</span>")
...
/obj/item/weapon/paper_bin/attack_hand(mob/user as mob)
	...
	var/response = ""
	if(!papers.len > 0)
		response = alert(user, "Do I take regular paper, or Carbon copy paper?", "Paper type request", "Regular", "Carbon-Copy", "Cancel")
...
/obj/item/weapon/paper_bin/attackby(obj/item/weapon/i as obj, mob/user as mob)
	if(istype(i, /obj/item/weapon/paper))
		...
		to_chat(user, "<span class='notice'>I loosen \the [i] and add its papers into \the [src].</span>")
