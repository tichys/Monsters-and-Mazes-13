/obj/machinery/papershredder
	name = "paper shredder"
	desc = "For those documents I don't want seen."
	...
	if(istype(W, /obj/item/weapon/storage))
		...
		if(paper_result)
			if(paperamount == max_paper)
				to_chat(user, "<span class='warning'>\The [src] is full; please empty it before I continue.</span>")
				return
...
/obj/machinery/papershredder/proc/empty_bin(var/mob/living/user, var/obj/item/weapon/storage/empty_into)
	...
	if(empty_into)
		if(paperamount)
			to_chat(user, "<span class='notice'>I fill \the [empty_into] with as much shredded paper as it will carry.</span>")
		else
			to_chat(user, "<span class='notice'>I empty \the [src] into \the [empty_into].</span>")

	else
		to_chat(user, "<span class='notice'>I empty \the [src].</span>")
	update_icon()
...
/obj/item/weapon/shreddedp/proc/burnpaper(var/obj/item/weapon/flame/lighter/P, var/mob/user)
	...
	user.visible_message("<span class='warning'>\The [user] holds \the [P] up to \the [src]. It looks like \he's trying to burn it!</span>", \
		"<span class='warning'>I hold \the [P] up to \the [src], burning it slowly.</span>")
	...
	user.visible_message("<span class='danger'>\The [user] burns right through \the [src], turning it to ash. It flutters through the air before settling on the floor in a heap.</span>", \
		"<span class='danger'>I burn right through \the [src], turning it to ash. It flutters through the air before settling on the floor in a heap.</span>")
	FireBurn()
