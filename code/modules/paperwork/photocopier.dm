...
/obj/machinery/photocopier/Topic(href, href_list)
	if(href_list["copy"])
		...
		if(copyitem)
			...
			to_chat(usr, "<span class='notice'>I take \the [copyitem] out of \the [src].</span>")
			copyitem = null
			updateUsrDialog()
	...
	if(istype(O, /obj/item/weapon/paper) || istype(O, /obj/item/weapon/photo) || istype(O, /obj/item/weapon/paper_bundle))
		if(!copyitem)
			if(!user.unEquip(O, src))
				return
			copyitem = O
			to_chat(user, "<span class='notice'>I insert \the [O] into \the [src].</span>")
		...
		if(toner <= 10) //allow replacing when low toner is affecting the print darkness
			if(!user.unEquip(O, src))
				return
			to_chat(user, "<span class='notice'>I insert the toner cartridge into \the [src].</span>")
...
/obj/machinery/photocopier/proc/copy(var/obj/item/weapon/paper/copy, var/need_toner=1)
	...
	else			//no toner? shitty copies for me!
