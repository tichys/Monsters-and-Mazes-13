...
/obj/item/weapon/photo/attackby(obj/item/weapon/P as obj, mob/user as mob)
	if(istype(P, /obj/item/weapon/pen))
		var/txt = sanitize(input(user, "What would I like to write on the back?", "Photo Writing", null)  as text, 128)
...
/obj/item/weapon/photo/verb/rename()
	...
	var/n_name = sanitizeSafe(input(usr, "What would I like to label the photo?", "Photo Labelling", null)  as text, MAX_NAME_LEN)
...
/obj/item/weapon/storage/photo_album
	...
	storage_slots = DEFAULT_BOX_STORAGE //yes, that's storage_slots. Photos are w_class 1 so this has as many slots equal to the number of photos I could put in a box
...
/obj/item/device/camera/attack_self(mob/user as mob)
	on = !on
	update_icon()
	to_chat(user, "I switch the camera [on ? "on" : "off"].")
	return
...
/obj/item/device/camera/proc/get_mobs(turf/the_turf as turf)
	...
	for(var/mob/living/carbon/A in the_turf)
		...
		if(!mob_detail)
			mob_detail = "I can see [A] on the photo[(A.health / A.maxHealth) < 0.75 ? " - [A] looks hurt":""].[holding ? " [holding]":"."]. "
		else
			mob_detail += "I can also see [A] on the photo[(A.health / A.maxHealth)< 0.75 ? " - [A] looks hurt":""].[holding ? " [holding]":"."]."
	return mob_detail
