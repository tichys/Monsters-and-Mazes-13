...
/obj/machinery/power/apc/attackby(obj/item/W, mob/user)
	...
	if(isCrowbar(W) && opened)
				...
				if (has_electronics==1)
					...
					if ((stat & BROKEN))
						user.visible_message(\
							...
							"<span class='notice'>I broke the charred power control board and remove the remains.</span>",
							"I hear a crack!")
		...
		if(opened)
			if (cell)
				...
				if (has_electronics==1 && terminal)
					...
					to_chat(user, "I screw the circuit electronics into place.")
				else if (has_electronics==2)
					has_electronics = 1
					stat |= MAINT
					playsound(src.loc, 'sound/items/Screwdriver.ogg', 50, 1)
					to_chat(user, "I unfasten the electronics.")
		...
		if(emagged)
			...
			to_chat(user, "I must close the panel")
			...
			if(src.allowed(usr) && !isWireCut(APC_WIRE_IDSCAN))
				locked = !locked
				to_chat(user, "I [ locked ? "lock" : "unlock"] the APC interface.")
				update_icon()
			else
				to_chat(user, "<span class='warning'>Access denied.</span>")
		...
		user.visible_message("<span class='warning'>[user.name] adds cables to the APC frame.</span>", \
							"I start adding cables to the APC frame...")
		...
		if(do_after(user, 50, src))
			if(terminal && opened && has_electronics!=2)
				...
				to_chat(user, "<span class='notice'>I cut the cables and dismantle the power terminal.</span>")
				qdel(terminal)
	else if (istype(W, /obj/item/weapon/module/power_control) && opened && has_electronics==0 && !((stat & BROKEN)))
		user.visible_message("<span class='warning'>[user.name] inserts the power control board into [src].</span>", \
							"I start to insert the power control board into the frame...")
		playsound(src.loc, 'sound/items/Deconstruct.ogg', 50, 1)
		...
		if (((stat & BROKEN) || (hacker && !hacker.hacked_apcs_hidden)) \
			...
			user.visible_message("<span class='danger'>The APC cover was knocked down with the [W.name] by [user.name]!</span>", \
				"<span class='danger'>I knock down the APC cover with my [W.name]!</span>", \
				"I hear a bang")
			update_icon()
		else
			...
			user.visible_message("<span class='danger'>The [src.name] has been hit with the [W.name] by [user.name]!</span>", \
				"<span class='danger'>I hit the [src.name] with my [W.name]!</span>", \
				"I hear a bang")
			if(W.force >= 5 && W.w_class >= ITEM_SIZE_NORMAL && prob(W.force))
				...
				switch(roulette)
					if(1 to 10)
						locked = FALSE
						to_chat(user, "<span class='notice'>I manage to disable the lock on \the [src]!</span>")
					if(50 to 70)
						to_chat(user, "<span class='notice'>I manage to bash the lid open!</span>")
						opened = 1
					if(90 to 100)
		...
		if(opened)
			...
			if (do_after(user,6,src))
				if(prob(50))
					...
					to_chat(user, "<span class='warning'>I fail to [ locked ? "unlock" : "lock"] the APC interface.</span>")
				return 1
...
//		return
	...
	if(istype(user,/mob/living/carbon/human))
		...
		if(H.species.can_shred(H))
			user.visible_message("<span class='warning'>\The [user] slashes at \the [src]!</span>", "<span class='notice'>I slash at \the [src]!</span>")
	...
	if(usr == user && opened && (!issilicon(user)))
		if(cell)
			...
			user.visible_message("<span class='warning'>[user.name] removes the power cell from [src.name]!</span>",\
								 "<span class='notice'>I remove the power cell.</span>")
//			to_chat(user, "I remove the power cell.")
			charging = 0
			src.update_icon()
		return
	if(stat & (BROKEN|MAINT))
...
/obj/machinery/power/apc/proc/can_use(mob/user as mob, var/loud = 0) //used by attack_hand() and Topic()
	if (user.stat)
		to_chat(user, "<span class='warning'>I must be conscious to use [src]!</span>")
		return 0
	if(!user.client)
		return 0
	if(inoperable())
	...
	if(!user.IsAdvancedToolUser())
		return 0
	if(user.restrained())
		to_chat(user, "<span class='warning'>I must have free hands to use [src].</span>")
		return 0
	if(user.lying)
		to_chat(user, "<span class='warning'>I must stand to use [src]!</span>")
		return 0
	autoflag = 5
	if (istype(user, /mob/living/silicon))
	...
	if (istype(H) && prob(H.getBrainLoss()))
		to_chat(user, "<span class='danger'>I momentarily forget how to use [src].</span>")
		return 0
	return 1
...
/obj/machinery/power/apc/Topic(href, href_list)
	...
	if(!istype(usr, /mob/living/silicon) && (locked && !emagged))
		...
		to_chat(usr, "I must unlock the panel to use this!")
		return 1
