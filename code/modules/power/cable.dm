...
/obj/structure/cable/proc/cut_wire(obj/item/W, mob/user)
	...
	if(d1 == UP || d2 == UP)
		to_chat(user, "<span class='warning'>I must cut this cable from above.</span>")
		return
...
//I can use wires to heal robotics
...
/obj/item/stack/cable_coil/proc/set_cable_color(var/selected_color, var/user)
	...
	color = final_color
	to_chat(user, "<span class='notice'>I change \the [src]'s color to [lowertext(selected_color)].</span>")
...
/obj/item/stack/cable_coil/verb/make_restraint()
	...
	if(ishuman(M) && !M.incapacitated())
		...
		if(src.amount <= 14)
			to_chat(usr, "<span class='warning'>I need at least 15 lengths to make restraints!</span>")
			return
		var/obj/item/weapon/handcuffs/cable/B = new /obj/item/weapon/handcuffs/cable(usr.loc)
		B.color = color
		to_chat(usr, "<span class='notice'>I wind some cable together to make some restraints.</span>")
		src.use(15)
	else
		to_chat(usr, "<span class='notice'>I cannot do that.</span>")
	..()
...
/obj/item/stack/cable_coil/proc/turf_place(turf/simulated/F, mob/user)
	...
	if(get_dist(F,user) > 1) // Too far
		to_chat(user, "I can't lay cable at a place that far away.")
		return

	if(!F.is_plating())		// Ff floor is intact, complain
		to_chat(user, "I can't lay cable there unless the floor tiles are removed.")
		return
	...
	if(istype(F, /turf/simulated/open))
		if(!can_use(2))
			to_chat(user, "I don't have enough cable to do this!")
			return
		end_dir = DOWN
