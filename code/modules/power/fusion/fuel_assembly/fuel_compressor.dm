...
/obj/machinery/fusion_fuel_compressor/proc/do_fuel_compression(var/obj/item/thing, var/mob/user)
	if(istype(thing) && thing.reagents && thing.reagents.total_volume && thing.is_open_container())
		...
		if(thing.reagents.total_volume < 50)
			to_chat(user, "<span class='warning'>I need at least fifty units of material to form a fuel rod.</span>")
			return 1
		...
		if(!mat.is_fusion_fuel)
			...
			return
		if(M.get_amount() < 25)
			to_chat(user, "<span class='warning'>I need at least 25 [mat.sheet_plural_name] to make a fuel rod.</span>")
			return
