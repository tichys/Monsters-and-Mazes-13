...
/obj/machinery/light_construct/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if(isWrench(W))
		if (src.stage == 1)
			...
			to_chat(usr, "I begin deconstructing \a [src].")
		...
		if (src.stage == 3)
			to_chat(usr, "I have to unscrew the case first.")
			return
	...
	if(isWirecutter(W))
		...
		user.visible_message("[user.name] removes the wiring from [src].", \
			"I remove the wiring from [src].", "I hear a noise.")
	...
	if(isScrewdriver(W))
		if (src.stage == 2)
			...
			user.visible_message("[user.name] closes [src]'s casing.", \
				"I close [src]'s casing.", "I hear a noise.")
...
/obj/machinery/light/attack_generic(var/mob/user, var/damage)
	...
	if(status == LIGHT_EMPTY || status == LIGHT_BROKEN)
		to_chat(user, "That object is useless to me.")
		return
...
/obj/machinery/light/attackby(obj/item/W, mob/user)
	...
	if(istype(W, /obj/item/weapon/light))
		...
		if(prob(1 + W.force * 5))
			...
			user.visible_message("<span class='warning'>[user.name] smashed the light!</span>", "<span class='warning'>I smash the light!</span>", "I hear a tinkle of breaking glass")
	...
	// attempt to stick weapon into light socket
		...
		if(istype(W, /obj/item/weapon/screwdriver)) //If it's a screwdriver open it.
			playsound(src.loc, 'sound/items/Screwdriver.ogg', 75, 1)
			user.visible_message("[user.name] opens [src]'s casing.", "I open [src]'s casing.", "I hear a noise.")
			new construct_type(src.loc, src.dir, src)
			qdel(src)
			return

		to_chat(user, "I stick \the [W] into the light socket!")
...
/obj/machinery/light/attack_hand(mob/user)
	...
	if(istype(user,/mob/living/carbon/human))
		...
		if(H.species.can_shred(H))
			visible_message("<span class='warning'>[user.name] smashed the light!</span>", 3, "I hear a tinkle of breaking glass")
			broken()
			return
	...
	if(on)
		...
		if(prot > 0 || (COLD_RESISTANCE in user.mutations))
			to_chat(user, "I remove the [get_fitting_name()]")
		else if(TK in user.mutations)
			to_chat(user, "I telekinetically remove the [get_fitting_name()].")
		else
			to_chat(user, "I try to remove the [get_fitting_name()], but it's too hot and I don't want to burn my hand.")
