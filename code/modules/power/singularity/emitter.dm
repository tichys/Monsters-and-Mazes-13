		...
		switch(state)
			if(0)
				...
				user.visible_message("[user.name] secures [src] to the floor.", \
					"I secure the external reinforcing bolts to the floor.", \
					"I hear a ratchet")
				src.anchored = 1
			if(1)
				state = 0
				...
				user.visible_message("[user.name] unsecures [src] reinforcing bolts from the floor.", \
					"I undo the external reinforcing bolts.", \
					"I hear a ratchet")
				src.anchored = 0
			if(2)
			...
			if(1)
				if (WT.remove_fuel(0,user))
					...
					user.visible_message("[user.name] starts to weld [src] to the floor.", \
						"I start to weld [src] to the floor.", \
						"I hear welding")
					if (do_after(user,20,src))
						if(!src || !WT.isOn()) return
						state = 2
						to_chat(user, "I weld [src] to the floor.")
						connect_to_network()
				else
			...
			if(2)
				if (WT.remove_fuel(0,user))
					...
					user.visible_message("[user.name] starts to cut [src] free from the floor.", \
						"I start to cut [src] free from the floor.", \
						"I hear welding")
					if (do_after(user,20,src))
						if(!src || !WT.isOn()) return
						state = 1
						to_chat(user, "I cut [src] free from the floor.")
...
/obj/machinery/power/emitter/emag_act(var/remaining_charges, var/mob/user)
	if(!emagged)
		locked = 0
		emagged = 1
		user.visible_message("[user.name] emags [src].","<span class='warning'>I short out the lock.</span>")
		return 1
