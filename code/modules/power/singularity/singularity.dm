...
/obj/singularity/proc/expand(var/force_size = 0, var/growing = 1)
	...
	switch (temp_allowed_size)
		...
		if (STAGE_TWO) //1 to 3 does not check for the turfs if I put the gens right next to a 1x1 then its going to eat them.
			SetName("gravitational singularity")
			desc = "A gravitational singularity."
		...
		if(STAGE_SUPER)//SUPERSINGULO
			...
			icon_state = "singularity_s11"//uh, whoever drew that, I know that black holes are supposed to look dark right? What's this, the clown's singulo?
			pixel_x = -160
			pixel_y = -160
			grav_pull = 16
			consume_range = 5
		...
		if(M.stat == CONSCIOUS)
			if (istype(M,/mob/living/carbon/human))
				...
				if(istype(H.glasses,/obj/item/clothing/glasses/meson) && current_size != 11)
					to_chat(H, "<span class=\"notice\">I look directly into The [src.name], good thing I had my protective eyewear on!</span>")
					return
				else
					to_chat(H, "<span class=\"warning\">I look directly into The [src.name], but my eyewear does absolutely nothing to protect I from it!</span>")
		to_chat(M, "<span class='danger'>I look directly into The [src.name] and feel [current_size == 11 ? "helpless" : "weak"].</span>")
...
/obj/singularity/proc/smwave()
	for(var/mob/living/M in view(10, src.loc))
		if(prob(67))
			to_chat(M, "<span class=\"warning\">I hear an uneartly ringing, then what sounds like a shrilling kettle as I am washed with a wave of heat.</span>")
			to_chat(M, "<span class=\"notice\">Miraculously, it fails to kill you.</span>")
		else
			to_chat(M, "<span class=\"danger\">I hear an uneartly ringing, then what sounds like a shrilling kettle as I am washed with a wave of heat.</span>")
			to_chat(M, "<span class=\"danger\">I don't even have a moment to react as I am reduced to ashes by the intense radiation.</span>")
			M.dust()
