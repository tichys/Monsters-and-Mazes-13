...
/obj/machinery/power/smes/proc/make_terminal(const/mob/user)
	if (user.loc == loc)
		to_chat(user, "<span class='warning'>I must not be on the same tile as the [src].</span>")
		return 1
	...
	if (istype(tempLoc, /turf/space))
		to_chat(user, "<span class='warning'>I can't build a terminal on space.</span>")
		return 1
	else if (istype(tempLoc))
		if(!tempLoc.is_plating())
			to_chat(user, "<span class='warning'>I must remove the floor plating first.</span>")
			return 1
	if(check_terminal_exists(tempLoc, user, tempDir))
		return 1
	to_chat(user, "<span class='notice'>I start adding cable to the [src].</span>")
...
/obj/machinery/power/smes/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	if (!panel_open)
		to_chat(user, "<span class='warning'>I need to open access hatch on [src] first!</span>")
		return 0
	...
	if(isCoil(W) && !building_terminal)
		...
		if (CC.get_amount() < 10)
			to_chat(user, "<span class='warning'>I need more cables.</span>")
			building_terminal = 0
			return 0
		...
		user.visible_message(\
				...
				"<span class='notice'>I added cables to the [src].</span>")
		stat = 0
		return 0
	...
	if(isWelder(W))
		...
		if(WT.remove_fuel(0,user) && do_after(user, damage, src))
			to_chat(user, "I repair all structural damage to \the [src]")
			damage = 0
		return 0
		...
		if (istype(tempTDir))
			if(!tempTDir.is_plating())
				...
				to_chat(user, "<span class='notice'>I begin to cut the cables...</span>")
				...
				if(do_after(user, 50, src))
					...
					user.visible_message(\
						...
						"<span class='notice'>I cut the cables and dismantle the power terminal.</span>")
					terminals -= term
					qdel(term)
		building_terminal = 0
		return 0
