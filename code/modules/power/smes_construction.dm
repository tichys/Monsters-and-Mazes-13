...
// It also supports RCON System which allows me to operate it remotely, if properly set.
	...
	switch (intensity)
		if (0 to 15)
			...
			if (user_protected && prob(80))
				to_chat(h_user, "Small electrical arc almost burns my hand. Luckily I had my gloves on!")
			else
				to_chat(h_user, "Small electrical arc sparks and burns my hand as I touch the [src]!")
		...
		if (16 to 35)
			...
			if (user_protected && prob(25))
				to_chat(h_user, "Medium electrical arc sparks and almost burns my hand. Luckily I had my gloves on!")
			else
				to_chat(h_user, "Medium electrical sparks as I touch the [src], severely burning my hand!")
		...
		if (36 to 60)
			...
			s.start()
			if (user_protected)
				to_chat(h_user, "Strong electrical arc sparks between I and [src], ignoring my gloves and burning my hand!")
				h_user.adjustFireLoss(rand(25,60))
				h_user.Paralyse(8)
			else
				to_chat(h_user, "Strong electrical arc sparks between I and [src], knocking I out for a while!")
		...
		if (61 to INFINITY)
			...
			s.set_up(10,1,src)
			s.start()
			to_chat(h_user, "Massive electrical arc sparks between I and [src]. Last thing I can think about is \"Oh shit...\"")
...
/obj/machinery/power/smes/buildable/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	// No more disassembling of overloaded SMESs. I broke it, now enjoy the consequences.
	if (failing)
	...
	if (..())
		...
		if(isMultitool(W))
			...
			if(newtag)
				RCon_tag = newtag
				to_chat(user, "<span class='notice'>I changed the RCON tag to: [newtag]</span>")
			return
		...
		if(isCrowbar(W))
			if (terminals.len)
				to_chat(user, "<span class='warning'>I have to disassemble the terminal first!</span>")
				return

			playsound(get_turf(src), 'sound/items/Crowbar.ogg', 50, 1)
			to_chat(user, "<span class='warning'>I begin to disassemble the [src]!</span>")
			if (do_after(usr, 50 * cur_coils, src)) // More coils = takes longer to disassemble. It's complex so largest one with 6 coils will take 30s
				...
				to_chat(usr, "<span class='warning'>I have disassembled the SMES cell!</span>")
		...
		// Superconducting Magnetic Coil - Upgrade the SMES
			...
			if (cur_coils < max_coils)
				...
				if(!user.unEquip(W, src))
					return
				to_chat(usr, "I install the coil into the SMES unit!")
				component_parts += W
				recalc_coils()
			else
				to_chat(usr, "<span class='warning'>I can't insert more coils to this SMES unit!</span>")
