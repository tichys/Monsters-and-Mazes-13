...
/obj/item/ammo_casing/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isScrewdriver(W))
		...
		if(length(label_text) > 20)
			...
			to_chat(user, "<span class='notice'>I scratch the inscription off of [initial(BB)].</span>")
			BB.SetName(initial(BB.name))
		else
			to_chat(user, "<span class='notice'>I inscribe \"[label_text]\" into \the [initial(BB.name)].</span>")
...
/obj/item/ammo_magazine/attack_hand(mob/user)
	if(user.get_inactive_hand() == src)
		if(!stored_ammo.len)
			...
			stored_ammo-=C
			user.put_in_hands(C)
			user.visible_message("\The [user] removes \a [C] from [src].", "<span class='notice'>I remove \a [C] from [src].</span>")
			update_icon()
	else
		..()
		return
