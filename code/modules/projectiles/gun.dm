...
/obj/item/weapon/gun
	...
	var/multi_aim = 0 //Used to determine if I can target multiple people.
...
//Otherwise, if I want handle_click_empty() to be called, check in consume_next_projectile() and return null there.
/obj/item/weapon/gun/proc/special_check(var/mob/user)
	...
	if(HULK in M.mutations)
		to_chat(M, "<span class='danger'>My fingers are much too large for the trigger guard!</span>")
		return 0
		...
		if(P)
			if(process_projectile(P, user, user, pick(BP_L_FOOT, BP_R_FOOT)))
				...
				user.visible_message(
					...
					"<span class='danger'>I shoot myself in the foot with \the [src]!</span>"
					)
				M.unequip_item()
		else
...
/obj/item/weapon/gun/proc/handle_post_fire(mob/user, atom/target, var/pointblank=0, var/reflex=0)
	...
	if(!silenced && user)  // byond was freaking out about this
		if(reflex)
			user.visible_message(
				"<span class='reflex_shoot'><b>\The [user] fires \the [src][pointblank ? " point blank at \the [target]":""] by reflex!</b></span>",
				"<span class='reflex_shoot'>I fire \the [src] by reflex!</span>",
				"I hear a [fire_sound_text]!"
			)
		else
			user.visible_message(
				"<span class='danger'>\The [user] fires \the [src][pointblank ? " point blank at \the [target]":""]!</span>",
				"<span class='warning'>I fire \the [src]!</span>",
				"I hear a [fire_sound_text]!"
				)
			...
			switch(one_hand_penalty)
				if(1)
					if(prob(50)) //don't need to tell them every single time
						to_chat(user, "<span class='warning'>My aim wavers slightly.</span>")
				if(2)
					to_chat(user, "<span class='warning'>My aim wavers as I fire \the [src] with just one hand.</span>")
				if(3)
					to_chat(user, "<span class='warning'>I have trouble keeping \the [src] on target with just one hand.</span>")
				if(4 to INFINITY)
					to_chat(user, "<span class='warning'>I struggle to keep \the [src] on target with just one hand!</span>")
			...
			switch(one_hand_penalty)
				if(1)
					if(prob(50)) //don't need to tell them every single time
						to_chat(user, "<span class='warning'>My aim wavers slightly.</span>")
				if(2)
					to_chat(user, "<span class='warning'>My aim wavers as I try to hold \the [src] steady.</span>")
				if(3)
					to_chat(user, "<span class='warning'>I have trouble holding \the [src] steady.</span>")
				if(4 to INFINITY)
					to_chat(user, "<span class='warning'>I struggle to hold \the [src] steady!</span>")
...
/obj/item/weapon/gun/proc/process_accuracy(obj/projectile, mob/living/user, atom/target, var/burst, var/held_twohanded)
	...
	if (aim_targets && (target in aim_targets))
		//If I aim at someone beforehead, it'll hit more often.
		//Kinda balanced by fact I need like 2 seconds to aim
...
/obj/item/weapon/gun/proc/handle_suicide(mob/living/user)
	...
	if (istype(in_chamber))
		...
		if(istype(in_chamber, /obj/item/projectile/beam/lastertag))
			user.show_message("<span class = 'warning'>I feel rather silly, trying to commit suicide with a toy.</span>")
			mouthshoot = 0
			return
...
/obj/item/weapon/gun/proc/toggle_scope(mob/user, var/zoom_amount=2.0)
	//looking through a scope limits my periphereal vision
...
/obj/item/weapon/gun/proc/toggle_safety(var/mob/user)
	...
	update_icon()
	if(user)
		to_chat(user, "<span class='notice'>I switch the safety [safety_state ? "on" : "off"] on [src].</span>")
		last_safety_check = world.time
