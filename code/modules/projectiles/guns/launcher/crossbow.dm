...
/obj/item/weapon/arrow
	name = "bolt"
	desc = "It's got a tip for I - get the point?"
...
/obj/item/weapon/gun/launcher/crossbow/attack_self(mob/living/user as mob)
	if(tension)
		if(bolt)
			user.visible_message("[user] relaxes the tension on [src]'s string and removes [bolt].","I relax the tension on [src]'s string and remove [bolt].")
...
/obj/item/weapon/gun/launcher/crossbow/proc/draw(var/mob/user as mob)
	...
	current_user = user
	user.visible_message("[user] begins to draw back the string of [src].","<span class='notice'>I begin to draw back the string of [src].</span>")
	tension = 1

	while(bolt && tension && loc == current_user)
		if(!do_after(user, 20, src)) //crossbow strings don't just magically pull back on their own.
			user.visible_message("[usr] stops drawing and relaxes the string of [src].","<span class='warning'>I stop drawing back and relax the string of [src].</span>")
			tension = 0
			update_icon()
			return
		...
		if(tension >= max_tension)
			tension = max_tension
			to_chat(usr, "[src] clunks as I draw the string to its maximum tension!")
			return

		user.visible_message("[usr] draws back the string of [src]!","<span class='notice'>I continue drawing back the string of [src]!</span>")
...
/obj/item/weapon/gun/launcher/crossbow/attackby(obj/item/W as obj, mob/user as mob)
	if(!bolt)
		if (istype(W,/obj/item/weapon/arrow) && user.unEquip(W, src))
			bolt = W
			user.visible_message("[user] slides [bolt] into [src].","I slide [bolt] into [src].")
			update_icon()
			return
			...
			if (R.use(1))
				...
				bolt.loc = src
				update_icon()
				user.visible_message("[user] jams [bolt] into [src].","I jam [bolt] into [src].")
				superheat_rod(user)
			return
...
/obj/item/weapon/crossbowframe/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/stack/rods))
		if(buildstate == 0)
			var/obj/item/stack/rods/R = W
			if(R.use(3))
				to_chat(user, "<span class='notice'>I assemble a backbone of rods around the wooden stock.</span>")
				buildstate++
				update_icon()
			else
				to_chat(user, "<span class='notice'>I need at least three rods to complete this task.</span>")
			return
	else if(isWelder(W))
		if(buildstate == 1)
			...
			if(T.remove_fuel(0,user))
				...
				to_chat(user, "<span class='notice'>I weld the rods into place.</span>")
			buildstate++
			update_icon()
		return
	else if(isCoil(W))
		...
		if(buildstate == 2)
			if(C.use(5))
				to_chat(user, "<span class='notice'>I wire a crude cell mount into the top of the crossbow.</span>")
				buildstate++
				update_icon()
			else
				to_chat(user, "<span class='notice'>I need at least five segments of cable coil to complete this task.</span>")
			return
		else if(buildstate == 4)
			if(C.use(5))
				to_chat(user, "<span class='notice'>I string a steel cable across the crossbow's lath.</span>")
				buildstate++
				update_icon()
			else
				to_chat(user, "<span class='notice'>I need at least five segments of cable coil to complete this task.</span>")
		...
		if(buildstate == 3)
			...
			if(P.use(3))
				to_chat(user, "<span class='notice'>I assemble and install a heavy plastic lath onto the crossbow.</span>")
				buildstate++
				update_icon()
			else
				to_chat(user, "<span class='notice'>I need at least three plastic sheets to complete this task.</span>")
			return
	else if(isScrewdriver(W))
		if(buildstate == 5)
			to_chat(user, "<span class='notice'>I secure the crossbow's various parts.</span>")
