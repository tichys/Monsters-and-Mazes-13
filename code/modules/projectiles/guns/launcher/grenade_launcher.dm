...
/obj/item/weapon/gun/launcher/grenade/proc/pump(mob/M as mob)
	...
	if(next)
		...
		chambered = next
		to_chat(M, "<span class='warning'>I pump [src], loading \a [next] into the chamber.</span>")
	else
		to_chat(M, "<span class='warning'>I pump [src], but the magazine is empty.</span>")
	update_icon()
...
/obj/item/weapon/gun/launcher/grenade/proc/load(obj/item/weapon/grenade/G, mob/user)
	...
	user.visible_message("\The [user] inserts \a [G] into \the [src].", "<span class='notice'>I insert \a [G] into \the [src].</span>")
...
/obj/item/weapon/gun/launcher/grenade/proc/unload(mob/user)
	if(grenades.len)
		...
		grenades.len--
		user.put_in_hands(G)
		user.visible_message("\The [user] removes \a [G] from [src].", "<span class='notice'>I remove \a [G] from \the [src].</span>")
	else
...
/obj/item/weapon/gun/launcher/grenade/underslung/load(obj/item/weapon/grenade/G, mob/user)
	...
	if(!user.unEquip(G, src))
		return
	chambered = G
	user.visible_message("\The [user] load \a [G] into \the [src].", "<span class='notice'>I load \a [G] into \the [src].</span>")

/obj/item/weapon/gun/launcher/grenade/underslung/unload(mob/user)
	if(chambered)
		user.put_in_hands(chambered)
		user.visible_message("\The [user] removes \a [chambered] from \the[src].", "<span class='notice'>I remove \a [chambered] from \the [src].</span>")
		chambered = null
	else
		to_chat(user, "<span class='warning'>\The [src] is empty.</span>")
