/obj/item/weapon/gun/launcher/money
	...
	description_info = "Load money into the cannon by picking it up with the gun, or feeding it directly by hand. Use in my hand to configure how much money I want to fire per shot."
	...
	description_antag = "Sliding a cryptographic sequencer into the receptacle will short the motors and override their speed. If I set the cannon to dispense 100 thaler or more, this might make a handy weapon."
...
/obj/item/weapon/gun/launcher/money/proc/unload_receptacle(mob/user)
	...
	to_chat(user, "<span class='notice'>I eject [receptacle_value] thaler from [src]'s receptacle.</span>")
	receptacle_value = 0
...
/obj/item/weapon/gun/launcher/money/proc/absorb_cash(var/obj/item/weapon/spacecash/bling, mob/user)
	...
	to_chat(user, "<span class='notice'>I load [bling] into [src].</span>")
	qdel(bling)
...
/obj/item/weapon/gun/launcher/money/attack_self(mob/user as mob)
	var/disp_amount = min(input(user, "How many thaler do I want to dispense at a time? (0 to [src.receptacle_value])", "Money Cannon Settings", 20) as num, receptacle_value)

	if (disp_amount < 1)
		to_chat(user, "<span class='warning'>I have to dispense at least one thaler at a time!</span>")
		return

	src.dispensing = disp_amount
	to_chat(user, "<span class='notice'>I set [src] to dispense [dispensing] thaler at a time.</span>")
...
/obj/item/weapon/gun/launcher/money/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/spacecash/))
		...
		if(bling.worth < 1)
			to_chat(user, "<span class='warning'>I can't seem to get the bills to slide into the receptacle.</span>")
			return

		receptacle_value += bling.worth
		to_chat(user, "<span class='notice'>I slide [bling.worth] thaler into [src]'s receptacle.</span>")
		qdel(bling)
...
/obj/item/weapon/gun/launcher/money/emag_act(var/remaining_charges, var/mob/user)
	...
	if(!emagged)
		emagged = 1
		to_chat(user, "<span class='notice'>I slide the sequencer into [src]... only for it to spit it back out and emit a motorized squeal!</span>")
		var/datum/effect/effect/system/spark_spread/s = new()
		s.set_up(3, 1, src)
		s.start()
	else
