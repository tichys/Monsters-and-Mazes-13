...
/obj/item/weapon/gun/launcher/net/proc/load(obj/item/weapon/net_shell/S, mob/user)
	if(chambered)
		...
		return
	if(!user.unEquip(S, src))
		return
	chambered = S
	user.visible_message("\The [user] inserts \a [S] into \the [src].", "<span class='notice'>I insert \a [S] into \the [src].</span>")

/obj/item/weapon/gun/launcher/net/proc/unload(mob/user)
	if(chambered)
		user.visible_message("\The [user] removes \the [chambered] from \the [src].", "<span class='notice'>I remove \the [chambered] from \the [src].</span>")
