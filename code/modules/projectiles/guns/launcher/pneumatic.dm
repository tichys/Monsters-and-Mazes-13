/obj/item/weapon/gun/launcher/pneumatic
	...
	var/force_divisor = 400                             // Force equates to speed. Speed/5 equates to a damage multiplier for whoever I hit.
...
/obj/item/weapon/gun/launcher/pneumatic/verb/set_pressure() //set amount of tank pressure.
	...
	if (N)
		pressure_setting = N
		to_chat(usr, "I dial the pressure valve to [pressure_setting]%.")
...
/obj/item/weapon/gun/launcher/pneumatic/proc/eject_tank(mob/user) //Remove the tank.
	...
	to_chat(user, "I twist the valve and pop the tank out of [src].")
...
/obj/item/weapon/gun/launcher/pneumatic/proc/unload_hopper(mob/user)
	if(item_storage.contents.len > 0)
		...
		to_chat(user, "I remove [removing] from the hopper.")
	else
...
/obj/item/weapon/gun/launcher/pneumatic/attackby(obj/item/W as obj, mob/user as mob)
	if(!tank && istype(W,/obj/item/weapon/tank) && user.unEquip(W, src))
		tank = W
		user.visible_message("[user] jams [W] into [src]'s valve and twists it closed.","I jam [W] into [src]'s valve and twist it closed.")
		update_icon()
...
/obj/item/weapon/cannonframe/attackby(obj/item/W as obj, mob/user as mob)
	if(istype(W,/obj/item/pipe))
		if(buildstate == 0)
			qdel(W)
			to_chat(user, "<span class='notice'>I secure the piping inside the frame.</span>")
			buildstate++
			update_icon()
			return
		...
		if(buildstate == 2)
			...
			if(M.use(5))
				to_chat(user, "<span class='notice'>I assemble a chassis around the cannon frame.</span>")
				buildstate++
				update_icon()
			else
				to_chat(user, "<span class='notice'>I need at least five metal sheets to complete this task.</span>")
			return
	else if(istype(W,/obj/item/device/transfer_valve))
		if(buildstate == 4)
			qdel(W)
			to_chat(user, "<span class='notice'>I install the transfer valve and connect it to the piping.</span>")
			buildstate++
			update_icon()
			return
	else if(isWelder(W))
		if(buildstate == 1)
			...
			if(T.remove_fuel(0,user))
				...
				to_chat(user, "<span class='notice'>I weld the pipe into place.</span>")
				buildstate++
				update_icon()
		if(buildstate == 3)
			var/obj/item/weapon/weldingtool/T = W
			if(T.remove_fuel(0,user))
				if(!src || !T.isOn()) return
				playsound(src.loc, 'sound/items/Welder2.ogg', 100, 1)
				to_chat(user, "<span class='notice'>I weld the metal chassis together.</span>")
				buildstate++
				update_icon()
		if(buildstate == 5)
			var/obj/item/weapon/weldingtool/T = W
			if(T.remove_fuel(0,user))
				if(!src || !T.isOn()) return
				playsound(src.loc, 'sound/items/Welder2.ogg', 100, 1)
				to_chat(user, "<span class='notice'>I weld the valve into place.</span>")
...
/obj/item/weapon/gun/launcher/pneumatic/small
	...
	desc = "It looks smaller than my garden variety cannon"
