...
/obj/item/weapon/syringe_cartridge/attackby(obj/item/I, mob/user)
	if(istype(I, /obj/item/weapon/reagent_containers/syringe) && user.unEquip(I, src))
		syringe = I
		to_chat(user, "<span class='notice'>I carefully insert [syringe] into [src].</span>")
		sharp = 1
		name = "syringe dart"
		update_icon()
...
/obj/item/weapon/syringe_cartridge/attack_self(mob/user)
	if(syringe)
		to_chat(user, "<span class='notice'>I remove [syringe] from [src].</span>")
...
/obj/item/weapon/gun/launcher/syringe/attack_self(mob/living/user as mob)
	if(next)
		user.visible_message("[user] unlatches and carefully relaxes the bolt on [src].", "<span class='warning'>I unlatch and carefully relax the bolt on [src], unloading the spring.</span>")
		next = null
	else if(darts.len)
		playsound(src.loc, 'sound/weapons/flipblade.ogg', 50, 1)
		user.visible_message("[user] draws back the bolt on [src], clicking it into place.", "<span class='warning'>I draw back the bolt on the [src], loading the spring!</span>")
		next = darts[1]
