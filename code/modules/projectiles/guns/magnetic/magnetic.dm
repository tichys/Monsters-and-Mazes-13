/obj/item/weapon/gun/magnetic
	...
	var/gun_unreliable = 15                                    // Percentage chance of detonating in my hands.
	...
	var/load_sheet_max = 1									   // Maximum number of "sheets" I can load from a stack.
...
/obj/item/weapon/gun/magnetic/examine(var/mob/user)
	...
	if(.)
		...
		if(!cell || !capacitor)
			to_chat(user, "<span class='notice'>The capacitor charge indicator is blinking <font color ='[COLOR_RED]'>red</font>. Maybe I should check the cell or capacitor.</span>")
		else
