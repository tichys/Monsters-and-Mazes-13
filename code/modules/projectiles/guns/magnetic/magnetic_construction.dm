...
/obj/item/weapon/coilgun_assembly/attackby(var/obj/item/thing, var/mob/user)
	...
	if(istype(thing, /obj/item/stack/material) && construction_stage == 1)
		...
		if(reinforcing_with.name == DEFAULT_WALL_MATERIAL) // Steel
			if(reinforcing.get_amount() < 5)
				to_chat(user, "<span class='warning'>I need at least 5 [reinforcing.singular_name]\s for this task.</span>")
				return
			reinforcing.use(5)
	...
	if(isCoil(thing) && construction_stage == 5)
		...
		if(cable.get_amount() < 5)
			to_chat(user, "<span class='warning'>I need at least 5 lengths of cable for this task.</span>")
			return
		cable.use(5)
