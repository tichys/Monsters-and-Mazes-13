	...
	if(!is_jammed && prob(jam_chance))
		...
		if(istype(user))
			if(prob(user.skill_fail_chance(SKILL_WEAPONS, 100, SKILL_PROF)))
				...
				to_chat(user, "<span class='notice'>I reflexively clear the jam on \the [src].</span>")
				is_jammed = 0
...
/obj/item/weapon/gun/projectile/proc/load_ammo(var/obj/item/A, mob/user)
	if(istype(A, /obj/item/ammo_magazine))
		...
		switch(AM.mag_type)
			if(MAGAZINE)
				...
				if(!user.unEquip(AM, src))
					return
				ammo_magazine = AM
				user.visible_message("[user] inserts [AM] into [src].", "<span class='notice'>I insert [AM] into [src].</span>")
			...
			if(SPEEDLOADER)
				...
				for(var/obj/item/ammo_casing/C in AM.stored_ammo)
					...
					if(C.caliber == caliber)
						...
						count++
				if(count)
					user.visible_message("[user] reloads [src].", "<span class='notice'>I load [count] round\s into [src].</span>")
		...
		user.visible_message("[user] inserts \a [C] into [src].", "<span class='notice'>I insert \a [C] into [src].</span>")
...
/obj/item/weapon/gun/projectile/proc/unload_ammo(mob/user, var/allow_dump=1)
	if(is_jammed)
		user.visible_message("\The [user] begins to unjam [src].", "I clear the jam and unload [src]")
	...
	if(ammo_magazine)
		...
		user.visible_message("[user] removes [ammo_magazine] from [src].", "<span class='notice'>I remove [ammo_magazine] from [src].</span>")
		...
		if(allow_dump && (load_method & SPEEDLOADER))
			...
			if(T)
				...
				loaded.Cut()
			if(count)
				user.visible_message("[user] unloads [src].", "<span class='notice'>I unload [count] round\s from [src].</span>")
		else if(load_method & SINGLE_CASING)
			var/obj/item/ammo_casing/C = loaded[loaded.len]
			loaded.len--
			user.put_in_hands(C)
			user.visible_message("[user] removes \a [C] from [src].", "<span class='notice'>I remove \a [C] from [src].</span>")
	else
