...
/obj/item/weapon/gun/projectile/dartgun/proc/add_beaker(var/obj/item/weapon/reagent_containers/glass/B, mob/user)
	...
	if(!user.unEquip(B, src))
		return
	beakers |= B
	user.visible_message("\The [user] inserts \a [B] into [src].", "<span class='notice'>I slot [B] into [src].</span>")

/obj/item/weapon/gun/projectile/dartgun/proc/remove_beaker(var/obj/item/weapon/reagent_containers/glass/B, mob/user)
	mixing -= B
	beakers -= B
	user.put_in_hands(B)
	user.visible_message("\The [user] removes \a [B] from [src].", "<span class='notice'>I remove [B] from [src].</span>")
