...
/obj/item/weapon/gun/projectile/colt/detective/verb/rename_gun()
	set name = "Name Gun"
	set category = "Object"
	set desc = "Rename my gun. If you're the detective."
	...
	var/input = sanitizeSafe(input("What do I want to name the gun?","Rename gun"), MAX_NAME_LEN)
...
/obj/item/weapon/gun/projectile/colt/detective/verb/reskin_gun()
	set name = "Reskin gun"
	set category = "Object"
	set desc = "Click to reskin my gun."

	var/mob/M = usr
	if(M.incapacitated())
		return

	var/datum/detective_gun_skin/choice = input(M,"What do I want to skin the gun to?","Reskin Gun", unique_reskin) as null|anything in gun_options
	if(src && choice && !M.incapacitated() && in_range(M,src))
		icon_state = choice.icon_state
		unique_reskin = choice
		if(!unique_name)
			SetName(choice.name)
		to_chat(M, "My gun is now skinned as \a [choice]. Say hello to my new friend.")
		return 1
