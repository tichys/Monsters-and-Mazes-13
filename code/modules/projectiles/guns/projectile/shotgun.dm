...
/obj/item/weapon/gun/projectile/shotgun/doublebarrel/attackby(var/obj/item/A as obj, mob/user as mob)
	if(w_class > 3 && (istype(A, /obj/item/weapon/circular_saw) || istype(A, /obj/item/weapon/melee/energy) || istype(A, /obj/item/weapon/gun/energy/plasmacutter)))
		...
		if(do_after(user, 30, src))	//SHIT IS STEALTHY EYYYYY
			...
			force = 5
			one_hand_penalty = 0
			slot_flags &= ~SLOT_BACK	//I can't sling it on my back
			slot_flags |= (SLOT_BELT|SLOT_HOLSTER) //but I can wear it on my belt (poorly concealed under a trenchcoat, ideally) - or in a holster, why not.
