...
/obj/item/weapon/gun/projectile/heavysniper/handle_post_fire(mob/user, atom/target, var/pointblank=0, var/reflex=0)
	...
	if(user && user.skill_check(SKILL_WEAPONS, SKILL_PROF))
		to_chat(user, "<span class='notice'>I work the bolt open with a reflexive motion, ejecting [chambered]!</span>")
		unload_shell()
...
/obj/item/weapon/gun/projectile/heavysniper/attack_self(mob/user as mob)
	...
	if(bolt_open)
		if(chambered)
			to_chat(user, "<span class='notice'>I work the bolt open, ejecting [chambered]!</span>")
			unload_shell()
		else
			to_chat(user, "<span class='notice'>I work the bolt open.</span>")
	else
		to_chat(user, "<span class='notice'>I work the bolt closed.</span>")
...
/obj/item/weapon/gun/projectile/heavysniper/special_check(mob/user)
	if(bolt_open)
		to_chat(user, "<span class='warning'>I can't fire [src] while the bolt is open!</span>")
		return 0
	return ..()
