...
/obj/item/projectile/proc/check_fire(atom/target as mob, var/mob/living/user as mob)  //Checks if I can hit them or not.
...
/obj/item/projectile/proc/launch_from_gun(atom/target, mob/user, obj/item/weapon/gun/launcher, var/target_zone, var/x_offset=0, var/y_offset=0)
	if(user == target) //Shooting myself
...
/obj/item/projectile/proc/attack_mob(var/mob/living/target_mob, var/distance, var/miss_modifier=0)
	...
	//hit messages
	if(silenced)
		to_chat(target_mob, "<span class='danger'>I have been hit in the [parse_zone(def_zone)] by \the [src]!</span>")
	else
		target_mob.visible_message("<span class='danger'>\The [target_mob] is hit by \the [src] in the [parse_zone(def_zone)]!</span>")//X has fired Y is now given by the guns so I cant tell who shot I if I could not see the shooter
...
//"Tracing" projectile
/obj/item/projectile/test //Used to see if I can hit them.
...
//Helper proc to check if I can hit them or not.
