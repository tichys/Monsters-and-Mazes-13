...
/obj/item/weapon/gun/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(istype(W, /obj/item/weapon/card/id) && is_secure_gun())
		if(!registered_owner)
			...
			user.visible_message("[user] swipes an ID through \the [src], registering it.", "I swipe an ID through \the [src], registering it.")
		else
			to_chat(user, "This weapon is already registered, I must reset it first.")
	else
		..()
...
/obj/item/weapon/gun/emag_act(var/charges, var/mob/user)
	if(!charges)
		...
		req_access.Cut()
		req_one_access.Cut()
		to_chat(user, "The authorization chip fries, giving I full use of \the [src].")
		return 1
	else
		return ..()
...
/obj/item/weapon/gun/proc/reset_registration()
	...
	if(allowed(usr))
		usr.visible_message("[usr] presses the reset button on \the [src], resetting its registration.", "I press the reset button on \the [src], resetting its registration.")
		registered_owner = null
		GLOB.registered_weapons -= src
		verbs -= /obj/item/weapon/gun/proc/reset_registration
...
/obj/item/weapon/gun/proc/authorize(var/mode, var/authorized, var/by)
	...
	if(M)
		to_chat(M, "<span class='notice'>my [src.name] has been [authorized ? "granted" : "denied"] [firemodes[mode]] fire authorization by [by].</span>")
