...
/obj/aiming_overlay/proc/toggle_permission(var/perm)
	...
	var/aim_message = "<span class='[use_span]'>[aiming_at ? "\The [aiming_at] is" : "My targets are"] [message].</span>"
	to_chat(owner, aim_message)
	if(aiming_at)
		to_chat(aiming_at, "<span class='[use_span]'>I am [message].</span>")
...
/obj/aiming_overlay/proc/update_aiming()
	...
	if(!(aiming_with in owner) || (istype(owner, /mob/living/carbon/human) && (owner.l_hand != aiming_with && owner.r_hand != aiming_with)))
		to_chat(owner, "<span class='warning'>I must keep hold of my weapon!</span>")
	else if(owner.eye_blind)
		to_chat(owner, "<span class='warning'>I am blind and cannot see my target!</span>")
	else if(!aiming_at || !istype(aiming_at.loc, /turf))
		to_chat(owner, "<span class='warning'>I have lost sight of my target!</span>")
	else if(owner.incapacitated() || owner.lying || owner.restrained())
		to_chat(owner, "<span class='warning'>I must be conscious and standing to keep track of my target!</span>")
	else if(aiming_at.is_invisible_to(owner))
		to_chat(owner, "<span class='warning'>My target has become invisible!</span>")
	else if(!(aiming_at in view(owner)))
		to_chat(owner, "<span class='warning'>My target is too far away to track!</span>")
	else
		cancel_aim = 0
...
/obj/aiming_overlay/proc/aim_at(var/mob/target, var/obj/thing)
	...
	if(owner.incapacitated())
		to_chat(owner, "<span class='warning'>I cannot aim a gun in my current state.</span>")
		return
	if(owner.lying)
		to_chat(owner, "<span class='warning'>I cannot aim a gun while prone.</span>")
		return
	if(owner.restrained())
		to_chat(owner, "<span class='warning'>I cannot aim a gun while handcuffed.</span>")
		return
	var/head_aim = owner.zone_sel.selecting == BP_HEAD ? "'s head" : ""
	if(aiming_at)
		if(aiming_at == target)
			return
		cancel_aiming(1)
		owner.visible_message("<span class='danger'>\The [owner] turns \the [thing] on \the [target][head_aim]!</span>")
	else
		owner.visible_message("<span class='danger'>\The [owner] aims \the [thing] at \the [target][head_aim]!</span>")

	if(owner.client)
		owner.client.add_gun_icons()
	to_chat(target, "<span class='danger'>I now have a gun pointed at you. No sudden moves!</span>")
	aiming_with = thing
	aiming_at = target
...
/obj/aiming_overlay/proc/toggle_active(var/force_state = null)
	...
	if(owner.client)
		if(active)
			to_chat(owner, "<span class='notice'>I will now aim rather than fire.</span>")
			owner.client.add_gun_icons()
		else
			to_chat(owner, "<span class='notice'>I will no longer aim rather than fire.</span>")
