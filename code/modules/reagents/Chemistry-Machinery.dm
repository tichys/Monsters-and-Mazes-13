...
/obj/machinery/chem_master/attackby(var/obj/item/weapon/B as obj, var/mob/user as mob)
	...
	if(istype(B, /obj/item/weapon/reagent_containers/glass))
		...
		to_chat(user, "I add the pill bottle into the dispenser slot!")
		src.updateUsrDialog()
...
/obj/machinery/chem_master/Topic(href, href_list, state)
	...
	if(beaker)
		...
		if (href_list["analyze"])
			...
			var/name = sanitizeSafe(input(usr,"Name:","Name my pill!","[reagents.get_master_reagent_name()] ([amount_per_pill]u)"), MAX_NAME_LEN)
...
/obj/machinery/chem_master/proc/create_bottle(mob/user)
	var/name = sanitizeSafe(input(usr,"Name:","Name my bottle!",reagents.get_master_reagent_name()), MAX_NAME_LEN)
...
/obj/machinery/reagentgrinder/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(istype(O,/obj/item/weapon/storage/plants))
		...
		if(!O.contents.len)
			to_chat(user, "I empty \the [O] into \the [src].")
		else
			to_chat(user, "I fill \the [src] from \the [O].")
...
/obj/machinery/reagentgrinder/proc/hurt_hand(mob/living/carbon/human/user)
	...
	var/dam = rand(10, 15)
	user.visible_message("<span class='danger'>\The [user]'s hand gets caught in \the [src]!</span>", "<span class='danger'>My hand gets caught in \the [src]!</span>")
