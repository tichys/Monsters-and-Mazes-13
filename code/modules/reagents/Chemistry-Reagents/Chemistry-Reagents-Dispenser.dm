...
/datum/reagent/ethanol/affect_ingest(var/mob/living/carbon/M, var/alien, var/removed)
	...
	var/effective_dose = M.chem_doses[type] * strength_mod * (1 + volume/60) //drinking a LOT will make I go down faster
...
/datum/reagent/radium/affect_blood(var/mob/living/carbon/M, var/alien, var/removed)
	M.apply_effect(10 * removed, IRRADIATE, blocked = 0) // Radium may increase my chances to cure a disease
	if(M.virus2.len)
		for(var/ID in M.virus2)
			...
			if(prob(5))
				...
				if(prob(50))
					M.apply_effect(50, IRRADIATE, blocked = 0) // curing it that way may kill I instead
					var/absorbed = 0
	...
	if(ishuman(M))
		...
		if(H.head)
			if(H.head.unacidable)
				to_chat(H, "<span class='danger'>my [H.head] protects I from the acid.</span>")
				remove_self(volume)
				return
			else if(removed > meltdose)
				to_chat(H, "<span class='danger'>my [H.head] melts away!</span>")
				qdel(H.head)
				H.update_inv_head(1)
				H.update_hair(1)
				removed -= meltdose
		...
		if(H.wear_mask)
			if(H.wear_mask.unacidable)
				to_chat(H, "<span class='danger'>my [H.wear_mask] protects I from the acid.</span>")
				remove_self(volume)
				return
			else if(removed > meltdose)
				to_chat(H, "<span class='danger'>my [H.wear_mask] melts away!</span>")
				qdel(H.wear_mask)
		...
		if(H.glasses)
			if(H.glasses.unacidable)
				to_chat(H, "<span class='danger'>my [H.glasses] partially protect I from the acid!</span>")
				removed /= 2
			else if(removed > meltdose)
				to_chat(H, "<span class='danger'>my [H.glasses] melt away!</span>")
				qdel(H.glasses)
