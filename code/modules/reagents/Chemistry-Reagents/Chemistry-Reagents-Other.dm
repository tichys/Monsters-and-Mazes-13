		...
		if(iscultist(M))
			...
			if(volume > 5)
				...
				if(prob(10)) //Only annoy them a /bit/
					to_chat(M,"<span class='danger'>I feel my insides curdle and burn!</span> \[<a href='?src=\ref[src];deconvert=\ref[M]'>Give Into Purity</a>\]")
...
/datum/reagent/helium
	name = "Helium"
	description = "A noble gas. It makes my voice squeaky."
...
/datum/reagent/carbon_dioxide/affect_blood(var/mob/living/carbon/human/M, var/alien, var/removed)
	...
	if(warning_message && prob(warning_prob))
		to_chat(M, "<span class='warning'>I feel [warning_message].</span>")
