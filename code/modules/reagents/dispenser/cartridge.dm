...
/obj/item/weapon/reagent_containers/chem_disp_cartridge/proc/setLabel(L, mob/user = null)
	if(L)
		if(user)
			to_chat(user, "<span class='notice'>I set the label on \the [src] to '[L]'.</span>")

		label = L
		SetName("[initial(name)] - '[L]'")
	else
		if(user)
			to_chat(user, "<span class='notice'>I clear the label on \the [src].</span>")
		label = ""
...
/obj/item/weapon/reagent_containers/chem_disp_cartridge/attack_self()
	..()
	if (is_open_container())
		to_chat(usr, "<span class = 'notice'>I put the cap on \the [src].</span>")
		atom_flags ^= ATOM_FLAG_OPEN_CONTAINER
	else
		to_chat(usr, "<span class = 'notice'>I take the cap off \the [src].</span>")
		atom_flags |= ATOM_FLAG_OPEN_CONTAINER
...
/obj/item/weapon/reagent_containers/chem_disp_cartridge/afterattack(obj/target, mob/user , flag)
	if (!is_open_container() || !flag)
		...
		to_chat(user, "<span class='notice'>I fill \the [src] with [trans] units of the contents of \the [target].</span>")
		...
		to_chat(user, "<span class='notice'>I transfer [trans] units of the solution to \the [target].</span>")
