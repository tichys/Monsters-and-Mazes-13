...
/obj/machinery/chemical_dispenser/proc/add_cartridge(obj/item/weapon/reagent_containers/chem_disp_cartridge/C, mob/user)
	...
	if(user)
		if(user.unEquip(C))
			to_chat(user, "<span class='notice'>I add \the [C] to \the [src].</span>")
		else
			return
...
/obj/machinery/chemical_dispenser/attackby(obj/item/weapon/W, mob/user)
	if(istype(W, /obj/item/weapon/reagent_containers/chem_disp_cartridge))
		...
		var/label = input(user, "Which cartridge would I like to remove?", "Chemical Dispenser") as null|anything in cartridges
		if(!label) return
		var/obj/item/weapon/reagent_containers/chem_disp_cartridge/C = remove_cartridge(label)
		if(C)
			to_chat(user, "<span class='notice'>I remove \the [C] from \the [src].</span>")
			C.loc = loc
		...
		if(!RC.is_open_container())
			to_chat(user, "<span class='warning'>I don't see how \the [src] could dispense reagents into \the [RC].</span>")
			return
		...
		RC.loc = src
		update_icon()
		to_chat(user, "<span class='notice'>I set \the [RC] on \the [src].</span>")
