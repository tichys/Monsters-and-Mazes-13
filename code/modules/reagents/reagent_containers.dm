...
/obj/item/weapon/reagent_containers/proc/standard_splash_mob(var/mob/user, var/mob/target) // This goes into afterattack
	...
	if(user.a_intent == I_HELP)
		to_chat(user, "<span class='notice'>I can't splash people on help intent.</span>")
		return 1
	...
	user.visible_message("<span class='danger'>[target] has been splashed with something by [user]!</span>", "<span class = 'notice'>I splash the solution onto [target].</span>")
	reagents.splash(target, reagents.total_volume)
	return 1

/obj/item/weapon/reagent_containers/proc/self_feed_message(var/mob/user)
	to_chat(user, "<span class='notice'>I eat \the [src]</span>")
...
/obj/item/weapon/reagent_containers/proc/standard_feed_mob(var/mob/user, var/mob/target) // This goes into attack
	...
	if(istype(target, /mob/living/carbon))
		if(target == user)
			...
			if(!H.check_has_mouth())
				to_chat(user, "Where do I intend to put \the [src]? \The [H] doesn't have a mouth!")
				return
			var/obj/item/blocked = H.check_mouth_coverage()
			if(blocked)
				to_chat(user, "<span class='warning'>\The [blocked] is in the way!</span>")
...
/obj/item/weapon/reagent_containers/proc/standard_pour_into(var/mob/user, var/atom/target) // This goes into afterattack and yes, it's atom-level
	...
	to_chat(user, "<span class='notice'>I transfer [trans] unit\s of the solution to \the [target].</span>")
	return 1
...
/obj/item/weapon/reagent_containers/examine(mob/user)
	...
	if(hasHUD(user, HUD_SCIENCE))
		...
		to_chat(user, "<span class='notice'>Using my chemistry knowledge, I indentify the following reagents in \the [src]: [reagents.get_reagents(!user.skill_check(SKILL_CHEMISTRY, SKILL_PROF), 5)].</span>")