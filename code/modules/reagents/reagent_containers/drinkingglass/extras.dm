	...
	if(istype(I, /obj/item/weapon/glass_extra))
		...
		if(can_add_extra(GE))
			...
			if(!user.unEquip(GE, src))
				return
			to_chat(user, "<span class=notice>I add \the [GE] to \the [src].</span>")
			update_icon()
		else
		...
		FS.pixel_y = 0
		to_chat(user, "<span class=notice>I add \the [FS] to \the [src].</span>")
		update_icon()
	else
		return ..()
...
/obj/item/weapon/reagent_containers/food/drinks/glass2/attack_hand(mob/user as mob)
	...
	var/choice = input(user, "What would I like to remove from the glass?") as null|anything in extras
	if(!choice || !(choice in extras))
		return

	if(user.put_in_active_hand(choice))
		to_chat(user, "<span class=notice>I remove \the [choice] from \the [src].</span>")
		extras -= choice
	else
