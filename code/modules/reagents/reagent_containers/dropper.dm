...
/obj/item/weapon/reagent_containers/dropper
	...
	afterattack(var/obj/target, var/mob/user, var/proximity)
		...
		if(reagents.total_volume)
			...
			if(!target.is_open_container() && !ismob(target) && !istype(target, /obj/item/weapon/reagent_containers/food) && !istype(target, /obj/item/clothing/mask/smokable/cigarette)) //I can inject humans and food but I cant remove the shit.
			...
			if(ismob(target))
				...
				if(istype(target, /mob/living/carbon/human))
					...
					if(safe_thing)
						...
						user.visible_message("<span class='warning'>[user] tries to squirt something into [target]'s eyes, but fails!</span>", "<span class='notice'>I transfer [trans] units of the solution.</span>")
						return
				...
				user.visible_message("<span class='warning'>[user] squirts something into [target]'s eyes!</span>", "<span class='notice'>I transfer [trans] units of the solution.</span>")
			...
			if(!target.is_open_container() && !istype(target,/obj/structure/reagent_dispensers))
				to_chat(user, "<span class='notice'>I cannot directly remove reagents from [target].</span>")
				return
			...
			to_chat(user, "<span class='notice'>I fill the dropper with [trans] units of the solution.</span>")
