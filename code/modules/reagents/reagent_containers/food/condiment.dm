...
///////////////////////////////////////////////Condiments
//Notes by Darem: The condiments food-subtype is for stuff I don't actually eat but I use to modify existing food. They all
//	leave empty containers when used up and can be filled/re-filled with other items. Formatting for first section is identical
//	to mixed-drinks code. If I want an object that starts pre-loaded, I need to make it in addition to the other code.
	...
	if(istype(W, /obj/item/weapon/pen) || istype(W, /obj/item/device/flashlight/pen))
		...
		if(length(tmp_label) > 10)
			...
			if(length(tmp_label))
				...
				to_chat(user, "<span class='notice'>I remove the label.</span>")
				label_text = null
				on_reagent_change()
		return
...
/obj/item/weapon/reagent_containers/food/condiment/afterattack(var/obj/target, var/mob/user, var/proximity)
	...
	if(istype(target, /obj/item/weapon/reagent_containers/food/snacks)) // These are not opencontainers but we can transfer to them
		...
		if(!target.reagents.get_free_space())
			to_chat(user, "<span class='notice'>I can't add more condiment to \the [target].</span>")
			return

		var/trans = reagents.trans_to_obj(target, amount_per_transfer_from_this)
		to_chat(user, "<span class='notice'>I add [trans] units of the condiment to \the [target].</span>")
	else
		..()
...
/obj/item/weapon/reagent_containers/food/condiment/self_feed_message(var/mob/user)
	to_chat(user, "<span class='notice'>I swallow some of contents of \the [src].</span>")
	...
	if(reagents.reagent_list.len > 0)
		switch(reagents.get_master_reagent_type())
			if(/datum/reagent/nutriment/ketchup)
				name = "ketchup"
				desc = "I feel more American already."
				icon_state = "ketchup"
				center_of_mass = "x=16;y=6"
			...
			if(/datum/reagent/capsaicin)
				name = "hotsauce"
				desc = "I can almost TASTE the stomach ulcers now!"
				icon_state = "hotsauce"
				center_of_mass = "x=16;y=6"
			...
			if(/datum/reagent/sugar)
				...
				if (reagents.reagent_list.len==1)
					desc = "Looks like it is [reagents.get_master_reagent_name()], but I am not sure."
				else
