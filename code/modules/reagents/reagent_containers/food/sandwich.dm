...
/obj/item/weapon/reagent_containers/food/snacks/csandwich/attackby(obj/item/W as obj, mob/user as mob)
	...
	if(src.contents.len > sandwich_limit)
		to_chat(user, "<span class='wwarning'>If I put anything else on \the [src] it's going to collapse.</span>")
		return
	else if(istype(W,/obj/item/weapon/material/shard))
		if(!user.unEquip(W, src))
			return
		to_chat(user, "<span class='warning'>I hide [W] in \the [src].</span>")
		update()
		return
	else if(istype(W,/obj/item/weapon/reagent_containers/food/snacks))
		if(!user.unEquip(W, src))
			return
		to_chat(user, "<span class='warning'>I layer [W] over \the [src].</span>")
...
/obj/item/weapon/reagent_containers/food/snacks/csandwich/examine(mob/user)
	...
	to_chat(user, "<span class='warning'>I think I can see [O.name] in there.</span>")
...
/obj/item/weapon/reagent_containers/food/snacks/csandwich/attack(mob/M as mob, mob/user as mob, def_zone)
	...
	if(H && shard && M == user) //This needs a check for feeding the food to other people, but that could be abusable.
		to_chat(H, "<span class='warning'>I lacerate my mouth on a [shard.name] in the sandwich!</span>")
