...
/obj/item/weapon/reagent_containers/glass/attack_self()
	..()
	if(is_open_container())
		to_chat(usr, "<span class = 'notice'>I put the lid on \the [src].</span>")
		atom_flags ^= ATOM_FLAG_OPEN_CONTAINER
	else
		to_chat(usr, "<span class = 'notice'>I take the lid off \the [src].</span>")
		atom_flags |= ATOM_FLAG_OPEN_CONTAINER
	update_icon()
...
/obj/item/weapon/reagent_containers/glass/standard_feed_mob(var/mob/user, var/mob/target)
	if(!is_open_container())
		to_chat(user, "<span class='notice'>I need to open \the [src] first.</span>")
		return 1
...
/obj/item/weapon/reagent_containers/glass/afterattack(var/obj/target, var/mob/user, var/proximity)
	...
	if(user.a_intent == I_HURT)
		...
		if(reagents && reagents.total_volume)
			to_chat(user, "<span class='notice'>I splash the contents of \the [src] onto [target].</span>") //They are on harm intent, aka wanting to spill it.
...
/obj/item/weapon/reagent_containers/glass/bucket/attackby(var/obj/D, mob/user as mob)
	...
	if(isprox(D))
		...
		if(reagents.total_volume < 1)
			...
			to_chat(user, "<span class='notice'>I wet \the [D] in \the [src].</span>")
