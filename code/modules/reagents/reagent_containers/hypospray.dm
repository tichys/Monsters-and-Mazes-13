...
/obj/item/weapon/reagent_containers/hypospray/attack(mob/living/M as mob, mob/user as mob)
	...
	if(istype(H))
		...
		if(!affected)
			...
			to_chat(user, "<span class='danger'>I cannot inject a robotic limb.</span>")
			return

	user.setClickCooldown(DEFAULT_QUICK_COOLDOWN)
	user.do_attack_animation(M)
	to_chat(user, "<span class='notice'>I inject [M] with [src].</span>")
	to_chat(M, "<span class='notice'>I feel a tiny prick!</span>")
...
/obj/item/weapon/reagent_containers/hypospray/vial/attack_hand(mob/user as mob)
	if(user.get_inactive_hand() == src)
		if(loaded_vial)
			...
			loaded_vial = null
			to_chat(user, "I remove the vial from the [src].")
			update_icon()
...
/obj/item/weapon/reagent_containers/hypospray/vial/attackby(obj/item/weapon/W, mob/user as mob)
	if(istype(W, /obj/item/weapon/reagent_containers/glass/beaker/vial))
		if(!loaded_vial)
			...
			user.visible_message("<span class='notice'>[user] has loaded [W] into \the [src].</span>","<span class='notice'>I load \the [W] into \the [src].</span>")
			update_icon()
