	...
	if(ismob(target))//Blood!
		...
		if(istype(target, /mob/living/carbon))
			...
			if(prob(user.skill_fail_chance(SKILL_MEDICAL, 60, SKILL_BASIC)))
				to_chat(user, "<span class='warning'>I miss the vein!</span>")
			...
			to_chat(user, "<span class='notice'>I take a blood sample from [target].</span>")
		...
		if(!target.is_open_container() && !istype(target, /obj/structure/reagent_dispensers) && !istype(target, /obj/item/slime_extract))
			to_chat(user, "<span class='notice'>I cannot directly remove reagents from this object.</span>")
			return

		var/trans = target.reagents.trans_to_obj(src, amount_per_transfer_from_this)
		to_chat(user, "<span class='notice'>I fill the syringe with [trans] units of the solution.</span>")
		update_icon()
...
/obj/item/weapon/reagent_containers/syringe/proc/injectReagents(var/atom/target, var/mob/user)
	...
	if(!target.is_open_container() && !ismob(target) && !istype(target, /obj/item/weapon/reagent_containers/food) && !istype(target, /obj/item/slime_extract) && !istype(target, /obj/item/clothing/mask/smokable/cigarette) && !istype(target, /obj/item/weapon/storage/fancy/cigarettes))
		to_chat(user, "<span class='notice'>I cannot directly fill this object.</span>")
		return
	...
	to_chat(user, "<span class='notice'>I inject \the [target] with [trans] units of the solution. \The [src] now contains [src.reagents.total_volume] units.</span>")
...
/obj/item/weapon/reagent_containers/syringe/proc/injectMob(var/mob/living/carbon/target, var/mob/living/carbon/user, var/atom/trackTarget)
	...
	if(target != user)
		user.visible_message("<span class='warning'>\the [user] injects \the [target] with [visible_name]!</span>", "<span class='notice'>I inject \the [target] with [trans] units of the solution. \The [src] now contains [src.reagents.total_volume] units.</span>")
	else
		to_chat(user, "<span class='notice'>I inject myself with [trans] units of the solution. \The [src] now contains [src.reagents.total_volume] units.</span>")
