...
/obj/structure/reagent_dispensers/fueltank/attack_hand()
	if (rig)
		usr.visible_message("<span class='notice'>\The [usr] begins to detach \the [rig] from \the [src].</span>", "<span class='notice'>I begin to detach \the [rig] from \the [src].</span>")
		if(do_after(usr, 20, src))
			usr.visible_message("<span class='notice'>\The [usr] detaches \the [rig] from \the [src].</span>", "<span class='notice'>I detach [rig] from \the [src]</span>")
...
/obj/structure/reagent_dispensers/fueltank/attackby(obj/item/weapon/W as obj, mob/user as mob)
	...
	if (istype(W,/obj/item/weapon/wrench))
		...
		if (rig)
			...
			return ..()
		user.visible_message("\The [user] begins rigging \the [W] to \the [src].", "I begin rigging \the [W] to \the [src]")
		if(do_after(user, 20, src))
			if(!user.unEquip(W, src))
				return
			user.visible_message("<span class='notice'>\The [user] rigs \the [W] to \the [src].</span>", "<span class='notice'>I rig \the [W] to \the [src].</span>")
		...
		if(user.a_intent != I_HURT)
			to_chat(user, "<span class='warning'>I almost got \the [W] too close to [src]! That could have ended very badly for you.</span>")
			return

		user.visible_message("<span class='warning'>\The [user] draws closer to the fuel tank with \the [W].</span>", "<span class='warning'>I draw closer to the fuel tank with \the [W].</span>")
		if(do_after(user, 50, src))
			log_and_message_admins("triggered a fuel tank explosion with \the [W].")
			user.visible_message("<span class='danger'>\The [user] puts \the [W] to \the [src]!</span>", "<span class='danger'>I put \the [W] to \the [src] and with a moment of lucidity I realize, this might not have been the smartest thing you've ever done.</span>")
			src.explode()
...
/obj/structure/reagent_dispensers/water_cooler/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if (istype(W,/obj/item/weapon/wrench))
		src.add_fingerprint(user)
		if(anchored)
			user.visible_message("\The [user] begins unsecuring \the [src] from the floor.", "I start unsecuring \the [src] from the floor.")
		else
			user.visible_message("\The [user] begins securing \the [src] to the floor.", "I start securing \the [src] to the floor.")

		if(do_after(user, 20, src))
			if(!src) return
			to_chat(user, "<span class='notice'>I [anchored? "un" : ""]secured \the [src]!</span>")
			anchored = !anchored
		return
	else
		return ..()
