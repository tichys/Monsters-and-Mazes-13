	...
	if(istype(W, /obj/item/device/destTagger))
		...
		if(O.currTag)
			if(src.sortTag != O.currTag)
				to_chat(user, "<span class='notice'>I have labeled the destination as [O.currTag].</span>")
				if(!src.sortTag)
			...
			to_chat(user, "<span class='warning'>I need to set a destination first!</span>")

	else if(istype(W, /obj/item/weapon/pen))
		switch(alert("What would I like to alter?",,"Title","Description", "Cancel"))
			if("Title")
				...
				"<span class='notice'>I title \the [src]: \"[str]\"</span>",\
				"I hear someone scribbling a note.")
				SetName("[name] ([str])")
				if(!examtext && !nameset)
					nameset = 1
					update_icon()
			...
			if("Description")
				...
				"<span class='notice'>I label \the [src]: \"[examtext]\"</span>",\
				"I hear someone scribbling a note.")
	return
...
/obj/item/stack/package_wrap/afterattack(var/obj/target as obj, mob/user as mob, proximity)
	...
	if(target in user)
		return
	if(user in target) //no wrapping closets that I am inside - it's not physically possible
		return
	...
	if (istype(target, /obj/item) && !(istype(target, /obj/item/weapon/storage) && !istype(target,/obj/item/weapon/storage/box)))
		...
		if (src.get_amount() >= 1)
			...
			"<span class='notice'>I wrap \the [target], leaving [src.get_amount()] units of paper on \the [src].</span>",\
			"I hear someone taping paper around a small object.")
		else
		...
		if (src.get_amount() >= 3 && !O.opened)
			...
			user.visible_message("\The [user] wraps \a [target] with \a [src].",\
			"<span class='notice'>I wrap \the [target], leaving [src.get_amount()] units of paper on \the [src].</span>",\
			"I hear someone taping paper around a large object.")
		...
		to_chat(user, "<span class='notice'>The object I am trying to wrap is unsuitable for the sorting machinery!</span>")
...
/obj/item/device/destTagger/OnTopic(user, href_list, state)
	if(href_list["nextTag"] && href_list["nextTag"] in GLOB.tagger_locations)
		...
		to_chat(user, "<span class='notice'>I set [src] to <b>[src.currTag]</b>.</span>")
	...
	if(href_list["nextTag"] == "CUSTOM")
		...
		if(CanUseTopic(user, state))
			if(dest && lowertext(dest) != "none")
				src.currTag = dest
				to_chat(user, "<span class='notice'>I designate a custom location on [src], set to <b>[src.currTag]</b>.</span>")
				playsound(src.loc, 'sound/machines/chime.ogg', 50, 1)
			else
				src.currTag = 0
				to_chat(user, "<span class='notice'>I clear [src]'s custom location.</span>")
				playsound(src.loc, 'sound/machines/chime.ogg', 50, 1)
			. = TOPIC_REFRESH
		else
			. = TOPIC_HANDLED
...
/obj/machinery/disposal/deliveryChute/flush()
	...
	if(prob(35))
		for(var/mob/living/carbon/human/L in src)
			...
			to_chat(L, "\The [src]'s mechanisms crush my [E.name]!")
...
/obj/machinery/disposal/deliveryChute/attackby(var/obj/item/I, var/mob/user)
	...
	if(isScrewdriver(I))
		...
		if(W.remove_fuel(1,user))
			to_chat(user, "I start slicing the floorweld off the delivery chute.")
			if(do_after(user,20, src))
				playsound(src.loc, 'sound/items/Welder2.ogg', 100, 1)
				if(!src || !W.isOn()) return
				to_chat(user, "I sliced the floorweld off the delivery chute.")
