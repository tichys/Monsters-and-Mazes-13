...
a /datum/desgin on the linked R&D console. I can then print them out in a fasion similar to a regular lathe. However, instead of
...
/obj/machinery/r_n_d/circuit_imprinter/attackby(var/obj/item/O as obj, var/mob/user as mob)
	...
	if(default_part_replacement(user, O))
		return
	if(panel_open)
		to_chat(user, "<span class='notice'>I can't load \the [src] while it's opened.</span>")
		return 1
	if(!linked_console)
	...
	if(!istype(O, /obj/item/stack/material))
		to_chat(user, "<span class='notice'>I cannot insert this item into \the [src]!</span>")
		return 0
	if(stat)
		return 1
	...
	if(t)
		if(do_after(usr, 16, src))
			if(stack.use(amount))
				to_chat(user, "<span class='notice'>I add [amount] sheet\s to \the [src].</span>")
