...
/obj/machinery/message_server
	...
	var/list/spamfilter = list("I have won", "my prize", "male enhancement", "shitcurity", \
			"are happy to inform you", "account number", "enter my PIN")
...
/obj/machinery/message_server/attack_hand(user as mob)
	to_chat(user, "I toggle PDA message passing from [active ? "On" : "Off"] to [active ? "Off" : "On"]")
	active = !active
	power_failure = 0
	update_icon()
...
/obj/machinery/message_server/attackby(obj/item/weapon/O as obj, mob/living/user as mob)
	if (active && !(stat & (BROKEN|NOPOWER)) && (spamfilter_limit < MESSAGE_SERVER_DEFAULT_SPAM_LIMIT*2) && \
		...
		qdel(O)
		to_chat(user, "I install additional memory and processors into message server. Its filtering capabilities been enhanced.")
	else
		..(O, user)
...
/obj/machinery/message_server/proc/send_to_department(var/department, var/message, var/tone)
	...
	for(var/mob/living/carbon/human/H in GLOB.human_mob_list)
		...
		if(J.department_flag & department)
			to_chat(H, "<span class='notice'>my [pda.name] alerts I to the fact that somebody is requesting my presence at my department.</span>")
			reached++
