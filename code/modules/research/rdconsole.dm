...
allow a player to attempt to re-sync with nearby consoles. I can also force it to disconnect from a specific console.

The imprinting and construction menus do NOT require toxins access to access but all the other menus do. However, if I leave it
on a menu, nothing is to stop the person from using the options on that menu (although they won't be able to change to a different
one). I can also lock the console on the settings menu if you're feeling paranoid and I don't want anyone messing with it who
doesn't have toxins access.

When a R&D console is destroyed or even partially disassembled, I lose all research data on it. However, there are two ways around
this dire fate:
...
operations to skip that console. This is useful if I want to make a "public" R&D console or, for example, give the engineers
a circuit imprinter with certain designs on it and don't want it accidentally updating. The downside of this method is that I have
...
it's entirety. I can then take the disk to any R&D console and upload it's data to it. This method is a lot more secure (since it
...
/obj/machinery/computer/rdconsole/attackby(var/obj/item/weapon/D as obj, var/mob/user as mob)
	...
	if(istype(D, /obj/item/weapon/disk))
		...
		to_chat(user, "<span class='notice'>I add \the [D] to the machine.</span>")
	else
...
/obj/machinery/computer/rdconsole/emp_act(var/remaining_charges, var/mob/user)
	if(!emagged)
		...
		emagged = 1
		to_chat(user, "<span class='notice'>I I disable the security protocols.</span>")
		return 1
...
/obj/machinery/computer/rdconsole/OnTopic(user, href_list)
	if(href_list["menu"]) //Switches menu screens. Converts a sent text string into a number. Saves a LOT of code.
		...
		screen = 0.0
		if(!sync)
			to_chat(usr, "<span class='notice'>I must connect to the network first.</span>")
		else
			. = TOPIC_HANDLED
		...
		griefProtection()
		var/choice = alert("R&D Console Database Reset", "Are I sure I want to reset the R&D console's database? Data lost cannot be recovered.", "Continue", "Cancel")
		. = TOPIC_HANDLED
		if(choice == "Continue")
			screen = 0.0
			qdel(files)
...
/obj/machinery/computer/rdconsole/attack_hand(mob/user as mob)
	...
	var/dat = ""
	files.RefreshResearch()
	switch(screen) //A quick check to make sure I get the right screen when a device is disconnected.
		if(2 to 2.9)
			if(linked_destroy == null)
				screen = 2.0
