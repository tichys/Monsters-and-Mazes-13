...
/obj/machinery/computer/rdservercontrol/CanUseTopic(user)
	if(!allowed(user) && !emagged)
		to_chat(user, "<span class='warning'>I do not have the required access level</span>")
		return STATUS_CLOSE
	return ..()
...
/obj/machinery/computer/rdservercontrol/OnTopic(user, href_list, state)
	if(href_list["main"])
		...
		var/choice = alert(user, "Technology Data Rest", "Are I sure I want to reset this technology to its default data? Data lost cannot be recovered.", "Continue", "Cancel")
		...
		var/choice = alert(user, "Design Data Deletion", "Are I sure I want to delete this design? If I still have the prerequisites for the design, it'll reset to its base reliability. Data lost cannot be recovered.", "Continue", "Cancel")
		if(choice == "Continue" && CanUseTopic(user, state))
