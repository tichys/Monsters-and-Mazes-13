...
/obj/machinery/shieldgen/attack_hand(mob/user as mob)
	if(locked)
		to_chat(user, "The machine is locked, I am unable to use it.")
		return
	if(is_open)
	...
	if (src.active)
		user.visible_message("<span class='notice'>\icon[src] [user] deactivated the shield generator.</span>", \
			"<span class='notice'>\icon[src] I deactivate the shield generator.</span>", \
			"I hear heavy droning fade out.")
		src.shields_down()
	else
		if(anchored)
			user.visible_message("<span class='notice'>\icon[src] [user] activated the shield generator.</span>", \
				"<span class='notice'>\icon[src] I activate the shield generator.</span>", \
...
/obj/machinery/shieldgen/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(isScrewdriver(W))
		...
		if(anchored)
			...
			to_chat(user, "<span class='notice'>'I unsecure the [src] from the floor!</span>")
			if(active)
			...
			playsound(src.loc, 'sound/items/Ratchet.ogg', 100, 1)
			to_chat(user, "<span class='notice'>I secure the [src] to the floor!</span>")
			anchored = 1
