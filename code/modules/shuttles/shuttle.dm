...
/datum/shuttle
	...
	var/defer_initialisation = FALSE //this shuttle will/won't be initialised automatically. If set to true, I am responsible for initialzing the shuttle manually.
...
//If I want to conditionally cancel shuttle launches, that logic must go in short_jump(), long_jump() or attempt_move()
...
//	log_debug("destination: [destination]")
	for(var/turf/src_turf in turf_translation)
		...
		if(src_turf.is_solid_structure()) //in case someone put a hole in the shuttle and I were lucky enough to be under it
			...
			for(var/mob/M in A)
				spawn(0)
					if(istype(M, /mob/living/carbon))
						if(M.buckled)
							to_chat(M, "<span class='warning'>Sudden acceleration presses I into my chair!</span>")
							shake_camera(M, 3, 1)
						else
							to_chat(M, "<span class='warning'>The floor lurches beneath me!</span>")
							shake_camera(M, 10, 1)
