...
/spell/aoe_turf/conjure/summon/before_cast()
	..()
	if(name_summon)
		var/newName = sanitize(input("Would I like to name my summon?") as null|text, MAX_NAME_LEN)
		if(newName)
...
/spell/aoe_turf/conjure/summon/bats/empower_spell()
	...
	return "My bats are now stronger."

/spell/aoe_turf/conjure/summon/bear
	name = "Summon Bear"
	desc = "This spell summons a permanent bear companion that will follow my orders."
	feedback = "BR"
...
/spell/aoe_turf/conjure/summon/bear/empower_spell()
	...
	switch(spell_levels[Sp_POWER])
		if(1)
			...
			return "My bear has been upgraded from a cub to a whelp."
		if(2)
			...
			return "My bear has been upgraded from a whelp to an adult."
		if(3)
			...
			return "My bear has been upgraded from an adult to an alpha."
		if(4)
			...
			return "My bear is now worshiped as a god amongst bears."