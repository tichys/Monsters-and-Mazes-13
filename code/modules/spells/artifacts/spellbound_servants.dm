...
/datum/spellbound_type/proc/spawn_servant(var/atom/a, var/mob/master, var/mob/user)
	...
	var/name_choice = sanitize(input(H, "Choose a name. If I leave this blank, it will be defaulted to my current characters.", "Name change") as null|text, MAX_NAME_LEN)
	if(name_choice)
...
/datum/spellbound_type/apprentice
	name = "Apprentice"
	desc = "Summon my trusty apprentice, equipped with their very own spellbook."
...
/datum/spellbound_type/apprentice/set_antag(var/datum/mind/M, var/mob/master)
	GLOB.wizards.add_antagonist_mind(M,1,ANTAG_APPRENTICE,"<b>I am an apprentice-type Servant! You�re just an ordinary Wizard-To-Be, with no special abilities, but do not need robes to cast spells. Follow my teacher�s orders!</b>")

/datum/spellbound_type/servant
	var/spiel = "I don't do anything in particular."

/datum/spellbound_type/servant/set_antag(var/datum/mind/M, var/mob/master)
	GLOB.wizards.add_antagonist_mind(M,1,ANTAG_SERVANT, "<b>I am a [name]-type Servant!</b> [spiel]")
...
/datum/spellbound_type/servant/caretaker
	...
	spiel = "<i>'The last enemy that will be destroyed is death.'</i> I can perceive any injuries with simple sight, and heal them with the Trance spell; potentially even reversing death itself! However, this comes at a price; Trance will become increasingly harder to use as I use it, until I can use it no longer. Be cautious, and aid my Master in any way possible!"
...
/datum/spellbound_type/servant/champion
	...
	spiel = "My sword and armor are second to none, but I have no unique supernatural powers beyond summoning the sword to my hands. Protect my Master with my life!"
...
/datum/spellbound_type/servant/familiar
	...
	spiel = "This form of mine is weak in comparison to my transformed form, but that certainly won�t pose a problem, considering the fact that I have an alternative. Whatever it is I can turn into, use its powers wisely and serve my Master as well as possible!"
...
/datum/spellbound_type/servant/familiar/modify_servant(var/list/equipment, var/mob/living/carbon/human/H)
	var/familiar_type
	switch(input(H,"Choose my desired animal form:", "Form") as anything in list("Space Pike", "Mouse", "Cat", "Bear"))
		if("Space Pike")
...
/datum/spellbound_type/servant/fiend
	...
	spiel = "The Summoning Ritual has bound I to this world with limited access to my infernal powers; you�ll have to be strategic in how I use them. Follow my Master�s orders as well as I can!"
...
/datum/spellbound_type/servant/infiltrator
	name = "Infiltrator"
	desc = "A spy and a manipulator to the end, capable of hiding in plain sight and falsifying information to my heart's content."
	spiel = "On the surface, I am a completely normal person, but is that really all I am? People are so easy to fool, do as my Master says, and do it with style!"
...
/datum/spellbound_type/servant/overseer
	...
	spiel = "Physicality is not something I am familiar with. Indeed, injuries cannot slow I down, but I can�t fight back, either! In addition to this, I can reach into the void and return the soul of a single departed crewmember via the revoke death verb, if so desired; this can even revive my Master, should they fall in combat before I do. Serve them well."
...
/obj/item/weapon/summoning_stone/attack_self(var/mob/user)
	if(user.z in GLOB.using_map.admin_levels)
		to_chat(user, "<span class='warning'>I cannot use \the [src] here.</span>")
		return
