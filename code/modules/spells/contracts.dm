...
/obj/item/weapon/contract/attack_self(mob/user as mob)
	if(contract_master == null)
		to_chat(user, "<span class='notice'>I bind the contract to my soul, making I the recipient of whatever poor fool's soul that decides to contract with you.</span>")
		contract_master = user
		return

	if(contract_master == user)
		to_chat(user, "I can't contract with myself!")
		return

	var/ans = alert(user,"The contract clearly states that signing this contract will bind my soul to \the [contract_master]. Are I sure I want to continue?","[src]","Yes","No")
...
/obj/item/weapon/contract/proc/contract_effect(mob/user as mob)
	to_chat(user, "<span class='warning'>I have signed my soul over to \the [contract_master] and with that my unbreakable vow of servitude begins.</span>")
	return 1
...
/obj/item/weapon/contract/apprentice/contract_effect(mob/user as mob)
	if(user.mind.special_role == ANTAG_APPRENTICE)
		to_chat(user, "<span class='warning'>I am already a wizarding apprentice!</span>")
		return 0
	if(user.mind.special_role == ANTAG_SERVANT)
		to_chat(user, "<span class='notice'>I am a servant. I have no need of apprenticeship.</span>")
		return 0
	if(GLOB.wizards.add_antagonist_mind(user.mind,1,ANTAG_APPRENTICE,"<b>I am an apprentice! My job is to learn the wizarding arts!</b>"))
		to_chat(user, "<span class='notice'>With the signing of this paper I agree to become \the [contract_master]'s apprentice in the art of wizardry.</span>")
		return 1
	return 0
...
/obj/item/weapon/contract/wizard/telepathy
	...
	desc = "The edges of the contract grow blurry when I look away from them. To be fair, actually reading it gives I a headache."
	color = "#fcc605"
...
/obj/item/weapon/contract/wizard/telepathy/contract_effect(mob/user as mob)
	...
	to_chat(H, "<span class='notice'>I expand my mind outwards.</span>")
	return 1

/obj/item/weapon/contract/wizard/tk
	name = "telekinesis contract"
	desc = "This contract makes my mind buzz. It promises to give me the ability to move things with my mind. At a price."
	color = "#990033"
...
/obj/item/weapon/contract/wizard/tk/contract_effect(mob/user as mob)
	...
	if(!(TK in user.mutations))
		...
		to_chat(user, "<span class='notice'>I feel my mind expanding!</span>")
		return 1
	return 0

/obj/item/weapon/contract/boon
	name = "boon contract"
	desc = "this contract grants I a boon for signing it."
	var/path
...
/obj/item/weapon/contract/boon/contract_effect(mob/user as mob)
	..()
	if(user.mind.special_role == ANTAG_SERVANT)
		to_chat(user, "<span class='warning'>As a servant I find myself unable to use this contract.</span>")
		return 0
...
/obj/item/weapon/contract/boon/wizard/horsemask
	...
	desc = "This contract is more horse than my mind has room for."
