...
/spell/hand/cast_check(skipcharge = 0,mob/user = usr, var/list/targets)
	...
	if(user.get_active_hand())
		to_chat(holder, "<span class='warning'>I need an empty hand to cast this spell.</span>")
		return FALSE
	return TRUE
...
/spell/hand/cast(list/targets, mob/user)
	if(current_hand)
		cancel_hand()
	if(user.get_active_hand())
		to_chat(user, "<span class='warning'>I need an empty hand to cast this spell.</span>")
		return FALSE
...
/spell/hand/charges/cast(list/targets, mob/user)
	...
	if(.)
		casts = max_casts
		to_chat(user, "I ready the [name] spell ([casts]/[casts] charges).")
