...
/spell/proc/perform(mob/user = usr, skipcharge = 0) //if recharge is started is important for the trigger spells
	...
	do
		...
		if(!check_valid_targets(targets)) //make sure we HAVE something
			break
		if(cast_check(1,user, targets)) //we check again, otherwise I can choose a target and then wait for when I am no longer able to cast (I.E. Incapacitated) to use it.
...
/spell/proc/adjust_var(mob/living/target = usr, type, amount) //handles the adjustment of the var when the spell is used. has some hardcoded types
	switch(type)
		...
		if("paralysis")
			...
			target.vars[type] += amount //I bear no responsibility for the runtimes that'll happen if I try to adjust non-numeric or even non-existant vars
	return
...
/spell/proc/cast_check(skipcharge = 0,mob/user = usr, var/list/targets) //checks if the spell can be cast based on its settings; skipcharge is used when an additional cast_check is called inside the spell
	...
	if(!user_turf)
		to_chat(user, "<span class='warning'>I cannot cast spells in null space!</span>")
	...
	if(holder == user)
		if(istype(user, /mob/living/simple_animal))
			...
			if(SA.purge)
				to_chat(SA, "<span class='warning'>The null sceptre's power interferes with my own!</span>")
				return 0
		...
		if(!(spell_flags & GHOSTCAST))
			if(!(spell_flags & NO_SOMATIC))
				...
				if(L.incapacitated(INCAPACITATION_STUNNED|INCAPACITATION_RESTRAINED|INCAPACITATION_BUCKLED_FULLY|INCAPACITATION_FORCELYING|INCAPACITATION_KNOCKOUT))
					to_chat(user, "<span class='warning'>I can't cast spells while incapacitated!</span>")
					return 0
...
/spell/proc/quicken_spell()
	...
	if(charge_type == Sp_RECHARGE)
		if(cooldown_reduc)
			...
			charge_max = round( max(cooldown_min, initial(charge_max) * ((level_max[Sp_SPEED] - spell_levels[Sp_SPEED]) / level_max[Sp_SPEED] ) ) ) //the fraction of the way I am to max speed levels is the fraction I lose
