...
/obj/item/weapon/spellbook/attack_self(mob/user as mob)
	...
	if (user.mind.special_role != ANTAG_WIZARD)
		if (user.mind.special_role != ANTAG_APPRENTICE)
			to_chat(user, "I can't make heads or tails of this book.")
			return
		...
		to_chat(user, "I notice the apprentice-proof lock is on. Luckily I am beyond such things.")
	interact(user)
	...
	if(reagent)
		...
		if(istype(I,/obj/item/stack))
			...
			if(S.amount < S.max_amount)
				to_chat(usr, "<span class='warning'>I must sacrifice [S.max_amount] stacks of [S]!</span>")
				return
		qdel(I)
	to_chat(user, "<span class='notice'>My sacrifice was accepted!</span>")
	has_sacrificed = 1
...
/obj/item/weapon/spellbook/interact(mob/user as mob)
	...
	if(temp)
		...
		dat = "<center><h3>[spellbook.title]</h3><i>[spellbook.title_desc]</i><br>I have [uses] spell slot[uses > 1 ? "s" : ""] left.</center><br>"
		...
		dat += "<center><b>To use a contract, first bind it to my soul, then give it to someone to sign. This will bind their soul to me.</b></center><br>"
		for(var/i in 1 to spellbook.spells.len)
			...
			dat += "<br><i>[desc]</i><br>"
		dat += "<center><A href='byond://?src=\ref[src];reset=1'>Re-memorize my spellbook.</a></center>"
...
/obj/item/weapon/spellbook/OnTopic(var/mob/living/carbon/human/user, href_list)
	if(href_list["lock"])
		...
		if(initial(spellbook.max_uses) != spellbook.max_uses || uses != spellbook.max_uses)
			temp = "I have already purchased things using this spellbook!"
		else
			src.set_spellbook(/datum/spellbook)
			temp = "I have reverted back to the Book of Tomes."
		. = TOPIC_REFRESH
		...
		if(uses < spellbook.spells[path])
			to_chat(user, "<span class='notice'>I do not have enough spell slots to purchase this.</span>")
			return TOPIC_HANDLED
		send_feedback(path) //feedback stuff
		if(ispath(path,/datum/spellbook))
			src.set_spellbook(path)
			temp = "I have chosen a new spellbook."
		else
		...
		if(istype(A))
			...
			has_sacrificed = 0
			user.spellremove()
			temp = "All spells and investments have been removed. I may now memorize a new set of spells."
			feedback_add_details("wizard_spell_learned","UM") //please do not change the abbreviation to keep data processing consistent. Add a unique id to any new spells
		else
			to_chat(user, "<span class='warning'>I must be in the wizard academy to re-memorize my spells.</span>")
		. = TOPIC_REFRESH
...
/obj/item/weapon/spellbook/proc/invest()
	if(uses < 1)
		return "I don't have enough slots to invest!"
	if(investing_time)
		return "I can only invest one spell slot at a time."
	uses--
	START_PROCESSING(SSobj, src)
	investing_time = world.time + (15 MINUTES)
	return "I invest a spellslot and will recieve two in return in 15 minutes."
	...
	for(var/spell/S in user.mind.learned_spells)
		if(istype(S,spell_path))
			...
			if(S.can_improve(Sp_SPEED) && S.can_improve(Sp_POWER))
				switch(alert(user, "Do I want to upgrade this spell's speed or power?", "Spell upgrade", "Speed", "Power", "Cancel"))
					if("Speed")
