/spell/targeted/heal_target
	...
	message = "I feel a pleasant rush of heat move through my body."
...
/spell/targeted/heal_target/major
	...
	message = "My body feels like a furnace."
...
/spell/targeted/heal_target/sacrifice/empower_spell()
	...
	return "I will now heal organ and brain damage, as well as virtually purge all radiation."
...
/spell/targeted/heal_target/trance/cast(var/list/targets, var/mob/user)
	for(var/t in targets)
		...
		to_chat(L,"<span class='notice'>I will be in stasis for [time/10] second\s</span>")
...
/spell/targeted/revoke/cast(var/list/targets, var/mob/living/user)
	if(alert(user, "Are I sure?", "Alert", "Yes", "No") == "Yes" && alert(user, "Are I ABSOLUTELY SURE?", "Alert", "Absolutely!", "No") == "Absolutely!")
		var/should_wait = 1
		for(var/t in targets)
			var/mob/living/M = t
			M.rejuvenate()
...
/spell/targeted/revoke/proc/check_for_revoke(var/list/targets)
	...
	to_chat(holder,"<span class='notice'>\The [src] refreshes as it seems it could not bring back the souls of those I healed.</span>")
