...
/spell/targeted/shapeshift/proc/destroyed_transformer(var/mob/target) //Juuuuust in case
	...
	to_chat(current, "<span class='danger'>I suddenly feel as if this transformation has become permanent...</span>")
...
/spell/targeted/shapeshift/baleful_polymorph/empower_spell()
	...
	return "My target will now stay in their polymorphed form for [duration/10] seconds."
...
/spell/targeted/shapeshift/corrupt_form/empower_spell()
	...
	switch(spell_levels[Sp_POWER])
		if(1)
			duration *= 2
			return "I will now stay corrupted for [duration/10] seconds."
		if(2)
			newVars = list("name" = "\proper corruption incarnate",
						...
						"maxHealth" = 125)
			duration = 0
			return "I revel in the corruption. There is no turning back."
