/datum/job/submap
	...
	var/info = "I have survived a terrible disaster. Make the best of things that I can."
	var/rank
	var/branch
...
/datum/job/submap/is_restricted(var/datum/preferences/prefs, var/feedback)
	...
	if(LAZYLEN(whitelisted_species) && !(prefs.species in whitelisted_species))
		to_chat(feedback, "<span class='boldannounce'>My current species, [prefs.species], is not permitted as crew on \a [owner.archetype.descriptor].</span>")
		return TRUE
