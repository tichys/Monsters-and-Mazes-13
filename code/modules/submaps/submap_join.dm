...
/datum/submap/proc/check_general_join_blockers(var/mob/new_player/joining, var/datum/job/submap/job)
	...
	if(jobban_isbanned(joining, "Offstation Roles"))
		to_chat(joining, "<span class='warning'>I am banned from playing offstation roles.</span>")
		return FALSE
...
/datum/submap/proc/join_as(var/mob/new_player/joining, var/datum/job/submap/job)
	...
	if(istype(character))
		...
		to_chat(character, "<B>I am [job.total_positions == 1 ? "the" : "a"] [job.title] of the [name].</B>")

		if(job.supervisors)
			to_chat(character, "<b>As a [job.title] I answer directly to [job.supervisors].</b>")
