...
/datum/surgery_step/cavity/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, scraping around inside [target]'s [affected.name] with \the [tool]!</span>")
...
/datum/surgery_step/cavity/make_space/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start making some space inside [target]'s [affected.cavity_name] cavity with \the [tool]." )
	target.custom_pain("The pain in my chest is living hell!",1,affecting = affected)
	affected.cavity = 1
	..()
...
/datum/surgery_step/cavity/make_space/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I make some space inside [target]'s [affected.cavity_name] cavity with \the [tool].</span>" )
...
/datum/surgery_step/cavity/close_space/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start mending [target]'s [affected.cavity_name] cavity wall with \the [tool]." )
	target.custom_pain("The pain in my chest is living hell!",1,affecting = affected)
	affected.cavity = 0
	..()
...
/datum/surgery_step/cavity/close_space/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I mend [target]'s [affected.cavity_name] cavity walls with \the [tool].</span>" )
...
/datum/surgery_step/cavity/place_item/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start putting \the [tool] inside [target]'s [affected.cavity_name] cavity." )
	target.custom_pain("The pain in my chest is living hell!",1,affecting = affected)
...
/datum/surgery_step/cavity/place_item/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I put \the [tool] inside [target]'s [affected.cavity_name] cavity.</span>" )
	if (tool.w_class > affected.cavity_max_w_class/2 && prob(50) && !BP_IS_ROBOTIC(affected) && affected.sever_artery())
		to_chat(user, "<span class='warning'>I tear some blood vessels trying to fit such a big object in this cavity.</span>")
		affected.owner.custom_pain("I feel something rip in my [affected.name]!", 1,affecting = affected)
...
/datum/surgery_step/cavity/implant_removal/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start poking around inside [target]'s [affected.name] with \the [tool]" )
	target.custom_pain("The pain in my [affected.name] is living hell!",1,affecting = affected)
	..()
...
/datum/surgery_step/cavity/implant_removal/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if (loot.len)
		...
		if (prob(find_prob))
			...
			"<span class='notice'>I take [obj] out of incision on [target]'s [affected.name]s with \the [tool].</span>" )
			...
			"<span class='notice'>There's something inside [target]'s [affected.name], but I just missed it this time.</span>" )
	else
		...
		"<span class='notice'>I could not find anything inside [target]'s [affected.name].</span>" )
