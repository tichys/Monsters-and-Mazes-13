...
/datum/surgery_step/internal/fix_organ/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	for(var/obj/item/organ/internal/I in affected.internal_organs)
		if(I && I.damage > 0 && !BP_IS_ROBOTIC(I) && (!I.status & ORGAN_DEAD || I.can_recover()) && (I.surface_accessible || affected.how_open() >= (affected.encased ? 3 : 2)))
			...
			"I start treating damage to [target]'s [I.name] with [tool_name]." )

	target.custom_pain("The pain in my [affected.name] is living hell!",100,affecting = affected)
	..()
	...
	for(var/obj/item/organ/internal/I in affected.internal_organs)
		if(I && I.damage > 0 && !BP_IS_ROBOTIC(I) && (I.surface_accessible || affected.how_open() >= (affected.encased ? SURGERY_ENCASED : SURGERY_RETRACTED)))
			if(I.status & ORGAN_DEAD && I.can_recover())
				...
				"<span class='notice'>I treat damage to [target]'s [I.name] with [tool_name], though it needs to be recovered further.</span>" )
			else
				user.visible_message("<span class='notice'>[user] treats damage to [target]'s [I.name] with [tool_name].</span>", \
				"<span class='notice'>I treat damage to [target]'s [I.name] with [tool_name].</span>" )
			I.surgical_fix(user)
...
/datum/surgery_step/internal/fix_organ/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, getting mess and tearing the inside of [target]'s [affected.name] with \the [tool]!</span>")
	var/dam_amt = 2
...
/datum/surgery_step/internal/detatch_organ/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	var/organ_to_remove = input(user, "Which organ do I want to separate?") as null|anything in attached_organs
	if(!organ_to_remove)
		return 0
...
/datum/surgery_step/internal/detatch_organ/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start to separate [target]'s [target.op_stage.current_organ] with \the [tool]." )
	target.custom_pain("Someone's ripping out my [target.op_stage.current_organ]!",100)
	..()
...
/datum/surgery_step/internal/detatch_organ/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I have separated [target]'s [target.op_stage.current_organ] with \the [tool].</span>")
...
/datum/surgery_step/internal/detatch_organ/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, slicing an artery inside [target]'s [affected.name] with \the [tool]!</span>")
...
/datum/surgery_step/internal/remove_organ/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	var/organ_to_remove = input(user, "Which organ do I want to remove?") as null|anything in removable_organs
	if(!organ_to_remove)
		return 0
...
/datum/surgery_step/internal/remove_organ/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start removing [target]'s [target.op_stage.current_organ] with \the [tool].")
	target.custom_pain("The pain in my [affected.name] is living hell!",100,affecting = affected)
	..()
...
/datum/surgery_step/internal/remove_organ/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I have removed [target]'s [target.op_stage.current_organ] with \the [tool].</span>")
...
/datum/surgery_step/internal/remove_organ/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, damaging [target]'s [affected.name] with \the [tool]!</span>")
...
/datum/surgery_step/internal/replace_organ/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if(BP_IS_CRYSTAL(O) && !BP_IS_CRYSTAL(affected))
		to_chat(user, "<span class='warning'>I cannot install a crystalline organ into a non-crystalline bodypart.</span>")
		return SURGERY_FAILURE

	if(!BP_IS_CRYSTAL(O) && BP_IS_CRYSTAL(affected))
		to_chat(user, "<span class='warning'>I cannot install a non-crystalline organ into a crystalline bodypart.</span>")
		return SURGERY_FAILURE

	if(BP_IS_ROBOTIC(affected) && !BP_IS_ROBOTIC(O))
		to_chat(user, "<span class='warning'>I cannot install a naked organ into a robotic body.</span>")
		return SURGERY_FAILURE
...
/datum/surgery_step/internal/replace_organ/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start transplanting \the [tool] into [target]'s [affected.name].")
	target.custom_pain("Someone's rooting around in my [affected.name]!",100,affecting = affected)
	..()
...
/datum/surgery_step/internal/replace_organ/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='notice'>I have transplanted \the [tool] into [target]'s [affected.name].</span>")
...
/datum/surgery_step/internal/replace_organ/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, damaging \the [tool]!</span>")
...
/datum/surgery_step/internal/attach_organ/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	var/obj/item/organ/organ_to_replace = input(user, "Which organ do I want to reattach?") as null|anything in attachable_organs
	if(!organ_to_replace)
		return 0
	if(organ_to_replace.parent_organ != affected.organ_tag)
		to_chat(user, "<span class='warning'>I can't find anywhere to attach [organ_to_replace] to!</span>")
		return SURGERY_FAILURE
...
/datum/surgery_step/internal/attach_organ/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	target.custom_pain("Someone's digging needles into my [target.op_stage.current_organ]!",100)
	..()
...
/datum/surgery_step/internal/attach_organ/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, damaging the flesh in [target]'s [affected.name] with \the [tool]!</span>")
	affected.take_external_damage(20, used_weapon = tool)
...
/datum/surgery_step/internal/treat_necrosis/can_use(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	var/obj/item/organ/internal/organ_to_fix = input(user, "Which organ do I want to regenerate?") as null|anything in dead_organs
	if(!organ_to_fix)
		return 0
...
/datum/surgery_step/internal/treat_necrosis/begin_step(mob/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"I start applying medication to the affected tissue in [target]'s [target.op_stage.current_organ] with \the [tool].")

	target.custom_pain("Something in my [target.op_stage.current_organ] is causing I a lot of pain!",50,affecting = target.op_stage.current_organ)
	..()
...
/datum/surgery_step/internal/treat_necrosis/end_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	if (trans > 0)
		...
		user.visible_message("<span class='notice'>[user] applies [trans] unit\s of the solution to affected tissue in [target]'s [affected.name]</span>.", \
			"<span class='notice'>I apply [trans] unit\s of the solution to affected tissue in [target]'s [affected.name] with \the [tool].</span>")
	qdel(temp_reagents)
...
/datum/surgery_step/internal/treat_necrosis/fail_step(mob/living/user, mob/living/carbon/human/target, target_zone, obj/item/tool)
	...
	"<span class='warning'>My hand slips, applying [trans] units of the solution to the wrong place in [target]'s [affected.name] with the [tool]!</span>")
