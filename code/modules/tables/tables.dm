/obj/structure/table
	...
	icon_state = "frame"
	desc = "It's a table, for putting things on. Or standing on, if I really want to."
	density = 1
	anchored = 1
	...
	// Convert if/when I can easily get stacks of these.
	var/carpeted = 0
...
/obj/structure/table/attackby(obj/item/weapon/W, mob/user)
	...
	if(carpeted && isCrowbar(W))
		user.visible_message("<span class='notice'>\The [user] removes the carpet from \the [src].</span>",
		                              "<span class='notice'>I remove the carpet from \the [src].</span>")
	...
	if(!carpeted && material && istype(W, /obj/item/stack/tile/carpet))
		...
		if(C.use(1))
			user.visible_message("<span class='notice'>\The [user] adds \the [C] to \the [src].</span>",
			                              "<span class='notice'>I add \the [C] to \the [src].</span>")
			carpeted = 1
			update_icon()
			return 1
		else
			to_chat(user, "<span class='warning'>I don't have enough carpet!</span>")
	...
	if(health < maxhealth && isWelder(W))
		...
		if(F.welding)
			to_chat(user, "<span class='notice'>I begin reparing damage to \the [src].</span>")
			playsound(src.loc, 'sound/items/Welder.ogg', 50, 1)
			if(!do_after(user, 20, src) || !F.remove_fuel(1, user))
				return
			user.visible_message("<span class='notice'>\The [user] repairs some damage to \the [src].</span>",
			                              "<span class='notice'>I repair some damage to \the [src].</span>")
...
/obj/structure/table/proc/common_material_add(obj/item/stack/material/S, mob/user, verb) // Verb is actually verb without 'e' or 'ing', which is added. Works for 'plate'/'plating' and 'reinforce'/'reinforcing'.
	...
	if(!istype(M))
		to_chat(user, "<span class='warning'>I cannot [verb]e \the [src] with \the [S].</span>")
		return null

	if(manipulating) return M
	manipulating = 1
	to_chat(user, "<span class='notice'>I begin [verb]ing \the [src] with [M.display_name].</span>")
	if(!do_after(user, 20, src) || !S.use(1))
		manipulating = 0
		return null
	user.visible_message("<span class='notice'>\The [user] [verb]es \the [src] with [M.display_name].</span>", "<span class='notice'>I finish [verb]ing \the [src].</span>")
	manipulating = 0
	return M
...
/obj/structure/table/proc/common_material_remove(mob/user, material/M, delay, what, type_holding, sound)
	if(!M.stack_type)
		to_chat(user, "<span class='warning'>I am unable to remove the [what] from this table!</span>")
		return M
	...
	user.visible_message("<span class='notice'>\The [user] begins removing the [type_holding] holding \the [src]'s [M.display_name] [what] in place.</span>",
	                              "<span class='notice'>I begin removing the [type_holding] holding \the [src]'s [M.display_name] [what] in place.</span>")
	if(sound)
	...
	user.visible_message("<span class='notice'>\The [user] removes the [M.display_name] [what] from \the [src].</span>",
	                              "<span class='notice'>I remove the [M.display_name] [what] from \the [src].</span>")
...
/obj/structure/table/proc/dismantle(obj/item/weapon/wrench/W, mob/user)
	...
	user.visible_message("<span class='notice'>\The [user] begins dismantling \the [src].</span>",
	                              "<span class='notice'>I begin dismantling \the [src].</span>")
	playsound(src.loc, 'sound/items/Ratchet.ogg', 50, 1)
	if(!do_after(user, 20, src))
		manipulating = 0
		return
	user.visible_message("<span class='notice'>\The [user] dismantles \the [src].</span>",
	                              "<span class='notice'>I dismantle \the [src].</span>")
