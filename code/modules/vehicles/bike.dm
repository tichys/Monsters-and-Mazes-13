...
/obj/vehicle/bike/verb/kickstand()
	...
	if(kickstand)
		...
		if(istype(src.loc,/turf/space))
			to_chat(usr, "<span class='warning'> I don't think kickstands work in space...</span>")
			return
...
/obj/vehicle/bike/attackby(obj/item/W as obj, mob/user as mob)
	if(open)
		if(istype(W, /obj/item/weapon/engine))
			...
			to_chat(user, "I pop out \the [engine] from \the [src].")
			unload_engine()
			return 1
	return ..()
...
/obj/vehicle/bike/MouseDrop_T(var/atom/movable/C, mob/user as mob)
	if(!load(C))
		to_chat(user, "<span class='warning'> I were unable to load \the [C] onto \the [src].</span>")
		return

/obj/vehicle/bike/attack_hand(var/mob/user as mob)
	if(user == load)
		unload(load)
		to_chat(user, "I unbuckle myself from \the [src]")
...
/obj/vehicle/bike/Move(var/turf/destination)
	...
	//these things like space, not turf. Dragging shouldn't weigh I down.
	if(!pulledby)
