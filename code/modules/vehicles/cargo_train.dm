...
/obj/vehicle/train/cargo/trolley/attackby(obj/item/weapon/W as obj, mob/user as mob)
	if(open && isWirecutter(W))
		...
		user.visible_message("<span class='notice'>[user] [passenger_allowed ? "cuts" : "mends"] a cable in [src].</span>","<span class='notice'>I [passenger_allowed ? "cut" : "mend"] the load limiter cable.</span>")
	else
		..()
...
/obj/vehicle/train/cargo/engine/RunOver(var/mob/living/carbon/human/H)
	...
	if(is_train_head() && istype(load, /mob/living/carbon/human))
		...
		to_chat(D, "<span class='danger'>I ran over [H]!</span>")
...
/obj/vehicle/train/cargo/engine/verb/start_engine()
	...
	turn_on()
	if (on)
		to_chat(usr, "I start [src]'s engine.")
	else
...
/obj/vehicle/train/cargo/engine/verb/stop_engine()
	...
	turn_off()
	if (!on)
		to_chat(usr, "I stop [src]'s engine.")
...
/obj/vehicle/train/cargo/trolley/load(var/atom/movable/C)
	if(ismob(C) && !passenger_allowed)
		return 0
	if(!istype(C,/obj/machinery) && !istype(C,/obj/structure/closet) && !istype(C,/obj/structure/largecrate) && !istype(C,/obj/structure/reagent_dispensers) && !istype(C, /mob/living/carbon/human))
		return 0

	//if there are any items I don't want to be able to interact with, add them to this check
...
/obj/vehicle/train/cargo/engine/update_car(var/train_length, var/active_engines)
	...
	if(!is_train_head() || !on)
		...
		move_delay = max(0, (-car_limit * active_engines) + train_length - active_engines)	//limits base overweight so I cant overspeed trains
