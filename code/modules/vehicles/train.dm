...
/obj/vehicle/train/Bump(atom/Obstacle)
	...
	if(emagged)
		if(istype(A, /mob/living))
			...
			M.apply_effects(5, 5)				//knock people down if I hit them
			...
			if(istype(load, /mob/living/carbon/human))
				...
				to_chat(D, "<span class='warning'>I hit [M]!</span>")
...
/obj/vehicle/train/relaymove(mob/user, direction)
	...
	if(!T)
		to_chat(user, "I can't find a clear area to step onto.")
		return 0
	...
	to_chat(user, "<span class='notice'>I climb down from [src].</span>")
...
/obj/vehicle/train/MouseDrop_T(var/atom/movable/C, mob/user as mob)
	...
	if(istype(C,/obj/vehicle/train))
		...
		if(!load(C))
			to_chat(user, "<span class='warning'>I were unable to load [C] on [src].</span>")
...
/obj/vehicle/train/proc/attach_to(obj/vehicle/train/T, mob/user)
	...
	if(user)
		to_chat(user, "<span class='notice'>I hitch \the [src] to \the [T].</span>")
...
/obj/vehicle/train/proc/unattach(mob/user)
	...
	to_chat(user, "<span class='notice'>I unhitch \the [src] from \the [lead].</span>")
	lead = null
