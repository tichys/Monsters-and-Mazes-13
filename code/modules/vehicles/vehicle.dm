...
/obj/vehicle
	...
	var/attack_log = null
	var/on = 0
	var/health = 0	//do not forget to set health for my vehicle!
	var/maxhealth = 0
...
/obj/vehicle/New()
	..()
	//spawn the cell I want in each vehicle
		...
		if(T.welding)
			if(health < maxhealth)
				if(open)
					...
					user.visible_message("<span class='warning'>\The [user] repairs \the [src]!</span>","<span class='notice'>I repair \the [src]!</span>")
				else
...
/obj/vehicle/emag_act(var/remaining_charges, mob/user as mob)
	if(!emagged)
		...
		if(locked)
			locked = 0
			to_chat(user, "<span class='warning'>I bypass [src]'s controls.</span>")
		return 1
...
/obj/vehicle/proc/insert_cell(var/obj/item/weapon/cell/C, var/mob/living/carbon/human/H)
	...
	cell = C
	powercheck()
	to_chat(usr, "<span class='notice'>I install [C] in [src].</span>")

/obj/vehicle/proc/remove_cell(var/mob/living/carbon/human/H)
	if(!cell)
		return

	to_chat(usr, "<span class='notice'>I remove [cell] from [src].</span>")
