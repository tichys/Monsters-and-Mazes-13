...
/mob/living/proc/can_ventcrawl()
	...
	if(!(/mob/living/proc/ventcrawl in verbs))
		to_chat(src, "<span class='warning'>I don't possess the ability to ventcrawl!</span>")
		return FALSE
	if(incapacitated())
		to_chat(src, "<span class='warning'>I cannot ventcrawl in my current state!</span>")
		return FALSE
...
/mob/living/carbon/slime/can_ventcrawl()
	if(Victim)
		to_chat(src, "<span class='warning'>I cannot ventcrawl while feeding.</span>")
		return FALSE
	. = ..()
...
/mob/living/proc/ventcrawl_carry()
	for(var/atom/A in contents)
		if(!is_allowed_vent_crawl_item(A))
			to_chat(src, "<span class='warning'>I can't carry \the [A] while ventcrawling!</span>")
			return FALSE
	return TRUE
...
/mob/proc/start_ventcrawl()
	...
	for(var/obj/machinery/atmospherics/unary/U in range(1))
		if(is_type_in_list(U,ventcrawl_machinery) && Adjacent(U) && U.can_crawl_through())
			pipes |= U
	if(!pipes || !pipes.len)
		to_chat(src, "There are no pipes that I can ventcrawl into within range!")
		return
	if(pipes.len == 1)
		pipe = pipes[1]
	else
			...
			if(vent_found.air_contents && !issilicon(src))
				...
				switch(vent_found.air_contents.return_pressure())
					if(0 to HAZARD_LOW_PRESSURE)
						to_chat(src, "<span class='danger'>I feel a rushing draw pulling I into the vent!</span>")
					if(HAZARD_LOW_PRESSURE to WARNING_LOW_PRESSURE)
						to_chat(src, "<span class='warning'>I feel a strong drag pulling I into the vent.</span>")
					if(WARNING_HIGH_PRESSURE to HAZARD_HIGH_PRESSURE)
						to_chat(src, "<span class='warning'>I feel a strong current pushing I away from the vent.</span>")
					if(HAZARD_HIGH_PRESSURE to INFINITY)
						to_chat(src, "<span class='danger'>I feel a roaring wind pushing I away from the vent!</span>")
