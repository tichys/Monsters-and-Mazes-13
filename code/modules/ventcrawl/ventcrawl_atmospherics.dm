...
/obj/machinery/atmospherics/proc/ventcrawl_to(var/mob/living/user, var/obj/machinery/atmospherics/target_move, var/direction)
	if(target_move)
		if(is_type_in_list(target_move, ventcrawl_machinery) && target_move.can_crawl_through())
			...
			user.visible_message("I hear something squeezing through the ducts.", "I climb out the ventilation system.")
		...
		if((direction & initialize_directions) || is_type_in_list(src, ventcrawl_machinery) && src.can_crawl_through()) //if we move in a way the pipe can connect, but doesn't - or we're in a vent
			...
			user.visible_message("I hear something squeezing through the pipes.", "I climb out the ventilation system.")
