 ...
 *
 *   The easiest way to make effective tests is to create a "template" if I intend to run the same test over and over and make my actual
 *   tests be a "child object" of those templates.  Be sure and name my templates with the word "template" somewhere in var/name.
 *
 ...
 *
 *   If my test requires a significant amount of time...cheat on the timers.  Either speed up the process/life runs or do as we did in the timers for the shuttle
...
// if I want plain output, use dm.sh -DUNIT_TEST -DUNIT_TEST_PLAIN baystation12.dme
...
datum/unit_test
	...
	var/async = 0           // If the check can be left to do it's own thing, I must define a check_result() proc if I use this.
...
/proc/do_unit_test(datum/unit_test/test, end_time, skip_disabled_tests = TRUE)
	...
	if(world.time > end_time)
		test.fail("Unit Tests Ran out of time")   // This should never happen, and if it does either fix my unit tests to be faster or if I can make them async checks.
		return
...
/proc/check_unit_test(datum/unit_test/test, end_time)
	if(world.time > end_time)
		test.fail("Unit Tests Ran out of Time")// If we're going to run out of time, most likely it's here.  If I can't speed up my unit tests then add time to the timeout at the top.
		return 1
