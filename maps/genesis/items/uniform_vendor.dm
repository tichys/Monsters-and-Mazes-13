...
/obj/machinery/uniform_vendor/attack_hand(mob/user)
	...
	dat += "<hr>"
	if(!ID)
		dat += "Insert my ID card to proceed."
	else
...
/obj/machinery/uniform_vendor/attackby(var/obj/item/weapon/W, var/mob/user)
	if(istype(W, /obj/item/weapon/clothingbag))
		if(W.contents.len)
			to_chat(user, "<span class='notice'>I must empty \the [W] before I can put it in \the [src].</span>")
			return
		to_chat(user, "<span class='notice'>I put \the [W] into \the [src]'s recycling slot.</span>")
		qdel(W)
	else if(istype(W, /obj/item/weapon/card/id) && !ID && user.unEquip(W, src))
		to_chat(user, "<span class='notice'>I slide \the [W] into \the [src]!</span>")
		ID = W
		attack_hand(user)
	else
		..()
...
/*	Outfit structures
	...
	The one exception to the above is the command department, due to the fact that I have to be an officer to
