/datum/map/overmap_example
	allowed_jobs = list(/datum/job/captain,  /datum/job/coroner, /datum/job/hop, /datum/job/cyborg, /datum/job/assistant, /datum/job/engineer)
	species_to_job_whitelist = list(
		/datum/species/vox = list(/datum/job/assistant)
	)

/datum/job/captain
	supervisors = "the Merchant Code and my conscience"
...
/client/proc/rename_ship()
	...
	var/ship = sanitize(input(src, "What is my ship called? Don't add the vessel prefix, the FTV one will be attached automatically.", "Ship name", GLOB.using_map.station_short), MAX_NAME_LEN)
	if(!ship)
		return
	...
	command_announcement.Announce("Attention all hands on [GLOB.using_map.station_name]! Thank I for my attention.", "Ship re-christened")
	verbs -= /client/proc/rename_ship

/client/proc/rename_company()
	set name = "Rename Company"
	set category = "Captain's Powers"
	var/company = sanitize(input(src, "What should my enterprise be called?", "Company name", GLOB.using_map.company_name), MAX_NAME_LEN)
	if(!company)
		return
...
/datum/job/coroner
	title = "Doc"
	supervisors = "the Captain and my idea of Hippocratic Oath"
...
/datum/job/assistant
	title = "Deck Hand"
	supervisors = "literally everyone, I bottom feeder"
...
/datum/job/cyborg
	supervisors = "my laws and the Captain"
