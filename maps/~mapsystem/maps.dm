...
/datum/map/New()
	...
	if(!allowed_jobs)
		allowed_jobs = subtypesof(/datum/job)-/datum/job // there we go, the "-/datum/job" wasn't there, so NOPE was on the job select
	if(!planet_size)
...
/datum/map/proc/bolt_saferooms()
	return // overriden by genesis

/datum/map/proc/unbolt_saferooms()
	return // overriden by genesis

/datum/map/proc/make_maint_all_access(var/radstorm = 0) // parameter used by genesis
	maint_all_access = 1
	priority_announcement.Announce("The maintenance access requirement has been revoked on all maintenance airlocks.", "Attention!")

/datum/map/proc/revoke_maint_all_access(var/radstorm = 0) // parameter used by genesis
	maint_all_access = 0
